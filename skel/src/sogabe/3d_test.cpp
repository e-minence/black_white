...
void	YT_Init3DTest(GAME_PARAM *gp)
{
	...
	//ジョブワークを確保
	gt=(G3D_TEST *)gp->job_work=(G3D_TEST *)GFL_HEAP_AllocClearMemory(gp->heapID,sizeof(G3D_TEST));
...
void	YT_Main3DTest(GAME_PARAM *gp)
{
	...
	if(gt->update){
...

		{
			...
			MTX_RotX43(&mtx43_x,FX_SinIdx(gt->rotx),FX_CosIdx(gt->rotx));
			MTX_RotY43(&mtx43_y,FX_SinIdx(gt->roty),FX_CosIdx(gt->roty));
			MTX_RotZ43(&mtx43_z,FX_SinIdx(gt->rotz),FX_CosIdx(gt->rotz));
			MTX_MultVec43(&gt->vec,&mtx43_x,&gt->vec);
			MTX_MultVec43(&gt->vec,&mtx43_y,&gt->vec);
			MTX_MultVec43(&gt->vec,&mtx43_z,&gt->vec);

			OS_Printf("vec.x:%08x vec.y:%08x vec.z:%08x\n",gt->vec.x,gt->vec.y,gt->vec.z);

//			rot_x=FX_Atan2((gt->vec.y-FX32_ONE),(gt->vec.z-FX32_ONE));
//			rot_y=FX_Atan2((gt->vec.z-FX32_ONE),(gt->vec.x-FX32_ONE));
//			rot_z=FX_Atan2((gt->vec.y-FX32_ONE),(gt->vec.x-FX32_ONE));
			height=gt->vec.y-FX32_ONE;
			width=gt->vec.z-FX32_ONE;
			if(width){
				rot_x=FX_Atan2Idx(height,width);
			}
			else{
				if(height>0){
					rot_x=0;
			...
			height=gt->vec.z-FX32_ONE;
			width=gt->vec.x-FX32_ONE;
			if(width){
				rot_y=FX_Atan2Idx(height,width);
			}
			else{
				if(height>0){
					rot_y=0;
			...
			width=gt->vec.x-FX32_ONE;
			if(width){
				rot_z=FX_Atan2Idx(height,width);
			}
			else{
				if(height>0){
					rot_z=0;

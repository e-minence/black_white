...
{
	int i, j;
	STATUS_CONTROL*	stc = (STATUS_CONTROL*)GFL_HEAP_AllocClearMemory( setup->heapID, sizeof(STATUS_CONTROL) );
	TEAM_CONTROL** p_tc = (TEAM_CONTROL**)GFL_HEAP_AllocClearMemory
							( setup->heapID, setup->teamCount * sizeof(TEAM_CONTROL*) );
	STATUS_CLACT_P** p_tact = (STATUS_CLACT_P**)GFL_HEAP_AllocClearMemory
	...
	ADDSTATUS_CALLBACK_WORK* ascw = (ADDSTATUS_CALLBACK_WORK*)GFL_HEAP_AllocClearMemoryLo
	...
	for( i=0; i<stc->teamCount; i++ ){
		STATUS_CLACT_P*	tact;
		tact = (STATUS_CLACT_P*)GFL_HEAP_AllocClearMemory( stc->heapID, sizeof(STATUS_CLACT_P) );
...
{
	int i, j;
	DRAWSTATUS_CALLBACK_WORK* dscw = (DRAWSTATUS_CALLBACK_WORK*)GFL_HEAP_AllocClearMemory
...
static void GetStatusCgx( u32 resID, GFL_BMP_DATA* bmp )
{
	...
	u8* src = (u8*)GFL_OBJGRP_GetVramTransCGRPointer( resID );
...
static void SetStatusCgx( u32 resID, GFL_BMP_DATA* bmp )
{
	...
	u8* dst = (u8*)GFL_OBJGRP_GetVramTransCGRPointer( resID );

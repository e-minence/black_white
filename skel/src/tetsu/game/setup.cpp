...
{
	GAME_SYSTEM*	gs = (GAME_SYSTEM*)GFL_HEAP_AllocMemory( heapID, sizeof(GAME_SYSTEM) );
	gs->heapID = heapID;
...
static void	bg_init( GAME_SYSTEM* gs )
{
	...
	//背景色パレット作成＆転送
	{
		u16* plt = (u16 *)GFL_HEAP_AllocClearMemoryLo( gs->heapID, 16*2 );
			...
			case CLACT_RESLOAD_CGX:
				{
					...
					GFL_VRAM_TYPE vtype = (GFL_VRAM_TYPE)(clactResList[++i]);

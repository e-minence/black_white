...
{
	STATUSWIN_CONTROL* swc = (STATUSWIN_CONTROL*)GFL_HEAP_AllocClearMemory( setup->heapID, sizeof(STATUSWIN_CONTROL) );
	TEAM_CONTROL** p_tc = (TEAM_CONTROL**)GFL_HEAP_AllocClearMemory
...
static void TeamStatusDraw( GFL_BMP_DATA* bmp, TEAM_CONTROL* tc, WINPOS_DATA* wd, HEAPID heapID )
{
	...
	char* text = (char *)GFL_HEAP_AllocClearMemory( heapID, 32 );
	char* p_text;
...
{
	MSGWIN_CONTROL* mwc = (MSGWIN_CONTROL*)GFL_HEAP_AllocClearMemory( setup->heapID, sizeof(MSGWIN_CONTROL) );
	mwc->setup = *setup;
...
{
	int i, j;
	MAPWIN_CONTROL*		mpwc = (MAPWIN_CONTROL*)GFL_HEAP_AllocClearMemory( setup->heapID, sizeof(MAPWIN_CONTROL) );
	TEAM_CONTROL**		p_tc = (TEAM_CONTROL**)GFL_HEAP_AllocClearMemory
							( setup->heapID, setup->teamCount * sizeof(TEAM_CONTROL*) );
	MAPWIN_CLACT_P**	p_tact = (MAPWIN_CLACT_P**)GFL_HEAP_AllocClearMemory
	...
	for( i=0; i<mpwc->teamCount; i++ ){
		...
		tact = (MAPWIN_CLACT_P*)GFL_HEAP_AllocClearMemory( mpwc->heapID, sizeof(MAPWIN_CLACT_P) );
...
{
	int i, j;
	MAPWIN_CALLBACK_WORK* mpcw = (MAPWIN_CALLBACK_WORK*)GFL_HEAP_AllocClearMemory

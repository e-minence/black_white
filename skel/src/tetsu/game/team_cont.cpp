...
extern const SUMMON_TYPETABLE	summonTypeTable[9];
...
{
	int i;
	TEAM_CONTROL* tc = (TEAM_CONTROL*)GFL_HEAP_AllocClearMemory( heapID, sizeof(TEAM_CONTROL) );
	tc->heapID = heapID;
	tc->gs = gs;
	tc->playerCount = 0;
	...
	//背景色パレット作成＆転送
	{
		u16* plt = (u16 *)GFL_HEAP_AllocClearMemoryLo( heapID, 16*2 );
		plt[2] = 0x07c0;
		plt[3] = 0x05e0;

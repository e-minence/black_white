...
{
	PLAYER_CONTROL* pc = (PLAYER_CONTROL*)GFL_HEAP_AllocClearMemory( heapID, sizeof(PLAYER_CONTROL) );
	pc->gs = gs;
	pc->targetAct = targetAct;
	pc->netID = netID;

	pc->moveDir = (PLAYER_MOVE_DIR)0;
...
static BOOL jumpControl( PLAYER_CONTROL* pc )
{
	...
	case 0:
		...
		pc->moveDir = (PLAYER_MOVE_DIR)(pc->contDirection + pc->moveDir);
		pc->jumpFlag = TRUE;
		pc->subSeq++;
		retFlag = FALSE;
		break;
...
void MainPlayerControl( PLAYER_CONTROL* pc )
{
	...
	if( pc->subSeq ){
		pc->contCommand = (PLAYER_CONTROL_COMMAND)pc->nowCommand;//サブシーケンス動作中
	}

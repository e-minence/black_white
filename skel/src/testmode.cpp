...
static void	g2d_load( TESTMODE_WORK * testmode )
{
	...
	//パレット作成＆転送
	{
		u16* plt = (u16*)GFL_HEAP_AllocClearMemoryLo( testmode->heapID, 16*2 );
	...
	//文字表示パラメータワーク作成
	{
		GFL_TEXT_PRINTPARAM* param = (GFL_TEXT_PRINTPARAM*)
				GFL_HEAP_AllocClearMemoryLo( testmode->heapID,sizeof(GFL_TEXT_PRINTPARAM));
		*param = default_param;
...
static GFL_PROC_RESULT TestModeProcInit(GFL_PROC * proc, int * seq, void * pwk, void * mywk)
{
	...
	testmode = (TESTMODE_WORK*)GFL_PROC_AllocWork( proc, sizeof(TESTMODE_WORK), heapID );
...
{
	TESTMODE_WORK * testmode = (TESTMODE_WORK *) mywk;
...
static GFL_PROC_RESULT TestModeProcEnd(GFL_PROC * proc, int * seq, void * pwk, void * mywk)
{
	...
	GFL_PROC_FreeWork((GFL_PROC*)mywk);

...
struct _NET_PARAM
{
    ...
    GFL_CLWK* pCLACT[KEY_YT_CLACT_MAX];  // 描画してるclact
    PLAYER_PARAM* pp;
    u32 seq;
    BOOL bGameStart;
};
...
enum CommCommandBattle_e {
    KEY_YT_NET_COMMAND_ANIM = GFL_NET_CMD_COMMAND_MAX,  ///< アニメコマンド送信  b
    KEY_YT_NET_COMMAND_POS,                             ///< 位置コマンド送信c
    KEY_YT_NET_COMMAND_PLAYERANIM,            ///< プレーヤーのアニメーション情報の送信d
    KEY_YT_NET_COMMAND_DELETE,            /// CLACTを消す命令e
    KEY_YT_NET_COMMAND_YOSSYANIM,     ///ヨッシーのアニメーションf
    KEY_YT_NET_COMMAND_ANIM_CREATE,   ///< アニメーション作成10
    TY_NET_COMMAND_SCREENMAKE,
    TY_NET_COMMAND_SCREENMAKE_ROT,
    KEY_YT_NET_COMMAND_PLAY_SE,
    KEY_YT_NET_COMMAND_GAMERESULT,     ///< ゲーム結果
    KEY_YT_NET_COMMAND_END,
    //------------------------------------------------ここまで
    KEY_YT_NET_COMMAND_MAX   // 終端--------------これは移動させないでください
};
...
static void _recvPlayGameover(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
    ...
    if(netID==GFL_NET_GetNetID(pNet->pNetHandle[1])){  // 送信者本人
        KEY_YT_PlayerSetWinLoseFlag(&gp->ps[pResult->player], pResult->bWin);
    }
    else{
        KEY_YT_PlayerSetWinLoseFlag(&gp->ps[pResult->player^1], !pResult->bWin);
    }
}
...
static void _recvYossyAnim(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
    ...
    KEY_YT_YossyBirthAnimeTaskSet(gp,NULL,pYossy->posx,pYossy->posy,pYossy->count);
}
...
static void _recvScreenMake(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
    ...
    if(pNet->pp){
        KEY_YT_PlayerScreenMakeNetFunc(pNet->pp, pSMake->player_no, pSMake->old_line_no, pSMake->new_line_no);
    }
}
...
static void _recvScreenMakeRot(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
    ...
    if(pNet->pp){
        KEY_YT_PlayerRotateScreenMakeNetFunc(pNet->pp, pSMake->player_no, pSMake->line_no, pSMake->flag);
    }
}
...
static void _recvCLACTAnimCreate(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
    ...
    if(pNet->pCLACT[pAnim->clactNo]==NULL){
        pNet->pCLACT[pAnim->clactNo] = KEY_YT_InitNetworkFallChr(gp,netID,pAnim->type,0);
...
static void _recvCLACTAnim(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
    ...
    GF_ASSERT(pAnim->clactNo < KEY_YT_CLACT_MAX);
...
static void _recvCLACTPlayerAnim(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
    ...
    if(pNet->pp == NULL){
        pNet->pp = KEY_YT_InitPlayer(gp,pAnim->player_no,pAnim->player_no,FALSE);
    }
    else{
        KEY_YT_NetPlayerChrTrans(gp, pNet->pp, pAnim->player_no, pAnim->anm_no, pAnim->line_no, pAnim->rot, pAnim->pat_no);
    }
}
...
BOOL KEY_YT_NET_IsParent(NET_PARAM* pNet)
{
...
void KEY_YT_NET_SendPlayerAnmReq(int player_no,int anm_no,int pat_no,u16 line_no,int rot,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        //GFL_NET_SendData(pNet->pNetHandle[1], KEY_YT_NET_COMMAND_PLAYERANIM, &CommPlayerAnmSt);
    }
}
...
void KEY_YT_NetSendYossyBirthAnime(u8 posx,u8 posy,u8 count,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        //GFL_NET_SendData(pNet->pNetHandle[1], KEY_YT_NET_COMMAND_YOSSYANIM, &sYossy);
    }
}
...
void KEY_YT_NET_SendAnmCreate(int clactno,u16 type,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        //GFL_NET_SendData(pNet->pNetHandle[1], KEY_YT_NET_COMMAND_ANIM_CREATE, &CommAnmCreateSt);
    }
}
...
void KEY_YT_NET_SendAnmReq(int clactno,u16 anmseq,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        //GFL_NET_SendData(pNet->pNetHandle[1], KEY_YT_NET_COMMAND_ANIM, &CommAnmSt);
    }
}
...
void KEY_YT_NET_SendPlayerScreenMake(int player_no, int old_line_no, int new_line_no,NET_PARAM* pNet)
{
...
void KEY_YT_NET_SendPlayerRotateScreenMake(u8 player_no, u8 line_no, u8 flag,NET_PARAM* pNet)
{
...
void KEY_YT_NET_DeleteReq(int clactno,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        //GFL_NET_SendData(pNet->pNetHandle[1], KEY_YT_NET_COMMAND_DELETE, &CommDelSt);
    }
}
...
void KEY_YT_NET_SendPosReq(int clactno,s16 x, s16 y,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        CommPosSt.y = y;
        //OS_TPrintf("位置送信 %d %d\n",x,y);
        //GFL_NET_SendData(pNet->pNetHandle[1], KEY_YT_NET_COMMAND_POS, &CommPosSt);
    }
}
...
void KEY_YT_NET_Init(GAME_PARAM* gp, BOOL bParent)
{
    GFL_TCBSYS* tcbsys;
...
//	GFL_TCB_AddTask(gp->tcbsys,KEY_YT_NET_Main,pNet,1);
}
 ...
 */
//-----------------------------------------------------------------------------
void KEY_YT_NET_PlaySE(int se_no, NET_PARAM* pNet)
{
    GFL_SOUND_PlaySE(se_no);
    if(pNet){
//        GFL_NET_SendData(pNet->pNetHandle[1], KEY_YT_NET_COMMAND_PLAY_SE, &se_no);
    }
}
 ...
 */
//-----------------------------------------------------------------------------
void KEY_YT_NET_SendGameResult(int player, int bWin, NET_PARAM* pNet)
{
    COMM_RESULT_ST sResult;
    if(pNet){
        sResult.player = player;
        sResult.bWin = bWin;
//        GFL_NET_SendData(pNet->pNetHandle[1], KEY_YT_NET_COMMAND_GAMERESULT, &sResult);
    }
}

...
#include "../sogabe/fall_obj_anm.h"

#include "key_fallchr.h"

static	void	KEY_YT_MainFallChr(GFL_TCB *tcb,void *work);
static	void	KEY_YT_FallStart(FALL_CHR_PARAM *fcp,KEY_YT_PLAYER_STATUS *ps);
static	int		KEY_YT_LandingCheck(FALL_CHR_PARAM *fcp,KEY_YT_PLAYER_STATUS *ps,GFL_CLWK *clwk,NET_PARAM* pNet);
static	int		KEY_YT_LandingStart(FALL_CHR_PARAM *fcp,KEY_YT_PLAYER_STATUS *ps,int fall_line,int stop_line,int height,NET_PARAM* pNet);
static	int		KEY_YT_LandingCheckRensa(FALL_CHR_PARAM *fcp,KEY_YT_PLAYER_STATUS *ps,GFL_CLWK *clwk,NET_PARAM* pNet);
static	int		KEY_YT_LandingStartRensa(FALL_CHR_PARAM *fcp,KEY_YT_PLAYER_STATUS *ps,int stop_line,int height,NET_PARAM* pNet);
static	GFL_CLWK	*KEY_YT_ClactWorkAdd(FALL_CHR_PARAM *fcp);
static	void	KEY_YT_AnmSeqSet(FALL_CHR_PARAM *fcp,int flag, NET_PARAM* pNet);
static	void	KEY_YT_ChrPosSet(FALL_CHR_PARAM *fcp, NET_PARAM* pNet);
static	void	KEY_YT_RotateActSet(FALL_CHR_PARAM *fcp);
static	void	KEY_YT_YossyBirth(GAME_PARAM *gp,FALL_CHR_PARAM *fcp);
static	void	KEY_YT_YossyBirthAnime(GFL_TCB *tcb,void *work);

typedef struct{
	GAME_PARAM			*gp;
	KEY_YT_PLAYER_STATUS	*ps;
	int					seq_no;
	int					pat_no;
	int					wait;
	int					pos_x;
 ...
 */
//-----------------------------------------------------------------------------
FALL_CHR_PARAM	*KEY_YT_InitFallChr(GAME_PARAM *gp,u8 player_no,u8 type,u8 line_no)
{
	...
	GFL_TCB_AddTask(gp->tcbsys,KEY_YT_MainFallChr,fcp,TCB_PRI_FALL_CHR);
	gp->clact->clact_work[fcp->clact_no]=fcp->clwk=KEY_YT_ClactWorkAdd(fcp);

	GFL_CLACT_WK_GetWldPos(fcp->clwk,&fcp->now_pos);

    //通信対戦時には、作成typeを送信
    if(pNet){
        KEY_YT_NET_SendAnmCreate(fcp->clact_no, type, pNet);
    }
 ...
 */
//-----------------------------------------------------------------------------
GFL_CLWK* KEY_YT_InitNetworkFallChr(GAME_PARAM *gp,u8 player_no,u8 type,u8 line_no)
{
	FALL_CHR_PARAM	*fcp=(FALL_CHR_PARAM *)GFL_HEAP_AllocClearMemory(gp->heapID,sizeof(FALL_CHR_PARAM));
    GFL_CLWK* pCLWK;
 ...
 */
//-----------------------------------------------------------------------------
static void KEY_YT_DeleteFallChr(GFL_CLWK* clwk,FALL_CHR_PARAM *fcp,GFL_TCB *tcb,NET_PARAM* pNet)
{
	//通信対戦時には、消す命令を送信する
    if(pNet){
        KEY_YT_NET_DeleteReq(fcp->clact_no, pNet);
    }
...
static	void	KEY_YT_MainFallChr(GFL_TCB *tcb,void *work)
{
    ...
    NET_PARAM*          pNet = gp->pNetParam;
	KEY_YT_PLAYER_STATUS	*ps=(KEY_YT_PLAYER_STATUS *)&gp->ps[fcp->player_no];
...
//		GFL_CLACT_WK_SetAutoAnmSpeed(clwk,0);
	...
	case SEQ_FALL_CHR_READY:
		if(fcp->now_pos.y<KEY_YT_READY_Y_POS){
			fcp->now_pos.y+=KEY_YT_READY_FALL_SPEED;
		}
		else{
			fcp->now_pos.y=KEY_YT_READY_Y_POS;
			KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_STOP,pNet);
			fcp->seq_no++;
		}
		KEY_YT_ChrPosSet(fcp, pNet);
		break;
	case SEQ_FALL_CHR_FALL_INIT:
		if(ps->fall_wait){
			break;
		}
		KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_FALL,pNet);
		KEY_YT_FallStart(fcp,ps);
		fcp->fall_wait=gp->default_fall_wait;
		fcp->fall_wait_tmp=gp->default_fall_wait;
		fcp->seq_no++;
	case SEQ_FALL_CHR_FALL:
		if(fcp->rotate_flag){
			KEY_YT_RotateActSet(fcp);
			break;
		}
		if(ps->status.no_active_flag){
			break;
		}
		if((GFL_NET_KEY_GetCont(fcp->player_no)&PAD_KEY_DOWN)==0){
...
//		GFL_CLACT_WK_SetAutoAnmSpeed(clwk,0);
		if((fcp->fall_wait)&&((GFL_NET_KEY_GetCont(fcp->player_no)&PAD_KEY_DOWN)==0)){
			fcp->fall_wait--;
		}
		else{
			fcp->fall_wait=fcp->fall_wait_tmp;
			switch(KEY_YT_LandingCheck(fcp,ps,clwk,pNet)){
			case KEY_YT_ACT_VANISH:
				GFL_CLACT_WK_SetAutoAnmSpeed(clwk,1*FX32_ONE);
				fcp->seq_no=SEQ_FALL_CHR_VANISH_INIT;
				break;
			case KEY_YT_ACT_LANDING:
				KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_LANDING,pNet);
				GFL_CLACT_WK_SetAutoAnmSpeed(clwk,1*FX32_ONE);
				if((fcp->type==KEY_YT_CHR_GREEN_EGG_U)||(fcp->type==KEY_YT_CHR_RED_EGG_U)){
			...
			default:
				if((GFL_NET_KEY_GetCont(fcp->player_no)&PAD_KEY_DOWN)==0){
					fcp->now_pos.y+=KEY_YT_FALL_SPEED;
				}
				else{
					fcp->now_pos.y+=KEY_YT_FALL_SPEED*2;
				}
				KEY_YT_ChrPosSet(fcp, pNet);
				break;
			}
	...
	case SEQ_FALL_CHR_STOP:
		...
		if(fcp->overturn_flag){
			KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_OVERTURN,pNet);
			KEY_YT_ChrPosSet(fcp, pNet);
				...
				if(fcp->rotate_flag==0){
					...
					if(fcp->seq_no==SEQ_FALL_CHR_STOP){
						...
						if(ps->stop[ps->stoptbl[fcp->line_no]][i+1]){
							KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_TUBURE,pNet);
						}
					}
				}
			}
	...
	case SEQ_FALL_CHR_OVERTURN:
		if(GFL_CLACT_WK_CheckAnmActive(clwk)==FALSE){
			if(fcp->type<KEY_YT_CHR_GREEN_EGG_U){
				fcp->dir^=1;
			}
			else if(fcp->type<KEY_YT_CHR_GREEN_EGG){
				fcp->type=((fcp->type-KEY_YT_CHR_GREEN_EGG_U)^1)+KEY_YT_CHR_GREEN_EGG_U;
				KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_FALL,pNet);
			}
			else{
				KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_EGG,pNet);
			}
			fcp->overturn_flag=0;
			fcp->offset_pos.y=0;
			KEY_YT_ChrPosSet(fcp, pNet);
		...
		if(fcp->wait_value){
			...
			if(--fcp->wait_value==0){
				if(fcp->egg_make_flag){
					if(fcp->type==KEY_YT_CHR_GREEN_EGG_U){
						fcp->type=KEY_YT_CHR_GREEN_EGG;
						KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_EGG,pNet);
						fcp->seq_no=SEQ_FALL_CHR_YOSSY_BIRTH;
						fcp->egg_make_flag=0;
						ps->egg_make_count--;
					}
					else if(fcp->type==KEY_YT_CHR_RED_EGG_U){
						fcp->type=KEY_YT_CHR_RED_EGG;
						KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_EGG,pNet);
						fcp->seq_no=SEQ_FALL_CHR_YOSSY_BIRTH;
						fcp->egg_make_flag=0;
						ps->egg_make_count--;
                        ...
                        KEY_YT_DeleteFallChr(clwk,fcp,tcb,pNet);
					}
				}
				else{
					if((fcp->type==KEY_YT_CHR_GREEN_EGG_U)||(fcp->type==KEY_YT_CHR_RED_EGG_U)){
	...
	case SEQ_FALL_CHR_YOSSY_BIRTH:
		...
		if(fcp->birth_wait){
			if(fcp->birth_wait>KEY_YT_BIRTH_SPEED){
				fcp->birth_wait-=KEY_YT_BIRTH_SPEED;
			}
			else{
				fcp->birth_wait=0;
			}
	...
	case SEQ_FALL_CHR_RENSA_FALL_P:
		{
			...
			if(ps->status.no_active_flag==KEY_YT_RENSA_FLAG){
				switch(KEY_YT_LandingCheckRensa(fcp,ps,clwk,pNet)){
				case KEY_YT_ACT_VANISH:
					fcp->seq_no=SEQ_FALL_CHR_VANISH_INIT;
					for(y=0;y<KEY_YT_HEIGHT_MAX;y++){
				...
				case KEY_YT_ACT_LANDING:
					...
					break;
				default:
					fcp->now_pos.y+=KEY_YT_FALL_SPEED*4;
					KEY_YT_ChrPosSet(fcp, pNet);
					for(y=0;y<KEY_YT_HEIGHT_MAX;y++){
						fcp_rensa=ps->rensa[y];
						if((fcp_rensa!=fcp)&&(fcp_rensa)){
							fcp_rensa->now_pos.y+=KEY_YT_FALL_SPEED*4;
							KEY_YT_ChrPosSet(fcp_rensa, pNet);
						}
					}
					break;
				}
	...
	case SEQ_FALL_CHR_VANISH_INIT:
		KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_VANISH,pNet);
		fcp->seq_no++;
...
static	int		KEY_YT_LandingCheck(FALL_CHR_PARAM *fcp,KEY_YT_PLAYER_STATUS *ps,GFL_CLWK *clwk,NET_PARAM* pNet)
{
	...
	for(height=0;height<KEY_YT_HEIGHT_MAX;height++){
	...
	if(fcp->now_pos.y>=height_tbl[height]){
		...
		KEY_YT_ChrPosSet(fcp, pNet);
		return KEY_YT_LandingStart(fcp,ps,check_fall_line,check_stop_line,height,pNet);
	}

	return KEY_YT_ACT_FALL;
}
	...
	if(height){
		{
			...
			if(fcp->type==fcp_under->type){
				...
				if(height-1){
					...
					KEY_YT_AnmSeqSet(fcp_under,KEY_YT_ANM_STOP,pNet);
				}
				return KEY_YT_ACT_VANISH;
			}
			else if((fcp->type==KEY_YT_CHR_TERESA)&&(fcp_under->type==KEY_YT_CHR_DEKATERESA)){
				fcp_under->type=KEY_YT_CHR_TERESA;
				KEY_YT_AnmSeqSet(fcp_under,KEY_YT_ANM_STOP,pNet);
				return KEY_YT_ACT_VANISH;
			}
			else if((fcp_under->type==KEY_YT_CHR_TERESA)&&(fcp->type==KEY_YT_CHR_DEKATERESA)){
				fcp_under->seq_no=SEQ_FALL_CHR_VANISH_INIT;
				ps->stop[stop_line][height-1]=NULL;
				//さらに下のキャラがいた場合はつぶれを元に戻す
				if(height-1){
				...
				fcp->type=KEY_YT_CHR_TERESA;
				KEY_YT_AnmSeqSet(fcp,KEY_YT_ANM_FALL,pNet);
				return KEY_YT_ACT_FALL;
			}
			else{
				KEY_YT_AnmSeqSet(fcp_under,KEY_YT_ANM_TUBURE,pNet);
			}
		}
	}
	//stop配列にセット
...
static	int		KEY_YT_LandingCheckRensa(FALL_CHR_PARAM *fcp,KEY_YT_PLAYER_STATUS *ps,GFL_CLWK *clwk,NET_PARAM* pNet)
{
	...
	if(fcp->now_pos.y>=height_tbl[height]){
		fcp->now_pos.y=height_tbl[height];
		KEY_YT_ChrPosSet(fcp, pNet);
		return KEY_YT_LandingStartRensa(fcp,ps,check_stop_line,height,pNet);
	}
...
static GFL_CLWK* KEY_YT_ClactWorkAdd(FALL_CHR_PARAM *fcp)
{
	...
	s16			pos_data[][KEY_YT_LINE_MAX][2]={
					//PLAYER1
					{
						//LINE0
						{
	...
	u8			soft_pri[]={
					...
					KEY_YT_PRI_GESSO,
					KEY_YT_PRI_TERESA,
					KEY_YT_PRI_DEKATERESA,
					KEY_YT_PRI_GREEN_EGG_U,
					KEY_YT_PRI_GREEN_EGG_D,
					KEY_YT_PRI_RED_EGG_U,
					KEY_YT_PRI_RED_EGG_D,
					KEY_YT_PRI_GREEN_EGG,
					KEY_YT_PRI_RED_EGG,
				};
	u8			bg_pri[]={
					2,		//KEY_YT_PRI_KURIBO,
					...
					2,		//KEY_YT_PRI_GESSO,
					2,		//KEY_YT_PRI_TERESA,
					2,		//KEY_YT_PRI_DEKATERESA,
					2,		//KEY_YT_PRI_GREEN_EGG_U,
					2,		//KEY_YT_PRI_GREEN_EGG_D,
					2,		//KEY_YT_PRI_RED_EGG_U,
					2,		//KEY_YT_PRI_RED_EGG_D,
					2,		//KEY_YT_PRI_GREEN_EGG,
					2,		//KEY_YT_PRI_RED_EGG,
				};
...
static	void	KEY_YT_ChrPosSet(FALL_CHR_PARAM *fcp, NET_PARAM* pNet)
{
	...
	//通信対戦時には、座標を送信する
    if(pNet){
        KEY_YT_NET_SendPosReq(fcp->clact_no, pos.x, pos.y, pNet);
    }
}
...
static	void	KEY_YT_AnmSeqSet(FALL_CHR_PARAM *fcp,int flag,NET_PARAM* pNet)
{
	...
	switch(flag){
	case KEY_YT_ANM_LANDING:
		KEY_YT_NET_PlaySE(SE_LANDING,pNet);
	case KEY_YT_ANM_STOP:
	case KEY_YT_ANM_TUBURE:
		//タマゴの場合はアニメセットしない
		if(fcp->type>=KEY_YT_CHR_GREEN_EGG_U){
			return;
		}
	case KEY_YT_ANM_FALL:
		anm_seq+=fcp->type;
		break;
	case KEY_YT_ANM_OVERTURN:
		//タマゴの場合は専用アニメをセット
		if(fcp->type>=KEY_YT_CHR_GREEN_EGG){
			anm_seq=NANR_fall_obj_GREEN_EGG_OVERTURN+(fcp->type-KEY_YT_CHR_GREEN_EGG);
		}
		else if(fcp->type>=KEY_YT_CHR_GREEN_EGG_U){
			anm_seq=NANR_fall_obj_GREEN_EGG_OVERTURN_UD+(fcp->type-KEY_YT_CHR_GREEN_EGG_U);
		}
		else{
			anm_seq+=fcp->type;
		}
	...
	case KEY_YT_ANM_EGG:
		...
		break;
	case KEY_YT_ANM_VANISH:
		KEY_YT_NET_PlaySE(SE_VANISH,pNet);
		break;
	default:
		//ありえない数値はバグなので止める
		GF_ASSERT(0);
    ...
    if(pNet){
        KEY_YT_NET_SendAnmReq(fcp->clact_no, anm_seq, pNet);
    }
}
...
static	void	KEY_YT_RotateActSet(FALL_CHR_PARAM *fcp)
{
	...
	if(fcp->rotate_flag==fcp->line_no){
		fcp->speed_value=-KEY_YT_ROTATE_SPEED;
	}
	else{
		fcp->speed_value=KEY_YT_ROTATE_SPEED;
	}
 ...
 */
//-----------------------------------------------------------------------------
void	KEY_YT_EggMakeCheck(KEY_YT_PLAYER_STATUS *ps)
{
	int				line_no=0;
	int				egg_line;
	int				egg_height;
	...
	while(ps->status.egg_make_check_flag){
		if(ps->status.egg_make_check_flag&1){
			...
			chr_count=0;
			egg_count=1;
			for(egg_height=0;egg_height<KEY_YT_HEIGHT_MAX;egg_height++){
				fcp_top=ps->stop[egg_line][egg_height];
				if(fcp_top){
					if((fcp_top->type==KEY_YT_CHR_GREEN_EGG_D)||(fcp_top->type==KEY_YT_CHR_RED_EGG_D)){
						break;
					}
				}
			}
			if(egg_height==KEY_YT_HEIGHT_MAX){
				egg_height=0;
			}
			for(;egg_height<KEY_YT_HEIGHT_MAX;egg_height++){
				fcp_top=ps->stop[egg_line][egg_height];
				if(fcp_top){
					if((fcp_top->type==KEY_YT_CHR_GREEN_EGG_U)||(fcp_top->type==KEY_YT_CHR_RED_EGG_U)){
						GFL_CLACT_WK_SetSoftPri(fcp_top->clwk,KEY_YT_PRI_GREEN_EGG_U);
						break;
					}
				}
			}
			//見つからなかったらアサート
			GF_ASSERT(egg_height!=KEY_YT_HEIGHT_MAX);
			...
			while(egg_search){
				...
				case KEY_YT_CHR_GREEN_EGG_U:
				case KEY_YT_CHR_RED_EGG_U:
					egg_search=0;
					break;
				case KEY_YT_CHR_GREEN_EGG_D:
				case KEY_YT_CHR_RED_EGG_D:
					GFL_CLACT_WK_SetSoftPri(fcp_p->clwk,KEY_YT_PRI_GREEN_EGG_D);
					//タマゴではさんだ個数を計算
					fcp_p=ps->stop[egg_line][egg_height];
					fcp_p->chr_count=chr_count*egg_count;
					fcp_p->birth_wait=KEY_YT_BIRTH_WAIT*fcp_p->chr_count;
					...
					}
					{
						...
						while((ps->stop[egg_line][height])&&(height<KEY_YT_HEIGHT_MAX)){
							fcp_p=ps->stop[egg_line][height];
					...
					egg_search=0;
					break;
				case KEY_YT_CHR_GREEN_EGG:
				case KEY_YT_CHR_RED_EGG:
					egg_count++;
					chr_count+=fcp_p->chr_count;
					break;
				case KEY_YT_CHR_DEKATERESA:
					chr_count+=2;
					break;
				default:
					chr_count++;
...
static	void	KEY_YT_YossyBirth(GAME_PARAM *gp,FALL_CHR_PARAM *fcp)
{
	...
	}
    {
        ...
        KEY_YT_YossyBirthAnimeTaskSet(gp,ps,posx,posy,fcp->chr_count);
        KEY_YT_NetSendYossyBirthAnime(posx,posy,fcp->chr_count,gp->pNetParam);
    }
	...
	//タマゴの上にキャラがいた場合は、連鎖落下シーケンスをセット
	{
		...
		for(;height<KEY_YT_HEIGHT_MAX;height++){
			...
			if(fcp_rensa){
				...
				KEY_YT_AnmSeqSet(fcp_rensa,KEY_YT_ANM_STOP,pNet);
...
void KEY_YT_YossyBirthAnimeTaskSet(GAME_PARAM *gp,KEY_YT_PLAYER_STATUS *ps,u8 pos_x,u8 pos_y,u8 count)
{
	...
	GFL_TCB_AddTask(gp->tcbsys,KEY_YT_YossyBirthAnime,yba,TCB_PRI_PLAYER);
}

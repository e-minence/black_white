...
static	void	KEY_YT_MainGameAct(GAME_PARAM *gp);
static	void	KEY_YT_ExitGame(GAME_PARAM *gp);
static	void	KEY_YT_ClactResourceLoad( KEY_YT_CLACT_RES *clact_res, int flag, u32 heapID );
static	int		KEY_YT_ReadyCheck(GAME_PARAM *gp,KEY_YT_PLAYER_STATUS *ps);
static	void	KEY_YT_ReadyAct(GAME_PARAM *gp,int player_no);
static	BOOL	KEY_YT_FallCheck(GAME_PARAM *gp,KEY_YT_PLAYER_STATUS *ps);
static	void	KEY_YT_CheckFlag(GFL_TCB *tcb,void *work);
static	void	KEY_YT_AdjAllVanishCheck(GAME_PARAM *gp,int player_no,int *deka_flag,int *kara_flag);
 ...
 */
//-----------------------------------------------------------------------------
void	KEY_YT_InitGame(GAME_PARAM *gp)
{
    PLAYER_PARAM* pp;

    // セルアクターユニット作成
	gp->clact->p_unit = GFL_CLACT_UNIT_Create( KEY_YT_CLACT_MAX, gp->heapID );

	//エリアマネージャ初期化
	gp->clact_area=GFL_AREAMAN_Create(KEY_YT_CLACT_MAX,gp->heapID);
	...
	//セルアクターリソース読み込み
	KEY_YT_ClactResourceLoad(&gp->clact->res, gp->raster_flag, gp->heapID);
	...
	//フラグ関連初期
	{
		...
		GFL_STD_MemClear(&gp->ps[0],sizeof(KEY_YT_PLAYER_STATUS));
		GFL_STD_MemClear(&gp->ps[1],sizeof(KEY_YT_PLAYER_STATUS));

		for(x=0;x<KEY_YT_LINE_MAX;x++){
			gp->ps[0].falltbl[x]=x;
			gp->ps[1].falltbl[x]=x;
			gp->ps[0].stoptbl[x]=x;
			gp->ps[1].stoptbl[x]=x;
		...
		gp->default_fall_wait=KEY_YT_DEFAULT_FALL_WAIT;
	}
...
//		seed=date.year+date.month*0x100+date.day*0x10000+time.hour*0x100000+(time.minute+time.second)*0x1000000;
    ...
    if(gp->pNetParam){  // 通信にあわせてマリオの初期化を行います
      ...
      //      if(KEY_YT_NET_IsParent(gp->pNetParam) && (player_no == 1)){
        //        continue;
          //  }
           // else if(!KEY_YT_NET_IsParent(gp->pNetParam) && (player_no == 0)){
             //   continue;
//            }
            pp = KEY_YT_InitPlayer(gp,player_no,player_no,TRUE);
            KEY_YT_InitPlayerAddTask(gp, pp, player_no);
        }
    }
    else{
        pp = KEY_YT_InitPlayer(gp,0,0,FALSE);
        KEY_YT_InitPlayerAddTask(gp, pp, 0);
		gp->ps[0].exist_flag=1;
    }

	//ゲームフラグチェックタスクセット
	gp->check_tcb=GFL_TCB_AddTask(gp->tcbsys,KEY_YT_CheckFlag,gp,TCB_PRI_PLAYER);

	KEY_YT_JobNoSet(gp,KEY_YT_MainGameNo);
 ...
 */
//-----------------------------------------------------------------------------
void	KEY_YT_MainGame(GAME_PARAM *gp)
{
	//ゲームシーケンス処理
	KEY_YT_MainGameAct(gp);
...
//			}
//			else
  //          if(gp->ps[player_no].exist_flag==0){
	//			continue;
	//		}
			...
			case SEQ_GAME_READY_CHECK:
				switch(KEY_YT_ReadyCheck(gp,&gp->ps[player_no])){
				case KEY_YT_READY_MAKE:
					gp->ps[player_no].fall_count++;
					KEY_YT_ReadyAct(gp,player_no);
				case KEY_YT_READY_ALREADY:
					gp->ps[player_no].fall_wait=KEY_YT_FALL_WAIT;
					gp->game_seq_no[player_no]++;
					break;
				default:
					break;
			...
			case SEQ_GAME_FALL_CHECK:
				if(KEY_YT_FallCheck(gp,&gp->ps[player_no])==TRUE){
			...
			if(ps->fall[x][y]){
				{
					...
					if(pos.y<KEY_YT_READY_NEXT_Y_POS){
						return KEY_YT_READY_WAIT;
					}
				}
				break;
			}
...
static	void	KEY_YT_ReadyAct(GAME_PARAM *gp,int player_no)
{
	...
	KEY_YT_AdjAllVanishCheck(gp,player_no,&deka_flag,&kara_flag);
	...
	while(i){
		...
		//デカキャラは、確率を低くする
		if((type==KEY_YT_CHR_TERESA)&&(deka_flag!=2)){
			if((__GFL_STD_MtRand()%5==0)||(deka_flag==1)){
				type=KEY_YT_CHR_DEKATERESA;
			}
		}
		else{
			//タマゴの殻発生確率
			if((__GFL_STD_MtRand()%5==0)&&(kara_flag==0)){
				type=KEY_YT_CHR_GREEN_EGG_U+__GFL_STD_MtRand()%4;
			}
		}
		gp->ps[player_no].ready[line][0]=KEY_YT_InitFallChr(gp,player_no,type,line);
		i--;
	}
}
		...
		if(ps->status.overturn_flag){
			{
				...
				if((x==KEY_YT_LINE_MAX)&&(y==KEY_YT_HEIGHT_MAX)){
		...
		if(ps->status.no_active_flag==0){
			{
				...
				for(x=0;x<KEY_YT_LINE_MAX;x++){
					fcp=ps->stop[x][KEY_YT_HEIGHT_MAX-1];
					if(fcp){

  //                      if(gp->pNetParam){ //通信してる場合は勝敗を送る
    //                        KEY_YT_NET_SendGameResult(player_no,FALSE,gp->pNetParam);
      //                  }
        //                else{
                            ps->status.win_lose_flag|=KEY_YT_GAME_LOSE;
                            gp->ps[player_no^1].status.win_lose_flag|=KEY_YT_GAME_WIN;
          //              }
					}
				}
			}
		...
		if((ps->status.no_active_flag==0)&&(ps->fall_count>2)){
			{
				...
				if((x==KEY_YT_LINE_MAX)&&(y==KEY_YT_HEIGHT_MAX)){
					switch(KEY_YT_ReadyCheck(gp,ps)){
					case KEY_YT_READY_ALREADY:
						for(x=0;x<KEY_YT_LINE_MAX;x++){
							for(y=0;y<KEY_YT_HEIGHT_MAX;y++){
								if(ps->fall[x][y]){
					...
					case KEY_YT_READY_WAIT:

//                        if(gp->pNetParam){ //通信してる場合は勝敗を送る
        //                    KEY_YT_NET_SendGameResult(player_no,TRUE,gp->pNetParam);
  //                      }
    //                    else{
                            ps->status.win_lose_flag|=KEY_YT_GAME_WIN;
                            gp->ps[player_no^1].status.win_lose_flag|=KEY_YT_GAME_LOSE;
      //                  }
					default:
						break;
					}
...
static	void	KEY_YT_AdjAllVanishCheck(GAME_PARAM *gp,int player_no,int *deka_flag,int *kara_flag)
{
	...
	if(gp->ps[player_no].fall_count<KEY_YT_ADJ_FALL_COUNT){
		return;
	}

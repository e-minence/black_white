...
#endif
                adr++;
                CommRingPuts(&_pComm->recvServerRing[i], adr, mcSize-1, 1510);//__LINE__); // マスターと差が出るので数字に置き換えました 06.10.23
                adr += (mcSize-1);
...
//        DEBUG_DUMP(adr,recvSize,"子機データ受信");
        ...
        //}
        CommRingPuts(&_pComm->recvRing , adr, recvSize, 1535);//__LINE__); // マスターと差が出るので数字に置き換えました 06.10.23
    }
}
  ...
  //      }
        ...
        for(i = 1;i < mcSize; i++){
            if(adr[i] != CS_NONE){  //何かが送られてきてたら
                CommRingPuts(&_pComm->recvMidRing[aid] , adr, mcSize, 1622);//__LINE__); // マスターと差が出るので数字に置き換えました 06.10.23
...
//                OHNO_PRINT(" まるまま中間リングに入れる %d %d \n", aid, CommRingDataSize(&_pComm->recvMidRing[aid]));
        ...
        if(!(adr[0] & _SEND_NO_DATA)){
            CommRingPuts(&_pComm->recvMidRing[aid] , adr, mcSize, 1630);//__LINE__); // マスターと差が出るので数字に置き換えました 06.10.23
...
//            DEBUG_DUMP(adr,_RECV_BUFF_SIZE_PARENT,"MP 受け取った");
        CommRingPuts(&_pComm->recvServerRing[aid] , adr, _RECV_BUFF_SIZE_PARENT, 1646);//__LINE__); // マスターと差が出るので数字に置き換えました 06.10.23
    }
}

...
#コマンド引数
#NCGファイル、出力先、1コマの幅（キャラ）、1コマの高さ（キャラ）、1動作のアニメフレーム、動作数、反転処理開始コマナンバー(1オリジン)
ncg_file_name = ARGV.shift
output_path = ARGV.shift
width_height = ARGV.shift.to_i
one_tex_width = width_height
one_tex_height = width_height
frame = ARGV.shift.to_i
#↓ここを変更。コレだと反転フラグが固定になるので、外部ファイルをみて判定するようにする
#動作数、反転処理開始コマナンバー(1オリジン)は外部ファイルに格納されているとみなす
#num = ARGV.shift.to_i
#reverse_start = ARGV.shift.to_i
#↓変更後
rev_info = open(ARGV.shift,"r")
line = rev_info.gets
column = line.split ","
num = column[0].chomp.to_i
reverse_start = column[1].chomp.to_i
...
if (reverse_start > 0) then
	#反転テクスチャを追加する
	start = reverse_start-1
	max = frame
	n = 0
	
	while n < max
		idx = start+n
		rev_tex = maker[idx].MakeReverseTex
		maker << rev_tex
		n+=1
	end
end
...
#テンプレートを選択
template = ""
if (width_height == 8) then
	template = "poke_org64x64.imd"
elsif (width_height == 4) then
	template = "poke_org32x32.imd"
else
	p "OutputFailed!!"
	return
end

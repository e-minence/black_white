...
sub convData
{
	...
	print( FILEOUT_OFS pack( "i", oct(&getPeaceParamForTag( \@filedata, "ALLDATA", "OFSMAX") )) );
	print( FILEOUT_OFS pack( "i", oct(&getPeaceParamForTag( \@filedata, "ALLDATA", "OFSUNIT")) ) );
		...
		for( $i=0; $i<@LINEPOS_NAME; $i++ )
		{
			...
			do
			{
				...
				if( ($work_number != $BYTEWORK_INDEX_OVER) && ($one_datasiz != $BYTEWORK_INDEX_OVER) )
				{
					...
					if( $one_datasiz == 1 )
					{
						print( FILEOUT_LINEPOS pack( "c", oct($work_number) ) );
					}
					elsif( $one_datasiz == 2 )
					{
						print( FILEOUT_LINEPOS pack( "s", oct($work_number) ) );
					}
					elsif( $one_datasiz == 4 )
					{
						print( FILEOUT_LINEPOS pack( "i", oct($work_number) ) );
					}
					$datasiz += $one_datasiz;
		...
		for( $i=0; $i<@CAMERA_NAME; $i++ )
		{
			...
			do
			{
				...
				if( ($work_number != $BYTEWORK_INDEX_OVER) && ($one_datasiz != $BYTEWORK_INDEX_OVER) )
				{
					...
					if( $one_datasiz == 1 )
					{
						print( FILEOUT_CAMERA pack( "c", oct($work_number) ) );
					}
					elsif( $one_datasiz == 2 )
					{
						print( FILEOUT_CAMERA pack( "s", oct($work_number) ) );
					}
					elsif( $one_datasiz == 4 )
					{
						print( FILEOUT_CAMERA pack( "i", oct($work_number) ) );
					}
					$datasiz += $one_datasiz;

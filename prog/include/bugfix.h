...
//GFBTS2027【GTSでポケモンを探す時、検索失敗した後に「選ぶ」を押すとブラックアウト後停止】
#define BUGFIX_AF_GFBTS2027_2010107

//【ユニオン友達リストの追加が排他的になっていない】
// 同じ人とユニオン接続し続けていても３２人に到達してしまいます。
#define BUGFIX_AF_GFGTS2032_20101028
...
/*
 * GFBTS2018
 * ライモンジム：コースタートレーナー全滅時に、トレーナーを非表示に戻す
 *
 * -c04gym0101.ev
 *
 *  担当：斉藤(マ)
 */
#define BUGFIX_AF_GFBTS2018_100930

/*
 * GFBTS2019
 * 地下水脈の穴：手持ちが全滅した状態でタッグバトルに勝利したとき、手持ちの回復処理を行う
 *
 * -r03r0301.ev
 *
 *  担当：斉藤(マ)
 */
#define BUGFIX_AF_GFBTS2019_100930

/*
 * SYSTEM97
 * R13：カザカミの天候中にキャモメイベントを進めるとはまる
 *
 * -r13.ev
 *
 *  担当：斉藤(マ)
 */
#define BUGFIX_AF_SYSTEM97_101014
...
/*
 *  フィーリングチェックの仕様変更。（任天堂様、TPC様と検討していた０点になってしまう問題）
 *  １、シンクロチェックでタッチ操作ミスが起こったときに専用のメッセージを表示
 *  ２、結果表示画面で、０～９点のとき、メッセージ変更。
 *
 *  ２においては、GMMにて変更するので、コミット済みです。
 *  ですので、以下のdefineをなくしても２のみ反映されてしまいます。
 *
 *  担当：名木橋
 */
...
#define BUGFIX_AF_SYSTEM175_101101  //キャンセルした時には出ないように修正
 ...
 *【通信エラー表示中に回転しているリンクのSEが鳴り続ける】
 *
 * ABが対戦相手のサーチを行い、Aが「対戦相手を　探すのを　やめますか？」で
 * 「はい」を選択と同時にB側で「相手との　接続が　切断されました」のエラー
 * が発生した際、対戦相手サーチ画面で回転しているリンクのSEが通信エラー表示
 * 中でも鳴り続けています。
 * 日本版BTS7759の再報告です。
 *
 *  担当：名木橋
 */
#define BUGFIX_AF_SYSTEM165_101021


/*
  ポケモンドリームワールドの変更
  特定の状況でシンクIDコードを登録するとPDWとBWでポケモンの状態が不統一になる
「体験版」状態にて、ポケモンを寝かせていない状態の「ホワイト」のシンクIDコード
登録後、「PDW」に移動すると、直前に寝かせていたポケモンが「PDW」中で寝ている
状態となっています。
...
/*
> ■System #217
> 【侵入者がワープゾーンからハイリンクを退出すると同時にターゲットが
> 　Cギアの「WIRELESS」アイコンをタッチすると、ターゲットがとまる】
>
> 侵入者がハイリンク内にあるワープゾーン（自分のハイリンク、
> 他人のハイリンク問わず）からハイリンクを退出すると同時に
> ターゲットがCギアの「WIRELESS」アイコンをタッチすると、
> ターゲットの画面が暗転し、とまってしまいます。
>
> ・「WIRELESS」以外で発生するかどうかは未確認です。
>
> →NETのモードを切り替える際に dev_wireless オーバーレイが
> 　一瞬アンロードされますが、その間に「WIRELESS」アイコンを
> 　タッチすると、アンロードされた関数を呼び出して止まります。


  通信終了時に 通信のオーバーレイ先にある関数を呼んでしまうタイミングがございました。
  担当：大野
...
症状：
3人目のユーザーがミッションを達成した後、自分のハイリンクに
戻ってきた際に、ハイルツリーが表示されない。

再現手順：
2人目のユーザーのハイリンクで2人まで参加できるミッション
（アイテム隠しなど）を開始し、後から（C-GEARから）参加した
3人目のユーザーが先にミッションを達成する。

→upper_version (リビジョン44822) で再現を確認いたしました。
...
/*
> ■System #207
>【生まれたシキジカのフォルムが必ずはるのすがたになっている】
>
> シキジカのタマゴを受け取ってタマゴを孵すと、
> 季節にかかわらず必ずフォルムNo.0(はるのすがた)で生まれてしまいます。
>
  タマゴを受け取った直後に、手持ちポケモンのフォルムを季節と一致するようにセットします。
  （図鑑登録は行いません。）
  担当：小幡
*/
#define BUGFIX_AF_SYSTEM207_20101104


/*
 *  誤って適用してしまった“フリーフォール＆じゅうりょく問題”の修正箇所を無効にするための措置。
 *  バイナリレベルでのプログラムコード差異を極小化するため、このマクロを適用することで
 *  （誤適用により行われている）関数呼び出しを、他の、まったく無意味な関数呼び出しに置き換える。
 *
 *  担当：田谷
 */
#define FREEFALL_FIX_SAFE_REMOVAL

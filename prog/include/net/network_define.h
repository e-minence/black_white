...
#define SYASHI_DOWNLOAD_GGID  (0x00001381)   //映画２０１０専用 MB不具合用
//#define SYACHI_LEG_GGID (0x00001380)         //大会レギュレーション配信専用

#define NET_DREAMWORLD_VER (1)  //ドリームワールド通信バーション（かならず整数）


#define POKEMON_IRC_CONNECTCODE (12)  //赤外線接続コード(デバッグ用 本番申請中 2010.04.27)
...
//>├───┼──────────┼───────────┼─────────┼────┤
//>│1     │GTS                 │POKEMON_PASO_PARAM    │1                 │136     │
//>├───┼──────────┼───────────┼─────────┼────┤
//>│2     │GTSネゴシエーション │POKEMON_PASO_PARAM    │3                 │136     │
//>├───┼──────────┼───────────┼─────────┼────┤
//>│3     │バトルビデオ        │POKE_BATTLEVIDEO_PARAM│12                │108     │
//>├───┼──────────┼───────────┼─────────┼────┤
//>│4     │バトルサブウェイ    │_B_TOWER_POKEMON      │3                 │56      │
//>└───┴──────────┴───────────┴─────────┴────┘
#endif

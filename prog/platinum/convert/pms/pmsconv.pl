...
my $PMSAIKOTOBA_FILENAME = "pms_aikotoba.dat";
...
sub main {
	...
	else
	{
		...
		#t[ĥƂ
		if( $mode == MODE_AIKOTOBA ){
			output_aikotoba_list($ARGV[2]);
		}
	}
}
...
#===============================================================
# あいことば　生成用重複なし単語リスト tomoya 080407
#===============================================================
sub output_aikotoba_list {
	my $file = shift;
	my $path;
	my @aikotoba_file = ();
	my @aikotoba_idxfile = ();
	my @check_string = ('トレーナー', 'ステータス', 'ひと', 'せいかつ', 'きもち');
	my $aikotoba_use;
	my $i, $j;
	my $strid_a, $strid_b;
	my $skip;
	...
	foreach $tmp (@CategoryParam) {
		...
		#チェックするカテゴリが確認する
		$aikotoba_use = 0;
		for( $i=0; $i<@check_string; $i++ ){

			if( $$tmp[CPELEM_TITLE] =~ /$check_string[$i]/ ){
#				print $$tmp[CPELEM_TITLE]."\n";
				$aikotoba_use = 1;
			}
		}
		...
		if( $aikotoba_use == 1 ){
			...
			for ( $i=0; $i<@datStrID; $i++ ) {
				@aikotoba_idxfile = (@aikotoba_idxfile, $datStrID[$i] );
#				print $datStrID[$i]." : ".$SrcString[$datStrID[$i]]."\n";
			}
		}
	}

	@aikotoba_idxfile = sort {$a <=> $b} @aikotoba_idxfile;

	#重複しない単語だけのテーブルを、@aikotoba_fileに生成する
	for( $i=0; $i<@aikotoba_idxfile; $i++ ){

#		print $aikotoba_idxfile[$i]."\n";

		#重複チェック
		$strid_a = $aikotoba_idxfile[$i];
		$skip = 0;		
		for( $j=0; $j<@aikotoba_idxfile; $j++ ){
	
			if( $i != $j ){
				$strid_b = $aikotoba_idxfile[$j];
			
#				print $strid_a." - ".$strid_b."\n";
#				print $SrcString[$strid_a]." eq ".$SrcString[$strid_b]."\n";
				
				if( $SrcString[$strid_a] eq $SrcString[$strid_b] ){
#					print $strid_a." - ".$strid_b."\n";
#					print $SrcString[$strid_a]." eq ".$SrcString[$strid_b]."\n";
					$skip = 1;
				}
		...
		#重複しないものの単語IDだけ追加	
		if( $skip == 0 ){
			@aikotoba_file = (@aikotoba_file, $aikotoba_idxfile[$i]);
		}		
	}

	#ためしに表示する場合1
	if(0){
		for( $i=0; $i<@aikotoba_file; $i++ ){
			print $i." : ".$SrcString[$aikotoba_file[$i]]."\n";
		}
	}

	#あいことば単語データ書き出し
	open( FILEOUT, ">$PMSAIKOTOBA_FILENAME" );
	binmode( FILEOUT );
	for( $i=0; $i<@aikotoba_file; $i++ ){
		print( FILEOUT pack("I", $aikotoba_file[$i]) );
	}
	close( FILEOUT );
...
sub aikotoba_arc{
	...
	@dir_tbl	= split( /\\/, $path );
	$file		= $dir_tbl[ @dir_tbl-1 ];
	$dir_tbl[ @dir_tbl-1 ] = "";
	$path		= join( "\\\\", @dir_tbl );
	
	$sys_str = 'nnsarc -c -n '.$file.' '.$PMSAIKOTOBA_FILENAME;
	system( $sys_str );
	
	$sys_str = 'cp '.$file.' '.$path;
	system( $sys_str );

...
#define	REC_DATA_SEND_SIZE	(128)	//対戦録画データ送信バッファサイズ
...
//SCIO_SioWaitMessageWithRecData用
typedef struct{
	u8		command_code;		//命令コードインデックス
	u8		dummy;
	u16		size;
...
extern	void	SCIO_SioWaitMessageWithRecData(BATTLE_WORK *bw,int send_client);
...
extern	void	ScioBattleRecvData(BATTLE_WORK *bw,int flag);

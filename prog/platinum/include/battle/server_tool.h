...
extern	BOOL	ST_ServerTypeCheckTableParaGet(BATTLE_WORK *bw,int pos,u8 *type_w,u8 *type_p,u8 *value);
...
extern	BOOL	ST_ServerHukitobasiCheck(BATTLE_WORK *bw,SERVER_PARAM *sp);
...
extern	BOOL	ST_ServerPCCTokuseiCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int client_no);
...
extern	int		ST_DamageLossCalc(BATTLE_WORK *bw,SERVER_PARAM *sp,int damage);
extern	int		ST_CriticalCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int attack,int defence,int critical_count,u32 side_condition);
...
extern	BOOL	ST_BattleRecordStopCheck(BATTLE_WORK *bw,SERVER_PARAM *sp);

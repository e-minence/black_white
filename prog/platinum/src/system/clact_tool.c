...
static BOOL CATS_LoadResourceCellArcH_Private(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int type, int id);
 ...
 */
//--------------------------------------------------------------
BOOL CATS_LoadResourceCharArcH(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int trans_mode, int id)
{
	CLACT_U_RES_OBJ_PTR obj;
	...
	obj = CLACT_U_ResManagerResAddArcChar_ArcHandle(crp->resManager[0], arcHandle, data_id, comp, id, trans_mode, csp->heap_area);
 ...
 */
//--------------------------------------------------------------
s8 CATS_LoadResourcePlttArcH(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int num, int trans_mode, int id)
{
	CLACT_U_RES_OBJ_PTR obj;
	int ret;
	...
	obj = CLACT_U_ResManagerResAddArcPltt_ArcHandle(crp->resManager[1], arcHandle, data_id, comp, id, trans_mode, num, csp->heap_area);
...
u8 CATS_LoadResourcePlttWorkArcH(PALETTE_FADE_PTR pfd, FADEREQ req, CATS_SYS_PTR csp, 
	CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int num, int trans_mode, int id)
{
	int pal_no;

	pal_no = CATS_LoadResourcePlttArcH(csp, crp, arcHandle, data_id, comp, num, trans_mode, id);
 ...
 */
//--------------------------------------------------------------
BOOL CATS_LoadResourceCellArcH(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int id)
{
	return CATS_LoadResourceCellArcH_Private(csp, crp, arcHandle, data_id, comp, 2, id);
}
 ...
 */
//--------------------------------------------------------------
BOOL CATS_LoadResourceCellAnmArcH(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int id)
{
	return CATS_LoadResourceCellArcH_Private(csp, crp, arcHandle, data_id, comp, 3, id);
}
 ...
 */
//--------------------------------------------------------------
BOOL CATS_LoadResourceMCellArcH(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int id)
{
	return CATS_LoadResourceCellArcH_Private(csp, crp, arcHandle, data_id, comp, 4, id);
}
 ...
 */
//--------------------------------------------------------------
BOOL CATS_LoadResourceMCellAnmArcH(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int id)
{
	return CATS_LoadResourceCellArcH_Private(csp, crp, arcHandle, data_id, comp, 5, id);
}
...
static BOOL CATS_LoadResourceCellArcH_Private(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* arcHandle, int data_id, int comp, int type, int id)
{
	...
	obj = CLACT_U_ResManagerResAddArcKindCell_ArcHandle(crp->resManager[type], arcHandle, data_id, comp, id, type, csp->heap_area);
 ...
 */
//--------------------------------------------------------------
void CATS_ObjectUpdateCapEx( CATS_ACT_PTR cap )
{
	CLACT_AnmFrameChg( cap->act, FX32_CONST( 2 ) );
}
...
{
	return CLACT_GetAnmSeqNum( cap->act );
}
 ...
 */
//--------------------------------------------------------------
void CATS_ObjectAnimeSeqCheckSetCap( CATS_ACT_PTR cap, u32 seq )
{
	CLACT_AnmChgCheck( cap->act, seq );
}
 ...
 */
//--------------------------------------------------------------
void CATS_ObjectPosMoveCapFx32(CATS_ACT_PTR cap, fx32 x, fx32 y)
{
	const VecFx32* p_vec;
	VecFx32 p_vec2;
	
	p_vec = CLACT_GetMatrix( cap->act );
	
	p_vec2.x = p_vec->x + x;
	p_vec2.y = p_vec->y + y;
	p_vec2.z = p_vec->z;
	
	CLACT_SetMatrix( cap->act, &p_vec2 );	
}
 ...
 */
//--------------------------------------------------------------
void CATS_ObjectPosSetCapFx32(CATS_ACT_PTR cap, fx32 x, fx32 y)
{
	const VecFx32* p_vec;
	VecFx32 p_vec2;
 ...
 */
//--------------------------------------------------------------
void CATS_ObjectPosGetCapFx32(CATS_ACT_PTR cap, fx32* x, fx32* y)
{
	const VecFx32* p_vec;
	VecFx32 p_vec2;
 ...
 */
//--------------------------------------------------------------
void CATS_ObjectPosSetCapFx32_SubSurface(CATS_ACT_PTR cap, fx32 x, fx32 y, fx32 sub_surface_y)
{
	if( CLACT_VramTypeGet( cap->act ) == NNS_G2D_VRAM_TYPE_2DSUB ){
		CATS_ObjectPosSetCapFx32( cap, x, y + sub_surface_y );
	}
	else {
		CATS_ObjectPosSetCapFx32( cap, x, y );
	}
}
 ...
 */
//--------------------------------------------------------------
void CATS_ObjectPosGetCapFx32_SubSurface(CATS_ACT_PTR cap, fx32* x, fx32* y, fx32 sub_surface_y)
{
	CATS_ObjectPosGetCapFx32(cap, x, y);
...
void CATS_ObjectScaleGet(CLACT_WORK_PTR act, f32* sx, f32* sy)
{
	...
	*sx = FX_FX32_TO_F32( p_vec->x );
	*sy = FX_FX32_TO_F32( p_vec->y );
}
...
//----------------------------------------------------------------------------
/**
 *	@brief	アニメーションシーケンス拡張アトリビュート取得
 *
 *	@param	cap			ワーク
 *	@param	seqidx		取得するシーケンス数
 *
 *	@retval	ユーザー拡張アトリビュートデータ
 *	@retval	CLACT_USERATTR_NONE		データなし
 */
//-----------------------------------------------------------------------------
u32 CATS_ObjectUserAttrAnimSeqGet( CATS_ACT_PTR cap, u32 seqidx )
{
	return CLACT_GetUserAttrAnimSeq( cap->act, seqidx );
}

//----------------------------------------------------------------------------
/**
 *	@brief	今のアニメーションシーケンス拡張アトリビュート取得
 *
 *	@param	cap			ワーク
 *
 *	@retval	ユーザー拡張アトリビュートデータ
...
{
	return CLACT_GetUserAttrNowAnimSeq( cap->act );
}


//----------------------------------------------------------------------------
/**
 *	@brief	アニメーションフレーム拡張アトリビュート取得
 *
 *	@param	cap			ワーク
 *	@param	seqidx		取得するアニメーションシーケンス
 *	@param	frameidx	取得するアニメーションフレーム
 *
 *	@retval	ユーザー拡張アトリビュートデータ
 *	@retval	CLACT_USERATTR_NONE		データなし
 */
...
{
	return CLACT_GetUserAttrAnimFrame( cap->act, seqidx, frameidx );
}

//----------------------------------------------------------------------------
/**
 *	@brief	今のアニメーションフレーム拡張アトリビュート取得
 *
 *	@param	cap		ワーク
 *
 *	@retval	ユーザー拡張アトリビュートデータ
...
{
	return CLACT_GetUserAttrNowAnimFrame( cap->act );
}


//----------------------------------------------------------------------------
/**
 *	@brief	セルデータ拡張アトリビュート取得
 *
 *	@param	cap			ワーク
 *	@param	cellidx		セルデータIDX
 *
 *	@retval	ユーザー拡張アトリビュートデータ
 *	@retval	CLACT_USERATTR_NONE		データなし
 */
...
{
	return CLACT_GetUserAttrCell( cap->act, cellidx );
}
		...
		CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* hdl, int data_id, BOOL comp, int trans_mode, int id)
{
	...
	obj = CLACT_U_ResManagerResAddArcChar_ArcHandle( crp->resManager[0], hdl, data_id, comp, id, trans_mode, csp->heap_area);
//	obj = CLACT_U_ResManagerResAddArcChar(crp->resManager[0], arc_id, data_id, comp, id, trans_mode, csp->heap_area);
 ...
 */
//--------------------------------------------------------------
void CATS_ChangeResourceCharArcH(
		CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* p_handle, int data_id, BOOL comp, int id )
{
	...
	CLACT_U_ResManagerResChgArcChar_ArcHandle(
		crp->resManager[0], obj, p_handle, data_id, comp, csp->heap_area );
 ...
 */
//--------------------------------------------------------------
void CATS_ChangeResourcePlttArcH(
		CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* p_handle, int data_id, BOOL comp, int id )
{
	CLACT_U_RES_OBJ_PTR	obj;
	...
	CLACT_U_ResManagerResChgArcPltt_ArcHandle(
		crp->resManager[1], obj, p_handle, data_id, comp, csp->heap_area );

	CLACT_U_PlttManagerReTrans( obj );
}

void CATS_ChangeResourcePlttArcH_PlttWork( 
	CATS_SYS_PTR csp, CATS_RES_PTR crp, PALETTE_FADE_PTR pfd, FADEREQ req, ARCHANDLE* p_handle, int data_id, BOOL comp, int id, int num, NNS_G2D_VRAM_TYPE type )
{
	CLACT_U_RES_OBJ_PTR	obj;
	int pal_no;
	...
	obj = CLACT_U_ResManagerGetIDResObjPtr( crp->resManager[1], id );
	CLACT_U_ResManagerResChgArcPltt_ArcHandle( crp->resManager[1], obj, p_handle, data_id, comp, csp->heap_area );
	obj = CLACT_U_ResManagerGetIDResObjPtr( crp->resManager[1], id );
	pltt   = CLACT_U_ResManagerGetResObjResPltt( obj );
	pal_no = CLACT_U_PlttManagerGetPlttNo( obj, type );
	
	base = pltt->pRawData;
		
	if (pal_no != -1){
		PaletteWorkSet( pfd, base, req, pal_no * 16, num * 0x20);
	}

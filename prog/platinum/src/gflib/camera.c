...
static void SetCamPosByTarget_Dist_Ang(GF_CAMERA_PTR camera_ptr)
{
	...
	camera_ptr->lookat.camPos.z =
		...
		FX_Mul(
			FX_Mul(FX_CosIdx(camera_ptr->Angle.y),camera_ptr->Distance),
			   FX_CosIdx(camera_ptr->Angle.x)
			   );
...
void GFC_CameraLookAtZ(void)
{
	...
	if (GF_Camera->BindTarget != NULL){
		...
		{
			VecFx32 camPos = GF_Camera->lookat.camPos;
			VecFx32 camUp = GF_Camera->lookat.camUp;
			const VecFx32 *target = GF_Camera->BindTarget;
//			fx16 sinx = FX_SinIdx(GF_Camera->Angle.x);
			fx16 cosx = FX_CosIdx(GF_Camera->Angle.x);
			fx16 siny = FX_SinIdx(GF_Camera->Angle.y);
			fx16 cosy = FX_CosIdx(GF_Camera->Angle.y);
			fx16 sinz = FX_SinIdx(GF_Camera->Angle.z);
			fx16 cosz = FX_CosIdx(GF_Camera->Angle.z);
			
			camPos.z = FX_Mul( cosy, GF_Camera->Distance );
			camPos.z = FX_Mul( camPos.z, cosx );
			camPos.z += target->z;
			camUp.x += FX_Mul( sinz, cosy );
			camUp.y += cosz;
			camUp.z += -FX_Mul( sinz, siny );
		...
		NNS_G3dGlbLookAt(
			&GF_Camera->lookat.camPos,
			&GF_Camera->lookat.camUp,
			&GF_Camera->lookat.target);
	}
}

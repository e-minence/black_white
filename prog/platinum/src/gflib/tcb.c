...
#ifdef PM_DEBUG
#define DEBUG_TCB_ADD_ERROR_PRINT_MAX
#endif
...
static TCB_PTR AddTask( TCBSYS* tcbsys, TCB_FUNC func, void* work, u32 pri )
{
	...
	if(get == NULL)
	{
		#ifdef DEBUG_TCB_ADD_ERROR_PRINT_MAX
		OS_Printf( "TCB ADD ERROR !! " );
		OS_Printf( " \"tcb_max = %d, ", tcbsys->tcb_max );
		OS_Printf( "tcb stack = %d\" ", tcbsys->tcb_stack_ptr );
		#endif
		//登録しようとしたがタスクがなかった

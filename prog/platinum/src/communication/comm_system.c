...
static void _commWorkClear(void)
{
    ...
    // サーバ側送受信エリアの初期化
    MI_CpuClear8(_pComm->pServerRecvBufRing, _pComm->packetSizeMax * machineMax);
    ...
    MI_CpuClear8(_pComm->pMidRecvBufRing, _pComm->packetSizeMax * machineMax );
    for(i = 0; i < machineMax; i++){
...
static void _commRecvParentCallback(u16 aid, u16 *data, u16 size)
{
    ...
    if(_transmissonType() == _DS_MODE){
        ...
        if(!(adr[0] & _SEND_NO_DATA)){
            CommRingPuts(&_pComm->recvMidRing[aid] , adr, mcSize);
        }
...
//        DEBUG_DUMP(adr,12,"ca");
        ...
        if(adr[0] == _INVALID_HEADER){
            ...
            adr++;
            CommRingPuts(&_pComm->recvServerRing[aid], adr, mcSize-1);
            _pComm->bFirstCatch[aid]=FALSE;
        }
    }
}

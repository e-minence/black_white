...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_FNOTE)
	SVLD_SetCrc(GMDATA_ID_FNOTE);
#endif //CRC_LOADCHECK
}
...
#if	S2423_060817_FIX	// 修正後
	// 12/31 <-> 1/1 の場合は、差が2年以上
...
static void FNOTE_SioDataSave( FNOTE_DATA * dat, FNOTE_WK_SIO * wk )
{
	...
	case FNOTE_ID_PL_LOBBY_CHAT:	// 通信：WIFI広場おしゃべり(挨拶と同じくIDと名前のみ)
	case FNOTE_ID_PL_LOBBY_TOY_GET:	// 通信：WIFI広場タッチトイをもらった
	...
	case FNOTE_ID_PL_LOBBY_MINIGAME:// 通信：WIFI広場ミニゲーム(コンテストと同じくresultを使用)
 ...
 * @param	result	順位
 * @param	heap	ヒープID
 * @param   id		FNOTE_ID_PL_TOWER ～ FNOTE_ID_PL_ROULETTE
 *
 * @return	作成したワーク
 */
//--------------------------------------------------------------------------------------------
void * FNOTE_ActionFrontierDataMake( u32 heap, u32 id )
{
	FNOTE_WK_ACTION * wk = FNOTE_ActionWorkAlloc( heap );
	wk->id  = id;
	return wk;
...
//--------------------------------------------------------------------------------------------
/**
 * データ作成 [ 通信：データIDのみの指定全般 ]
 *
 * @param	heap	ヒープID
 * @param   id		通信系のFNOTE_ID_???
 *
 * @return	作成したワーク
 */
//--------------------------------------------------------------------------------------------
void * FNOTE_SioIDOnlyDataMake( u32 heap, u32 id )
{
	FNOTE_WK_SIO * wk = FNOTE_SioWorkAlloc( heap );
	wk->id     = id;
	return wk;
 ...
 */
//--------------------------------------------------------------------------------------------
void * FNOTE_Sio_ID_Name_DataMake( STRCODE * name, u8 sex, u32 heap, u32 id )
{
	FNOTE_WK_SIO * wk = FNOTE_SioWorkAlloc( heap );
	wk->id     = id;
	wk->sex1   = sex;
...
//--------------------------------------------------------------------------------------------
/**
 * データ作成 [ 通信：WIFI広場ミニゲーム ]
 *
 * @param   game_type	WFLBY_GAMETYPE
 * @param	result		順位
 * @param	heap		ヒープID
 *
 * @return	作成したワーク
 */
//--------------------------------------------------------------------------------------------
void * FNOTE_SioLobbyMinigameDataMake(int game_type, u32 heap, u32 id )
{
	FNOTE_WK_SIO * wk = FNOTE_SioWorkAlloc( heap );
	wk->id     = id;
	wk->result = game_type;

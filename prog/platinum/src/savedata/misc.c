...
struct _MISC {
	...
	u16 gds_send_box:10;		//GDS:ボックス送信済みフラグ(1bit=1box)
	u16 gds_send_dress:1;		//GDS:ドレスアップ送信済みフラグ
	u16 gds_send_video:1;		//GDS:バトルビデオ送信済みフラグ
	u16 			:4;			//余り
...
//--------------------------------------------------------------
/**
 * @brief   GDS：ボックス送信済みフラグをセットする
 *
 * @param   misc		
 * @param   box_no		ボックス番号
 * @param   flag		0=フラグリセット、1=フラグセット(送信済み扱い)
 */
//--------------------------------------------------------------
void MISC_SetGdsSend_Box(MISC *misc, int box_no, int flag)
{
	if(flag == 0){
		misc->gds_send_box &= 0x3ff ^ (1 << box_no);
	}
	else{
		misc->gds_send_box |= 1 << box_no;
	}
}

//--------------------------------------------------------------
/**
 * @brief   GDS：ボックス送信済みフラグを取得する
 *
 * @param   misc		
 * @param   box_no		ボックス番号
 *
 * @retval  TRUE:送信済み、　FALSE:まだ送信していない
 */
//--------------------------------------------------------------
BOOL MISC_GetGdsSend_Box(MISC *misc, int box_no)
{
	if(misc->gds_send_box & (1 << box_no)){
		return TRUE;
	}
	return FALSE;
}
...
//--------------------------------------------------------------
/**
 * @brief   GDS：ドレス送信済みフラグをセットする
 *
 * @param   misc		
 * @param   flag		0=フラグリセット、1=フラグセット(送信済み扱い)
 */
//--------------------------------------------------------------
void MISC_SetGdsSend_Dress(MISC *misc, int flag)
{
	misc->gds_send_dress = flag;
}

//--------------------------------------------------------------
/**
 * @brief   GDS：ドレス送信済みフラグを取得する
 *
 * @param   misc		
 *
 * @retval  TRUE:送信済み、　FALSE:まだ送信していない
 */
//--------------------------------------------------------------
BOOL MISC_GetGdsSend_Dress(MISC *misc)
{
	return misc->gds_send_dress;
}

//--------------------------------------------------------------
/**
 * @brief   GDS：バトルビデオ送信済みフラグをセットする
 *
 * @param   misc		
 * @param   flag		0=フラグリセット、1=フラグセット(送信済み扱い)
 */
//--------------------------------------------------------------
void MISC_SetGdsSend_Video(MISC *misc, int flag)
{
	misc->gds_send_video = flag;
}

//--------------------------------------------------------------
/**
 * @brief   GDS：バトルビデオ送信済みフラグを取得する
 *
 * @param   misc		
 *
 * @retval  TRUE:送信済み、　FALSE:まだ送信していない
 */
//--------------------------------------------------------------
BOOL MISC_GetGdsSend_Video(MISC *misc)
{
	return misc->gds_send_video;
}

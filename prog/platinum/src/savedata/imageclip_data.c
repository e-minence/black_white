 ...
 */
//-----------------------------------------------------------------------------
static void get_poke_mat( IMC_POKE* ip, u8* x, u8* y, s8* pri )
{
	int gx, gy;
	int gpri;
...
static u8 imcsavedata_Acce9Get( const u32* p_arry, u8 idx )
{
	...
	// 9こ以上はいけないので、9こにする
	// 2007/01/09　バグ技で9より大きい数が入っている可能性があるため修正　
 ...
 */
//-----------------------------------------------------------------------------
static void clean_item_savedata( IMC_ITEM_SAVEDATA* wk )
{
	int i;
...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_IMAGECLIPDATA)
	SVLD_CheckCrc(GMDATA_ID_IMAGECLIPDATA);
#endif //CRC_LOADCHECK
...
//	GT_DRESS
//==============================================================================
#include "application/battle_recorder/gds_dressup.h"
...
void ImcTelevision_to_GTDress(const IMC_TELEVISION_SAVEDATA * dress, GT_GDS_DRESS * gt_dress)
{
	...
	MI_CpuClear8(gt_dress, sizeof(GT_GDS_DRESS));
	
//	MI_CpuCopy8(dress, gt_dress, ImcSaveData_GetTelevisionWorkSize());
	gt_dress->init_flag = dress->init_flag;
	//gt_dress->pokemon = dress->pokemon;
	{
		gt_dress->pokemon.personal_rnd = dress->pokemon.personal_rnd;
		gt_dress->pokemon.idno = dress->pokemon.idno;
		gt_dress->pokemon.monsno = dress->pokemon.monsno;
		for(i = 0; i < GT_IMC_SAVEDATA_OYANAME_STR_NUM; i++){
			gt_dress->pokemon.oyaname[i] = dress->pokemon.oyaname[i];
		}
		gt_dress->pokemon.poke_pri = dress->pokemon.poke_pri;
		gt_dress->pokemon.poke_x = dress->pokemon.poke_x;
		gt_dress->pokemon.poke_y = dress->pokemon.poke_y;
		gt_dress->pokemon.form_id = dress->pokemon.form_id;
	}
	gt_dress->accessory_set_msk = dress->accessory_set_msk;
	gt_dress->title = *((GT_PMS_DATA*)(&dress->title));
	for(i = 0; i < GT_IMC_SAVEDATA_TELEVISION_ACCE_NUM; i++){
		gt_dress->acce[i] = *((GT_IMC_SAVEDATA_ACCESSORIE *)(&dress->acce[i]));
	}
...
void GTDress_to_ImcTelevision(const GT_GDS_DRESS * gt_dress, IMC_TELEVISION_SAVEDATA * dress)
{
	...
	tv_size = ImcSaveData_GetTelevisionWorkSize();
	
	MI_CpuClear8(dress, tv_size);

	dress->init_flag = gt_dress->init_flag;
	//dress->pokemon = gt_dress->pokemon;
	{
		dress->pokemon.personal_rnd = gt_dress->pokemon.personal_rnd;
		dress->pokemon.idno = gt_dress->pokemon.idno;
		dress->pokemon.monsno = gt_dress->pokemon.monsno;
		for(i = 0; i < IMC_SAVEDATA_OYANAME_STR_NUM; i++){
			dress->pokemon.oyaname[i] = gt_dress->pokemon.oyaname[i];
		}
		dress->pokemon.poke_pri = gt_dress->pokemon.poke_pri;
		dress->pokemon.poke_x = gt_dress->pokemon.poke_x;
		dress->pokemon.poke_y = gt_dress->pokemon.poke_y;
		dress->pokemon.form_id = gt_dress->pokemon.form_id;
	}
	dress->accessory_set_msk = gt_dress->accessory_set_msk;
	dress->title = *((PMS_DATA *)(&gt_dress->title));
	for(i = 0; i < GT_IMC_SAVEDATA_TELEVISION_ACCE_NUM; i++){
		dress->acce[i] = *((IMC_SAVEDATA_ACCESSORIE *)(&gt_dress->acce[i]));
	}
	dress->bg_id = gt_dress->bg_id;
	dress->country = gt_dress->country;
	
	//GT_GDS_DRESSには無いパラメータの初期値のセット
	for(i = 0; i < IMC_SAVEDATA_NICKNAME_STR_NUM; i++){
		dress->pokemon.nickname[i] = EOM_;
	}
	dress->pokemon.oya_sex = PM_MALE;
}

...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_GUINNESS)
	SVLD_SetCrc(GMDATA_ID_GUINNESS);
#endif //CRC_LOADCHECK
}
...
{
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_GUINNESS)
	SVLD_CheckCrc(GMDATA_ID_GUINNESS);
#endif //CRC_LOADCHECK
...
static u32* guinness_AllocMyRecord(SAVEDATA* sv,int heapID)
{
	...
	for(i = 0;i < GNS_RECORD_NUM;i++){
		...
		case GNSID_GLC_GPXPER:	//コンテスト優勝率
			...
			break;
		default:
			if(i >= GNSID_GLB_SINGLE && i <= GNSID_GLB_WIFI){
				data[i] = FrontierRecord_Get(frontier, recid[i], FRONTIER_RECORD_NOT_FRIEND);
			}
			else{

...
static void NutsTagBgGraphicSet( NTAG_WORK * wk, ARCHANDLE* p_handle );
...
static void NutsGraphicSet( NTAG_WORK * wk, ARCHANDLE* p_handle );
...
PROC_RESULT NutsTagProc_Init( PROC * proc, int * seq )
{
	...
	NutsTagBgGraphicSet( wk, p_handle );			// BGグラフィックセット
	NutsTagAlphaSet();
...
static void NutsTagBgGraphicSet( NTAG_WORK * wk, ARCHANDLE* p_handle )
{
	ArcUtil_HDL_BgCharSet(
		p_handle, NARC_ntag_gra_kinomi_bg_NCGR,
		wk->bgl, GF_BGL_FRAME2_M, 0, 0, 0, HEAPID_NUTSTAG );
	ArcUtil_HDL_ScrnSet(
		p_handle, NARC_ntag_gra_kinomi_bg_NSCR,
		wk->bgl, GF_BGL_FRAME2_M, 0, 0, 0, HEAPID_NUTSTAG );
	ArcUtil_HDL_ScrnSet(
		p_handle, NARC_ntag_gra_kinomi_bg02_NSCR,
...
{
	ArcUtil_HDL_BgCharSet(
		p_handle, NutsCgxArcGet(wk->dat->now_tag),
		wk->bgl, NUTS_GRA_FRM, NUTS_GRA_CGX, 0, 0, HEAPID_NUTSTAG );
	ArcUtil_HDL_PalSet(
		p_handle, NutsPalArcGet(wk->dat->now_tag),

...
static int panm_PokeVoiceSet(PORU_DEMO_WORK* wk,PORUDEMO_ANM* anm)
{
	...
	formid = PokeParaGet( wk->poke.pp, ID_PARA_form_no, NULL );
	switch(wk->poke.favolite){
	case PORU_FAVOLITE_KIRAI:
		Snd_PMVoicePlayEx( PV_PINCHNORMAL,wk->poke.monsno, 0, 127,wk->heapID, formid);
		break;
	default:
		Snd_PMVoicePlayEx( PV_NORMAL,wk->poke.monsno, 0, 127,wk->heapID, formid);
		break;
	}

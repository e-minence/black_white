...
//	memset( wk, 0, sizeof( BB_WORK ) );
	...
	{		
		...
		if ( wk->rule.ball_type == 100 ){
			///< レアゲーム
			int ball_res[] = {
				NARC_manene_r_ball_red_nsbmd,
				NARC_manene_r_ball_blue_nsbmd,
				NARC_manene_r_ball_yellow_nsbmd,
				NARC_manene_r_ball_green_nsbmd,
			};
			...
			BB_disp_Model_Load( &wk->p_client->bb3d_mane[ 0 ], wk->sys.p_handle_bb, NARC_manene_r_manene_nsbmd );
			BB_disp_Model_Load( &wk->p_client->bb3d_mane[ 1 ], wk->sys.p_handle_bb, NARC_manene_r_manene_ochita_nsbmd );
			BB_disp_Model_Load( &wk->p_client->bb3d_mane[ 2 ], wk->sys.p_handle_bb, NARC_manene_r_manene_fukki_nsbmd );		
			...
			///< アニメ設定
			{
				...
				///< 歩く
				anime = NARC_manene_r_manene_aruku_nsbca;
				...
				anime = NARC_manene_r_manene_ochita_nsbca;			
				D3DOBJ_AnmLoadH( &b3d->anm[ 0 ], &b3d->mdl, wk->sys.p_handle_bb, anime, HEAPID_BB, &wk->sys.allocator );
				D3DOBJ_AddAnm( &b3d->obj, &b3d->anm[ 0 ] );
				D3DOBJ_AnmSet( &b3d->anm[ 0 ], 0 );
				...
				anime = NARC_manene_r_manene_fukki_nsbca;			
				D3DOBJ_AnmLoadH( &b3d->anm[ 0 ], &b3d->mdl, wk->sys.p_handle_bb, anime, HEAPID_BB, &wk->sys.allocator );
				D3DOBJ_AddAnm( &b3d->obj, &b3d->anm[ 0 ] );
				D3DOBJ_AnmSet( &b3d->anm[ 0 ], 0 );
			}
//			OS_Printf( "レアゲームの読み込み\n" );
		}
		else {
			int ball_res[] = {
...
PROC_RESULT BalanceBallProc_Main( PROC* proc, int* seq )
{
	...
	case eBB_SEQ_RESULT:
		...
		if ( wk->result.score < BB_SCORE_MIN ){
			 wk->result.score = BB_SCORE_MIN;
		}
...
//		GF_BGL_BGControlSet( bgl, GF_BGL_FRAME0_M, &TextBgCntDat[ 0 ], GF_BGL_MODE_TEXT );
	...
	G2_SetBlendAlpha(  0, GX_BLEND_PLANEMASK_BG3 | GX_BLEND_PLANEMASK_OBJ, 7, 10 );					  

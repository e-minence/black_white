...
static void ImcRightSetAcceList( IMC_RIGHT_BOX* p_data, IMC_OBJLIST* objList, BOOL top_bottom, NNSG2dCharacterData** SWSP_UPchar  )
{
	...
	top = y + in_size_y;
	bottom = y + size_y - in_size_bottom;
	left = x + in_size_x;
	right = x + size_x - in_size_right;
#else
...
static void imcRbox_GetAcceEncSize( IMC_OBJLIST* objlist, int* enc_x, int* enc_y )
{
...
#if PLFIX_S2407
	int in_size_right, in_size_bottom;	// 食い込んでよいサイズXY
#endif
	...
	top		= mat_y + in_size_y;
	bottom	= mat_y + size_y - in_size_bottom;
	left	= mat_x + in_size_x;
	right	= mat_x + size_x - in_size_right;

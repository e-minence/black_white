...
static void IMC_PrevMainFrameSet( IMC_PREV_WORK* p_ip )
{
	...
	ArcUtil_HDL_BgCharSet( p_ip->draw_data.p_handle, 
	...
	p_scrn_buff = ArcUtil_HDL_ScrnDataGet( p_ip->draw_data.p_handle, 
			scrn_idx, FALSE,
...
static void IMC_PrevOamWndSet( IMC_PREV_WORK* p_ip )
{
	...
	IMC_DRAW_CLACT_LoadPlttData( &p_ip->draw_data, p_ip->draw_data.p_handle,
	...
	IMC_DRAW_CLACT_LoadCellData( &p_ip->draw_data, p_ip->draw_data.p_handle,
			NARC_imageclip_imgclip_win_contv_NCER, FALSE,
			IMC_PREV_MAIN_FRAME_OAM_CONT_ID );

	IMC_DRAW_CLACT_LoadCellAnmData( &p_ip->draw_data, p_ip->draw_data.p_handle,

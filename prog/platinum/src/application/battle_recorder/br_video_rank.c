...
static void ListPokeIcon_Add( BR_WORK* wk )
{
	...
	for ( i = 0; i < 5; i++ ){
		...
		for ( j = 0; j < 6; j++ ){
			monsno = wk->recv_data.outline_data_tmp[ now + i ]->head.monsno[ tbl[ tbl_index ][ j ] ];
			form   = wk->recv_data.outline_data_tmp[ now + i ]->head.monsno[ tbl[ tbl_index ][ j ] ];
			egg	   = 0;
...
static void ListPokeIcon_ReAdd( BR_WORK* wk )
{
	...
	
	{
		...
		for ( j = 0; j < 6; j++ ){
			monsno = wk->recv_data.outline_data_tmp[ pos ]->head.monsno[ tbl[ tbl_index ][ j ] ];
			form   = wk->recv_data.outline_data_tmp[ pos ]->head.monsno[ tbl[ tbl_index ][ j ] ];
			egg	   = 0;
			OS_Printf( "monsno = %d\n", monsno );
...
//	result = BattleRec_GTRec_Save( wk->save, &rwk->save_data, no );
	result = BattleRec_GDS_RecvData_Save( wk->save, no );

...
// br_tool.c
typedef struct _FADE_SYS * FADE_SYS_PTR;

extern BOOL IsPaletteSoftFade( FADE_SYS_PTR psp );
extern void PaletteSoftFadeDelete( FADE_SYS_PTR psp );
extern FADE_SYS_PTR PaletteSoftFade( PALETTE_FADE_PTR pfd, int heap_area, FADEREQ req,
								   u16 start_pos, u16 col_num, s8 wait, s8 param, 
								   u8 start_evy, u8 end_evy, u16 next_rgb, int tcb_pri );

extern BOOL IsFadeCheck( FADE_SYS_PTR psp );
...
extern void TouchList_InitRanking( TOUCH_LIST* tl, BR_WORK* wk, MSGDATA_MANAGER* man, const TOUCH_LIST_HEADER* head, int group_no, STRBUF* unit );
extern void TouchList_InitBoxShot( TOUCH_LIST* tl, BR_WORK* wk, const TOUCH_LIST_HEADER* head, STRBUF* str[20][2] );
extern int TouchList_Ranking_Main( TOUCH_LIST* tl, BR_WORK* wk, int group_no, STRBUF* unit );
...
extern void RedData_To_brs( BR_WORK* wk );
extern void RecData_Save( BR_WORK* wk, int no );

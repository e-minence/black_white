...
{
	FADE_SYS_PTR wk = (FADE_SYS_PTR)work;
 ...
 * @brief	状態チェック
 *
 * @param	psp			FADE_SYS_PTR
 *
 * @retval	BOOL		TRUE = 実行中
 *
 */
//--------------------------------------------------------------
BOOL IsPaletteSoftFade(FADE_SYS_PTR psp)
{
	GF_ASSERT(psp != NULL);
 ...
 */
//--------------------------------------------------------------
void PaletteSoftFadeDelete(FADE_SYS_PTR psp)
{
	GF_ASSERT(psp != NULL);
 ...
 */
//--------------------------------------------------------------
FADE_SYS_PTR PaletteSoftFade(PALETTE_FADE_PTR pfd, int heap_area, FADEREQ req,
							u16 start_pos, u16 col_num,
							s8 wait, s8 param,
							u8 start_evy, u8 end_evy, 
							u16 next_rgb, int tcb_pri)
{
	FADE_SYS_PTR wk = NULL;
	
	wk = sys_AllocMemory(heap_area, sizeof(FADE_SYS));
 ...
 */
//--------------------------------------------------------------
BOOL IsFadeCheck( FADE_SYS_PTR psp )
{
	if ( psp == NULL ){
 ...
 *
 * @param	trtype	
 * @param	data	1 = TR_TYPE, 2 = ncl, 3 = ncg
 *
 * @retval	int	
 *
 */
...
{
	return wifi_tr_type_tbl[ trtype ][ data ];
#if 0	
	int i;

	for ( i = 0; i < BR_UNION_CHAR_MAX; i++ ){
		
		if ( wifi_tr_type_tbl[ i ][ 0 ] != trtype ){ continue; }
		
		return wifi_tr_type_tbl[ i ][ data ];
	}
	
	return wifi_tr_type_tbl[ 0 ][ data ];
#endif
}
...
void TouchList_InitRanking( TOUCH_LIST* tl, BR_WORK* wk, MSGDATA_MANAGER* man, const TOUCH_LIST_HEADER* head, int group_no, STRBUF* unit )
{
	int i;
...
//	tl->bView = TRUE;
	...
	for ( i = 0; i < tl->lim; i++ ){		
		...
		STRBUF_SetNumber( str2, i + 1, 2, NUMBER_DISPTYPE_SPACE, NUMBER_CODETYPE_DEFAULT);			
 ...
 */
//--------------------------------------------------------------
int TouchList_Ranking_Main( TOUCH_LIST* tl, BR_WORK* wk, int group_no, STRBUF* unit )
{
	int i;
	int param = 0;	
	GF_BGL_BMPWIN* win;
...
{	
//	CATS_SystemActiveSet( br_wk, TRUE );
...
//	CATS_SystemActiveSet( br_wk, FALSE );
}
 ...
 */
//--------------------------------------------------------------
void RedData_To_brs( BR_WORK* wk )
{
...
void BR_ChangeDisplayVIntr( BR_WORK* wk, int mode )
{
	...
	VIntrTCB_Add( DispChange_TCB, &wk->disp, 4 );
}
...
{
	wk->view_tcb = VIntrTCB_Add( DrawTCB, wk, 3 );
}
 ...
 */
//--------------------------------------------------------------
void RecData_Save( BR_WORK* wk, int no )
{
	SAVE_RESULT result;
//	VIDEO_SEARCH_WORK* vwk = wk->sub_work;	
//	GT_BATTLE_REC_RECV* data = vwk->rec_data;
//	GT_BATTLE_REC_RECV* data = ( GT_BATTLE_REC_RECV* )BattleRec_WorkAdrsGet();

	result = BattleRec_GDS_RecvData_Save( wk->save, no );

	OS_Printf( "brs_save result = %d\n", result );
}
...
void BR_Response_BattleVideoSearch(void *work, const GDS_RAP_ERROR_INFO *error_info)
{
	...
	if( error_info->occ == TRUE ){
		...
		
		{
			...
			#if 0
				wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 3 ] = wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 0 ];
				wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 4 ] = wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 1 ];
				wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 5 ] = wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 2 ];
				wk->recv_data.outline_data_tmp[ recv_data->outline_data_num + i ] = wk->recv_data.outline_data_tmp[ i ];
				OS_Printf( "monsno = %d\n", wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 0 ] );
				OS_Printf( "monsno = %d\n", wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 1 ] );
				OS_Printf( "monsno = %d\n", wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 2 ] );
				OS_Printf( "monsno = %d\n", wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 3 ] );
				OS_Printf( "monsno = %d\n", wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 4 ] );
				OS_Printf( "monsno = %d\n", wk->recv_data.outline_data_tmp[ i ]->head.monsno[ 5 ] );
			#endif
			}
		//	recv_data->outline_data_num *= 2;
		}
	//	MI_CpuCopy8( &wk->recv_data.outline_data, wk->recv_data.outline_data_tmp, sizeof( GT_BATTLE_REC_OUTLINE_RECV ) * wk->recv_data.outline_data_num );
	}
...
{
	SAVE_RESULT res = BattleRec_SaveDataErase( wk->save, HEAPID_BR, wk->ex_param1 );
			
	if ( res == SAVE_RESULT_NG ){		
		GF_ASSERT( 0 );	
		return FALSE;
	}
	return TRUE;
...
static void TouchActionInit( BR_WORK* wk )
{
	...
	wk->touch_work.tcb = TCB_Add( TouchAction_TCB, wk, 0x1000 );

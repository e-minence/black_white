...
{	
	...
	case 1:
		if ( BR_PaletteFade( &wk->sys.master_color, eFADE_MODE_OUT ) ){
			wk->sub_seq++;
		}
		ColorConceChangePfd( wk->sys.pfd, FADE_MAIN_OBJ, TAG_PALETTE, 			 wk->sys.master_color, wk->sys.logo_color );
		ColorConceChangePfd( wk->sys.pfd, FADE_SUB_OBJ,	 LINE_OTHER_SUB_PALETTE, wk->sys.master_color, wk->sys.logo_color );
		ColorConceChangePfd( wk->sys.pfd, FADE_SUB_BG,	 LOGO_PALETTE, 			 wk->sys.master_color, wk->sys.logo_color );
		break;

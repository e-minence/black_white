...
//	プロトタイプ宣言
//==============================================================================
static int GDSRAP_SEQ_GDS_Initialize(GDS_RAP_WORK *gdsrap, GDS_RAP_SUB_PROCCESS_WORK *sub_work);
...
//==============================================================================
///インターネット接続
static const SUBPROCESS_FUNC SubSeq_InternetConnect[] = {
//	GDSRAP_SEQ_InternetConnect,
//	GDSRAP_SEQ_InternetConnectWait,
	GDSRAP_SEQ_GDS_Initialize,
	NULL,	//終端
};

///インターネット切断
static const SUBPROCESS_FUNC SubSeq_CleanupInet[] = {
//	GDSRAP_SEQ_CleanupInet,
...
int GDSRAP_Init(GDS_RAP_WORK *gdsrap, const GDSRAP_INIT_DATA *init_data)
{
	...
	gdsrap->gs_profile_id = init_data->gs_profile_id;
	...
	ret = POKE_NET_GDS_Initialize(gdsrap->gs_profile_id);
...
//--------------------------------------------------------------
/**
 * @brief   ポケモンWifiGDSライブラリの初期化を行います
 *
 * @param   gdsrap		
 *
 * @retval  
...
static int GDSRAP_SEQ_Error_DWC_Message(GDS_RAP_WORK *gdsrap, GDS_RAP_SUB_PROCCESS_WORK *sub_work)
{
    ...
    type =  mydwc_errorType(-gdsrap->ErrorCode);
...
int GDSRAP_Tool_Send_DressupUpload(GDS_RAP_WORK *gdsrap, GDS_PROFILE_PTR gpp, IMC_TELEVISION_SAVEDATA * dress)
{
	...
	//送信済みフラグON　＆　セーブ
	{
		MISC_SetGdsSend_Dress(SaveData_GetMisc(gdsrap->savedata), TRUE);
		SaveData_Save(gdsrap->savedata);
	}
...
int GDSRAP_Tool_Send_BoxshotUpload(GDS_RAP_WORK *gdsrap, int category_no, GDS_PROFILE_PTR gpp, const BOX_DATA *boxdata, int tray_number)
{
	...
	//送信済みフラグON　＆　セーブ
	{
		MISC_SetGdsSend_Box(SaveData_GetMisc(gdsrap->savedata), tray_number, TRUE);
		SaveData_Save(gdsrap->savedata);
	}
...
int GDSRAP_Tool_Send_BattleVideoUpload(GDS_RAP_WORK *gdsrap, GDS_PROFILE_PTR gpp)
{
	...
	//送信済みフラグON　＆　セーブ
	{
		MISC_SetGdsSend_Video(SaveData_GetMisc(gdsrap->savedata), TRUE);
		SaveData_Save(gdsrap->savedata);
	}
...
static int GDSRAP_MAIN_Send(GDS_RAP_WORK *gdsrap)
{
	...
	case POKE_NET_GDS_REQCODE_BATTLEDATA_SEARCH:	// バトルビデオ検索
		...
		case POKE_NET_GDS_REQUEST_BATTLEDATA_SEARCHTYPE_RANKING:	//最新30件
			ret = POKE_NET_GDS_BattleDataSearchRanking(gdsrap->response);
			break;
		case POKE_NET_GDS_REQUEST_BATTLEDATA_SEARCHTYPE_EXRANKING:	//お気に入り上位30件
			ret = POKE_NET_GDS_BattleDataSearchExRanking(gdsrap->response);
			break;
		}
		if(ret == TRUE){
...
static int Local_GetResponse(GDS_RAP_WORK *gdsrap)
{
	...
	int size;
	int result = TRUE;
	GDSRAP_RESPONSE_FUNC callback_func = NULL;
	...
	case POKE_NET_GDS_REQCODE_DRESSUPSHOT_REGIST:		// ドレスアップショット登録
		...
		callback_func = gdsrap->response_callback.func_dressupshot_regist;
		if(result == FALSE){	//送信済みフラグを戻して再セーブ
			MISC_SetGdsSend_Dress(SaveData_GetMisc(gdsrap->savedata), FALSE);
			SaveData_Save(gdsrap->savedata);
		}
		break;
	case POKE_NET_GDS_REQCODE_DRESSUPSHOT_GET:			// ドレスアップショット取得
		...
		callback_func = gdsrap->response_callback.func_dressupshot_get;
		break;
	case POKE_NET_GDS_REQCODE_BOXSHOT_REGIST:			// ボックスショット登録
		OS_TPrintf("受信：ボックスショット登録\n");
		result = GDS_RAP_RESPONSE_Boxshot_Upload(gdsrap, res);
		callback_func = gdsrap->response_callback.func_boxshot_regist;
		if(result == FALSE){	//送信済みフラグを戻して再セーブ
			MISC_SetGdsSend_Box(SaveData_GetMisc(gdsrap->savedata), 
				gdsrap->send_buf.sub_para.box.tray_number, FALSE);
			SaveData_Save(gdsrap->savedata);
		}
		break;
	case POKE_NET_GDS_REQCODE_BOXSHOT_GET:				// ボックスショット取得
		...
		callback_func = gdsrap->response_callback.func_boxshot_get;
		break;
	case POKE_NET_GDS_REQCODE_RANKING_GETTYPE:			// ランキングタイプ取得
		...
		callback_func = gdsrap->response_callback.func_ranking_type_get;
		break;
	case POKE_NET_GDS_REQCODE_RANKING_UPDATE:			// ランキング更新
		...
		callback_func = gdsrap->response_callback.func_ranking_update_get;
		break;
	case POKE_NET_GDS_REQCODE_BATTLEDATA_REGIST:		// バトルビデオ登録
		OS_TPrintf("受信：バトルビデオ登録\n");
		result = GDS_RAP_RESPONSE_BattleVideo_Upload(gdsrap, res);
		callback_func = gdsrap->response_callback.func_battle_video_regist;
		if(result == FALSE){	//送信済みフラグを戻して再セーブ
			MISC_SetGdsSend_Video(SaveData_GetMisc(gdsrap->savedata), FALSE);
			SaveData_Save(gdsrap->savedata);
		}
		break;
	case POKE_NET_GDS_REQCODE_BATTLEDATA_SEARCH:		// バトルビデオ登録
		OS_TPrintf("受信：バトルビデオ検索\n");
		result = GDS_RAP_RESPONSE_BattleVideo_Search_Download(gdsrap, res);
		callback_func = gdsrap->response_callback.func_battle_video_search_get;
		break;
	case POKE_NET_GDS_REQCODE_BATTLEDATA_GET:		// バトルビデオ取得
		OS_TPrintf("受信：バトルビデオ取得\n");
		result = GDS_RAP_RESPONSE_BattleVideo_Data_Download(gdsrap, res);
		callback_func = gdsrap->response_callback.func_battle_video_data_get;
		break;
	case POKE_NET_GDS_REQCODE_BATTLEDATA_FAVORITE:		// バトルビデオお気に入り登録
		OS_TPrintf("受信：バトルビデオお気に入り登録\n");
		result = GDS_RAP_RESPONSE_BattleVideo_Favorite_Upload(gdsrap, res);
		callback_func = gdsrap->response_callback.func_battle_video_favorite_regist;
		break;
	...
	if(callback_func != NULL){
		callback_func(gdsrap->response_callback.callback_work, &gdsrap->error_info);
	}

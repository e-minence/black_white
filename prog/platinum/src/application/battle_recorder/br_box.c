...
static void BoxShot_FontButton_Create( BR_WORK* wk )
{
	...
	bwk->obtn[ 0 ].cap = ExTag_Add( wk, NNS_G2D_VRAM_TYPE_2DSUB );
	bwk->obtn[ 1 ].cap = ExTag_Add( wk, NNS_G2D_VRAM_TYPE_2DSUB );
	FontOam_Create( &bwk->obtn[ 0 ], &wk->sys, NNS_G2D_VRAM_TYPE_2DSUB );
	FontOam_Create( &bwk->obtn[ 1 ], &wk->sys, NNS_G2D_VRAM_TYPE_2DSUB );
	FontOam_MsgSet( &bwk->obtn[ 0 ], &wk->sys, msg_302 );
	FontOam_MsgSet( &bwk->obtn[ 1 ], &wk->sys, msg_05 );

	CATS_ObjectPosSetCap( bwk->obtn[ 0 ].cap,  32, 232 );
	CATS_ObjectPosSetCap( bwk->obtn[ 1 ].cap, 128, 232 );
	...
	FONTOAM_SetDrawFlag( bwk->obtn[ 0 ].font_obj, TRUE );
	FONTOAM_SetDrawFlag( bwk->obtn[ 1 ].font_obj, TRUE );
	CATS_ObjectAnimeSeqSetCap( bwk->obtn[ 0 ].cap, 1 );
	CATS_ObjectAnimeSeqSetCap( bwk->obtn[ 1 ].cap, 0 );
}
...
static void BoxShot_FontButton_Delete( BR_WORK* wk )
{
	...
	FontOam_Delete( &bwk->obtn[ 0 ] );
	FontOam_Delete( &bwk->obtn[ 1 ] );
	CATS_ActorPointerDelete_S( bwk->obtn[ 0 ].cap );
	CATS_ActorPointerDelete_S( bwk->obtn[ 1 ].cap );
...
static void ButtonOAM_PosSet( BR_WORK* wk, int mode )
{
	...
	if ( mode == 0 ){
		///< 2個
		CATS_ObjectEnableCap( bwk->obtn[ 0 ].cap, TRUE );
		FONTOAM_SetDrawFlag( bwk->obtn[ 0 ].font_obj, TRUE );
		CATS_ObjectPosSetCap( bwk->obtn[ 1 ].cap, 128, 232 );
	}
	else {
		///< 1個
		CATS_ObjectEnableCap( bwk->obtn[ 0 ].cap, FALSE );
		FONTOAM_SetDrawFlag( bwk->obtn[ 0 ].font_obj, FALSE );
		CATS_ObjectPosSetCap( bwk->obtn[ 1 ].cap, 80, 232 );
	}
	FONTOAM_SetMat( bwk->obtn[ 0 ].font_obj, FONT_OAM_TAG_OX, FONT_OAM_TAG_OY );
	FONTOAM_SetMat( bwk->obtn[ 1 ].font_obj, FONT_OAM_TAG_OX, FONT_OAM_TAG_OY );
}

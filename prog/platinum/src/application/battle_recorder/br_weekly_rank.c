...
//	int				ranking_page_no;		///< ランキングのページ番号
//	int				ranking_page_rank_num;	///< 1ページに表示するランキングの数
//	int				ranking_page_max;		///< ページの総数
...
static void Ranking_WinRankPut( BR_WORK* wk );
...
//	TouchList_InitEx( &wk->sys.touch_list, wk, rwk->man_trtype, &rwk->tl_head );
	TouchList_InitRanking( &wk->sys.touch_list, wk, rwk->man_trtype, &rwk->tl_head, rwk->group_no, rwk->unit_str );
...
static void TL_InfoMake( BR_WORK* wk )
{
	...
	for ( i = 0; i < GT_TRAINER_VIEW_MAX; i++ ){
		if ( rwk->ranking_mode == BR_EX_DATA_THIS_WEEK_RANK ){
			rwk->tl_info[ 0 ][ i ].msg	= TL_THIS_WEEK_TR;
			rwk->tl_info[ 0 ][ i ].param= WiFi_TR_DataGet( rwk->this_week_data->ranking_data[ 0 ].trainer_type_ranking[ i ], 1 );
		}
		else {
			rwk->tl_info[ 0 ][ i ].msg	= TL_LAST_WEEK_TR;
			rwk->tl_info[ 0 ][ i ].param= WiFi_TR_DataGet( rwk->last_week_data->ranking_data[ 0 ].trainer_type_ranking[ i ], 1 );
		}
		rwk->tl_info[ 0 ][ i ].param_ex = rwk->last_week_data->ranking_data[ 0 ].trainer_type_score[ i ];
	}

	for ( i = 0; i < GT_YEAR_MONTH_NUM; i++ ){
		if ( rwk->ranking_mode == BR_EX_DATA_THIS_WEEK_RANK ){
			rwk->tl_info[ 1 ][ i ].msg	= TL_THIS_WEEK_YM;
			rwk->tl_info[ 1 ][ i ].param= rwk->this_week_data->ranking_data[ 1 ].birthday_month_ranking[ i ];
		}
		else {
			rwk->tl_info[ 1 ][ i ].msg	= TL_LAST_WEEK_YM;
			rwk->tl_info[ 1 ][ i ].param= rwk->last_week_data->ranking_data[ 1 ].birthday_month_ranking[ i ];
		}
		rwk->tl_info[ 1 ][ i ].param_ex = rwk->last_week_data->ranking_data[ 1 ].birthday_month_score[ i ];
	}

	for ( i = 0; i < GT_MONSNO_RANKING_MAX; i++ ){
		if ( rwk->ranking_mode == BR_EX_DATA_THIS_WEEK_RANK ){
			rwk->tl_info[ 2 ][ i ].msg	= TL_THIS_WEEK_MN;
			rwk->tl_info[ 2 ][ i ].param= rwk->this_week_data->ranking_data[ 2 ].monsno_ranking[ i ];
		}
		else {
			rwk->tl_info[ 2 ][ i ].msg	= TL_LAST_WEEK_MN;
			rwk->tl_info[ 2 ][ i ].param= rwk->last_week_data->ranking_data[ 2 ].monsno_ranking[ i ];
		}
		rwk->tl_info[ 2 ][ i ].param_ex = rwk->last_week_data->ranking_data[ 2 ].monsno_score[ i ];
	}	
}
...
static u32 Send_RECORD_Get( BR_WORK* wk, int no )
{
	...
	data = RECORD_Get( SaveData_GetRecord( wk->save ), RecordID_Table[ no ] );
...
//	no = ( rwk->ranking_page_no * rwk->ranking_page_rank_num );

	for ( i = 2; i < win_max; i++ ){
		str1 = MSGMAN_AllocString( wk->sys.man, msg_510 + rwk->group_no );		///< 順位：トレーナータイプ or 生まれ月 or すきなポケモン
		str2 = STRBUF_Create( 255, HEAPID_BR );
		temp = STRBUF_Create( 255, HEAPID_BR );
		
		str_num  = STRBUF_Create( 255, HEAPID_BR );
		str_unit = MSGMAN_AllocString( wk->sys.man_ranking, Score_Unit_IndexTable[ rwk->last_week_data->ranking_data[ rwk->ranking_no ].ranking_type - 1 ] );
		...
		case 0:
			{
				int view;
				if ( rwk->ranking_mode == BR_EX_DATA_THIS_WEEK_RANK ){
					view = WiFi_TR_DataGet( rwk->this_week_data->ranking_data[ rwk->ranking_no ].trainer_type_ranking[ no + i ], 1 );
				}
				else {
					view = WiFi_TR_DataGet( rwk->last_week_data->ranking_data[ rwk->ranking_no ].trainer_type_ranking[ no + i ], 1 );					
					{
						u64 score = rwk->last_week_data->ranking_data[ rwk->ranking_no ].trainer_type_score[ no + i ];
						STRBUF_SetNumber( str_num, score, Number_to_Unit_Get( score ), NUMBER_DISPTYPE_LEFT, NUMBER_CODETYPE_DEFAULT);
					}
				}
				str3 = MSGMAN_AllocString( rwk->man_trtype, view );
			}
			break;
		case 1:
			{
				int month;
				if ( rwk->ranking_mode == BR_EX_DATA_THIS_WEEK_RANK ){
					month = rwk->this_week_data->ranking_data[ rwk->ranking_no ].birthday_month_ranking[ no + i ];
				}
				else {
					month = rwk->last_week_data->ranking_data[ rwk->ranking_no ].birthday_month_ranking[ no + i ];
					{
						u64 score = rwk->last_week_data->ranking_data[ rwk->ranking_no ].birthday_month_score[ no + i ];						
						STRBUF_SetNumber( str_num, score, Number_to_Unit_Get( score ), NUMBER_DISPTYPE_LEFT, NUMBER_CODETYPE_DEFAULT);
					}
				}
		...
		case 2:
			{
				...
				int monsno;
				if ( rwk->ranking_mode == BR_EX_DATA_THIS_WEEK_RANK ){
					monsno = rwk->this_week_data->ranking_data[ rwk->ranking_no ].monsno_ranking[ no + i ];
				}
				else {
					monsno = rwk->last_week_data->ranking_data[ rwk->ranking_no ].monsno_ranking[ no + i ];
					{
						u64 score = rwk->last_week_data->ranking_data[ rwk->ranking_no ].monsno_score[ no + i ];						
						STRBUF_SetNumber( str_num, score, Number_to_Unit_Get( score ), NUMBER_DISPTYPE_LEFT, NUMBER_CODETYPE_DEFAULT);
					}
				}
				str3 = STRBUF_Create( 255, HEAPID_BR );
		...
		STRBUF_SetNumber( str2, no + 1, 2, NUMBER_DISPTYPE_SPACE, NUMBER_CODETYPE_DEFAULT);			
		...
		GF_BGL_BmpWinDataFill( &rwk->win_m[ i ], 0x00 );
		
		GF_STR_PrintColor( &rwk->win_m[ i ], FONT_SYSTEM, temp, 0, 0, MSG_NO_PUT, PRINT_COL_RANK, NULL );
		if ( rwk->ranking_mode == BR_EX_DATA_LAST_WEEK_RANK ){
			int width = FontProc_GetPrintStrWidth( FONT_SYSTEM, str_num, 0 );
			GF_STR_PrintColor( &rwk->win_m[ i ], FONT_SYSTEM, str_num, 16, 16, MSG_NO_PUT, PRINT_COL_RANK, NULL );
			GF_STR_PrintColor( &rwk->win_m[ i ], FONT_SYSTEM, str_unit, 16 + width, 16, MSG_NO_PUT, PRINT_COL_RANK, NULL );
		}
		
		GF_BGL_BmpWinOnVReq( &rwk->win_m[ i ] );
		STRBUF_Delete( str1 );
		STRBUF_Delete( str2 );
		STRBUF_Delete( str3 );
		STRBUF_Delete( temp );
	...
	case 0:
		{
			...
			str2 = MSGMAN_AllocString( rwk->man_trtype, WiFi_TR_DataGet( view, 1 ) );
		}
		break;
	case 1:
		{
...
/*
	rwk->ranking_page_no = 0;
	rwk->ranking_page_rank_num = RANKING_NUM_THIS_WEEK;
	
	if ( rwk->ranking_mode == BR_EX_DATA_LAST_WEEK_RANK ){		///< 先週のランキングなら表示件数を減らす	
		rwk->ranking_page_rank_num = RANKING_NUM_LAST_WEEK;
	}
	
	switch ( rwk->group_no ){
	case 0:
		///< トレーナーの種類
		rwk->ranking_page_max = ( GT_TRAINER_VIEW_MAX / rwk->ranking_page_rank_num );
		rwk->ranking_num = GT_TRAINER_VIEW_MAX;
		break;
	case 1:
		///< うまれたつき
		rwk->ranking_page_max = ( GT_YEAR_MONTH_NUM / rwk->ranking_page_rank_num );
		break;
	case 2:
		///< すきなポケモン
		rwk->ranking_page_max = ( GT_MONSNO_RANKING_MAX / rwk->ranking_page_rank_num );
		break;
	}
*/
}
...
/*
	case 2:
		TouchAction( wk );
		if ( rwk->ranking_page_no > 0 ){
			rwk->ranking_page_no--;
		}
		else {
			rwk->ranking_page_no = rwk->ranking_page_max - 1;
		}		
		BR_Main_SeqChange( wk, eRANK_RankChange );
		break;
	
	case 3:
		TouchAction( wk );
		rwk->ranking_page_no++;
		rwk->ranking_page_no %= rwk->ranking_page_max;
		BR_Main_SeqChange( wk, eRANK_RankChange );
		break;
*/		
	default:
...
static BOOL BR_WeeklyRanking_Init( BR_WORK* wk )
{
	...
	rwk->man_trtype = MSGMAN_Create( MSGMAN_TYPE_NORMAL, ARC_MSG, NARC_msg_trtype_dat, HEAPID_BR );
...
//			Snd_SePlay( eBR_SND_SEARCH );
	...
	case 3:
		...
		rwk->send_data.my_data[ 0 ].score = Send_RECORD_Get( wk, rwk->send_data.my_data[ 0 ].ranking_type );
		rwk->send_data.my_data[ 1 ].score = Send_RECORD_Get( wk, rwk->send_data.my_data[ 1 ].ranking_type );
		rwk->send_data.my_data[ 2 ].score = Send_RECORD_Get( wk, rwk->send_data.my_data[ 2 ].ranking_type );

		if( GDSRAP_Tool_Send_RankingUpdate(	BR_GDSRAP_WorkGet( wk ), ( GDS_PROFILE_PTR )&rwk->send_data.profile, rwk->send_data.my_data ) == TRUE ){
			wk->sub_seq++;
		}
		break;
...
static BOOL BR_WeeklyRanking_Main( BR_WORK* wk )
{
	...
	TouchList_Ranking_Main( &wk->sys.touch_list, wk, rwk->group_no, rwk->unit_str );
...
void BR_Response_RankingType(void *work, const GDS_RAP_ERROR_INFO *error_info)
{
	...
	/*
		for( i = 0; i < GT_RANKING_WEEK_NUM; i++ ){
			...
			rwk->ranking_type->ranking_type[ i ] = recv_type->ranking_type[ i ];
		}
	*/
	}
}

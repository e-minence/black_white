...
static void EMail_InfoDraw( EMAIL_WORK* wk, int no )
{
	...
	//	str	= MSGMAN_AllocString( man, msg_email_900 + no );	
		str	= MSGMAN_AllocString( man, msg_email_list_002 );	
		GF_STR_PrintExpand( win, FONT_SYSTEM, str, 0, 0, 0, EMAIL_PRINT_COL, 0, 0, NULL );
		STRBUF_Delete( str );
	}
...
//		CI_pv_ButtonManagerInit( wk );
	...
	if(0)
	{
		...
		len = STRBUF_GetLen( wk->param->str );
		...
		if ( len != 0 )
		{
			...
			s16 x,y;
			u16 code[2];
			STRBUF* str = STRBUF_Create( 2, HEAPID_EMAIL );

			STRBUF_GetStringCode( wk->param->str, wk->address, EMAIL_CODE_MAX );
			STRBUF_GetStringCode( wk->param->str, wk->temp, EMAIL_CODE_MAX );
...
//	InfoWinAdd( wk, eINFO_ADRS_INPUT );
...
static void EMail_ButtonCallBack( u32 button, u32 event, void* work )
{
	...
	if ( button < touch_btn )
	{
		...
		if ( button == EMAIL_BACK ){
			...
			GF_BGL_BmpWinFill( &wk->sys.win1[ 0 ], 0xFF, x, y, 8, 16 );
			GF_BGL_BmpWinOn( &wk->sys.win1[ 0 ] );
...
static BOOL EMail_EndCheck( EMAIL_WORK* wk, int end_set )
{
	...
	if ( wk->param->mode == EMAIL_INPUT_ADDRESS ){
		...
		if ( comp == 0 ){
			...
			OS_Printf( "１回目 と ２回目 が 違う\n");
		//	EMail_InfoDraw( wk, eINFO_ADRS_ERR );

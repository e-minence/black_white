...
#define BCT_FEVER_BACK_ANM_SPEED_START			( BCT_MARUNOMU_ANM_SPEED )
#define BCT_FEVER_BACK_ANM_SPEED_DIF			( FX32_CONST( 100 ) )
...
static void BCT_netID2DMatrixGet( u32 comm_num, u32 myplno, u32 targetplno, s32* p_x,  s32* p_y )
{
    // 通信人数に対応した、それぞれの通信IDの座標
    static const s32 NetIDMatrix[BCT_PLAYER_NUM][BCT_PLAYER_NUM][2] = {
        {   // 1人
            {0,0},
            {0,0},
            {0,0},
    ...
    // myplnoとtargetplnoがどのくらい離れているかチェック
    num = targetplno - myplno;
    if( num < 0 ){
        num = comm_num + num;
    }

    *p_x = NetIDMatrix[ comm_num-1 ][ num ][ 0 ];
    *p_y = NetIDMatrix[ comm_num-1 ][ num ][ 1 ];
}
...
static void BCT_CLIENT_MainBackInit( BCT_CLIENT_MAINBACK* p_wk, BCT_CLIENT_GRAPHIC* p_graphic, ARCHANDLE* p_handle, int comm_num, u32 plno, u32 heapID, NNSFndAllocator* p_allocator )
{
	...
	// アニメスピード設定
	p_wk->anm_speed = BCT_MARUNOMU_ANM_SPEED;
}

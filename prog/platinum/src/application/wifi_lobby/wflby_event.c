...
///	イベント管理システム関数
//=====================================
static void WFLBY_EVENTWK_Init( WFLBY_EVENTWK* p_wk, u32 heapID );
...
WFLBY_EVENT* WFLBY_EVENT_Init( WFLBY_ROOMWK* p_rmwk, u32 heapID )
{
	...
	// 個別イベント初期設定
	{
		...
		for( i=0; i<WFLBY_PLAYER_MAX; i++ ){
			WFLBY_EVENTWK_Init( &p_sys->private[i], heapID );
		}
	}

	// 全体イベント初期設定
	WFLBY_EVENTWK_Init( &p_sys->global, heapID );
...
void WFLBY_EVENTWK_SetSeq( WFLBY_EVENTWK* p_wk, u32 seq )
{
	...
	p_wk->data.seq = seq;
	WFLBY_EVENT_DEBUG_PRINT( "event seq = %d\n", seq );
}

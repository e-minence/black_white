...
enum {	// サブ面パレット
	...
	WFLBY_ROOM_BGSPL_NONE05,			// 
	WFLBY_ROOM_BGSPL_NONE06,			// 
	WFLBY_ROOM_BGSPL_NONE07,			// 
...
// OAM
//=====================================
#define WFLBY_ROOM_OAM_CONTNUM			( 16 )	// OAM管理数
#define WFLBY_ROOM_OAM_RESNUM			( 4 )	// リソース管理種類
#define WFLBY_ROOM_CLACT_NUM			( 16 )
...
#define WFLBY_LISTWIN_SIZY		( 16 )	// リスト最大サイズ
...
/// 広場専用メッセージ
//=====================================
#define WFLBY_DEFMSG_STRNUM		( 408 )
...
#define WFLBY_UNDERWIN_BGCG_END		( 400 )	// 背景とフレーム用CGXのオワリ
...
#define WFLBY_TRCARD_WIN_NAME_SIZX	( 20 )
...
#define WFLBY_TRCARD_WIN_VIPAIKOTOBA_X		( 1 )
#define WFLBY_TRCARD_WIN_VIPAIKOTOBA_Y		( 14 )
#define WFLBY_TRCARD_WIN_VIPAIKOTOBA_SIZX	( 19 )
#define WFLBY_TRCARD_WIN_VIPAIKOTOBA_SIZY	( 9 )
...
static const WFLBY_TRCARD_TRTYPE sc_WFLBY_TRCARD_TRTYPE[WFLBY_TRCARD_TRTYPE_NUM]={
	{  HERO,		TRTYPE_HERO,	43,	36 },
	{  HEROINE,		TRTYPE_HEROINE,	33,	35 },
...
static void WFLBY_ROOM_ListWin_Exit( WFLBY_ROOM_LISTWIN* p_wk );
...
static void WFLBY_ROOM_ErrWin_DrawDwcErr( WFLBY_ROOM_ERRMSG* p_wk, WFLBY_ROOM_DEFMSG* p_msg, int errno );
...
static void WFLBY_ROOM_UNDERWIN_Main( WFLBY_UNDER_WIN* p_wk, WFLBY_ROOMWK* p_room, u32 heapID );
...
static void WFLBY_ROOM_UNDERWIN_StartTrCard( WFLBY_UNDER_WIN* p_wk, WFLBY_ROOM_TALKMSG* p_boardwin, WFLBY_GRAPHICCONT* p_sys, WFLBY_ROOM_DEFMSG* p_msg, BOOL mydata, u32 heapID, const WFLBY_USER_PROFILE* cp_profile, BOOL vip, BOOL aikotoba, const WFLBY_AIKOTOBA_DATA* cp_aikotoba );
static void WFLBY_ROOM_UNDERWIN_EndTrCard( WFLBY_UNDER_WIN* p_wk, WFLBY_ROOM_TALKMSG* p_boardwin, WFLBY_GRAPHICCONT* p_sys );
static void WFLBY_ROOM_UNDERWIN_StartBttn( WFLBY_UNDER_WIN* p_wk, WFLBY_GRAPHICCONT* p_sys, u32 heapID, const WFLBY_USER_PROFILE* cp_profile );
...
static void WFLBY_ROOM_UNDERWIN_TrCard_Start( WFLBY_UNDER_WIN* p_ugwk, WFLBY_ROOM_TALKMSG* p_boardwin, WFLBY_TR_CARD* p_wk, WFLBY_GRAPHICCONT* p_sys, WFLBY_ROOM_DEFMSG* p_msg, BOOL mydata, ARCHANDLE* p_handle, u32 heapID, const WFLBY_USER_PROFILE* cp_profile, BOOL vip, BOOL aikotoba, const WFLBY_AIKOTOBA_DATA* cp_aikotoba );
...
static void WFLBY_ROOM_UNDERWIN_TrCard_ItemIcon_Init( WFLBY_TR_CARD* p_wk, WFLBY_GRAPHICCONT* p_sys, ARCHANDLE* p_handle, u32 heapID, WFLBY_ITEMTYPE itemno );
...
static void WFLBY_ROOM_UNDERWIN_Button_Start( WFLBY_GADGET_BTTN* p_wk, WFLBY_GRAPHICCONT* p_sys, const WFLBY_USER_PROFILE* cp_profile, ARCHANDLE* p_handle, u32 heapID );
...
PROC_RESULT WFLBY_ROOM_Init(PROC* p_proc, int* p_seq)
{
	...
	// ヒープ作成
	sys_CreateHeap( HEAPID_BASE_APP, HEAPID_WFLBY_ROOM, 0x37000 );
	sys_CreateHeap( HEAPID_BASE_APP, HEAPID_WFLBY_ROOMGRA, 0x40000 );
	...
	// タッチトイを表示できる状態なら表示する
	{
		...
		if( WFLBY_SYSTEM_CheckGetItem( cp_profile ) == TRUE ){
			WFLBY_ROOM_UNDERWIN_StartBttn( &p_wk->under_win, &p_wk->graphic,
					HEAPID_WFLBY_ROOM, 
					cp_profile );
		}
	}
	...
	// ガジェット初期化
	p_wk->p_gadget = WFLBY_GADGET_Init( p_wk->p_mapcont, p_wk->p_camera, p_wk->p_objcont, HEAPID_WFLBY_ROOM, HEAPID_WFLBY_ROOMGRA );
...
PROC_RESULT WFLBY_ROOM_Exit(PROC* p_proc, int* p_seq)
{
	...
	WFLBY_ROOM_ListWin_Exit( &p_wk->listwin );
 ...
 */
//-----------------------------------------------------------------------------
void WFLBY_ROOM_LISTWIN_Start_Ex( WFLBY_ROOMWK* p_wk, const BMPLIST_HEADER* cp_data, u16 list_p, u16 cursor_p, u8 cx, u8 szcx )
{
	WFLBY_ROOM_ListWin_Start( &p_wk->listwin, cp_data, &p_wk->graphic, list_p, cursor_p, HEAPID_WFLBY_ROOM, cx, WFLBY_LISTWIN_Y, szcx );
}
...
void WFLBY_ROOM_MSG_SetIdxPlayerName( WFLBY_ROOMWK* p_wk, u32 idx, u32 bufid )
{
	...
	if( idx == WFLBY_SYSTEM_GetMyIdx( p_wk->p_system ) ){
		cp_profile	= WFLBY_SYSTEM_GetMyProfile( p_wk->p_system );	
	}else{
...
static void WFLBY_ROOM_GraphicInit( WFLBY_GRAPHICCONT* p_sys, SAVEDATA* p_save, u32 heapID )
{
	...
	// ウィンドウ設定
	{
		...
		MenuWinGraphicSet(
				...
				WFLBY_ROOM_BGPL_SYSWIN, winnum, heapID );
		TalkWinGraphicSet(
	...
	// OAM
	{
		...
		// キャラクタマネージャー初期化
		InitCharManagerReg(&sc_WFLBY_ROOM_CHARMAN_INIT, GX_OBJVRAMMODE_CHAR_1D_128K, GX_OBJVRAMMODE_CHAR_1D_64K );
		// パレットマネージャー初期化
...
static void WFLBY_ROOM_RoomMain( WFLBY_ROOMWK* p_wk )
{
	...
	if( p_wk->fade_flag == FALSE ){
		WFLBY_ROOM_UNDERWIN_Main( &p_wk->under_win, p_wk, HEAPID_WFLBY_ROOM );
	}
	WFLBY_DEBUG_ROOM_PRINT_TIME_TICK_PRINT( __LINE__ );
...
static void WFLBY_ROOM_TalkWin_Exit( WFLBY_ROOM_TALKMSG* p_wk )
{
	...
	WFLBY_ROOM_TalkWin_StopTimeWait( p_wk );
...
static void WFLBY_ROOM_ErrWin_DrawDwcErr( WFLBY_ROOM_ERRMSG* p_wk, WFLBY_ROOM_DEFMSG* p_msg, int errno )
{
	...
	// メッセージ取得
	msgno = WFLBY_ERR_GetStrID( errno );	// メッセージＮＯ取得
	WFLBY_ROOM_Msg_SetNumber( p_msg, errno, 5, 0, NUMBER_DISPTYPE_ZERO );	// ＥＲＲＮＯ設定
...
static void WFLBY_ROOM_UNDERWIN_Main( WFLBY_UNDER_WIN* p_wk, WFLBY_ROOMWK* p_room, u32 heapID )
{
	...
	case WFLBY_UNDERWIN_SEQ_STARTFADEOUTWAIT:	
		if( WIPE_SYS_EndCheck() == TRUE ){
			// ガジェット書き込み
			WFLBY_ROOM_UNDERWIN_StartBttn( p_wk, &p_room->graphic,
					heapID, 
					WFLBY_SYSTEM_GetMyProfileLocal( p_room->p_system ) );
	...
	case WFLBY_UNDERWIN_SEQ_CHANGE:
		{
			...
			case WFLBY_UNDERWIN_DRAW_BTTN:
				...
				WFLBY_ROOM_UNDERWIN_StartBttn( p_wk, p_sys, heapID, cp_profile );
				break;
			...
			case WFLBY_UNDERWIN_DRAW_PROFILE:
				{
					...
					WFLBY_ROOM_UNDERWIN_StartTrCard( p_wk, &p_room->boardwin, p_sys, p_msg, mydata, heapID, cp_profile, vip, aikotoba, cp_aikotoba );
				}
				break;
...
static void WFLBY_ROOM_UNDERWIN_StartTrCard( WFLBY_UNDER_WIN* p_wk, WFLBY_ROOM_TALKMSG* p_boardwin, WFLBY_GRAPHICCONT* p_sys, WFLBY_ROOM_DEFMSG* p_msg, BOOL mydata, u32 heapID, const WFLBY_USER_PROFILE* cp_profile, BOOL vip, BOOL aikotoba, const WFLBY_AIKOTOBA_DATA* cp_aikotoba )
{
	...
	WFLBY_ROOM_UNDERWIN_TrCard_Start( p_wk, p_boardwin, &p_wk->tr_card, p_sys, p_msg, mydata, p_wk->p_handle, heapID, cp_profile, vip, aikotoba, cp_aikotoba );
...
static void WFLBY_ROOM_UNDERWIN_StartBttn( WFLBY_UNDER_WIN* p_wk, WFLBY_GRAPHICCONT* p_sys, u32 heapID, const WFLBY_USER_PROFILE* cp_profile )
{
	...
	WFLBY_ROOM_UNDERWIN_Button_Start( &p_wk->bttn, p_sys, cp_profile, 
...
static void WFLBY_ROOM_UNDERWIN_TrCard_Start( WFLBY_UNDER_WIN* p_ugwk, WFLBY_ROOM_TALKMSG* p_boardwin, WFLBY_TR_CARD* p_wk, WFLBY_GRAPHICCONT* p_sys, WFLBY_ROOM_DEFMSG* p_msg, BOOL mydata, ARCHANDLE* p_handle, u32 heapID, const WFLBY_USER_PROFILE* cp_profile, BOOL vip, BOOL aikotoba, const WFLBY_AIKOTOBA_DATA* cp_aikotoba )
{
	...
	if( aikotoba == FALSE ){
		WFLBY_ROOM_UNDERWIN_Common_LoadScrn( p_sys, p_handle, 
				NARC_wifi_lobby_other_profile_NSCR, 
	...
	// 名前
	{
		...
		WFLBY_ROOM_Msg_SetNumber( p_msg, 
				MyStatus_GetID_Low( p_mystatus ), 5, 0, NUMBER_DISPTYPE_SPACE );
	...
	if( aikotoba ){
		...
		WFLBY_ROOM_UNDERWIN_TrCard_WinPrint( p_wk, p_msg, WFLBY_TRCARD_WIN_VIPAIKOTOBA, 
				msg_hiroba_profile_07, 0, 0, col );

		// 合言葉設定
		WFLBY_ROOM_UNDERWIN_TrCard_WinSetAikotoba( p_wk, p_msg, cp_aikotoba );
		WFLBY_ROOM_UNDERWIN_TrCard_WinPrint( p_wk, p_msg, WFLBY_TRCARD_WIN_VIPAIKOTOBA, 
				msg_hiroba_profile_10, 8, 24, WFLBY_TRCARD_COL_BLACK );
		WFLBY_ROOM_UNDERWIN_TrCard_WinPrint( p_wk, p_msg, WFLBY_TRCARD_WIN_VIPAIKOTOBA, 
				msg_hiroba_profile_11, 88, 24, WFLBY_TRCARD_COL_BLACK );
		WFLBY_ROOM_UNDERWIN_TrCard_WinPrint( p_wk, p_msg, WFLBY_TRCARD_WIN_VIPAIKOTOBA, 
				msg_hiroba_profile_12, 8, 44, WFLBY_TRCARD_COL_BLACK );
		WFLBY_ROOM_UNDERWIN_TrCard_WinPrint( p_wk, p_msg, WFLBY_TRCARD_WIN_VIPAIKOTOBA, 
				msg_hiroba_profile_13, 88, 44, WFLBY_TRCARD_COL_BLACK );
	...
	WFLBY_ROOM_UNDERWIN_TrCard_ItemIcon_Init( p_wk, p_sys, p_handle, heapID, 
			WFLBY_SYSTEM_GetProfileItem( cp_profile ) );
...
static void WFLBY_ROOM_UNDERWIN_TrCard_TrView_Init( WFLBY_TR_CARD* p_wk, WFLBY_GRAPHICCONT* p_sys, ARCHANDLE* p_handle, u32 heapID, u32 trtype )
{
	...
	// 追加
	{
		...
		add.mat.y		= FX32_CONST( y ) + SUB_SURFACE_Y;
...
{
	u32 action;
	action = WFLBY_SYSTEM_GetProfileLastAction( cp_profile );

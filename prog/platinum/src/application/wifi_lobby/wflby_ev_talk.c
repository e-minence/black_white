...
//		p_str = WFLBY_ROOM_MSG_Get( p_rmwk, WFLBY_DEFMSG_TYPE_AISATSU, msg_hiroba_item_00_04 );
		...
		// 最後にしたことにガジェット交換を設定
		WFLBY_SYSTEM_SetLastAction( p_system, WFLBY_GAME_ITEMCHG );
		break;
	...
	case WFLBY_EV_TALK_A_SEQ_SEL_T_K:
		...
		WFLBY_ROOM_LISTWIN_Start_Ex( p_rmwk, &p_evwk->bmplist.bmplist, 0, 0, WFLBY_EV_TALK_TK_LISTWIN_X, WFLBY_EV_TALK_TK_LISTWIN_SIZX );
...
//		p_str = WFLBY_ROOM_MSG_Get( p_rmwk, WFLBY_DEFMSG_TYPE_AISATSU, msg_hiroba_osyaberi_00_06 );
		...
		// 最後にしたことに会話を設定
		WFLBY_SYSTEM_SetLastAction( p_system, WFLBY_GAME_TALK );
		break;
...
BOOL WFLBY_EV_DEF_PlayerA_SWTOPIC_Before( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_PLAYER_TOPIC_MSGMINI:	
		...
		WFLBY_ROOM_MSG_SetMinigame( p_rmwk, WFLBY_GAME_BALANCEBALL, 1 );
		WFLBY_ROOM_MSG_SetMinigame( p_rmwk, WFLBY_GAME_BALLOON, 2 );
		p_str = WFLBY_ROOM_MSG_Get( p_rmwk, WFLBY_DEFMSG_TYPE_HIROBA, msg_hiroba_infob_03 );
		WFLBY_ROOM_TALKWIN_PrintStr( p_rmwk, p_str );
		p_evwk->ret_seq = WFLBY_EV_DEF_PLAYER_TOPIC_MSG01;
		WFLBY_EVENTWK_SetSeq( p_wk, WFLBY_EV_DEF_PLAYER_TOPIC_MSGWAIT );
		break;
	...
	case WFLBY_EV_DEF_PLAYER_TOPIC_MSGEVE:	
		p_str = WFLBY_ROOM_MSG_Get( p_rmwk, WFLBY_DEFMSG_TYPE_HIROBA, msg_hiroba_infob_07 );
		WFLBY_ROOM_TALKWIN_PrintStr( p_rmwk, p_str );
		p_evwk->ret_seq = WFLBY_EV_DEF_PLAYER_TOPIC_MSG01;
		WFLBY_EVENTWK_SetSeq( p_wk, WFLBY_EV_DEF_PLAYER_TOPIC_MSGWAIT );
		break;
...
static BOOL WFLBY_EV_TALK_WK_AisatuFollowDraw( WFLBY_EV_TALK_WK* p_evwk, WFLBY_SYSTEM* p_system, WFLBY_ROOMWK* p_rmwk )
{
	...
	// 自分か相手の言語が不明でないか
	if( WFLBY_SYSTEM_GetProfileRagionCode( cp_mypr ) == WFLBY_REGIONCODE_ERR ){
		return FALSE;
	}
	if( WFLBY_SYSTEM_GetProfileRagionCode( cp_youpr ) == WFLBY_REGIONCODE_ERR ){
		return FALSE;
	}
...
static void WFLBY_EV_TOPIC_WK_InitBmpList( WFLBY_EV_TALK_BMPLIST* p_wk, WFLBY_ROOMWK* p_rmwk )
{
	...
	for( i=0; i<WFLBY_EV_TOPIC_LIST_SEL_NUM; i++ ){
		p_str = WFLBY_ROOM_MSG_Get( p_rmwk, WFLBY_DEFMSG_TYPE_HIROBA, msg_hiroba_infowin_01+i );

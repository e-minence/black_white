...
// ミニゲーム共通ワーク作成
static void WFLBY_APL_MINIGAME_InitWk( WFLBY_MINIGAME_WK* p_wk, WFLBY_SYSTEM* p_system );
 ...
 */
//-----------------------------------------------------------------------------
WFLBY_APL* WFLBY_APL_Init( BOOL check_skip, SAVEDATA* p_save, WFLBY_SYSTEM* p_system, u32 heapID )
{
	WFLBY_APL* p_wk;
...
static void WFLBY_APL_MINIGAME_InitWk( WFLBY_MINIGAME_WK* p_wk, WFLBY_SYSTEM* p_system )
{
	...
	// ミニゲーム共通ワークを作成
	cp_profile		= WFLBY_SYSTEM_GetMyProfile( p_system );
...
static void WFLBY_APLDATA_BS_Start( WFLBY_APL* p_sys, WFLBY_APLDATA* p_data )
{
...

	{
		...
		p_param = p_data->p_param;
		WFLBY_APL_MINIGAME_InitWk( &p_param->lobby_wk, p_sys->p_system );
	}

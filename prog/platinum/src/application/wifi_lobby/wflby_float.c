 ...
 */
//-----------------------------------------------------------------------------
static const WFLBY_FLOAT_DATA sc_WFLBY_FLOAT_DATA[WFLBY_ROOM_NUM][WFLBY_SEASON_NUM] = {
	// 火
	{
		{ 
...
void WFLBY_FLOAT_CONT_Main( WFLBY_FLOAT_CONT* p_sys )
{
	...
	for( i=0; i<WFLBY_FLOAT_MAX; i++ ){
		if( WFLBY_SYSTEM_GetFloatMove( p_sys->p_system, i ) == TRUE ){
		...
		if( anm_on == TRUE ){
			...
			case WFLBY_FLOAT_ANM_SOUND:
				...
				if( anm_result == TRUE ){
					...
					if( (idx % WFLBY_FLOAT_ON_NUM) == 2 ){
						// でかいフロート
						WFLBY_FLOAT_CONT_PlayMonsVoice( my_float_on, my_float, my_float_offs_chk, sc_WFLBY_FLOAT_DATA[ room ][ season ].monsno_big );

						if( sc_WFLBY_FLOAT_DATA[ room ][ season ].monsno_big_snd_pv != WFLBY_SND_RIDE_INVALIDITY ){
							WFLBY_FLOAT_CONT_PlaySe( my_float_on, my_float, my_float_offs_chk, sc_WFLBY_FLOAT_DATA[ room ][ season ].monsno_big_snd_pv );
						}
					}else{
						// ちっこいフロート
						WFLBY_FLOAT_CONT_PlayMonsVoice( my_float_on, my_float, my_float_offs_chk, sc_WFLBY_FLOAT_DATA[ room ][ season ].monsno_sml );

						if( sc_WFLBY_FLOAT_DATA[ room ][ season ].monsno_sml_snd_pv != WFLBY_SND_RIDE_INVALIDITY ){
							WFLBY_FLOAT_CONT_PlaySe( my_float_on, my_float, my_float_offs_chk, sc_WFLBY_FLOAT_DATA[ room ][ season ].monsno_sml_snd_pv );
						}
					}
			...
			case WFLBY_FLOAT_ANM_BODY:
				...
				if( anm_result == TRUE ){
					...
					if( (idx % WFLBY_FLOAT_ON_NUM) == 2 ){
						// でかいフロート
						WFLBY_FLOAT_CONT_PlaySe( my_float_on, my_float, my_float_offs_chk, sc_WFLBY_FLOAT_DATA[ room ][ season ].monsno_big_snd_ex );
					}else{
						// ちっこいフロート
						WFLBY_FLOAT_CONT_PlaySe( my_float_on, my_float, my_float_offs_chk, sc_WFLBY_FLOAT_DATA[ room ][ season ].monsno_sml_snd_ex );
					}
				}
				break;
			}
...
static void WFLBY_FLOAT_CONT_SetReserveLock( WFLBY_FLOAT_CONT* p_sys, u32 idx )
{
	...
	for( i=0; i<WFLBY_FLOAT_ON_NUM; i++ ){
		...
		if( now_station <= WFLBY_FLOAT_STATION_NUM ){

			// 駅ロックチェック
			WFLBY_3DMAPOBJCONT_FLOAT_GetSheetPos( p_sys->p_mapobjcont, idx, i, &matrix );
			lock_x = ((p_sys->station_pos[now_station].x+WFLBY_FLOAT_CONT_LOCK_X_OFS)*WFLBY_3DMATRIX_GRID_SIZ);
			if( lock_x > matrix.x ){
				// ロックかける
				WFLBY_SYSTEM_SetFloatReserveLock( p_sys->p_system, posidx, now_station );
			}
		}
	}
}

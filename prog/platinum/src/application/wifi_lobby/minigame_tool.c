...
#define MNGM_RESULT_BALLOON_DEF_X			( 68 )
#define MNGM_RESULT_BALLOON_SIZ_X			( 24 )
#define MNGM_RESULT_BALLOON_DEF_Y			( 42 )
#define MNGM_RESULT_BALLOON_SIZ_Y			( 40 )
...
static BOOL MNGM_RESULT_Balloon_Main( MNGM_BALLOON_WK* p_wk );
...
static void MNGM_TALKWIN_Exit( MNGM_TALKWIN* p_wk )
{
	...
	// ウィンドウと文字列バッファを破棄
	for( i=0; i<MNGM_TALKWIN_IDX_NUM; i++ ){
		MNGM_TALKWIN_CleanTimeWork( p_wk, i );
...
static MNGM_ENTRYWK* MNGM_ENTRY_CommonInit( const MNGM_ENRES_PARAM* cp_commparam, u32 gametype, u32 heapID )
{
	...
	// メッセージシステム初期化
	{
		...
		vip = MNGM_ENRES_PARAM_GetVipFlag( &p_wk->comm_param, p_wk->comm_param.my_playerid );
	...
	case MNGM_ENTRY_SEQ_PLLIST_ON:
		// プレイヤー分のデータ作成
		{
			...
			for( i=0; i<p_wk->comm_param.num; i++ ){
				...
				// 名前表示
				vip = MNGM_ENRES_PARAM_GetVipFlag( &p_wk->comm_param, i );
...
static void MNGM_COMMPDATA_Init( MNGM_COMM_PDATA* p_wk, const MNGM_ENRES_PARAM* cp_comm_param )
{
	...
	for( i=0; i<cp_comm_param->num; i++ ){
		p_wk->cp_status[i]	= CommInfoGetMyStatus( cp_comm_param->pnetid[i] );
...
static void MNGM_RESULT_Tcb_Balloon( TCB_PTR tcb, void* p_work )
{
	...
	case MNGM_RESULT_BALLOON_SEQ_BALLOON_ONWAIT:
		result = MNGM_RESULT_Balloon_Main( &p_wk->balloon );
		if( result == TRUE ){
	...
	case MNGM_RESULT_BALLOON_SEQ_SYNCWAIT:
		if( CommIsTimingSync( MNGM_SYNC_RESULT_END ) ){
			...
			if( p_wk->comm_param.replay ){
				...
				p_wk->seq = MNGM_RESULT_BALLOON_SEQ_RETRY_MAIN;
			}else{
				p_wk->seq ++;
			}
		}
...
{
	// リミットチェック
	if( num > MNGM_RESULT_BALLOON_CLACT_NUM ){
		num = MNGM_RESULT_BALLOON_CLACT_NUM;
	}
	p_wk->num = num;
	p_wk->seq = 0;
	p_wk->idx = 0;
...
static BOOL MNGM_RESULT_Balloon_Main( MNGM_BALLOON_WK* p_wk )
{
	...
	case MNGM_RESULT_BALLOON_SEQ_ON:
		...
		BmpMenuWinWrite( &p_wk->win, WINDOW_TRANS_OFF, 
				MNGM_RESULT_BALLOON_SYSWIN_CGX, MNGM_RESULT_BALLOON_SYSWIN_PAL );
	...
	case MNGM_RESULT_BALLOON_SEQ_BLN_ON:
		...
		CLACT_SetDrawFlag( p_wk->p_clwk[ p_wk->idx ], TRUE );
...
static void MNGM_RESULT_BallslowBalancePlayerTblDraw( MNGM_RESULTWK* p_wk, u32 plidx, u32 rank )
{
	...
	// 名前表示
	vip = MNGM_ENRES_PARAM_GetVipFlag( &p_wk->comm_param, plidx );

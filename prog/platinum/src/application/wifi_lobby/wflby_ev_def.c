...
enum{
	...
	WFLBY_EV_DEF_PLAYERIN_NORMAL_SEQ_MSG01,		// ”ガジェットをもらった”
	WFLBY_EV_DEF_PLAYERIN_NORMAL_SEQ_MSG02,		// ”たのしんでね！”
...
void WFLBY_EV_DEF_StartPlayer( WFLBY_ROOMWK* p_rmwk )
{
	...
	WFLBY_EVENT_SetPrivateEvent( p_event, plno, &sc_WFLBY_EV_DEF_PLAYERIN[ p_save->pl_inevno ], NULL );
...
void WFLBY_EV_DEF_StartNpc( WFLBY_ROOMWK* p_rmwk )
{
	...
	for( i=0; i<WFLBY_PLAYER_MAX; i++ ){
		if( plno != i ){	// プレイヤー以外
			WFLBY_EVENT_SetPrivateEvent( p_event, i, &sc_WFLBY_EV_DEF_NPCWAIT, NULL );
		}
	}
}
		...
		for( i=0; i<WFLBY_EV_DEF_EVNO_MAX; i++ ){
			...
			if( cp_data[no].event[i] == eventno ){
				...
				case WFLBY_EV_DEF_EVTYPE_DEF:
					...
					WFLBY_EVENT_SetPrivateEvent( p_event, plno, &cp_data[no].evdata, NULL );
					break;	
				// 割り込みイベント
				case WFLBY_EV_DEF_EVTYPE_INTR:
					...
					WFLBY_EVENT_SetPrivateIntrEvent( p_event, plno, &cp_data[no].evdata, NULL );
					break;		
				}
				return ;
			}
		...
		if( plno == WFLBY_SYSTEM_GetMyIdx( p_system ) ){
			...
			if( (WFLBY_ROOM_UNDERWIN_TrCardGetDraw( p_rmwk ) == TRUE) ){
				if( plno != WFLBY_ROOM_UNDERWIN_TrCardGetPlIdx( p_rmwk ) ){
					// NPC動作設定
					WFLBY_3DOBJCONT_SetWkMove( p_person, WFLBY_3DOBJCONT_MOVENPC );
				}
			}else{
				// NPC動作設定
				WFLBY_3DOBJCONT_SetWkMove( p_person, WFLBY_3DOBJCONT_MOVENPC );
...
static BOOL WFLBY_EV_DEF_PlayerInNormal_Before( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_PLAYERIN_NORMAL_SEQ_ADD:
		...
		// 落とす
		WFLBY_3DOBJCONT_SetWkMove( p_player, WFLBY_3DOBJCONT_MOVEFLYDOWN );
	...
	case WFLBY_EV_DEF_PLAYERIN_NORMAL_SEQ_FADEWAIT:
		if( (WFLBY_ROOM_GetFadeFlag( p_rmwk ) == FALSE) &&
			...
			WFLBY_3DOBJCONT_SetWkMove( p_player, WFLBY_3DOBJCONT_MOVENONE );
	...
	case WFLBY_EV_DEF_PLAYERIN_NORMAL_SEQ_MSG01:
		...
		WFLBY_ROOM_TALKWIN_PrintStr( p_rmwk, p_str );
		p_evwk->ret_seq = WFLBY_EV_DEF_PLAYERIN_NORMAL_SEQ_MSG02;
		WFLBY_EVENTWK_SetSeq( p_wk, WFLBY_EV_DEF_PLAYERIN_NORMAL_SEQ_MSGWAIT );
		break;
	...
	case WFLBY_EV_DEF_PLAYERIN_NORMAL_SEQ_END:
		// 基本動作の関数を設定
		WFLBY_3DOBJCONT_SetWkMove( p_player, WFLBY_3DOBJCONT_MOVEPLAYER );
		...
		// プレイヤーメイン常駐イベントを設定
		{
			...
			p_event = WFLBY_ROOM_GetEvent( p_rmwk );
			plno = WFLBY_ROOM_GetPlno( p_rmwk );
			WFLBY_EVENT_SetPrivateEvent( p_event, plno, &sc_WFLBY_EV_DEF_PLAYERMAIN, NULL );
	...
	case WFLBY_EV_DEF_PLAYERIN_MG_SEQ_ADD:
		{
			...
			// 割り込みイベントで切断処理
			WFLBY_EVENT_SetPrivateIntrEvent( p_event, plno, 
...

		{
			...
			if( st != WF2DMAP_OBJST_NONE ){
				...
				if( WFLBY_ROOM_UNDERWIN_TrCardGetDraw( p_rmwk ) == TRUE ){
					...
					if( (st == WF2DMAP_OBJST_WALK) ||
						...
						if( npc_id != plno ){
							...
							WFLBY_3DOBJCONT_SetWkMove( p_npc, p_plwk->infodraw_retmvtype );	// NPC動作再開
						}
						WFLBY_ROOM_UNDERWIN_TrCardOff( p_rmwk );
					}
				}
				...
				if( WFLBY_ROOM_GadGet_GetFlag( p_rmwk ) == TRUE ){
					...
					// 割り込みイベント発生
					WFLBY_EVENT_SetPrivateIntrEvent( p_event, plno, &sc_WFLBY_EV_DEF_ALL_GADGET, NULL );
				...
				if( WFLBY_SYSTEM_CheckTalkReq( p_system ) ){
					WFLBY_EVENT_SetPrivateIntrEvent( p_event, plno, &sc_WFLBY_EV_DEF_TALK_B, NULL );
					// イベント発動したら動作を一時停止
					WFLBY_3DOBJCONT_SetWkMove( p_player, WFLBY_3DOBJCONT_MOVENONE );
					WFLBY_ROOM_GadGet_SetStop( p_rmwk, TRUE );	// ボタン反応なし
					return FALSE;
					...
					// いたらその人と話す
					{
						...
						if( p_npc ){
							...
							if( (status == WF2DMAP_OBJST_NONE) && (talk_busy == FALSE) ){	
								...
								// イベント発動したら動作を一時停止
								WFLBY_3DOBJCONT_SetWkMove( p_player, WFLBY_3DOBJCONT_MOVENONE );
								WFLBY_3DOBJCONT_SetWkMove( p_npc, WFLBY_3DOBJCONT_MOVENONE );
...
static BOOL WFLBY_EV_DEF_PlayerOutNormal_Before( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_PLAYER_OUTNORMAL_SEQ_MSGSTART:			
		// 動作停止
		{
			...
			WFLBY_3DOBJCONT_SetWkMove( p_person, WFLBY_3DOBJCONT_MOVENONE );
		}
	...
	case WFLBY_EV_DEF_PLAYER_OUTNORMAL_SEQ_FLYUP:			
		...
		
		{
			...
			// 上に跳ぶ
			WFLBY_3DOBJCONT_SetWkMove( p_person, WFLBY_3DOBJCONT_MOVEFLYUP );
		}
...
static BOOL WFLBY_EV_DEF_PlayerOutSubChan_Before( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_PLAYER_SUBCHAN_SEQ_NG:
		{
			...
			WFLBY_3DOBJCONT_SetWkMove( p_player, moveno );
	...
	case WFLBY_EV_DEF_PLAYER_SUBCHAN_SEQ_NGMSG:
		...
		if( p_evwk->wait <= 0 ){	
			...
			p_str = WFLBY_ROOM_MSG_Get( p_rmwk, WFLBY_DEFMSG_TYPE_INFO, p_evwk->msg_idx );
			WFLBY_ROOM_TALKWIN_PrintStr( p_rmwk, p_str );
			// メッセージ完了待ち
			WFLBY_EVENTWK_AddSeq( p_wk );
		}
...
static BOOL WFLBY_EV_DEF_PlayerOutMiniGame_Before( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_PLAYER_MG_SEQ_INTR_START:
		...
		WFLBY_EVENT_SetPrivateIntrEvent( p_event, plno, &sc_WFLBY_EV_DEF_PLAYER_MGSTART, &p_evwk->mg_param );
	...
	case WFLBY_EV_FLOAT_SEQ_RET:				
		{
			...
			WFLBY_3DOBJCONT_SetWkMove( p_evwk->p_obj, WFLBY_3DOBJCONT_MOVEPLAYER );
			// ワーク破棄
			WFLBY_EVENTWK_DeleteWk( p_wk );
...
static BOOL WFLBY_EV_DEF_NpcWait_Before( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_NPCWAIT_SEQ_STATUS_CHECK:
		{
			...
			if( result ){
				...
				WFLBY_EVENT_SetPrivateIntrEvent( p_event, plno, &sc_WFLBY_EV_DEF_NPCJUMP, &p_evwk->intrparam );
			}else{
				
				// ステータス的にジャンプしていく必要がないなら
				// 普通に落とす
				WFLBY_3DOBJCONT_SetWkMove( p_npc, WFLBY_3DOBJCONT_MOVEFLYDOWN );
	...
	case WFLBY_EV_DEF_NPCWAIT_SEQ_END:
		...
		// メイン動作へ
		WFLBY_3DOBJCONT_SetWkMove( p_npc, WFLBY_3DOBJCONT_MOVENPC );
		WFLBY_EVENT_SetPrivateEvent( p_event, plno, &sc_WFLBY_EV_DEF_NPCMAIN, NULL );
...
static BOOL WFLBY_EV_DEF_NpcMain_Before( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_NPCMAIN_SEQ_MAIN:
		...
		if( WFLBY_SYSTEM_FLAG_GetUserOut(p_system, plno) ){
			...
			WFLBY_3DOBJCONT_SetWkMove( p_npc, WFLBY_3DOBJCONT_MOVENONE );	// 動作停止
			// NPC状態表示不可能
			WFLBY_ROOM_SetNpcInfoDrawBusy( p_rmwk, plno, TRUE );				
			...
			WFLBY_EVENT_SetPrivateEvent( p_event, plno, &sc_WFLBY_EV_DEF_NPCEND, NULL );
		...
		if( WFLBY_SYSTEM_Event_GetParade( p_system ) == TRUE ){
			...
			if( idx != DWC_LOBBY_USERIDTBL_IDX_NONE ){
				...
				WFLBY_EVENT_SetPrivateEvent( p_event, plno, &sc_WFLBY_EV_DEF_NPCFLOAT, &p_evwk->intrparam );
				break;
			}
		}
...
static BOOL WFLBY_EV_DEF_NpcEnd_Brfore( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_NPCEND_SEQ_OUT:
		WFLBY_3DOBJCONT_SetWkMove( p_npc, WFLBY_3DOBJCONT_MOVEFLYUP );	// 上に跳んでく
		WFLBY_EVENTWK_AddSeq( p_wk );
		break;
	...
	case WFLBY_EV_DEF_NPCEND_SEQ_END:
		...
		// 入室待ちへ
		{
			WFLBY_EVENT* p_event;
			p_event = WFLBY_ROOM_GetEvent( p_rmwk );
			WFLBY_EVENT_SetPrivateEvent( p_event, plno, &sc_WFLBY_EV_DEF_NPCWAIT, NULL );
		}
	...
	case WFLBY_EV_DEF_NPCFLOAT_SEQ_INIT:
		{
			...
			// 上に飛び上がる
			WFLBY_3DOBJCONT_SetWkMove( p_evwk->p_npc, WFLBY_3DOBJCONT_MOVEFLYUP );
	...
	case WFLBY_EV_DEF_NPCFLOAT_SEQ_MAIN:
		{
			...
			if( WFLBY_SYSTEM_GetFloatMove( p_system, p_evwk->float_idx ) == FALSE ){
...
static BOOL WFLBY_EV_DEF_NpcJump_Before( WFLBY_EVENTWK* p_wk, WFLBY_ROOMWK* p_rmwk, u32 plno )
{
	...
	case WFLBY_EV_DEF_NPCJUMP_SEQ_UP:
		...
		// その人の動作を変更
		WFLBY_3DOBJCONT_SetWkMove( p_npc, WFLBY_3DOBJCONT_MOVEFLYUP );
		WFLBY_EVENTWK_AddSeq( p_wk );
		break;

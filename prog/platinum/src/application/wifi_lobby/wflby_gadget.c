...
static void WFLBY_GADGET_OBJ_Draw( WFLBY_GADGET* p_sys, WFLBY_GADGET_OBJ* p_wk );
...
static void WFLBY_GADGET_ANM_Draw_Obj( WFLBY_GADGET* p_sys, WFLBY_GADGET_OBJ* p_wk );
 ...
 */
//-----------------------------------------------------------------------------
WFLBY_GADGET* WFLBY_GADGET_Init( WFLBY_MAPCONT* p_mapcont, WFLBY_CAMERA* p_camera, WFLBY_3DOBJCONT* p_objcont, u32 heapID, u32 gheapID )
{
	WFLBY_GADGET* p_sys;
...
//					WFLBY_GADGET_Start( p_sys, i, (gf_mtRand() % WFLBY_ITEM_NUM) );
					WFLBY_GADGET_Start( p_sys, i, WFLBY_ITEM_WINK02 );
				}
			}
		}
	}
...
//	pFunc[ p_wk->gadget_type ]( p_sys, p_wk );
	WFLBY_GADGET_ANM_Draw_Obj( p_sys, p_wk );		// 基本これで全部いける
}
	...
	for( i=0; i<WFLBY_GADGET_OBJ_MAX; i++ ){
		if( WFLBY_GADGET_OBJ_CheckRes( p_wk, i ) == TRUE ){
			if( cal_check == FALSE ){
				...
				if( WFLBY_3DMAPOBJ_MDL_BOXCheck( &p_sys->mdl[ p_wk->cp_objres[i]->mdl ], 
							&p_wk->obj[i] ) == FALSE ){
					// 矩形内にいないなら、出さない
					break;
				}

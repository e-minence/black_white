...
typedef struct _BOXAPP_WORK{
	...
	u32 gds_flag_reset_bit;		///<GDSボックス送信済みフラグリセット対象(ビット指定)
}BOXAPP_WORK;
...
PROC_RESULT BoxProc_End( PROC* proc, int* seq )
{
	...
	{//触ったボックスはGDS送信済みフラグをリセットする
		MISC *misc = SaveData_GetMisc(wk->saveData);
		int box_no = 0;
		OS_TPrintf("gds_flag_reset_bit = %d\n", wk->gds_flag_reset_bit);
		while(wk->gds_flag_reset_bit){
			if(wk->gds_flag_reset_bit & 1){
				MISC_SetGdsSend_Box(misc, box_no, 0);
			}
			wk->gds_flag_reset_bit >>= 1;
			box_no++;
		}
	}
...
static void SubSeq_Menu_WallPaper( BOXAPP_WORK* wk, u32* seq )
{
	...
	case SEQ_WPCHANGE_START:
		...
		wk->gds_flag_reset_bit |= 1 << BOXDAT_GetCureentTrayNumber( wk->boxData );
		(*seq) = SEQ_VIEWWAIT_RETURN;
		break;
...
static BOOL VParaSet_CatchPokeAzuke( BOXAPP_WORK* wk, u32 trayNum )
{
	...
	if( BOXDAT_PutPokemonBox( wk->boxData, trayNum, catchPoke->ppp ) )
	{
		vpara->cursor.poke_catch_flag = CURSOR_CATCH_NONE;
		CursorPointPokeUpdate( wk );
		wk->gds_flag_reset_bit |= 1 << trayNum;
		return TRUE;
	}
	return FALSE;
}

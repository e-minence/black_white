...
BOOL BoxAppView_IconSysInit(BOX_ICON_SYS* wk, BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, CLACT_SET_PTR actsys, ARCHANDLE* p_boxgra_handle )
{
	...
	ArcUtil_HDL_PalSysLoad(wk->arcHandle, PokeIconPalArcIndexGet(), NNS_G2D_VRAM_TYPE_2DMAIN, OBJPAL_ICON*0x20, 
			HEAPID_BOX_VIEW, &proxy);

	tmpbuf = ArcUtil_HDL_PalDataGet(wk->arcHandle, PokeIconPalArcIndexGet(), &palData, HEAPID_BOX_VIEW);
	if(tmpbuf)
	{
		BOOL  cmpFlag;
...
//	wk->cellBuf = ArcUtil_HDL_LoadEx(p_boxgra_handle, NARC_box_icon_lz_ncer, TRUE, HEAPID_BOX_VIEW, ALLOC_TOP, &(wk->cellBankSize));
	wk->cellBuf = ArcUtil_HDL_CellBankDataGet(p_boxgra_handle, NARC_box_icon_lz_ncer, TRUE, &(wk->cellBank), HEAPID_BOX_VIEW);
	wk->anmBuf = ArcUtil_HDL_AnimBankDataGet(p_boxgra_handle, NARC_box_icon_lz_nanr, TRUE, &(wk->anmBank), HEAPID_BOX_VIEW);

	wk->item_cellBuf = ArcUtil_HDL_CellBankDataGet(p_boxgra_handle, NARC_box_itemicon_lz_ncer, TRUE, &(wk->item_cellBank), HEAPID_BOX_VIEW);
	wk->item_anmBuf = ArcUtil_HDL_AnimBankDataGet(p_boxgra_handle, NARC_box_itemicon_lz_nanr, TRUE, &(wk->item_anmBank), HEAPID_BOX_VIEW);

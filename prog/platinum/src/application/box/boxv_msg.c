...
BOOL BoxAppView_MsgInit( MSG_VIEW_WORK* wk, BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, GF_BGL_INI* bgl, CLACT_SET_PTR actsys,
	 MSGDATA_MANAGER* msgman_boxmes, const WORDSET* wordset, int winType, ARCHANDLE* p_boxgra_handle )
{
	wk->vwk = vwk;
	wk->bgl = bgl;
	wk->vpara = vpara;
	...
	wk->markCharBuf = ArcUtil_HDL_CharDataGet( p_boxgra_handle, NARC_box_marking_lz_ncgr, TRUE, &(wk->markCharData), HEAPID_BOX_VIEW );

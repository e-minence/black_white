...
static void SetupBGGraphicData( BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, ARCHANDLE* p_handle );
static void Box3D_BGInit(void);
static void InitMainOBJ( BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, ARCHANDLE* p_handle );
...
BOOL BoxAppView_Init( BOXAPP_VIEW_WORK** vwk, const BOXAPP_VPARAM* vpara, const BOXAPP_WORK* mainwk )
{
	...
	if( wk != NULL )
	{
		...
		if( wk->bgl != NULL )
		{
			...
			ArcUtil_HDL_PalSysLoad(p_handle, NARC_box_m_obj_nclr, NNS_G2D_VRAM_TYPE_2DMAIN, 0, 
			...
			*vwk = wk;
			...
			flg &= BoxAppView_IconSysInit( &(wk->iconSysWork), wk, wk->vpara, wk->clactSys, p_handle );
			flg &= BoxAppView_TrayInit( &(wk->trayWork), wk, wk->vpara, wk->bgl, wk->clactSys );
			flg &= BoxAppView_CursorInit( &(wk->cursorWork), wk, wk->vpara, wk->clactSys, p_handle );
			flg &= BoxAppView_ButtonInit( &(wk->buttonWork), wk, wk->vpara, wk->clactSys, p_handle );
			flg &= BoxAppView_StatusInit( &(wk->statusWork), wk, wk->vpara, wk->bgl, wk->clactSys, BoxApp_GetMsgManager(mainwk), p_handle );
			flg &= BoxAppView_MsgInit( &(wk->msgWork), wk, wk->vpara, wk->bgl, wk->clactSys, BoxApp_GetMsgManager(mainwk), BoxApp_GetWordSet(mainwk), BoxApp_GetTalkWinType(mainwk), p_handle );
			flg &= BoxAppView_ExWinInit( &(wk->exWinWork), wk, wk->vpara, wk->bgl, wk->clactSys, p_handle );
			flg &= BoxAppView_PartyInit( &(wk->partyWork), wk, wk->vpara, wk->bgl, wk->clactSys, p_handle );
			...
			flg &= BoxAppView_CompareInit( &(wk->compareWork), wk, wk->vpara, wk->bgl, wk->clactSys, BoxApp_GetMsgManager(mainwk), p_handle );
...
static void SetupBGGraphicData( BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, ARCHANDLE* p_handle )
{
	ArcUtil_HDL_BgCharSet( p_handle, NARC_box_bg0_lz_ncgr, vwk->bgl, FRM_MAIN_MESSAGE, 0, 0, TRUE, HEAPID_BOX_VIEW );
	...
	ArcUtil_HDL_BgCharSet(p_handle, NARC_box_bg1_lz_ncgr, vwk->bgl, FRM_MAIN_STATUS, 0, 0, TRUE, HEAPID_BOX_VIEW);
	ArcUtil_HDL_PalSet( p_handle, NARC_box_bg1_nclr, PALTYPE_MAIN_BG, 0, 0x20*PAL_FIX_USE_MAX, HEAPID_BOX_VIEW );

	if( BoxAppVPara_GetBoxMode(vpara) != BOX_MODE_COMPARE )
	{
		ArcUtil_HDL_ScrnSet(p_handle, NARC_box_bg1_lz_nscr, vwk->bgl, FRM_MAIN_STATUS, 0, 0, TRUE, HEAPID_BOX_VIEW);
	}
	...
	BoxAppView_Reel_Setup( vwk->reelWork, p_handle );
	BoxAppView_SetupCompare( vwk->compareWork, p_handle );
...
static void InitMainOBJ( BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, ARCHANDLE* p_handle )
{
	BoxAppView_CursorActSet( &vwk->cursorWork, p_handle );
	BoxAppView_ButtonActSet( &vwk->buttonWork, p_handle );

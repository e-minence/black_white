 ...
 */
//------------------------------------------------------------------
BOOL BoxAppView_StatusInit( STATUS_VIEW_WORK* wk, BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, GF_BGL_INI* bgl, CLACT_SET_PTR actsys, const MSGDATA_MANAGER* msgman, ARCHANDLE* p_boxgra_handle )
{
	int i;
	...
	wk->infobackCellBuf = ArcUtil_HDL_CellBankDataGet(p_boxgra_handle, NARC_box_info_back_lz_ncer, TRUE, &(wk->infobackCellData), HEAPID_BOX_VIEW);
	wk->infobackAct = NULL;
...
static void StatusPrintCore( STATUS_VIEW_WORK* wk )
{
	...
	PokeGraArcDataGetPPP( &spriteArc, poke->poke_data, PARA_FRONT, FALSE );
	wk->sprite = SoftSpriteAdd( wk->spriteMan, &spriteArc, POKEIMAGE_WRITE_X, POKEIMAGE_WRITE_Y, 0, 0, NULL, NULL );

 ...
 */
//------------------------------------------------------------------
BOOL BoxAppView_ButtonInit( BUTTON_VIEW_WORK* wk, BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, CLACT_SET_PTR actsys, ARCHANDLE* p_boxgra_handle )
{
	wk->vwk = vwk;
	wk->actSys = actsys;
	wk->vpara = vpara;
	...
	wk->cellBuf = ArcUtil_HDL_CellBankDataGet(p_boxgra_handle, NARC_box_m_button_lz_ncer, TRUE, &(wk->cellBank), HEAPID_BOX_VIEW);
	wk->anmBuf = ArcUtil_HDL_AnimBankDataGet(p_boxgra_handle, NARC_box_m_button_lz_nanr, TRUE, &(wk->anmBank), HEAPID_BOX_VIEW);
 ...
 */
//------------------------------------------------------------------
void BoxAppView_ButtonActSet( BUTTON_VIEW_WORK* wk, ARCHANDLE* p_boxgra_handle )
{
	CLACT_HEADER  header;

	NNS_G2dInitImageProxy( &(wk->imgProxy) );
	ArcUtil_HDL_CharSysLoad( p_boxgra_handle, NARC_box_m_button_lz_ncgr, TRUE, CHAR_MAP_1D, 0,

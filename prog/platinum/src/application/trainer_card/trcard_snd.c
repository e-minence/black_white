...
//--------------------------------------------------------------
/**
 * @brief	使用中で、すでに再生終了しているものはクリアする！
 *
 * @param	wk		SND_BADGE_WORK型のポインタ
 *
 * @retval	none
...
//--------------------------------------------------------------
/**
 * @brief	渡されたバッチが使用中の時は、同じプレイヤーで再生する！
 *
 * @param	wk		SND_BADGE_WORK型のポインタ
 * @param	no		バッジナンバー(0から7)
 * @param	lv		バッジレベル(0～4)
...
//--------------------------------------------------------------
/**
 * @brief	プレイヤーが全て埋まっている時は、一番古いデータを消して、上書きする
 *
 * @param	wk		SND_BADGE_WORK型のポインタ
 * @param	no		バッジナンバー(0から7)
 * @param	lv		バッジレベル(0～4)
...
//--------------------------------------------------------------
/**
 * @brief	ソート(渡された値を先頭にする)
 *
 * @param	wk		SND_BADGE_WORK型のポインタ
 * @param	num		SND_BADGE_WORK.player[]のナンバー
 * @param	player	使用しているプレイヤー(player[?][0])
 * @param	no		使用しているバッジNO(player[?][1])
 *
 * @retval	none
 */
//--------------------------------------------------------------
...
//--------------------------------------------------------------
/**
 * @brief	バッチにより音程を操作する・レベルにより音程の微調整をする
 *
 * @param	wk		SND_BADGE_WORK型のポインタ
 * @param	no		バッジナンバー(0から7)
 * @param	lv		バッジレベル(0～4)

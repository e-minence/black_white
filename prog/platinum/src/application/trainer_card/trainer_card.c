...
static void SetTrCardBgGraphic( TR_CARD_WORK * wk, ARCHANDLE* p_handle );
...
PROC_RESULT TrCardProc_Init( PROC * proc, int * seq )
{
	...
	p_handle = ArchiveDataHandleOpen( ARC_TR_CASE_GRA, HEAPID_TR_CARD );
		...
		if (inPokeBookHold){
			...
			case 0:
				buf = ArcUtil_HDL_PalDataGet(
					p_handle, NARC_trainer_case_card_0_NCLR, &dat, HEAPID_TR_CARD );
				break;
			case 1:
				buf = ArcUtil_HDL_PalDataGet(
					p_handle, NARC_trainer_case_card_1_NCLR, &dat, HEAPID_TR_CARD );
				break;
			case 2:
				buf = ArcUtil_HDL_PalDataGet(
					p_handle, NARC_trainer_case_card_2_NCLR, &dat, HEAPID_TR_CARD );
				break;
			case 3:
				buf = ArcUtil_HDL_PalDataGet(
					p_handle, NARC_trainer_case_card_3_NCLR, &dat, HEAPID_TR_CARD );
				break;
			case 4:
				buf = ArcUtil_HDL_PalDataGet(
					p_handle, NARC_trainer_case_card_4_NCLR, &dat, HEAPID_TR_CARD );
				break;
			case 5:
				buf = ArcUtil_HDL_PalDataGet(
					p_handle, NARC_trainer_case_card_5_NCLR, &dat, HEAPID_TR_CARD );
				break;
			}
		}else{
			buf = ArcUtil_HDL_PalDataGet(
					p_handle, NARC_trainer_case_card_6_NCLR, &dat, HEAPID_TR_CARD );
		}
...
static void SetUniTrainerPalette(const u8 inTrainerNo, ARCHANDLE* p_handle)
{
	...
	NNSG2dPaletteData *dat;
	buf = ArcUtil_HDL_PalDataGet(
			p_handle, NARC_trainer_case_trdp_union_card_NCLR, &dat, HEAPID_TR_CARD );
...
static void SetTrCardBgGraphic( TR_CARD_WORK * wk, ARCHANDLE* p_handle )
{
	...
	// CARD PALETTE
	SetCardPalette(wk->TrCardData->CardRank, wk->TrCardData->PokeBookFlg, p_handle);
	...
	// UNSER_CASE_COVER PALETTE(UNDER_DISPLAY)
	{
		...
		NNSG2dPaletteData *dat;
		buf = ArcUtil_HDL_PalDataGet(
				p_handle, NARC_trainer_case_card_futa_NCLR, &dat, HEAPID_TR_CARD );
		DC_FlushRange( dat->pRawData, 16*2*16 );
		GX_LoadBGPltt( dat->pRawData, 0, 16*2*16 );
		sys_FreeMemoryEz(buf);
	}
	// CASE PALETTE
	SetCasePalette(wk->TrCardData->Version, p_handle);
	...
	if (wk->TrCardData->UnionTrNo == UNION_TR_NONE){
		{
			BOOL rc;
			//アーカイブデータ取得
			wk->TrArcData = ArcUtil_HDL_Load(	p_handle, NARC_trainer_case_card_trainer_NCGR,
		...
		if (wk->TrCardData->TrSex == PM_MALE){
			//男
			wk->TrScrnArcData = ArcUtil_HDL_ScrnDataGet(p_handle, NARC_trainer_case_card_trainer01_NSCR,
													0, &wk->ScrnData, HEAPID_TR_CARD);
		}else{
			//女
			wk->TrScrnArcData = ArcUtil_HDL_ScrnDataGet(p_handle, NARC_trainer_case_card_trainer02_NSCR,
													0, &wk->ScrnData, HEAPID_TR_CARD);
		}
	}else{
		//ユニオンルームで他の人のデータを見る時
	...
	ArcUtil_HDL_BgCharSet(
			p_handle, NARC_trainer_case_card_NCGR, wk->bgl, TRC_BG_CARD, 0, 0, 0, HEAPID_TR_CARD );
	ArcUtil_HDL_ScrnSet(
			p_handle, NARC_trainer_case_card_faca_NSCR, wk->bgl, TRC_BG_CARD, 0, 0, 0, HEAPID_TR_CARD );
	//CASE
	ArcUtil_HDL_BgCharSet(
			p_handle, NARC_trainer_case_card_case_NCGR, wk->bgl, TRC_BG_CASE, 0, 0, 0, HEAPID_TR_CARD );
	ArcUtil_HDL_ScrnSet(
			p_handle, NARC_trainer_case_card_case_NSCR, wk->bgl, TRC_BG_CASE, 0, 0, 0, HEAPID_TR_CARD );

	//UNDER_CASE
	ArcUtil_HDL_BgCharSet(
			p_handle, NARC_trainer_case_card_case2_NCGR, wk->bgl, TRC_BG_UNDER_CASE, 0, 0, 0, HEAPID_TR_CARD );
	ArcUtil_HDL_ScrnSet(
			p_handle, NARC_trainer_case_card_case2_NSCR, wk->bgl, TRC_BG_UNDER_CASE, 0, 0, 0, HEAPID_TR_CARD );
	//UNDER_CASE_COVER
	ArcUtil_HDL_BgCharSet(
			p_handle, NARC_trainer_case_card_futa_NCGR, wk->bgl, TRC_BG_COVER, 0, 0, 0, HEAPID_TR_CARD );
	ArcUtil_HDL_ScrnSet(

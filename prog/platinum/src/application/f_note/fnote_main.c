...
static void FNOTE_BgGraphicSet( FNOTE_WORK * wk )
{
	...
	p_handle = ArchiveDataHandleOpen( ARC_FNOTE_GRA, HEAPID_FANTASYNOTE );

	if( MyStatus_GetMySex( wk->myst ) == 0 ){
		ArcUtil_HDL_BgCharSet(
			p_handle, NARC_f_note_gra_note_boy_NCGR,
			wk->bgl, GF_BGL_FRAME2_M, 0, 0, 0, HEAPID_FANTASYNOTE );
		ArcUtil_HDL_ScrnSet(
			p_handle, NARC_f_note_gra_note_boy_NSCR,
			wk->bgl, GF_BGL_FRAME2_M, 0, 0, 0, HEAPID_FANTASYNOTE );
		ArcUtil_HDL_PalSet(
			p_handle, NARC_f_note_gra_note_boy_NCLR,
			PALTYPE_MAIN_BG, 0, 0, HEAPID_FANTASYNOTE );
	}else{
		ArcUtil_HDL_BgCharSet(
			p_handle, NARC_f_note_gra_note_girl_NCGR,
			wk->bgl, GF_BGL_FRAME2_M, 0, 0, 0, HEAPID_FANTASYNOTE );
		ArcUtil_HDL_ScrnSet(
			p_handle, NARC_f_note_gra_note_girl_NSCR,
			wk->bgl, GF_BGL_FRAME2_M, 0, 0, 0, HEAPID_FANTASYNOTE );
		ArcUtil_HDL_PalSet(
			p_handle, NARC_f_note_gra_note_girl_NCLR,
			PALTYPE_MAIN_BG, 0, 0, HEAPID_FANTASYNOTE );
	}

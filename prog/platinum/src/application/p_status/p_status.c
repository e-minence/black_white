...
static void PokeStatusBgGraphicSet( PST_WORK * wk, ARCHANDLE* p_handle );
...
static PROC_RESULT PokeStatusProc_Init( PROC * proc, int * seq )
{
	...
	PokeStatusBgGraphicSet( wk, p_handle );		// BGグラフィックセット
	PokeStatusAlphaSet();
...
static void PokeStatusBgGraphicSet( PST_WORK * wk, ARCHANDLE* p_handle )
{
	ArcUtil_HDL_BgCharSet(
		p_handle, NARC_pst_gra_p_st_bg_NCGR,
		wk->bgl, GF_BGL_FRAME3_M, 0, 0, 0, HEAPID_POKESTATUS );
	ArcUtil_HDL_ScrnSet(
		p_handle, NARC_pst_gra_p_st_s0_NSCR,
		wk->bgl, GF_BGL_FRAME3_M, 0, 0, 0, HEAPID_POKESTATUS );
	ArcUtil_HDL_PalSet(
		p_handle, NARC_pst_gra_p_st_bg_NCLR,
		PALTYPE_MAIN_BG, 0, 0, HEAPID_POKESTATUS );

	ArcUtil_HDL_ScrnSet(
		p_handle, NARC_pst_gra_p_st_s4b_NSCR,
		wk->bgl, GF_BGL_FRAME2_M, 0, 0, 0, HEAPID_POKESTATUS );

	ArcUtil_HDL_BgCharSet(
		p_handle, NARC_pst_gra_p_st_but_NCGR,
		wk->bgl, GF_BGL_FRAME0_S, 0, 0, 0, HEAPID_POKESTATUS );
	ArcUtil_HDL_BgCharSet(
		p_handle, NARC_pst_gra_p_st_sub_bg_NCGR,
		wk->bgl, GF_BGL_FRAME1_S, 0, 0, 0, HEAPID_POKESTATUS );
	ArcUtil_HDL_ScrnSet(
		p_handle, NARC_pst_gra_p_st_sub_NSCR,
		wk->bgl, GF_BGL_FRAME1_S, 0, 0, 0, HEAPID_POKESTATUS );
	ArcUtil_HDL_PalSet(
		p_handle, NARC_pst_gra_p_st_sub_NCLR,
...
//		Snd_PerapVoicePlay( NULL, PM_MALE, PV_VOL_DEFAULT, 0 );
		...
		Snd_PMVoiceWaitPlay( wk->pp.mons, wait, wk->pp.form );

...
static BOOL guru2Comm_WideUseSendWorkSend(
	GURU2COMM_WORK *g2c, u8 code, const void *buf, u32 size );
...
//--------------------------------------------------------------
static BOOL guru2Comm_WideUseSendWorkSend(
	GURU2COMM_WORK *g2c, u8 cmd, const void *buf, u32 size )
{
	GF_ASSERT( cmd < G2COMM_MAX );
	GF_ASSERT( size+1 <= GURU2_WIDEUSE_SENDWORK_SIZE );
		...
		int netID, int size, void *pBuff, void *pWork )
{
	...
	if( netID == 0 && CommGetCurrentID() != 0 &&	//親から
		wk->g2p->guru2_mode == GURU2MODE_POKESEL &&	//ポケモンセレクト中に
		((*buff) == G2COMM_GMSBIT_CANCEL) ){		//キャンセルシグナル受信
		Guru2Comm_PokeSelOyaEndTcbSet( wk );
#ifdef D_GURU2_PRINTF_ON
		OS_Printf( "ぐるぐる　ポケモン選択中に親終了受信\n" );
#endif
	}
}
 ...
 */
//--------------------------------------------------------------
static void CommCB_Main_Cancel(
		int netID, int size, void *pBuff, void *pWork )
{
#if 0
	GURU2COMM_WORK *g2c = pWork;
	...
	if( netID != 0 ){	//親以外
		if( CommGetCurrentID() == 0 ){
			...
			case CREC_REQ_RIDATU_CHECK:
				...
				#else
				g2c->comm_game_cancel_bit |= 1 << netID;
				trans_work.ridatu_kyoka = TRUE;
				CommStateSetLimitNum( CommGetConnectNum() );
				#endif
			...
			Guru2Comm_SendData( g2c, G2COMM_GM_CANCEL,
				&trans_work, sizeof(GURU2COMM_END_CHILD_WORK) );
			OS_Printf("子機%dから離脱を受け取った→送信\n",netID);
		}
	}else{
		...
		case CREC_REQ_RIDATU_CHECK:
			...
			if( recieve_work->ridatu_kyoka == TRUE ){
				g2c->oya_share_num = recieve_work->oya_share_num;
			}
	...
	GURU2COMM_WORK *g2c = pWork;
	u32 bit = 1 << netID;
	g2c->comm_game_cancel_bit |= bit;
	g2c->comm_game_join_bit &= ~bit;
#endif
}
...
//==============================================================================
//	ポケモン交換,ゲームメインで親機が抜ける際に発生する切断タスク
//==============================================================================
//--------------------------------------------------------------
///	PSEL_OYAEND_WORK
//--------------------------------------------------------------
...
//--------------------------------------------------------------
/**
 * ポケモンセレクトで親機が抜ける際に発生する切断タスク
 * @param
 * @retval
 */
//--------------------------------------------------------------
static void Guru2Comm_PokeSelOyaEndTcb( TCB_PTR tcb, void *wk )
{
	GURU2COMM_WORK *g2c = wk;
	GURU2COMM_PSEL_OYAEND_TCB_WORK *work = &g2c->pselend_work;
	...
	case 1:
		if( CommIsTimingSync(COMM_GURU2_TIMINGSYNC_NO) ){
			...
			CommStateUnionBconCollectionRestart();
			g2c->comm_psel_oya_end_flag = G2C_OYA_END_FLAG_END;
			TCB_Delete( tcb );
			return;
		}
		break;
...
//--------------------------------------------------------------
/**
 * ポケモン交換,受付で親機が抜ける際に発生する切断タスクセット
 * @param
 * @retval
 */
//--------------------------------------------------------------
...
{
	TCB_PTR tcb;
	g2c->comm_psel_oya_end_flag = G2C_OYA_END_FLAG_EXE;
	tcb = TCB_Add( Guru2Comm_PokeSelOyaEndTcb, g2c, 0 );
	GF_ASSERT( tcb != NULL );
}
 ...
 * @retval	BOOL TRUE=全員キャンセル
 */
//--------------------------------------------------------------
BOOL Guru2Comm_PokeSelCancelCheck( GURU2COMM_WORK *g2c )
{
	int count = 0;
	u32 bit = g2c->comm_game_cancel_bit;
	...
	if( (g2c->g2p->receipt_num-count) <= 1 ){ //自身しかいない
		return( TRUE );
	}

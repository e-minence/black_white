...
#define BMPPOS_NAME_X_3_1 (CCX-10)
#define BMPPOS_NAME_Y_3_1 (CCY-2)
#define BMPPOS_NAME_X_3_2 (CCX+2)
...
enum
{
	...
	SEQNO_MAIN_KO_OYA_POKESEL_CANCEL_TASK_END_WAIT,
	SEQNO_MAIN_KO_SIGNAL_JOIN_WAIT,
	SEQNO_MAIN_KO_SEND_JOIN_TIME_OVER_CANCEL,
...
//	if( InitTPNoBuff(TP_SYNC) != TP_OK ){ GF_ASSERT(0); }
	...
	
	{
		...
		disc->rotate_offs_fx = NUM_FX32( DATA_DiscOffsetAngle
				[g2m->g2p->receipt_num][CommGetCurrentID()] );
	}
	...
	WIPE_SYS_Start( WIPE_PATTERN_WMS,
		WIPE_TYPE_FADEIN, WIPE_TYPE_FADEIN,	WIPE_FADE_BLACK, 8, 1, HEAPID_GURU2 );
...
static RET Guru2Proc_FadeInWait( GURU2MAIN_WORK *g2m )
{
	if( WIPE_SYS_EndCheck() ){
		int oya_end = g2m->g2c->comm_psel_oya_end_flag;
	
		if( oya_end == G2C_OYA_END_FLAG_EXE ){	//ポケモン選択で切断中
			g2m->seq_no = SEQNO_MAIN_KO_OYA_POKESEL_CANCEL_TASK_END_WAIT;
			return( RET_NON );
		}
	
		if( oya_end == G2C_OYA_END_FLAG_END ){	//ポケモン選択で切断済み
			g2m->force_end_flag = TRUE;			//タイミングを待たずに終了
			g2m->seq_no = SEQNO_MAIN_COMM_ERROR_OYA_CANCEL_MSG;
			return( RET_CONT );
		}
...
static RET Guru2Proc_OyaSignalJoinWait( GURU2MAIN_WORK *g2m )
{
	...
	if( Guru2Comm_PokeSelCancelCheck(g2m->g2c) ){	//キャンセル数チェック
		g2m->force_end_flag = TRUE;
		g2m->seq_no = SEQNO_MAIN_COMM_ERROR_MEMBER_MSG;
		OS_Printf( "ぐるぐる　親　全員キャンセル\n" );
		return( RET_CONT );
	}
	...
	if( g2m->comm_wait_frame >= COMM_WAIT_ERROR_FRAME ){ //時間切れ
		g2m->comm_wait_frame = 0;
		
		if( Guru2MainCommJoinNumGet(g2m) ){		//参加者がいる
			g2m->seq_no = SEQNO_MAIN_OYA_SEND_JOIN_CLOSE;
			OS_Printf( "ぐるぐる　親　時間切れによる参加者締め切りへ\n" );
			return( RET_CONT );
		}else{	//子全員がポケモンセレクト中による時間切れ、タイムオーバー
			u16 bit = G2COMM_GMSBIT_CANCEL;
			Guru2Comm_SendData( g2m->g2c, G2COMM_GM_SIGNAL, &bit, 2 );
//			g2m->force_end_flag = TRUE;
			g2m->seq_no = SEQNO_MAIN_COMM_ERROR_MEMBER_MSG;
			OS_Printf( "ぐるぐる　親　時間切れによりキャンセル\n" );
			return( RET_CONT );
		}
	}
...
static RET Guru2Proc_OyaConnectNumCheck( GURU2MAIN_WORK *g2m )
{
	...
	CommStateSetErrorCheck( TRUE, TRUE ); //これ以後の切断はエラー扱い
...
static RET Guru2Proc_OyaSendPlayMax( GURU2MAIN_WORK *g2m )
{
	...
	if( ret == TRUE ){
		...
		OS_Printf( "ぐるぐる 子へ最大参加数を転送 max=%d\n",
				g2m->comm.play_max );
	}
...
static RET Guru2Proc_OyaSendPlayNo( GURU2MAIN_WORK *g2m )
{
	...
	if( g2m->play_send_count == 0 ){	//親
		...
		if( ret == TRUE ){
			...
			OS_Printf( "ぐるぐる 子(%d)へプレイ番号転送 no=%d\n",
...
{
	if( Guru2MainCommOyaCancelCheck(g2m) == TRUE ){
		g2m->seq_no = SEQNO_MAIN_COMM_ERROR_OYA_CANCEL_MSG;
		return( RET_CONT );
	}
	
	if( Guru2MainCommJoinCloseCheck(g2m) == TRUE ){
		g2m->force_end_flag = TRUE;
		g2m->seq_no = SEQNO_MAIN_COMM_ERROR_JOIN_CLOSE_MSG;
		return( RET_CONT );
	}
	
	{
		int ret = Guru2Comm_SendData( g2m->g2c, G2COMM_GM_JOIN, NULL, 0 );
		
		if( ret == TRUE ){
			g2m->comm_wait_frame = 0; //エラー時間を一旦クリア
			g2m->seq_no = SEQNO_MAIN_KO_SIGNAL_JOIN_WAIT;
			return( RET_NON );
		}
	}
...
//--------------------------------------------------------------
/**
 * メイン　子　ポケモンセレクト親切断によるセレクト通信終了タスク終了待ち
 * @param	g2m	GURU2MAIN_WORK
 * @retval	GURU2RET	GURU2RET
 */
//--------------------------------------------------------------
static RET Guru2Proc_KoOyaPokeSelCancelTaskEndWait( GURU2MAIN_WORK *g2m )
{
	int oya_end = g2m->g2c->comm_psel_oya_end_flag;
	
	if( oya_end == G2C_OYA_END_FLAG_END ){
		g2m->force_end_flag = TRUE;			//タイミングを待たずに終了
		g2m->seq_no = SEQNO_MAIN_COMM_ERROR_OYA_CANCEL_MSG;
		return( RET_CONT );
	}
...
//--------------------------------------------------------------
/**
 * メイン　子　参加シグナル送信後の親締め切り待ち
 * @param	g2m	GURU2MAIN_WORK
 * @retval	GURU2RET	GURU2RET
 */
//--------------------------------------------------------------
static RET Guru2Proc_KoSignalJoinWait( GURU2MAIN_WORK *g2m )
{
	if( Guru2MainCommOyaCancelCheck(g2m) == TRUE ){
		g2m->seq_no = SEQNO_MAIN_COMM_ERROR_OYA_CANCEL_MSG;
		return( RET_CONT );
...
//--------------------------------------------------------------
/**
 * メイン　子　時間オーバーによるキャンセル送信
 * @param	g2m	GURU2MAIN_WORK
 * @retval	GURU2RET	GURU2RET
 */
//--------------------------------------------------------------
static RET Guru2Proc_KoSendJoinTimeOverCancel( GURU2MAIN_WORK *g2m )
{
	Guru2Comm_SendData( g2m->g2c, G2COMM_GM_CANCEL, NULL, 0 );
	g2m->seq_no = SEQNO_MAIN_COMM_ERROR_MEMBER_MSG;
	return( RET_CONT );
}
...
static RET Guru2Proc_EggAddInit( GURU2MAIN_WORK *g2m )
{
	...
	for( i = 0; i < g2m->g2p->receipt_num; i++ ){
...
static RET Guru2Proc_GameEndInit( GURU2MAIN_WORK *g2m )
{
	...
	if( g2m->front_eggact->id == CommGetCurrentID() ){
...
static RET Guru2Proc_OyaGameEndDataSend( GURU2MAIN_WORK *g2m )
{
	...
	for( i = 0; i < g2m->comm.play_max; i++ ){
		eact = &g2m->egg.eact[i];
		result.egg_angle[eact->id] = (u16)FX32_NUM( eact->angle );
	}
...
static RET Guru2Proc_KoGameEndDataRecv( GURU2MAIN_WORK *g2m )
{
	if( g2m->comm.game_result_get_flag == TRUE ){
		...
		for( i = 0; i < g2m->comm.play_max; i++ ){
			eact = &g2m->egg.eact[i];
			eact->angle = NUM_FX32( res->egg_angle[eact->id] );
		}
...
{
	int i,id;
	int no = g2m->front_eggact->no;
	
	for( i = 0; i < g2m->comm.play_max; i++ ){
		id = g2m->egg.eact[no].id;
		
		Guru2NameWin_WriteIDColor( g2m,
				g2m->comm.my_name_buf[id],
				i, id );
...
static RET Guru2Proc_OmakeAreaCheck( GURU2MAIN_WORK *g2m )
{
	...
	
	{
		...
		id += gf_rand() % (DATA_KinomiTbl[g2m->comm.play_max][1] - id);
...
{
	if( CommIsTimingSync(COMM_GURU2_TIMINGSYNC_NO) ){
		Guru2MainFriendEggExchange( g2m, g2m->front_eggact->id );
...
static RET (* const DATA_Guru2ProcTbl[SEQNO_MAIN_MAX])( GURU2MAIN_WORK *g2m ) =
{
	...
	Guru2Proc_KoOyaPokeSelCancelTaskEndWait,
	Guru2Proc_KoSignalJoinWait,
	Guru2Proc_KoSendJoinTimeOverCancel,
	...
	GURU2MAIN_WORK *g2m, STRBUF *name, int no, u32 col )
{
	...
	GF_STR_PrintColor( bmp, FONT_SYSTEM, name, 4, 4, MSG_NO_PUT, col, NULL );
...
{
	EGGWORK *egg = &g2m->egg;
	EGGKAGE *ekage = &egg->ekage[act->id];
...
{
	EGGWORK *egg = &g2m->egg;
	EGGCURSOR *ecs = &egg->ecursor[act->id];
	
	ecs->use_flag = TRUE;
	ecs->eact = act;
	
	FRO_OBJ_InitAll( &ecs->robj,
		&egg->m_rmdl_cursor[act->id], &egg->m_ranm_cursor[act->id] );
}
...
static void EggCursor_Update( GURU2MAIN_WORK *g2m, EGGCURSOR *ecs )
{
	...
	FRO_ANM_Play(&egg->m_ranm_cursor[ecs->eact->id], FX32_ONE, ANMLOOP_ON);
}
...
static RET EggStart_FallEnd( EGGSTARTWORK *work )
{
	...
	if( eact->offs.y <= 0 ){
		...
		Guru2NameWin_WriteIDColor(
			work->g2m,
			work->g2m->comm.my_name_buf[eact->id],
			eact->name_no, eact->id );
...
#if 0	
	mc_pp = PokemonParam_AllocWork( HEAPID_GURU2 );
	fc_pp = PokemonParam_AllocWork( HEAPID_GURU2 );
	
	PokeCopyPPtoPP( m_pp, mc_pp );
	PokeCopyPPtoPP( f_pp, fc_pp );
#endif
	
	PokeCopyPPtoPP( f_pp, m_pp );
	
#if 0	
	sys_FreeMemoryEz( mc_pp );
	sys_FreeMemoryEz( fc_pp );
#endif
}

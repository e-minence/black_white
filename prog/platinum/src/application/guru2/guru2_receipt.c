...
//受送信データ
static void MakeSendData(SAVEDATA * sv, GURU2COMM_RC_SENDDATA * send_data);
...
PROC_RESULT Guru2Receipt_Init( PROC *proc, int *seq )
{
	...
	wk->MsgManager = MSGMAN_Create(
		MSGMAN_TYPE_NORMAL, ARC_MSG, NARC_msg_guru2_receipt_dat, HEAPID_GURU2 );
...
//	CommStateUnionRecordCornerChange();
	...
	if(CommGetCurrentID()==0){
		Union_BeaconChange( UNION_PARENT_MODE_GURUGURU_FREE );
	}
...
//				RecordMessagePrint( wk, msg_guru2_receipt_01_11, 1 );		// まぜています
				...
				Union_BeaconChange( UNION_PARENT_MODE_GURUGURUNOW );
			}else{
				GF_ASSERT(0);
			}
		}
...
static void MakeSendData(SAVEDATA * sv, GURU2COMM_RC_SENDDATA * send_data)
{
	...
	void * data_adrs;
	int i, max;
	u32 buf_size = GURU2COMM_RC_SEND_DATABUF_SIZE;
	...
	for (i = 0; i < max; i++) {
		...
		data_adrs = tbl[i].get_data(sv, HEAPID_GURU2, data_size);

...
enum {
	...
	BACKUP_LOAD_SUBSEQ_LOADWARING_MSGCHECK,
	...
	case BACKUP_LOAD_SUBSEQ_LOADMAIN:
		{
			...
			wk->load_bit = SaveData_GetLoadResult(wk->savedata);
			...
			if(wk->load_bit == 0){
				...
				wk->subseq = BACKUP_LOAD_SUBSEQ_LOADWARING_MSGCHECK;
			}
		#endif
		}
		break;
	
	case BACKUP_LOAD_SUBSEQ_LOADWARING_MSGCHECK:
		wk->subseq = BACKUP_LOAD_SUBSEQ_LOADWARING_MSGINIT;
		//表示したいメッセージ順にチェック
		if(wk->load_bit & NORMAL_BREAK_BIT){
			wk->load_bit &= 0xffffffff ^ (NORMAL_BREAK_BIT | NORMAL_NG_BIT);
			wk->warningID = warning02;
		}
		else if(wk->load_bit & NORMAL_NG_BIT){
			wk->load_bit ^= NORMAL_NG_BIT;
			wk->warningID = warning01;
		}
		else if(wk->load_bit & FRONTIER_BREAK_BIT){
			wk->load_bit &= 0xffffffff ^ (FRONTIER_BREAK_BIT | FRONTIER_NG_BIT);
			wk->warningID = warning06;
		}
		else if(wk->load_bit & FRONTIER_NG_BIT){
			wk->load_bit ^= FRONTIER_NG_BIT;
			wk->warningID = warning05;
		}
		else if(wk->load_bit & VIDEO_BREAK_BIT){
			wk->load_bit &= 0xffffffff ^ (VIDEO_BREAK_BIT | VIDEO_NG_BIT);
			wk->warningID = warning04;
		}
		else if(wk->load_bit & VIDEO_NG_BIT){
			wk->load_bit ^= VIDEO_NG_BIT;
			wk->warningID = warning03;
		}
		else{
			wk->subseq = BACKUP_LOAD_SUBSEQ_END;
		}

...
static void Menu_Guru2In( PLIST_WORK * wk, int * seq );
...
//	(const u32)Menu_ChgSelect,			// いれかえる
	...
	(const u32)Menu_Guru2In,			// けってい (ぐるぐる交換)
...
static void Menu_ItemMenuItemGet( PLIST_WORK * wk, int * seq )
{
	...
	if( wk->panel[wk->pos].item == 0 ){
		...
		if(wk->panel[wk->pos].item == ITEM_HAKKINDAMA && gira_form == FORMNO_GIRATINA_NORMAL){
			ret = SEQ_ITEM_MENU_GET_NEXT_DEMO;
		}
...
int PokeList_ItemMenuItemGetEndWaitNextDemo( PLIST_WORK * wk )
{
	if( GF_MSG_PrintEndCheck( wk->msg_index ) == 0 ){
		if( sys.trg & (PAD_BUTTON_DECIDE|PAD_BUTTON_CANCEL) ){
			...
			return SEQ_ITEM_MENU_GET_DEMO_WAIT;
		}
	}
	return SEQ_ITEM_MENU_GET_NEXT_DEMO;
}

...
static void PokeListBgGraphicSet( PLIST_WORK * wk, ARCHANDLE* p_handle );
...
static int PokeList_ItemSet( PLIST_WORK * wk, POKEMON_PARAM * pp, int * gira_form );
...
//static int PokeList_Guru2SelectEggCheck( PLIST_WORK * wk );
static int PokeList_Guru2EggSelectYes( void *work );
static int PokeList_Guru2EggSelectNo( void *work );
static int PokeList_Guru2SelectCancel( PLIST_WORK *wk );

static u8 PL_BattleTowerEnterCheck( PLIST_WORK * wk );
static u8 PL_BattleStageEnterCheck( PLIST_WORK * wk );
static u8 PL_BattleCastleEnterCheck( PLIST_WORK * wk );

static BOOL PL_TouchPanelCheck( PLIST_WORK * wk );

static GF_G3DMAN * PokeList_3D_Init(int heap_id);
static void PokeListSimpleSetUp(void);
static void PokeList_3D_Exit(GF_G3DMAN *g3Dman);

static int PokeItemSetWaitNextDemo_Bag( PLIST_WORK * wk );
static int PokeItemSetDemoWait_Bag( PLIST_WORK * wk );
...
//	sys_CreateHeap( HEAPID_BASE_APP, HEAPID_POKELIST, 0x20000 );
	sys_CreateHeap( HEAPID_BASE_APP, HEAPID_POKELIST, 0x30000 );	//パーティクルを出す為増やした

	p_handle = ArchiveDataHandleOpen( ARC_PLIST_GRA, HEAPID_POKELIST );
	...
	PokeListBgGraphicSet( wk, p_handle );
	...
	if( wk->dat->mode == PL_MODE_ITEMUSE || wk->dat->mode == PL_MODE_SHINKA ){
		...
		PokeListSmallMsgPut( wk, mes_pokelist_02_06, 1 );
#ifdef STAGE_MODE_ON
	}else if( wk->dat->mode == PL_MODE_BATTLE_STAGE ){
		PokeListSmallMsgPut( wk, mes_pokelist_02_06, 1 );
	}else if( wk->dat->mode == PL_MODE_BATTLE_CASTLE ){
		PokeListSmallMsgPut( wk, mes_pokelist_02_06, 1 );
#endif
	}else if( wk->dat->mode == PL_MODE_GURU2 ){
		PokeListSmallMsgPut( wk, mes_pokelist_10_01, 1 );
...
static PROC_RESULT PokeListProc_Main( PROC * proc, int * seq )
{
	...
	case SEQ_ITEMSET_WAIT:
		*seq = PokeItemSetWait_Bag( wk );
		break;
	case SEQ_ITEMSET_WAIT_NEXT_DEMO:
		*seq = PokeItemSetWaitNextDemo_Bag( wk );
		break;
	case SEQ_ITEMSET_DEMO_WAIT:
		*seq = PokeItemSetDemoWait_Bag( wk );
		break;
	...
	case SEQ_ITEM_MENU_GET:
		*seq = PokeList_ItemMenuItemGetEndWait( wk );
		break;
	case SEQ_ITEM_MENU_GET_NEXT_DEMO:
		*seq = PokeList_ItemMenuItemGetEndWaitNextDemo( wk );
		break;
	case SEQ_ITEM_MENU_GET_DEMO_WAIT:
		*seq = PokeList_ItemMenuItemGetEndDemoWait( wk );
		break;
	...
	case SEQ_SHEIMI_FORMDEMO:
		if(PokeList_FormDemoMain( wk ) == TRUE){
			PokeList_FormDemoOverlayUnload(wk);
			*seq = SEQ_ENDTRG_WAIT;
		}
		else{
			*seq = SEQ_SHEIMI_FORMDEMO;
		}
		break;
...
static int MainSeq_Main( PLIST_WORK * wk )
{
	...
	if( sel == SEL_POKE ){
		...
		if( wk->dat->mode == PL_MODE_GURU2 ){
			return PokeList_Guru2SelectCancel( wk );
		}
...
static void PokeListVramBankSet(void)
{
	GF_BGL_DISPVRAM tbl = {
		...
		GX_VRAM_OBJ_128_B,				// テクスチャイメージスロット
		GX_VRAM_OBJ_32_FG				// テクスチャパレットスロット
	};
	GF_Disp_SetBank( &tbl );
}
 ...
 * @param   ini		BGLデータ
 *
 * BG0はフォルムチェンジデモ中は3D面と2D面を切り替えるので、2D面設定部分を抜き出しておく
 */
//--------------------------------------------------------------
static void PokeListBgSet_Bg0(GF_BGL_INI *ini)
{
	// WINDOW (BMP)
...
//--------------------------------------------------------------
/**
 * @brief   BG0面を 2D、3D 切り替える
 *
 * @param   ini					BGLデータ
 * @param   list_bg_mode		LIST_BG0_MODE_???
 */
//--------------------------------------------------------------
void PokeListBG0_Change(PLIST_WORK *wk, int list_bg_mode)
{
	if(list_bg_mode == LIST_BG0_MODE_3D){	//3D面に切り替える
		GF_BGL_VisibleSet(GF_BGL_FRAME0_M, VISIBLE_OFF);
		GF_BGL_BGControlExit(wk->bgl, GF_BGL_FRAME0_M);		//2D面解放
		GX_SetGraphicsMode( GX_DISPMODE_GRAPHICS, GX_BGMODE_0, GX_BG0_AS_3D);
		
		wk->g3Dman = PokeList_3D_Init(HEAPID_POKELIST);
	}
	else{	//2D面に切り替える
		GF_Disp_GX_VisibleControl( GX_PLANEMASK_BG0, VISIBLE_OFF );
		PokeList_3D_Exit(wk->g3Dman);	//3D面解放
		GX_SetGraphicsMode( GX_DISPMODE_GRAPHICS, GX_BGMODE_0, GX_BG0_AS_2D);
		
		PokeListBgSet_Bg0(wk->bgl);
		GF_BGL_ClearCharSet( GF_BGL_FRAME0_M, 32, 0, HEAPID_POKELIST );
	}
}
...
static GF_G3DMAN * PokeList_3D_Init(int heap_id)
{
	...
	g3Dman = GF_G3DMAN_Init(heap_id, GF_G3DMAN_LNK, GF_G3DTEX_128K, 
		GF_G3DMAN_LNK, GF_G3DPLT_32K, PokeListSimpleSetUp);
	return g3Dman;
}
...
static void PokeListSimpleSetUp(void)
{
	...
	//ここでONにすると一瞬チラつくので1フレーム待った後、外側でONする
//    G2_SetBG0Priority(0);
...
static void PokeListBgGraphicSet( PLIST_WORK * wk, ARCHANDLE* p_handle )
{
	ArcUtil_HDL_BgCharSet(
		p_handle, NARC_plist_gra_p_list_NCGR,
		wk->bgl, GF_BGL_FRAME3_M, 0, 0, 0, HEAPID_POKELIST );
	ArcUtil_HDL_ScrnSet(
		p_handle, NARC_plist_gra_p_list_NSCR,
		wk->bgl, GF_BGL_FRAME3_M, 0, 0, 0, HEAPID_POKELIST );

	{
		...
		buf = ArchiveDataLoadAllocByHandle( p_handle, NARC_plist_gra_p_list_NCLR, HEAPID_POKELIST );
...
//	SystemFontPaletteLoad( PALTYPE_MAIN_BG, 0, HEAPID_POKELIST );
	...
	ArcUtil_HDL_BgCharSet(
		p_handle, NARC_plist_gra_list_but_NCGR,
		wk->bgl, GF_BGL_FRAME0_S, 0, 0, 0, HEAPID_POKELIST );
	ArcUtil_HDL_PalSet(
		p_handle, NARC_plist_gra_list_but_NCLR,
		PALTYPE_SUB_BG, 0x20, 0x20, HEAPID_POKELIST );
		
	ArcUtil_HDL_BgCharSet(
		p_handle, NARC_plist_gra_list_sub_NCGR,
		wk->bgl, GF_BGL_FRAME1_S, 0, 0, 0, HEAPID_POKELIST );
	ArcUtil_HDL_ScrnSet(
		p_handle, NARC_plist_gra_list_sub_NSCR,
		wk->bgl, GF_BGL_FRAME1_S, 0, 0, 0, HEAPID_POKELIST );
	ArcUtil_HDL_PalSet(
		p_handle, NARC_plist_gra_list_sub_NCLR,
...
static void PokePanelMakeNormal( PLIST_WORK * wk, const PANEL_POS * tbl )
{
	...
	for( i=0; i<6; i++ ){
		if( PokeList_PanelPPDataMake( wk, i ) == TRUE ){
			...
			PokeListIconAdd( wk, i, tbl[i].icon_px, tbl[i].icon_py, p_handle );		// アイコン表示
 ...
 * @param	wk		ポケモンリストのワーク
 *
 * @retval	"SEL_MOVE = カーソル移動"
 * @retval	"SEL_TOUCH = サブ画面のボタン"
 * @retval	"SEL_NONE = なにもしていない"
 */
//--------------------------------------------------------------------------------------------
...
static u8 BattleStageInModeMenuMake( PLIST_WORK * wk, u8 * prm );
static u8 BattleCastleInModeMenuMake( PLIST_WORK * wk, u8 * prm );
static u8 Guru2ModeMenuMake( PLIST_WORK * wk, u8 * prm );
...
static void PL_MenuPut( PLIST_WORK * wk )
{
	...
	case PL_MODE_SODATEYA:
		...
		break;
	case PL_MODE_GURU2:
		siz = Guru2ModeMenuMake( wk, prm );
		break;
#ifdef STAGE_MODE_ON
	case PL_MODE_BATTLE_STAGE:							//バトルステージ
		siz = BattleStageInModeMenuMake( wk, prm );
		break;
	case PL_MODE_BATTLE_CASTLE:							//バトルキャッスル
		siz = BattleCastleInModeMenuMake( wk, prm );
		break;
#endif
	default:
 ...
 */
//--------------------------------------------------------------------------------------------
u8 PokeList_BattleStageInCheck( PLIST_WORK * wk, u8 num );
static u8 BattleStageInModeMenuMake( PLIST_WORK * wk, u8 * prm )
{
	switch( PokeList_BattleStageInCheck( wk, wk->pos ) ){
	case 0:		// 参加できない
		prm[0] = MENU_STATUS;
		prm[1] = MENU_CANCEL;
		return 2;
 ...
 */
//--------------------------------------------------------------------------------------------
u8 PokeList_BattleCastleInCheck( PLIST_WORK * wk, u8 num );
static u8 BattleCastleInModeMenuMake( PLIST_WORK * wk, u8 * prm )
{
	switch( PokeList_BattleCastleInCheck( wk, wk->pos ) ){
	case 0:		// 参加できない
		prm[0] = MENU_STATUS;
		prm[1] = MENU_CANCEL;
		return 2;
...
u8 PokeList_BattleCastleInCheck( PLIST_WORK * wk, u8 num )
{
	...
	// バトルタワーチェック
	if( wk->dat->mode == PL_MODE_BATTLE_CASTLE ){
		if( BattleTowerExPokeCheck_MonsNo( wk->panel[num].mons ) == TRUE ){
			return 0;
		}
	}
...
static int PL_SelectEnter( PLIST_WORK * wk )
{
	...
	// バトルステージ用処理
	if( wk->dat->mode == PL_MODE_BATTLE_STAGE ){
		switch( PL_BattleStageEnterCheck(wk) ){
		case 0:		// 正常
			break;

		case 1:		// 同じポケモンが選ばれていない
			PokeListLargeMsgPut( wk, mes_pokelist_11_01, 1 );
			wk->next_seq = SEQ_BTLIN_ERR;
			Snd_SePlay( PLIST_SE_ERROR );
			return SEQ_MSG_WAIT;
		}
	...
	// バトルキャッスル用処理
	if( wk->dat->mode == PL_MODE_BATTLE_CASTLE ){
		switch( PL_BattleCastleEnterCheck(wk) ){
		case 0:		// 正常
			break;
...
//	下画面ボタン
//============================================================================================
#define	SUB_BUTTON_CGX	( 5 )			// 下画面ボタンのキャラ番号
#define	SUB_BUTTON_PAL	( 1 << 12 )		// 下画面ボタンの使用パレット
#define	SUB_BUTTON_SX	( 5 )			// 下画面ボタンのXサイズ
#define	SUB_BUTTON_SY	( 5 )			// 下画面ボタンのYサイズ
...
static int ItemUseCheck( PLIST_WORK * wk )
{
	...
	//「グラシデアのはな」使用　プラチナから追加
	if(wk->dat->item == ITEM_GURASIDEANOHANA
			&& GrashideaFeasibleCheck(PokeParty_GetMemberPointer(wk->dat->pp, wk->pos)) == TRUE){
		wk->dat->after_mons = FORMNO_SHEIMI_FLOWER;	//フォルム番号をセット
		sys_FreeMemoryEz(dat);
		PokeList_FormDemoOverlayLoad( wk );
		return SEQ_SHEIMI_FORMDEMO;
	}
...
static int PokeItemSetInit_Bag( PLIST_WORK * wk )
{
	...
	case POKEITEM_NONE:		// 何も持っていない
		...
		ret = PokeList_ItemSet( wk, pp, &gira_form );
 ...
 * @param	wk			ポケモンリストのワーク
 * @param	pp			POKEMON_PARAM
 * @param   gira_form	渡した相手がギラティナだった場合、フォルム番号(ギラティナでない場合は"-1")
 *
 * @return	次のシーケンス
 */
//--------------------------------------------------------------------------------------------
static int PokeList_ItemSet( PLIST_WORK * wk, POKEMON_PARAM * pp, int * gira_form )
{
	...
	*gira_form = PokeParaGirathinaFormChange( pp );
	...
	if(item == ITEM_HAKKINDAMA && (*gira_form) != -1){
		return SEQ_ITEMSET_WAIT_NEXT_DEMO;
	}
...
//--------------------------------------------------------------------------------------------
/**
 * ポケモンに持たせたアイテムをバッグに戻し、持っていたアイテムを再び持たせる
 *
 * @param	wk		ポケモンリストのワーク
 * @param	pp		POKEMON_PARAM
 * @param	set		ポケモン -> バッグ のアイテム
 * @param	get		バッグ -> ポケモン のアイテム
 *
 * @return	none
 */
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
/**
 * 持たせたメッセージ終了待ち＆フォルムチェンジデモ終了待ちへ移行
 *
 * @param	wk		ポケモンリストのワーク
 *
 * @return	次のシーケンス
...
//					PokeParaPut( pp, ID_PARA_item, &get_item );
				...
				if(set_item != ITEM_HAKKINDAMA && get_item == ITEM_HAKKINDAMA && gira_form != -1){
					ret = SEQ_ITEMSET_WAIT_NEXT_DEMO;
				}
				//ハッキンダマは1個しか持てないのでありえないけど一応。
				else if(set_item == ITEM_HAKKINDAMA && get_item == ITEM_HAKKINDAMA){
					ret = SEQ_ITEMSET_WAIT;	//ハッキンダマをハッキンダマに変えた場合はデモ無し
				}
			}
			GF_BGL_BmpWinDataFill( win, FBMP_COL_WHITE );
			PokeList_TalkMsgStart( wk );
...
static int PL_MailSetEnd( PLIST_WORK * wk )
{
	...
	ret = PokeList_ItemSet( wk, pp, &gira_form );
	if(get_item == ITEM_HAKKINDAMA && ret == SEQ_ITEMSET_WAIT 
...
static int PokeList_Guru2SelectEggCheck( PLIST_WORK * wk )
{
	if( wk->panel[wk->pos].egg == 1 ){		//卵選択
		CATS_ObjectPaletteSet( wk->act_work[PLIST_ACT_CURSOR1], 1 );
		PokeListLargeMsgPut( wk, mes_pokelist_10_02, 1 );
		wk->yesno.yes = PokeList_Guru2EggSelectYes;
		wk->yesno.no = PokeList_Guru2EggSelectNo;
		wk->next_seq = SEQ_YESNO_INIT;
	}else{									//卵ではない
		PokeListLargeMsgPut( wk, mes_pokelist_04_71, 1 );
		wk->next_seq = SEQ_MENUWAZA_ERR;	//技エラーSEQ流用
	}
...
//--------------------------------------------------------------
/**
 * ぐるぐる交換　卵選択後　YES選択　※キャンセル時のYesにも流用
 *
 * @param	work	ポケモンリストのワーク
 *
 * @return	次のシーケンス
...
//--------------------------------------------------------------
/**
 * ぐるぐる交換　卵選択後　No選択 ※キャンセル時のNoにも流用
 *
 * @param	work	ポケモンリストのワーク
 *
 * @return	次のシーケンス
...
{
	CATS_ObjectPaletteSet( wk->act_work[PLIST_ACT_CURSOR1], 1 );
	PokeListLargeMsgPut( wk, mes_pokelist_10_03, 1 );
	wk->yesno.yes = PokeList_Guru2EggSelectYes;
	wk->yesno.no = PokeList_Guru2EggSelectNo;
	wk->next_seq = SEQ_YESNO_INIT;
	return SEQ_MSG_WAIT;
...
// プレートグラフィックのキャラ位置
#define	PANEL_SCREEN_POS_TOP		( 0 )										// 先頭
#define	PANEL_SCREEN_POS_NORMAL		( PL_PANEL_SY )								// 通常
#define	PANEL_SCREEN_POS_NONE		( PANEL_SCREEN_POS_NORMAL + PL_PANEL_SY )	// いない
 ...
 * @param	heap	ヒープID
 * @param	top		先頭のグラフィック
 * @param	normal	通常のグラフィック
 * @param	none	いない場合のグラフィック
 *
 * @return	none
 */
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------
/**
 * @brief   フォルムデモ：オーバーレイロードして初期化関数呼び出し
 *
 * @param   wk		
 */
//--------------------------------------------------------------
void PokeList_FormDemoOverlayLoad( PLIST_WORK *wk )
{
	Overlay_Load(FS_OVERLAY_ID(form_demo), OVERLAY_LOAD_NOT_SYNCHRONIZE);
	PokeList_FormDemoInit( wk );
}

//--------------------------------------------------------------
/**
 * @brief   フォルムデモ：オーバーレイをアンロード
 *
 * @param   wk		
 */
//--------------------------------------------------------------
void PokeList_FormDemoOverlayUnload( PLIST_WORK *wk )
{
	Overlay_UnloadID(FS_OVERLAY_ID(form_demo));
}

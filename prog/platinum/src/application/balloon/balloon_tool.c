...
static const AIR_POSITION_DATA AirPositionDataTbl[][WFLBY_MINIGAME_MAX] = {
	{//player_maxそのままでアクセスできるように1origin化の為のダミー
		{128, 192+16,	MIXOBJ_ANMSEQ_AIR_0, PALOFS_SUB_AIR_D,	DIR_UP},		//下
	},
	...
	{//参加人数2人
		{128, 192+16,	MIXOBJ_ANMSEQ_AIR_0, PALOFS_SUB_AIR_D,	DIR_UP},		//下
		{128, 0-16,	MIXOBJ_ANMSEQ_AIR_3, PALOFS_SUB_AIR_U,	DIR_DOWN},		//上
	},
	{//参加人数3人
		{128, 192+16,	MIXOBJ_ANMSEQ_AIR_0, PALOFS_SUB_AIR_D,	DIR_UP},		//下
		{0-16, 192/2,	MIXOBJ_ANMSEQ_AIR_2, PALOFS_SUB_AIR_L,	DIR_RIGHT},		//左
		{256+16, 192/2,	MIXOBJ_ANMSEQ_AIR_1, PALOFS_SUB_AIR_R,	DIR_LEFT},	//右
	},
	{//参加人数4人
		{128, 192+16,	MIXOBJ_ANMSEQ_AIR_0, PALOFS_SUB_AIR_D,	DIR_UP},		//下
		{0-16, 192/2,	MIXOBJ_ANMSEQ_AIR_2, PALOFS_SUB_AIR_L,	DIR_RIGHT},		//左
		{128, 0-16,	MIXOBJ_ANMSEQ_AIR_3, PALOFS_SUB_AIR_U,	DIR_DOWN},		//上
		{256, 192/2,	MIXOBJ_ANMSEQ_AIR_1, PALOFS_SUB_AIR_R,	DIR_LEFT},	//右
	},
};
...
static const TCATS_OBJECT_ADD_PARAM_S AirObjParam = {
	0, 0, 0,		//x, y, z
	0, BALLOONSUB_SOFTPRI_AIR, PALOFS_SUB_AIR_D,	//アニメ番号、優先順位、パレット番号
...
static const TCATS_OBJECT_ADD_PARAM_S BoosterObjParam = {
	0, 0, 0,		//x, y, z
	0, BALLOONSUB_SOFTPRI_BOOSTER, PALOFS_SUB_BOOSTER_BLUE,//アニメ番号、優先順位、パレット番号
	NNS_G2D_VRAM_TYPE_2DSUB,		//描画エリア
	{	//使用リソースIDテーブル
		CHARID_SUB_BALLOON_MIX,			//キャラ
		PLTTID_SUB_OBJ_COMMON,			//パレット
...
//	カラースワップ
//--------------------------------------------------------------
//デフォルトパレットでの1Pのカラー開始位置(青)
#define PIPE_BG_COLOR_START_NO_PLAYER1		(0*16 + 0xd)
//デフォルトパレットでの2Pのカラー開始位置(赤)
#define PIPE_BG_COLOR_START_NO_PLAYER2		(1*16 + 0xd)
//デフォルトパレットでの3Pのカラー開始位置(緑)
#define PIPE_BG_COLOR_START_NO_PLAYER3		(2*16 + 0xd)
//デフォルトパレットでの4Pのカラー開始位置(黄)
#define PIPE_BG_COLOR_START_NO_PLAYER4		(3*16 + 0xd)

///パイプBG：プレイヤー位置毎のパレットカラー開始位置
static const u16 PlayerNoPaletteSwapTbl_Pipe[][WFLBY_MINIGAME_MAX] = {
	{//0人参加(ダミー)
		PIPE_BG_COLOR_START_NO_PLAYER1,
		PIPE_BG_COLOR_START_NO_PLAYER2,
		PIPE_BG_COLOR_START_NO_PLAYER3,
		PIPE_BG_COLOR_START_NO_PLAYER4,
	},
	{//1人参加	(下、左、上、右)：デバッグ用
		PIPE_BG_COLOR_START_NO_PLAYER1,
		PIPE_BG_COLOR_START_NO_PLAYER2,
...
//デフォルトパレットでの1Pのカラー開始位置(青)
#define AIR_OBJ_COLOR_START_NO_PLAYER1		(1*16 + 0xc)
//デフォルトパレットでの2Pのカラー開始位置(赤)
#define AIR_OBJ_COLOR_START_NO_PLAYER2		(2*16 + 0xc)
//デフォルトパレットでの3Pのカラー開始位置(緑)
#define AIR_OBJ_COLOR_START_NO_PLAYER3		(4*16 + 0xc)
//デフォルトパレットでの4Pのカラー開始位置(黄)
#define AIR_OBJ_COLOR_START_NO_PLAYER4		(3*16 + 0xc)

///エアーOBJ(サブ画面)：プレイヤー位置毎のパレットカラー開始位置(PLTTID_SUB_OBJ_COMMONからのoffset)
static const u16 PlayerNoPaletteSwapTbl_Air[][WFLBY_MINIGAME_MAX] = {
	{//0人参加(ダミー)
		AIR_OBJ_COLOR_START_NO_PLAYER1,
		AIR_OBJ_COLOR_START_NO_PLAYER2,
		AIR_OBJ_COLOR_START_NO_PLAYER3,
		AIR_OBJ_COLOR_START_NO_PLAYER4,
	},
	{//1人参加	(下、左、上、右)：デバッグ用
		AIR_OBJ_COLOR_START_NO_PLAYER1,
		AIR_OBJ_COLOR_START_NO_PLAYER2,
...
///エアーOBJ：プレイヤーカラーがパレット開始位置からいくつ入っているか
#define PLAYER_NO_PALETTE_COLOR_NUM_AIR		(4)
...
static int BalloonTool_AirStackAdd(BALLOON_GAME_PTR game, BALLOON_STATUS *bst, int air, int net_id);
...
static int BalloonTool_AirStackAdd(BALLOON_GAME_PTR game, BALLOON_STATUS *bst, int air, int net_id)
{
	...
	OS_TPrintf("追加されたair = %d, add_air = %d, stack = %d, net_id = %d\n", air, bst->add_air, bst->air_stack, net_id);
...
static int Air_ParamCreate(BALLOON_GAME_PTR game, const BALLOON_AIR_DATA * air_data, PLAYER_AIR_PARAM *air_param)
{
	...
	{//ジョイント到着までのフレーム数を返す
		...
		arrival_frame = FX_Mul(before_joint_offset << FX32_SHIFT, AIR_MOVE_SPEED);
	}
		...
		if(air_param->occ == TRUE){
			if(air_param->wait > 0){
				...
				if(game->balloon_occ == TRUE && game->balloon_no == air_param->balloon_no){
					//風船番号が一致しているならば空気注入
					BalloonTool_AirStackAdd(game, &game->bst, air_param->air, air_param->net_id);
				}
...
//--------------------------------------------------------------
/**
 * @brief   プレイヤー位置に従ってエアーOBJのカラーのスワップを行う
 *
 * @param   game		
 */
//--------------------------------------------------------------
void BalloonTool_PaletteSwap_Air(BALLOON_GAME_PTR game)
{
	...
	for(player = 0; player < WFLBY_MINIGAME_MAX; player++){
		for(i = 0; i < PLAYER_NO_PALETTE_COLOR_NUM_AIR; i++){
			color[player][i] = PaletteWork_ColorGet(game->pfd, FADE_SUB_OBJ, FADEBUF_SRC, 
				common_palno*16 + PlayerNoPaletteSwapTbl_Air[game->bsw->player_max][player] + i);
		}
	}
	...
	for(i = 0; i < game->bsw->player_max; i++){
		read_pos = i;
		write_pos = PlayerNoPaletteSwapTbl_Air[game->bsw->player_max][Balloon_NetID_to_PlayerPos(game, game->bsw->player_netid[i])] + common_palno*16;
...
void SioBooster_Appear(BALLOON_GAME_PTR game, SIO_BOOSTER_WORK *sio_booster, int booster_type, int net_id, int arrival_frame)
{
	...
	sio_move->start_wait = arrival_frame - FX_Mul(MATH_ABS(SIO_BOOSTER_APPEAR_OFFSET_Y) * FX32_ONE, SIO_BOOSTER_IN_SPEED);
	sio_move->seq = 0;
	sio_move->occ = TRUE;
}

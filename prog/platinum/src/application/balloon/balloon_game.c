...
#define BMPWIN_SUB_CGX_NAME_SIZE		(8 * 4)
...
//--------------------------------------------------------------
///フォントOAMの最大数
#define BALLOON_FONTOAM_MAX_MAIN	(BALLOON_COUNTER_MAX * BALLOON_COUNTER_KETA_MAX + 1)	//カウンター＋"CC"
	...
	case SEQ_MAIN:
		...
		case BALLOON_COUNTDOWN_START_WAIT:
			if(MNGM_COUNT_Wait(game->mgcount) == TRUE){
				...
				//ウィンドウ設定	WIPEとカウントダウンで使っているのでここで設定
				GX_SetVisibleWnd(GX_WNDMASK_W0);
				G2_SetWnd0Position(4, 192-32+8, 8*9+4, 192-8);	//カウンターウィンドウを囲む
				G2_SetWnd0InsidePlane(GX_WND_PLANEMASK_OBJ, FALSE);	//カウンターOBJしか表示されないよう設定
...
PROC_RESULT BalloonGameProc_End( PROC * proc, int * seq )
{
	...
	//割った数を結果画面用ワークへセット
	game->bsw->result_param.balloon = game->balloon_no;
...
static void BalloonSys_DefaultBmpWinAdd(BALLOON_GAME_WORK *game)
{
	...
	GF_BGL_BmpWinAdd(game->bgl, &game->win[BALLOON_BMPWIN_NAME_1], BALLOON_SUBFRAME_WIN, 
		0x12, 1, 8, 2, BMPWIN_SUB_TALK_COLOR, BMPWIN_SUB_CGX_OFFSET_NAME_1);
	GF_BGL_BmpWinAdd(game->bgl, &game->win[BALLOON_BMPWIN_NAME_2], BALLOON_SUBFRAME_WIN, 
		0x16, 0xe, 8, 2, BMPWIN_SUB_TALK_COLOR, BMPWIN_SUB_CGX_OFFSET_NAME_2);
	GF_BGL_BmpWinAdd(game->bgl, &game->win[BALLOON_BMPWIN_NAME_3], BALLOON_SUBFRAME_WIN, 
		1, 14, 8, 2, BMPWIN_SUB_TALK_COLOR, BMPWIN_SUB_CGX_OFFSET_NAME_3);
...
static void BalloonDefaultOBJSet(BALLOON_GAME_WORK *game, ARCHANDLE *hdl)
{
	//-- カウンター --//
	{
		...
		BalloonTool_FontOamCreate(game->bgl, game->crp, game->fontoam_sys,
			...
			BALLOON_BGPRI_COUNTER, BALLOON_SOFTPRI_COUNTER, 2);
		STRBUF_Delete(str0);
...
static void BalloonDefaultOBJSet_Sub(BALLOON_GAME_WORK *game, ARCHANDLE *hdl)
{
	...
	//カラースワップ
	BalloonTool_PaletteSwap_Air(game);
}
...
static void BalloonDefault3DSet(BALLOON_GAME_WORK *game, ARCHANDLE *hdl)
{
	...
	pos = Balloon_NetID_to_PlayerPos(game, CommGetCurrentID());
	
	//-- パイプ --//
    //モデルﾃﾞｰﾀ読み込み
	D3DOBJ_MdlLoadH(&pa->pipe_mdl, hdl, PlayerPosGraphicID[pos].pipe_mdl_id, HEAPID_BALLOON);
    //レンダーオブジェクトに登録
	...
	D3DOBJ_MdlLoadH(&pa->air_mdl[PIPE_3D_AIR_TYPE_SMALL], 
		hdl, PlayerPosGraphicID[pos].air_s_mdl_id, HEAPID_BALLOON);
    //モデルデータ読み込み
	D3DOBJ_MdlLoadH(&pa->air_mdl[PIPE_3D_AIR_TYPE_MIDDLE], 
		hdl, PlayerPosGraphicID[pos].air_m_mdl_id, HEAPID_BALLOON);
    //モデルデータ読み込み
	D3DOBJ_MdlLoadH(&pa->air_mdl[PIPE_3D_AIR_TYPE_BIG], 
		hdl, PlayerPosGraphicID[pos].air_l_mdl_id, HEAPID_BALLOON);
    //モデルデータ読み込み
	D3DOBJ_MdlLoadH(&pa->air_mdl[PIPE_3D_AIR_TYPE_SPECIAL], 
		hdl, PlayerPosGraphicID[pos].air_sp_mdl_id, HEAPID_BALLOON);

...
void PageData_Create(CB_MAIN_WORK* cbmw, int page_no)
{
	...
	for (i = 0; i < CB_DEF_ITEM_DATA_NUM; i++){
		...
		for ( j = 0; j < CB_DEF_CORE_MAX; j++ ){
			have = CB_Tool_IsSealCore( cbmw->cb_edit_data[ j ].core, i );
...
void SubObj_ResLoad(CATS_SYS_PTR csp, CATS_RES_PTR crp, PALETTE_FADE_PTR pfd, int chr_id, int pal_id, int cel_id, int cla_id, int vram, int pal_tg, int palnum, ARCHANDLE* p_handle)
{
	CATS_LoadResourceCharArcH(csp, crp,
							 p_handle, chr_id, TRUE,
							 vram,
							 CB_DEF_ARC_ID_GET( chr_id ));

	CATS_LoadResourcePlttWorkArcH(pfd, pal_tg,
								 csp, crp,
								 p_handle, pal_id, FALSE,
								 palnum, vram, CB_DEF_ARC_ID_GET( pal_id ));
	CATS_LoadResourceCellArcH(csp, crp,
							 p_handle, cel_id, TRUE,
							 CB_DEF_ARC_ID_GET( cel_id ));

	CATS_LoadResourceCellAnmArcH(csp, crp,
								p_handle, cla_id, TRUE,
								CB_DEF_ARC_ID_GET( cla_id ));	
}


void CB_Tool_SubSelect_ObjAdd(CB_MAIN_WORK* cbmw, ARCHANDLE* p_handle)
{
	SubObj_ResLoad(cbmw->sys.csp, cbmw->sys.crp, cbmw->sys.pfd,
				   ...
				   NNS_G2D_VRAM_TYPE_2DMAIN, FADE_MAIN_OBJ, 1, p_handle);
...
void CB_Select_TopResLoad(CB_MAIN_WORK* cbmw, ARCHANDLE* p_handle)
{
	...
	ArcUtil_HDL_BgCharSet(p_handle, chr_index,
					  cbmw->sys.bgl, frame, 0, 0, 1, HEAPID_CUSTOM_BALL);
	ArcUtil_HDL_ScrnSet(p_handle, scr_index,
					cbmw->sys.bgl, frame, 0, 0, 1, HEAPID_CUSTOM_BALL);
...
void CB_Select_BottomResLoad(CB_MAIN_WORK* cbmw, ARCHANDLE* p_handle)
{
	int arc_index = ARC_CUSTOM_BALL_GRA;
...
void CB_Edit_BottomResLoad(CB_MAIN_WORK* cbmw, ARCHANDLE* p_handle)
{
	int arc_index = ARC_CUSTOM_BALL_GRA;
	int chr_index = CB_BG_B_01_NCGR_BIN;
...
void CB_Tool_BTN_ResLoad(CB_MAIN_WORK* cbmw, ARCHANDLE* p_handle)
{
	CATS_SYS_PTR csp;
	CATS_RES_PTR crp;
	PALETTE_FADE_PTR pfd;
...
//	CB_Tool_SealBTN_ResLoad(cbmw);
	...
	CATS_LoadResourceCellArcH(csp, crp,
							 p_handle, CB_BUTTON_SC01_NCER_BIN, TRUE,
	...
	CATS_LoadResourceCellArcH(csp, crp,
							 p_handle, CB_BUTTON_SC02_NCER_BIN, TRUE,
	...
	CATS_LoadResourceCellArcH(csp, crp,
							 p_handle, CB_BUTTON_01_NCER_BIN, TRUE,
							 CB_ENUM_BTN_CELL_ID + CB_BUTTON_01_NCER_BIN);

	CATS_LoadResourceCellAnmArcH(csp, crp,
								p_handle, CB_BUTTON_01_NANR_BIN, TRUE,
								CB_ENUM_BTN_CELLANM_ID + CB_BUTTON_01_NANR_BIN);

	CATS_LoadResourceCharArcH(csp, crp,
							 p_handle, CB_BUTTON_KAKU01_NCGR_BIN, TRUE,
							 NNS_G2D_VRAM_TYPE_2DSUB,
							 CB_ENUM_BTN_EYE_CHAR_ID + CB_BUTTON_KAKU01_NCGR_BIN);

	CATS_LoadResourceCellArcH(csp, crp,
							 p_handle, CB_BUTTON_KAKU01_NCER_BIN, TRUE,
							 CB_ENUM_BTN_CELL_ID + CB_BUTTON_KAKU01_NCER_BIN);

	CATS_LoadResourceCellAnmArcH(csp, crp,
								p_handle, CB_BUTTON_KAKU01_NANR_BIN, TRUE,
...
void CB_Tool_BTN_Enable(CB_MAIN_WORK* cbmw, int flag)
{
	...
	for (i = 0; i < CB_ENUM_BTN_NUM; i++){
		...
		if ( cbmw->button_obj.cap[ i ] == NULL ){ continue; }

...
//----------------------------------------------------------------------------
/**
 *	@brief	カーソル動作がサブ画面に遷移しているか設定
 *
 *	@param	p_glb	アプリケーションデータ
 *	@param	flag	TRUE	動作OK	FALSE　動作NG
 */
//-----------------------------------------------------------------------------
void ZKN_ZukanCommonSubCursorFlagSet( ZKN_APL_DATA* p_glb, BOOL flag )
{
	ZKN_ZUKAN_COMMON_SUB_GLB * p_glbdata = p_glb->p_glb_data;
	p_glbdata->sub_cursor_flag = flag;
}
...
#if S2418_060817_FIX
		//  ZknZukanCommonSubApplyChangeで
		// 落とした、フラグを元に戻す
		p_glb->sub_common_sub_mode_flag = TRUE;
#endif	// S2418_060817_FIX
...
static void ZknZukanCommonSubButtonCallBack( u32 button_no, u32 event, void* p_work )
{
	...
	case BMN_EVENT_HOLD:
		// カーソルをその位置に設定
		ZKN_CURSOR_Cont( p_subwork->p_cursor, ZKN_CURSOR_CONT_SET_X, button_no );
...
#if S2418_060817_FIX
	if( ret == TRUE ){
		p_glb->sub_common_sub_mode_flag = FALSE;	// 動作を認めない
	}
#endif

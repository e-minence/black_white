...
int WorldTrade_MyPoke_Init(WORLDTRADE_WORK *wk, int seq)
{
	...
	WorldTrade_PokeInfoPrint2( wk->MsgManager, &wk->InfoWin[5], wk->UploadPokemonData.name, pp, &wk->InfoWin[10]);
...
static void BgGraphicSet( WORLDTRADE_WORK * wk )
{
	...
	// 上下画面ＢＧパレット転送
	ArcUtil_HDL_PalSet(    p_handle, NARC_worldtrade_poke_view_nclr, PALTYPE_MAIN_BG, 0, 16*3*2,  HEAPID_WORLDTRADE);
  ...
  //	TalkFontPaletteLoad( PALTYPE_SUB_BG,  WORLDTRADE_TALKFONT_PAL*0x20, HEAPID_WORLDTRADE );
	...
	// メイン画面BG1キャラ転送
	ArcUtil_HDL_BgCharSet( p_handle, NARC_worldtrade_poke_view_lz_ncgr, bgl, GF_BGL_FRAME1_M, 0, 16*5*0x20, 1, HEAPID_WORLDTRADE);

	// メイン画面BG1スクリーン転送
	ArcUtil_HDL_ScrnSet(   p_handle, NARC_worldtrade_mypoke_lz_nscr, bgl, GF_BGL_FRAME1_M, 0, 32*24*2, 1, HEAPID_WORLDTRADE);
							...
							Dpw_Tr_PokemonDataSimple *post )
{
	...
	WorldTrade_SysPrint( &win[2], levelbuf,  0, 0, 0, GF_PRINTCOLOR_MAKE(15,2,0) );
 ...
 */
//------------------------------------------------------------------
void WorldTrade_PokeInfoPrint2( MSGDATA_MANAGER *MsgManager, GF_BGL_BMPWIN win[], STRCODE *name, POKEMON_PARAM *pp, GF_BGL_BMPWIN oya_win[] )
{
	STRBUF *ornerbuf, *ornerlabel;
	STRBUF *oyalabel, *oyabuf;
	
	ornerbuf = STRBUF_Create( (PERSON_NAME_SIZE+EOM_SIZE)*2, HEAPID_WORLDTRADE );
	oyabuf = STRBUF_Create((PERSON_NAME_SIZE+EOM_SIZE)*2, HEAPID_WORLDTRADE);
	
	ornerlabel = MSGMAN_AllocString( MsgManager, msg_gtc_02_010  );
	STRBUF_SetStringCode( ornerbuf, name );
	
	oyalabel = MSGMAN_AllocString(MsgManager, msg_gtc_pl_oya);
	PokeParaGet(pp, ID_PARA_oyaname_buf, oyabuf);
	...
	WorldTrade_SysPrint( &oya_win[0], oyalabel,   0, 0, 0, GF_PRINTCOLOR_MAKE(15,2,0) );
	WorldTrade_SysPrint( &oya_win[1], oyabuf,   0, 0, 0, GF_PRINTCOLOR_MAKE(15,2,0) );
	
	STRBUF_Delete( ornerlabel );
	STRBUF_Delete( ornerbuf   );
	STRBUF_Delete( oyalabel );
	STRBUF_Delete( oyabuf   );
}

...
static void WFP2PMF_GraphicMsgBmpStrPut( WFP2PMF_DRAW* p_draw, u32 msg_idx, u32 heapID );
static void WFP2PMF_GraphicMsgBmpStrPutEx( WFP2PMF_DRAW* p_draw, u32 msg_idx, u8 wait, u32 heapID );
static void WFP2PMF_GraphicMsgBmpStrPutAll( WFP2PMF_DRAW* p_draw, u32 msg_idx, u32 heapID );
...
static void WFP2PMF_ErrCheck( WFP2PMF_WK* p_wk );
...
static void WFP2PMF_GraphicMsgBmpStrPut( WFP2PMF_DRAW* p_draw, u32 msg_idx, u32 heapID )
{
	WFP2PMF_GraphicMsgBmpStrPutEx( p_draw, msg_idx, 0, heapID );
}
...
static void WFP2PMF_StatusVChatOnBmpFade( WFP2PMF_WK* p_wk )
{
	if( WFP2PMF_StatusVChatOn( p_wk ) ){
		// BGMの音量を落とす
		Snd_VChatVolSet( _BGM_MAIN, SND_HANDLE_BGM);
	}
}
...
static void WFP2PMF_StatusVChatOffBmpFade( WFP2PMF_WK* p_wk )
{
	if( WFP2PMF_StatusVChatOff( p_wk ) ){
		Snd_PlayerSetInitialVolume( SND_HANDLE_BGM, BGM_WIFILOBBY_VOL );
	}
}
...
static void WFP2PMF_CommSendNewEntryNg( WFP2PMF_WK* p_wk )
{
	...
	for( i= 1; i<WIFI_P2PMATCH_NUM_MAX; i++ ){
		if( CommInfoIsBattleNewName( i ) == TRUE ){
			result.netID  = i;
			CommSendData( CNM_WFP2PMF_RESULT, &result, sizeof(WFP2PMF_COMM_RESULT) );
...
static void WFP2PMF_DrawOamInit( WFP2PMF_WK* p_wk, u32 aid, u32 heapID )
{
	...
	GF_ASSERT( p_wk->draw.objdata[aid].p_objwk == NULL );
...
{
	// メッセージ
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_107, heapID );
...
static BOOL WFP2PMF_OyaConnectWait( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	if( WFP2PMF_MatchCheck( p_wk ) == TRUE ){
		...
		p_wk->data.new_con = CommInfoGetNewNameID();
	}
...
static BOOL WFP2PMF_OyaConnectSelectInit00( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	// メッセージの表示
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_108, heapID );
...
{
	// メッセージ表示
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_122, heapID );
...
static BOOL WFP2PMF_OyaStartInit00( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_111, heapID );
...
static BOOL WFP2PMF_OyaStartSyncInit( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	if(p_wk->data.timer == 0){
		CommSendData( CNM_WFP2PMF_START, NULL, 0 );	// ゲーム開始
...
{
	// メッセージの表示
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_123, heapID );
...
{
	// メッセージ表示	このメンバーでよろしいですが？
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_124, heapID );
...
static BOOL WFP2PMF_OyaEnd3Init( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_125, heapID );
...
{
	// メッセージ表示	このメンバーでよろしいですが？
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_120, heapID );
...
static BOOL WFP2PMF_OyaVchatInit00( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	// メッセージ表示	このメンバーでよろしいですが？
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, idx, heapID );
...
static BOOL WFP2PMF_KoEntryInit( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_118, heapID );
...
static BOOL WFP2PMF_KoEntryOkInit( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	// メッセージ
	WFP2PMF_WordSetPlayerNameSet( &p_wk->draw, p_init, COMM_PARENT_ID, 0, heapID );
	WFP2PMF_GraphicMsgBmpStrPutEx( &p_wk->draw, msg_wifilobby_129, WFP2PMF_MSGENDWAIT, heapID );
...
static BOOL WFP2PMF_KoEntryNgInit( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	// メッセージ
	WFP2PMF_WordSetPlayerNameSet( &p_wk->draw, p_init, COMM_PARENT_ID, 0, heapID );
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_130, heapID );
...
static BOOL WFP2PMF_KoConnectInit( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	// メッセージ
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_126, heapID );
...
static BOOL WFP2PMF_KoConLockInit00( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	// メッセージ表示	このメンバーでよろしいですが？
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_136, heapID );
...
static BOOL WFP2PMF_KoConLockReCon( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	WFP2PMF_WordSetPlayerNameSet( &p_wk->draw, p_init, COMM_PARENT_ID, 0, heapID );
	WFP2PMF_GraphicMsgBmpStrPut( &p_wk->draw, msg_wifilobby_014, heapID );
...
static BOOL WFP2PMF_KoConLockReCon01( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	...
	// 再接続
	if( CommWifiPofinStart( p_wk->data.oya_friendid ) ){
...
static BOOL WFP2PMF_KoConLockReConGraphicReset00( WFP2PMF_WK* p_wk, WFP2PMF_INIT* p_init, u32 heapID )
{
	if( WIPE_SYS_EndCheck() ){
		...
		// ワークのクリア
		{
			...
			memset( p_wk->data.vchat_tmp, 0, sizeof(u8)*WIFI_P2PMATCH_NUM_MAX );// １つ前のVCHAT
			memset( p_wk->data.vchat_now, 0, sizeof(u8)*WIFI_P2PMATCH_NUM_MAX );// １つ前のVCHAT

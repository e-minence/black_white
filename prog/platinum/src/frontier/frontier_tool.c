...
void Frontier_EnemyPokeMake( u8 set_num, u16 tr_index, u8 level, B_TOWER_POKEMON* rental_poke, u16* enemy_poke_index, B_TOWER_POKEMON* enemy_poke, u8* pow_rnd, u32* personal_rnd );
...
{
	int i;
	u8	buf8,waza_pp,poke_level,pow;
	u16	buf16;
	u32	buf32;
...
//B_TOWER_TRAINER_ROM_DATA* RomTrainerDataAlloc(B_TOWER_PARTNER_DATA *tr_data,u16 tr_no,int heapID);
...
B_TOWER_TRAINER_ROM_DATA* RomTrainerDataAlloc(B_TOWER_PARTNER_DATA *tr_data,u16 tr_no,int heapID)
{
	...
	trd = Frontier_TrainerRomDataGet(tr_no,heapID,arcTR);

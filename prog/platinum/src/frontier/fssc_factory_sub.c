...
static void FactoryScr_TradeCountInc( FACTORY_SCRWORK* wk );
...
/*****************************************************/
/*****************************************************/
void FactoryScr_SaveRestPlayData( FACTORY_SCRWORK* wk );
...
void FactoryScr_SetLose( FACTORY_SCRWORK* wk )
{
	...
	//現在の5連勝などを保存する必要がある！
	FactoryScr_SaveRestPlayData( wk );		//セーブデータに代入
...
u16	FactoryScr_GetAddBtlPoint( FACTORY_SCRWORK* wk )
{
	...
	if( lap >= FACTORY_LAP_MAX ){
		return bppoint_normal[ FACTORY_LAP_MAX ];		//テーブルが+1しているので
	}

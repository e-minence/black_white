...
static void CastleScr_InitDataSet( CASTLE_SCRWORK* wk )
{
	...
	Frontier_EnemyPokeMake(	Castle_GetEnemyPokeNum(wk->type,CASTLE_FLAG_TOTAL),
							wk->tr_index[wk->round], 0, 
							NULL,								//B_TOWER_POKEMON
							wk->enemy_poke_index, 
...
static void CastleScr_ContinuePokePara( CASTLE_SCRWORK* wk )
{
	...
	for( i=0; i < count ;i++ ){
		...
		buf = (u16)CASTLEDATA_GetPlayData(	wk->castle_savedata, 
											CASTLEDATA_ID_TEMOTI_HP, i, 0, NULL );
		OS_Printf( "ロード時のHP = %d\n", buf );
		PokeParaPut( pp, ID_PARA_hp, &buf );			//HP

		buf = (u16)CASTLEDATA_GetPlayData(	wk->castle_savedata, 
											CASTLEDATA_ID_TEMOTI_PP, i, 0, NULL );
		PokeParaPut( pp, ID_PARA_pp1, &buf );			//PP1

		buf = (u16)CASTLEDATA_GetPlayData(	wk->castle_savedata, 
											CASTLEDATA_ID_TEMOTI_PP, i, 1, NULL );
		PokeParaPut( pp, ID_PARA_pp2, &buf );			//PP2

		buf = (u16)CASTLEDATA_GetPlayData(	wk->castle_savedata, 
											CASTLEDATA_ID_TEMOTI_PP, i, 2, NULL );
		PokeParaPut( pp, ID_PARA_pp3, &buf );			//PP3

		buf = (u16)CASTLEDATA_GetPlayData(	wk->castle_savedata, 
											CASTLEDATA_ID_TEMOTI_PP, i, 3, NULL );
		PokeParaPut( pp, ID_PARA_pp4, &buf );			//PP4

		buf = (u16)CASTLEDATA_GetPlayData(	wk->castle_savedata, 
											CASTLEDATA_ID_TEMOTI_CONDITION, i, 0, NULL );
		PokeParaPut( pp, ID_PARA_condition, &buf );		//CONDITION
...
/*****************************************************/
/*****************************************************/
void CastleScr_SaveRestPlayData( CASTLE_SCRWORK* wk );
...
void CastleScr_SaveRestPlayData( CASTLE_SCRWORK* wk )
{
	...
	//更新されていたら、"最高連勝記録時"の"残りCP"も更新する
	if( after == wk->rensyou ){

		cp = FrontierRecord_Get(SaveData_GetFrontier(wk->sv), 
								CastleScr_GetCPRecordID(wk->type),
	...
	for( i=0; i < count ;i++ ){
		...
		buf16[0] = PokeParaGet( pp, ID_PARA_pp1, NULL );		//PP1
		CASTLEDATA_PutPlayData( wk->castle_savedata, CASTLEDATA_ID_TEMOTI_PP, i, 0, buf16 );

		buf16[0] = PokeParaGet( pp, ID_PARA_pp2, NULL );		//PP2
		CASTLEDATA_PutPlayData( wk->castle_savedata, CASTLEDATA_ID_TEMOTI_PP, i, 1, buf16 );

		buf16[0] = PokeParaGet( pp, ID_PARA_pp3, NULL );		//PP3
		CASTLEDATA_PutPlayData( wk->castle_savedata, CASTLEDATA_ID_TEMOTI_PP, i, 2, buf16 );

		buf16[0] = PokeParaGet( pp, ID_PARA_pp4, NULL );		//PP4
		CASTLEDATA_PutPlayData( wk->castle_savedata, CASTLEDATA_ID_TEMOTI_PP, i, 3, buf16 );

		buf16[0] = PokeParaGet( pp, ID_PARA_condition, NULL );	//CONDITION
		CASTLEDATA_PutPlayData( wk->castle_savedata, CASTLEDATA_ID_TEMOTI_CONDITION, i, 0, buf16 );
...
void CastleScr_SetLose( CASTLE_SCRWORK* wk )
{
	...
	//現在の5連勝などを保存する必要がある！
	CastleScr_SaveRestPlayData( wk );		//セーブデータに代入
...
//]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]
//
//castle_tool.c	Castle_RentalPokeMake
//
//先に親機のレンタルを決定、
...
u16	CastleScr_GetAddBtlPoint( CASTLE_SCRWORK* wk )
{
	...
	if( lap >= CASTLE_LAP_MAX ){
		return bppoint_normal[ CASTLE_LAP_MAX ];		//テーブルが+1しているので
	}


	return bppoint_normal[ lap ];
}

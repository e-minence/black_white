...
#define NORMAL_PAL2		(1)				//通常パレット番号(2本使用している)
...
static BOOL Stage_BtlCountLeaderCheck( u16 btl_count );
...
static void RankWrite( STAGE_WORK* wk, GF_BGL_BMPWIN* win, u8 num, u32 x, u32 y )
{
	NUMFONT_WriteNumber( wk->num_font, num, 2, NUMFONT_MODE_SPACE, win, x, y );
...
static void ScrnPalChg( GF_BGL_INI* ini, u8 csr_pos, u8 flag )
{
	...
	if( flag == NORMAL_PAL_CHG ){
		...
		//OS_Printf( "sx = %d\n", sx );
		//OS_Printf( "sy = %d\n", sy );
		GF_BGL_ScrPalChange( ini, BS_FRAME_BG, px, py, sx, sy, NORMAL_PAL2 );
	}
...
static BOOL Stage_BtlCountLeaderCheck( u16 btl_count )
{
	...
	//ブレーンが登場する戦闘回数だったら、「???」以外はパレット変更
	if( (btl_count == STAGE_LEADER_SET_1ST) || (btl_count == STAGE_LEADER_SET_2ND) ){
		return TRUE;
	}
...
static void Stage_SeqSubTypeSelYesNo( STAGE_WORK* wk )
{
	...
	wk->msg_index = Stage_EasyMsg( wk, msg_stage_room_41, FONT_TALK );

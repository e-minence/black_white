...
static const FACTORY_POKE_RANGE OpenLevelEnemyPokemonRange[] = {
	...
	{220,	351,	950,	24},
	{300,	351,	950,	31},
	{312,	759,	950,	31},		//ブレーン用
};

///レベル50で自分が使用出来るポケモンデータのIndex取得範囲(現状敵と範囲は一緒)
#define Level50MinePokemonRange		Level50EnemyPokemonRange
///オープンレベルで自分が使用出来るポケモンデータのIndex取得範囲(現状敵と範囲は一緒)
#define OpenLevelMinePokemonRange	OpenLevelEnemyPokemonRange
...
#define MINE_POKE_POW_RND_END		( Level50MinePokemonRange[LV50_ENEMY_POKE_RANGE_MAX-1].pow_rnd )
...
BOOL Factory_PokemonIndexCreate(const u16 check_pokeno[], const u16 check_itemno[], 
	int check_num, int get_count, u16 get_pokeindex[], int heap_id, const FACTORY_POKE_RANGE *poke_range, u16 trade_count)
{
	...
	while(set_count != get_count){
		...
		if( (set_count >= get_count - range_up) && (range->pow_rnd < MINE_POKE_POW_RND_END) ){
			...
			//    MINE_POKE_POW_RND_END = 31 //ブレーンと通常TRの最強の時は、1つ上のデータを見ない！
			//                               //テーブルオーバーしないようにする
		}else{
...
void Factory_RentalPokeMake( u8 lap, u8 level, u16* rental_poke_index, B_TOWER_POKEMON* rental_poke, u8* pow_rnd, u32* personal_rnd, u16 trade_count, u16* poke_check_tbl, u16* item_check_tbl );
void Factory_EnemyPokeMake( u8 set_num, u16 tr_index, u8 level, B_TOWER_POKEMON* rental_poke, u16* enemy_poke_index, B_TOWER_POKEMON* enemy_poke, u8* pow_rnd, u32* personal_rnd );
...
void Factory_RentalPokeMake( u8 lap, u8 level, u16* rental_poke_index, B_TOWER_POKEMON* rental_poke, u8 *pow_rnd, u32* personal_rnd, u16 trade_count, u16* poke_check_tbl, u16* item_check_tbl )
{
	...
	if( poke_check_tbl == NULL ){
		Factory_PokemonIndexCreate( NULL, NULL, 0,
									...
									HEAPID_WORLD, poke_range, trade_count );	//被りチェック無し
	}else{
		...
		Factory_PokemonIndexCreate( poke_check_tbl, poke_check_tbl, FACTORY_RENTAL_POKE_MAX,
									FACTORY_RENTAL_POKE_MAX, rental_poke_index, 
									HEAPID_WORLD, poke_range, trade_count );	//被りチェック有り
	}

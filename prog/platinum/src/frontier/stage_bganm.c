...
typedef struct _STAGE_BGANM_WORK{
	...
	GF_BGL_INI *bgl;
	u16 *scrn[STAGE_BG_ANIME_PATTERN][STAGE_BG_SCREEN_SIZE];	///<転送スクリーンデータ
	u8 wait;					///<アニメウェイト
...
#if 1	//07.11.29 キャラを差し替えたいとのことなので試しに入れてみた
	{
		ARCHANDLE *hdl_bg;
		...
		if( sbw->work_no == 0 ){
			idx = BS_STAGE_NCGR_BIN;
		}else{
			idx = BS_STAGE02_NCGR_BIN;
		}

		ArcUtil_HDL_BgCharSet(	hdl_bg, idx, 
								sbw->bgl, FRMAP_FRAME_MAP, 0, 0, TRUE, HEAPID_FRONTIERMAP );
		ArchiveDataHandleClose(hdl_bg);
	}
#endif

...
static void CsrMoveSub( ROULETTE_WORK* wk );
...
#if 0
		PaletteFadeReq( wk->pfd, PF_BIT_MAIN_BG, 0xffff, 0, 
						16,									//開始濃度
						0,									//最終濃度 
						0x0000 );							//変更後の色
		SoftFadePfd( wk->pfd, FADE_MAIN_BG, 0, 16*13, 0, 0x0000 );
#endif
...
static void CsrMove( ROULETTE_WORK* wk, int key )
{
	...
	if( wk->csr_wait >= csr_wait_tbl[level][0] ){
		...
		RouletteObj_SetObjPos(	wk->p_csr,
								PANEL_START_X + (PANEL_WIDTH_X*(wk->csr_pos%ROULETTE_PANEL_H_MAX)),
								PANEL_START_Y + (PANEL_WIDTH_Y*(wk->csr_pos/ROULETTE_PANEL_H_MAX)));
	}
...
static void CsrMoveSub( ROULETTE_WORK* wk )
{
	...
	RouletteObj_SetObjPos( wk->p_csr, GetCsrX(wk,wk->csr_pos,0), GetCsrY(wk,wk->csr_pos) );

	//カーソル位置を送信
	if( Roulette_CommCheck(wk->type) == TRUE ){
		Roulette_CommSetSendBuf( wk, ROULETTE_COMM_RANK_CSR_POS, wk->csr_pos );
	}
	return;
}
...
static void Roulette_EvPanelChoice( ROULETTE_WORK* wk )
{
	...
	for( ev_i=0; ev_i < ROULETTE_EV_APPEAR_MAX ;ev_i++ ){
		...
		//ここは自分のラップ数でパネルを選出なので問題ないはず(この後に通信で渡すので)
		switch( wk->lap ){
		case 0:
		...
		if( flag == 1 ){
			...
			OS_Printf( "現在何人目か = %d\n", (wk->rensyou % ROULETTE_LAP_ENEMY_MAX) );
			if( challenge != 0xff ){
				switch( (wk->rensyou % ROULETTE_LAP_ENEMY_MAX) ){
				case 0:

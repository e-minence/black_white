...
/*****************************************************/
/*****************************************************/
void RouletteScr_SaveRestPlayData( ROULETTE_SCRWORK* wk );
...
void RouletteScr_SetLose( ROULETTE_SCRWORK* wk )
{
	...
	//現在の5連勝などを保存する必要がある！
	RouletteScr_SaveRestPlayData( wk );		//セーブデータに代入
...
u16	RouletteScr_GetAddBtlPoint( ROULETTE_SCRWORK* wk )
{
	...
	if( lap >= ROULETTE_LAP_MAX ){
		return bppoint_normal[ ROULETTE_LAP_MAX ];		//テーブルが+1しているので
	}

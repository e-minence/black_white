...
BOOL FSSC_RouletteParentCheckWait( FSS_TASK * core );
BOOL FSSC_RouletteTalkMsgAppear(FSS_TASK* core);
static BOOL WaitRouletteParentCheckWait( FSS_TASK * core );
...
BOOL FSSC_RouletteScrWork( FSS_TASK * core )
{
	...
	case FR_ID_SAVE_REST_PLAY_DATA:
		RouletteScr_SaveRestPlayData( br_scr_wk );
		break;
	...
	//親の決定をクリア
	case FR_ID_CLEAR_PARENT_CHECK_FLAG:
		br_scr_wk->parent_check_flag = 0;
		break;

	//相手の決定が採用されたか取得
	case FR_ID_GET_PAIR_DECIDE_CHECK:
		*work = 0;

		if( Roulette_CommCheck(br_scr_wk->type) == TRUE ){
			if( br_scr_wk->parent_check_flag >= ROULETTE_SCR_MENU_MAX ){
				//*work = 1;												//子選択が採用
...
BOOL FSSC_RouletteParentCheckWait( FSS_TASK * core )
{
	...
	FSST_SetWait( core, WaitRouletteParentCheckWait );
	return 1;
}
...
static BOOL WaitRouletteParentCheckWait( FSS_TASK * core )
{
	...
	if( br_scr_wk->parent_check_flag == ROULETTE_SCR_MENU_NONE ){
		return 0;
	}

	br_scr_wk->recieve_count = 0;
	*ret_wk = br_scr_wk->parent_check_flag;
	return 1;
}

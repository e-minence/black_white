...
static	void	TCB_CommandSelectAuto(TCB_PTR tcb,void *work)
{
	...
	//録画データの整合性をチェック
	data=BattleWorkBattlePlay(tcs->bw,tcs->client_no);
	//コマンドがない
	if((!data)||
	//コマンドが範囲外
...
static	void	TCB_WazaSelectAuto(TCB_PTR tcb,void *work)
{
	...
	//録画データの整合性をチェック
	data=BattleWorkBattlePlay(tws->bw,tws->client_no);
	//コマンドがない
	if((!data)||
	//コマンドが範囲外
...
static	void	TCB_DirSelectAuto(TCB_PTR tcb,void *work)
{
	...
	//録画データの整合性をチェック
	data=BattleWorkBattlePlay(tds->bw,tds->client_no);
	//コマンドがない
	if((!data)||
	//コマンドが範囲外
	   (data>CLIENT_MAX)){
...
static	void	TCB_ItemSelectAuto(TCB_PTR tcb,void *work)
{
	...
	isrp.item_no=BattleWorkBattlePlay(tis->bw,tis->client_no);
	data=BattleWorkBattlePlay(tis->bw,tis->client_no);
	isrp.page=data&0x0f;
...
static	void	TCB_PokemonSelectAuto(TCB_PTR tcb,void *work)
{
	...
	//録画データの整合性をチェック
	data=BattleWorkBattlePlay(tps->bw,tps->client_no);
	//コマンドがない
	if((!data)||
	//すでに
	//コマンドが範囲外
...
//	SCIO_CommandSelectReturnSet(tyns->bw,tyns->client_no,BattleWorkBattlePlay(tyns->bw,tyns->client_no));
	SCIO_ServerQueClear(tyns->bw,tyns->client_no,tyns->command_code);
	sys_FreeMemoryEz(work);
	TCB_Delete(tcb);
}
...
static	void	TCB_YesNoSelectAuto(TCB_PTR tcb,void *work)
{
	...
	//録画データの整合性をチェック
	data=BattleWorkBattlePlay(tyns->bw,tyns->client_no);
	//コマンドがない
	if((!data)||
	//コマンドが範囲外
	...
	case 4:
		{
			...
			PokeVoiceWaitPlaySet(BattleWorkPerapVoiceGet(tke->bw,tke->client_no),
								 PV_POKEDOWN,tke->monsno,tke->form_no,pan,POKE_KIZETSU_VOL,tke->henshin_flag,HEAPID_BATTLE, wait);
...
void	CT_UISet(BATTLE_WORK *bw,CLIENT_PARAM *cp)
{
	...
	if(fight_type&FIGHT_TYPE_AI){
		...
		else if(cp->client_type!=BattleWorkClientTypeGet(bw,CommGetCurrentID())){
			cp->ui.UI_CommandSelect=&TCB_CommandSelectSIO;
			cp->ui.UI_WazaSelect=&TCB_WazaSelectSIO;
			cp->ui.UI_DirSelect=&TCB_DirSelectSIO;
			cp->ui.UI_ItemSelect=&TCB_ItemSelectSIO;

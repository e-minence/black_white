...
static const PARTS_TRANS_PARAM NameTrans[][4] = {
	{//GAUGE_TYPE_AA
		{0x13 * 0x20, 5 * 0x20},		///<1枚目OAM：上段
		{0x1b * 0x20, 5 * 0x20},		///<1枚目OAM：下段
		{0x50 * 0x20, 3 * 0x20},		///<2枚目OAM：上段
		{0x58 * 0x20, 3 * 0x20},		///<2枚目OAM：下段
	},
	{//GAUGE_TYPE_BB
...
static const PARTS_TRANS_PARAM LvFontTrans[][2] = {
	{//GAUGE_TYPE_AA
		{0x53 * 0x20, 2 * 0x20},		///<1枚目OAM
		{0x5B * 0x20, 2 * 0x20},		///<2枚目OAM
	},
	{//GAUGE_TYPE_BB
...
static const PARTS_TRANS_PARAM LvTrans[][2] = {
	{//GAUGE_TYPE_AA
		{0x55 * 0x20, 3 * 0x20},		///<1枚目OAM
		{0x5d * 0x20, 3 * 0x20},		///<2枚目OAM
	},
	{//GAUGE_TYPE_BB
...
enum{
	EXPBAR_FAST_HIGH = 0x33 * 0x20,		///<EXPバー：1枚目のOAMの描画開始位置
	EXPBAR_SECOND_HIGH = 0x70 * 0x20,	///<EXPバー：2枚目のOAMの描画開始位置
	
	EXPBAR_NEXT_OAM = 5,		///<1枚目のOAMが持つEXPバーのキャラ数
};
 ...
 */
//--------------------------------------------------------------
void Gauge_ResourceLoad(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE* hdl, PALETTE_FADE_PTR pfd, int gauge_type)
{
...
//	CATS_LoadResourcePlttArc(csp, crp, hdl, obj_head->id[CLACT_U_PLTT_RES], 0, 
...
//		CATS_LoadResourcePlttArc(csp, crp, hdl, obj_head->id[CLACT_U_PLTT_RES], 
//			0, 1, NNS_G2D_VRAM_TYPE_2DMAIN, obj_head->id[CLACT_U_PLTT_RES]);
		CATS_LoadResourcePlttWorkArcH(pfd, FADE_MAIN_OBJ, csp, crp, 
			hdl, GAGE_PALETTE_NCLR, 
			0, 1, NNS_G2D_VRAM_TYPE_2DMAIN, PLTTID_GAUGE);
		CATS_LoadResourceCellArcH(csp, crp, hdl, obj_head->id[CLACT_U_CELL_RES], 
...
void Gauge_ActorResourceSet(GAUGE_WORK *gauge)
{
	...
	Gauge_ResourceLoad(csp, crp, hdl, pfd, gauge->type);
...
void Gauge_NumModeChange(GAUGE_WORK *gauge)
{
	...
	if(gauge->num_mode == TRUE){	//数値モード
		//HPバーの頭と蓋を消去
	#if 0	//platinumで変更 2007.04.19(木) matsuda
...
//			0x20);
		...
		MI_CpuCopy16(parts, (void*)((u32)obj_vram + HPBarHeadTrans[gauge->type].pos + 0x20
			+ image->vramLocation.baseAddrOfVram[NNS_G2D_VRAM_TYPE_2DMAIN]), 
			0x20);
		parts = GetGaugePartsAdrs(GP_HPBAR_NUM_RIGHT);
		...
		//HPラインの切れ端を元に戻す
		//左側はHPBAR_HEADで上書きされるので、右側のみ描画する
		parts = GetGaugePartsAdrs(GP_HPBAR_CLEAR);
		MI_CpuCopy16(parts, (void*)((u32)obj_vram + HPBarCapTrans[gauge->type].pos + 0x20
			+ image->vramLocation.baseAddrOfVram[NNS_G2D_VRAM_TYPE_2DMAIN]), 
			0x20);
...
static void GaugeParts_LevelFontDraw(GAUGE_WORK *gauge)
{
	NNSG2dImageProxy * image;
	const u8 *level_buf;
	const u8 *level_top_buf;
	int gp, gp_top;
	
	if(gauge->sex == PARA_MALE){
		gp = GP_SEX_MALE;
		gp_top = GP_SEX_MALE_TOP;
	}
	else if(gauge->sex == PARA_FEMALE){
		gp = GP_SEX_FEMALE;
		gp_top = GP_SEX_FEMALE_TOP;
	}
	else{
		gp = GP_SEX_UNK;
		gp_top = GP_SEX_UNK_TOP;
	}
	...
	//形成された文字バッファをVRAMに直書き
	{
		...
		MI_CpuCopy16(level_buf, (void*)((u32)obj_vram + LvFontTrans[gauge->type][1].pos
			+ image->vramLocation.baseAddrOfVram[NNS_G2D_VRAM_TYPE_2DMAIN]), 
...
static void GaugeLevelUpEffTask(TCB_PTR tcb, void *work)
{
	...
	case 1:
		...
		if(level_eff->evy >= GAUGE_LEVELUP_EVY_MAX){
			level_eff->evy = GAUGE_LEVELUP_EVY_MAX;
			level_eff->seq++;
		}

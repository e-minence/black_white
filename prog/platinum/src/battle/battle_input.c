...
ALIGN4 static const u16 BackGroundPaletteID[] = {
	BATTLE_W_00_NCLR,			//BG_ID_BASIC
	BATTLE_W_01_NCLR,			//BG_ID_SEA
	BATTLE_W_02_NCLR,			//BG_ID_CITY
	BATTLE_W_03_NCLR,			//BG_ID_FOREST
	BATTLE_W_04_NCLR,			//BG_ID_MOUNTAIN
	BATTLE_W_05_NCLR,			//BG_ID_SNOW_MOUNTAIN
	BATTLE_W_06_NCLR,			//BG_ID_ROOM_A
	BATTLE_W_07_NCLR,			//BG_ID_ROOM_B
	BATTLE_W_08_NCLR,			//BG_ID_ROOM_C
	BATTLE_W_09_NCLR,			//BG_ID_CAVE_A
	BATTLE_W_10_NCLR,			//BG_ID_CAVE_B
	BATTLE_W_11_NCLR,			//BG_ID_CAVE_C
	BATTLE_W_12_NCLR,			//BG_ID_BIG_FOUR_A
	BATTLE_W_13_NCLR,			//BG_ID_BIG_FOUR_B
	BATTLE_W_14_NCLR,			//BG_ID_BIG_FOUR_C
	BATTLE_W_15_NCLR,			//BG_ID_BIG_FOUR_D
	BATTLE_W_16_NCLR,			//BG_ID_CHAMPION
	BACK_GROUND_PALETTE_NONE,	//BG_ID_HAKAI
	BACK_GROUND_PALETTE_NONE,	//BG_ID_TOWER
	BACK_GROUND_PALETTE_NONE,	//BG_ID_FACTORY
	BACK_GROUND_PALETTE_NONE,	//BG_ID_ROULETTE
	BACK_GROUND_PALETTE_NONE,	//BG_ID_CASTLE
	BACK_GROUND_PALETTE_NONE,	//BG_ID_STAGE
};
...
//	BINPUT_DefaultFrameSet(bgl);
	...
	//スクリーン全読み込み
	{
		...
		for(i = 0; i < SCRNBUF_MAX; i++){
			...
			arc_data = ArcUtil_ScrnDataGet(ARC_BATT_BG, scrn_data_id, 1, 
	...
	{
		u16 *def_wk;
		int bg_id = BattleWorkBGIDGet(bw);
		int bg_data_id;
		...
		PaletteWorkSet_Arc(pfd, ARC_BATT_BG, bg_data_id, HEAPID_BATTLE, FADE_SUB_BG, 0, 0);
		if(BackGroundPaletteID[bg_id] != BACK_GROUND_PALETTE_NONE){
			PaletteWorkSet_Arc(pfd, ARC_BATT_BG, BackGroundPaletteID[bg_id], HEAPID_BATTLE,
				FADE_SUB_BG, 0x20, 0);
		}
...
void BINPUT_DefaultDataSet(BI_PARAM_PTR bip)
{
	...
	//キャラデータVRAM転送
	{
		...
		ArcUtil_BgCharSet(ARC_BATT_BG, cgr_data_id, bgl, GF_BGL_FRAME0_S, 
...
//	Sub_BackScrnOnlyVisible();
	...
	if(bmd->cgr_id != NONE_ID && (force_put == TRUE || bmd->cgr_id != old_bmd->cgr_id)){
		...
		ArcUtil_HDL_BgCharSet(hdl_bg, cgr_data_id, bgl, GF_BGL_FRAME0_S, 
			0, BI_BG_CGR_SIZE, 1, HEAPID_BATTLE);
		if(bip->sex == PM_MALE){
			ArcUtil_HDL_BgCharSet(hdl_bg, BATTLE_W_BOY_NCGR_BIN, bgl, GF_BGL_FRAME0_S,
				BG_PLAYER_WALL_OFFSET, BG_PLAYER_WALL_SIZE, 1, HEAPID_BATTLE);
		}
		else{
			ArcUtil_HDL_BgCharSet(hdl_bg, BATTLE_W_GIRL_NCGR_BIN, bgl, GF_BGL_FRAME0_S,
				BG_PLAYER_WALL_OFFSET, BG_PLAYER_WALL_SIZE, 1, HEAPID_BATTLE);
		}
	}
	//PALETTE
...
//		1, NNS_G2D_VRAM_TYPE_2DSUB, PLTTID_INPUT_FONTOBJ_COMMON);
	...
	//-- ついでにここで戦闘カーソルも生成 --//
	{
		PALETTE_FADE_PTR pfd;
		pfd = BattleWorkPfdGet(bip->bw);
		BCURSOR_ResourceLoad(csp, crp, pfd, HEAPID_BATTLE, CHARID_BATTLE_CURSOR, PLTTID_BATTLE_CURSOR,
...
static int TPCallback_PlayBackStop(BI_PARAM_PTR bip, int tp_ret, int panel_pal)
{
	...
	case RECT_HIT_NONE:
	default:
		//カーソルを表示しないので、CursorMoveでは特別処理を入れないと処理できない
		//その為ここでキー判定させる
		if(sys.trg & PAD_BUTTON_CANCEL){
			tp_ret = TRUE;
			panel_pal = PlayBackStopMenuPaletteNo[0];
			break;
		}
		return tp_ret;
	case TRUE:

...
BOOL		ST_ServerTypeCheckTableParaGet(BATTLE_WORK *bw,int pos,u8 *type_w,u8 *type_p,u8 *value);
...
BOOL		ST_ServerHukitobasiCheck(BATTLE_WORK *bw,SERVER_PARAM *sp);
...
BOOL		ST_ServerPCCTokuseiCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int client_no);
...
int			ST_DamageLossCalc(BATTLE_WORK *bw,SERVER_PARAM *sp,int damage);
int			ST_CriticalCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int attack,int defence,int critical_count,u32 side_condition);
...
BOOL		ST_BattleRecordStopCheck(BATTLE_WORK *bw,SERVER_PARAM *sp);
...
static	int		ST_ServerTraceClientNoGet(BATTLE_WORK *bw,SERVER_PARAM *sp,int def1,int def2);
...
//技のシーケンス内でも呼ばれるので,battle_aiのオーバーレイからはずす
int				ClientAIPokeSelectAI(BATTLE_WORK *bw,int client_no);
int				ClientAIReshuffleSelMonsNoGet(BATTLE_WORK *bw,int client_no);
int				AIWazaTypeGetPP(BATTLE_WORK *bw,SERVER_PARAM *sp,POKEMON_PARAM *pp,int waza_no);
...
#if AFTER_MASTER_070219_BT_FIX == 0
	sp->psp[client_no].item=			PokeParaGet(pp,ID_PARA_item,			0);
#endif //AFTER_MASTER_070219_BT_FIX
...
//		attackpri2=WT_WazaDataParaGet(wazano2,ID_WTD_attackpri);
	...
	if(attackpri1==attackpri2){
		...
		if((sensei1)&&(sensei2)){
			...
			else if((agi1==agi2)&&(BattleWorkRandGet(bw)&1)){
				ret=2;
			}
		}
			...
			if((agi1==agi2)&&(BattleWorkRandGet(bw)&1)){
				ret=2;
			}
		}
		else{
...
#ifdef PM_DEBUG
		if(((BattleWorkRandGet(bw)%100)<kakuritsu)||(sp->dbp.debug_flag&DEBUG_FLAG_ADD_STATUS)){
			//追加効果起動になったら、フラグをオンにする
			sp->server_status_flag|=SERVER_STATUS_FLAG_TSUIKA;
		}
#else
		if((BattleWorkRandGet(bw)%100)<kakuritsu){
			//追加効果起動になったら、フラグをオンにする
			sp->server_status_flag|=SERVER_STATUS_FLAG_TSUIKA;
		}
#endif
...
//		range=WT_WazaDataParaGet(waza_no,ID_WTD_attackrange);
	...
	//ゆびをふるなどで、対象を選ばずに技が出た時の処理
	else if((range==RANGE_TUBOWOTUKU)&&(flag==1)){
		{
			...
			if((fight_type&FIGHT_TYPE_2vs2)&&((BattleWorkRandGet(bw)%2)==0)){
...
void	ST_ServerCounterWorkInit(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	for(i=0;i<CLIENT_MAX;i++){
		...
		sp->agi_rand[i]=BattleWorkRandGet(bw);	///<素早さチェックに使用する乱数値
	}
...
BOOL	ST_ServerTypeCheckTableParaGet(BATTLE_WORK *bw,int pos,u8 *type_w,u8 *type_p,u8 *value)
{
	...
	if(pos>=NELEMS(TypeCheckTable)){
		pos=BattleWorkRandGet(bw)%(NELEMS(TypeCheckTable));
		ret=FALSE;
	}
...
BOOL	ST_ServerHukitobasiCheck(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(sp->psp[sp->attack_client].level>=sp->psp[sp->defence_client].level){
	...
	else{
		{
			...
			level=(((BattleWorkRandGet(bw)&0xff)*(sp->psp[sp->attack_client].level+sp->psp[sp->defence_client].level))>>8)+1;
...
BOOL	ST_EscapeActCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int client_no)
{
	...
	//特殊能力にげあしは100%逃げれる
		...
		if(sp->psp[client_no].agi<sp->psp[client_no^1].agi){
			...
			if(escape>(BattleWorkRandGet(bw)%256)){
				ret=TRUE;
			}
		}
		//自分の方が早ければ100%逃げれる
...
BOOL	ST_ServerPCCTokuseiCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int client_no)
{
	...
	case TOKUSYU_DAPPI:
		...
		if((sp->psp[client_no].condition&CONDITION_BAD)&&
		   (sp->psp[client_no].hp)&&
		   ((BattleWorkRandGet(bw)%10<3)||(sp->dbp.debug_flag&DEBUG_FLAG_TOKUSYU))){
#else
		if((sp->psp[client_no].condition&CONDITION_BAD)&&
		   (sp->psp[client_no].hp)&&
		...
		case SPAC_WEATHER:
			if(sp->weather_check_flag==0){
				...
				case WEATHER_SYS_MIST2:			// 霧払いの霧
					...
					ret=SPAC_WAZA_SEQ;
					break;
				case WEATHER_SYS_HIGHSUNNY:		// 日本晴れ
					seq_no=SUB_SEQ_FIELD_HARE;
					ret=SPAC_WAZA_SEQ;
					break;
				case WEATHER_SYS_TRICKROOM:		// トリックルーム
					seq_no=SUB_SEQ_FIELD_TRICKROOM;
					ret=SPAC_WAZA_SEQ;
					break;
				default:
					break;
		...
		case SPAC_TRACE:
			{
				...
				for(i=0;i<client_set_max;i++){
					...
					sp->defence_client_work=ST_ServerTraceClientNoGet(bw,sp,def1,def2);
...
//										switch(WT_WazaDataParaGet(waza_no,ID_WTD_battleeffect)){
										...
										case 38:
											if((pow_temp<150)||
											  ((pow_temp==150)&&(BattleWorkRandGet(bw)&1))){
												pow_temp=150;
										...
										case 227:
											if((pow_temp<120)||
											  ((pow_temp==120)&&(BattleWorkRandGet(bw)&1))){
												pow_temp=120;
												sp->waza_work=waza_no;
											}
											break;
										default:
											if((pow_temp<80)||
											  ((pow_temp==80)&&(BattleWorkRandGet(bw)&1))){
												pow_temp=80;
												sp->waza_work=waza_no;
											}
											break;
									...
									default:
										if((pow_temp<pow)||
										  ((pow_temp==pow)&&(BattleWorkRandGet(bw)&1))){
											pow_temp=pow;
											sp->waza_work=waza_no;
										}
										break;
						...
						//見つからなかった場合は、持っている技からランダムで選択
							...
							sp->waza_work=sp->psp[no].waza[BattleWorkRandGet(bw)%pos];
							sp->client_work=client_no;
							seq_no=SUB_SEQ_YOCHIMU;
							ret=SPAC_WAZA_SEQ;
						}
					...
					if(BattleWorkFightTypeGet(bw)&FIGHT_TYPE_2vs2){
						{
							...
							if((sp->psp[def[0]].hp)&&(sp->psp[def[0]].item)&&
							   (sp->psp[def[1]].hp)&&(sp->psp[def[1]].item)){
								sp->item_work=sp->psp[def[BattleWorkRandGet(bw)&1]].item;
								ret=SPAC_WAZA_SEQ;
							}
...
BOOL	ST_ServerTenkiyaCheck(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	for(i=0;i<BattleWorkClientSetMaxGet(bw);i++){
		...
		if((sp->psp[sp->client_work].monsno==MONSNO_AUSU)&&
		   (sp->psp[sp->client_work].hp)&&
		   (ST_ServerTokuseiGet(sp,sp->client_work)==TOKUSYU_MARUTITAIPU)){
...
//	wdcp_d.atc=ItemParamGet(item,ITEM_PRM_ATTACK,HEAPID_BATTLE);
	...
	//アイテムでギラティナが持つと技の威力を1.5倍
	if((wdcp_a.eqp==SOUBI_DORAGONGHOSTUP)&&
	  ((wazatype==DRAGON_TYPE)||(wazatype==GHOST_TYPE))&&
	   (wdcp_a.monsno==MONSNO_KIMAIRAN)){
		wazapow=wazapow*(100+wdcp_a.atc)/100;
	}
...
int		ST_DamageLossCalc(BATTLE_WORK *bw,SERVER_PARAM *sp,int damage)
{
	...
	if(damage){
		damage*=(100-(BattleWorkRandGet(bw)%16));
		damage/=100;
		if(damage==0){
			damage=1;
		}
...
//	eqp=ItemParamGet(item,ITEM_PRM_EQUIP,HEAPID_BATTLE);
	...
	if(BattleWorkRandGet(bw)%CriticalTable[work]==0){
...
BOOL	ST_BattleRecordStopCheck(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	if(BattleWorkBattleRecStopFlagGet(bw)){
		sp->server_seq_no=SERVER_BATTLE_RECORD_STOP_NO;
		return TRUE;
	}
	return FALSE;
}
...
int		ST_ServerParamDataGet(BATTLE_WORK *bw,SERVER_PARAM *sp,int id,int client_no)
{
	...
	case ID_SP_act_work:
		...
		break;
	case ID_SP_server_seq_no:
		return sp->server_seq_no;
		break;
	case ID_SP_next_server_seq_no:
		return sp->next_server_seq_no;
		break;
	}
...
//技のシーケンス内でも呼ばれるので,battle_ai、bc_common2のオーバーレイからはずれたここに置く
//============================================================================================
/**
 *	ポケモン選択AI
 *
...
int	ClientAIReshuffleSelMonsNoGet(BATTLE_WORK *bw,int client_no)
{
	...
	return sp->ai_reshuffle_sel_mons_no[client_no];
}

...
#define HENSHIN_ITEM_ZOUSYOKU_FIX			//コメントアウトをはずすことで修正　2006.10.26
...
void	ClientEnd(BATTLE_WORK *bw,CLIENT_PARAM *cp,int mode)
{
	...
	ArchiveDataHandleClose( cp->handle );	///< handle close
...
static	void	ClientSioWaitMessage(BATTLE_WORK *bw,CLIENT_PARAM *cp)
{
	WITH_REC_DATA	*wrd=(WITH_REC_DATA *)&cp->client_buffer[0];

	BattleWorkBattleRecordRecvDataSet(bw,wrd->size,wrd->buf);
...
static	void	ClientEscapeMessage(BATTLE_WORK *bw,CLIENT_PARAM *cp)
{
	...
	BattleWorkBattleRecordRecvDataSet(bw,emp->size,emp->buf);
...
static	void	ClientGiveUpMessage(BATTLE_WORK *bw,CLIENT_PARAM *cp)
{
	GIVEUP_MESSAGE_PARAM	*gmp=(GIVEUP_MESSAGE_PARAM *)&cp->client_buffer[0];

	BattleWorkBattleRecordRecvDataSet(bw,gmp->size,gmp->buf);

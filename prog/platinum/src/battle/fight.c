...
static	const	u32	BattleServerVersion=0x140;		//戦闘サーババージョン
													//履歴
													//0x100　国内版
													//0x110　北米版
													//0x120　欧州版
...
static	void	FightInit(PROC *proc)
{
	...
	if(bw->fight_type & FIGHT_TYPE_SAFARI){
		...
		bw->numfont_hp = NUMFONT_Create(0xe, 2, 3, HEAPID_BATTLE);
	}
	else{
...
static	void	FightSystemBoot(BATTLE_WORK *bw,BATTLE_PARAM *bp)
{
	...
	//ボックスのGDS送信済みフラグリセットの為にMISCをコピー
	if(bp->savedata != NULL){
		bw->misc = SaveData_GetMisc(bp->savedata);
	}
...
static	BOOL	FightWinLoseDemoInit(PROC *proc)
{
	...
	//録画記録モード
	if(bp->fight_type&FIGHT_TYPE_MULTI){
		vdd->rec_mode = RECMODE_COLOSSEUM_MULTI;
	}
	else if(bp->fight_type&FIGHT_TYPE_2vs2){
		vdd->rec_mode = RECMODE_COLOSSEUM_DOUBLE;
	}
	else{
		vdd->rec_mode = RECMODE_COLOSSEUM_SINGLE;
	}
...
static	void	FightSioServerBootCheck(BATTLE_WORK *bw,BATTLE_PARAM *bp)
{
	...
	if(id[0]==sio_id){
		...
		if(bw->fight_type&FIGHT_TYPE_MULTI){
			...
			case 2:
				if((work&1)==0){
					bp->battle_status_flag|=BATTLE_STATUS_FLAG_REC_CHILD;
				}
				break;
			case 1:
			case 3:

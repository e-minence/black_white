...
typedef struct
{
    ...
    void *recvPtr;  //受信バッファの32バイトアライメントしていないポインタ
	void *heapPtr;
...
//-----#endif //TESTOHNO
{
    ...
    _dWork->orgPtr = pTemp;
	_dWork->recvPtr = sys_AllocMemory(heapID, (SIZE_RECV_BUFFER * (maxConnectNum-1)) + 32);
    _dWork->recvBuffer = (u8 *)( ((u32)_dWork->recvPtr + 31) / 32 * 32 );
...
#endif
    _dWork->heapPtr = sys_AllocMemory(heapID, heapSize + 32);
//----#endif //TESTOHNO
...
void mydwc_free()
{
    if(_dWork){
        ...
        sys_FreeMemory( _dWork->heapID, _dWork->recvPtr  );
...
static void setConnectionBuffer(int index)
{
    ...
    for(i=0,j=0;i< _dWork->maxConnectNum; i++){
        if(DWC_GetMyAID()!=i){
            DWC_SetRecvBuffer( i, &(_dWork->recvBuffer[SIZE_RECV_BUFFER * j]), SIZE_RECV_BUFFER );
            j++;
...
static int _DebugDWCCount=0;

static void dwcDebugCountRecv(void)
{
    if(_dWork==NULL){
        return;
    }
...
//    MYDWC_DEBUGPRINT("%d ",_DebugDWCCount);
    _DebugDWCCount=0;
}
#endif
...
#pragma unused(param, index)
    MYDWC_DEBUGPRINT("ConnectionClosedCallback %x\n",(u32)_dWork->disconnectCallback);
...
//            // サーバがコネクションをクローズした。
         (!CommLocalIsWiFiQuartetGroup(CommStateGetServiceNo()) && (DWC_GetNumConnectionHost() == 1))){
            // 全てのコネクションがクローズされた場合         
			...
			if(	_dWork->state == MDSTATE_CANCEL )
			{
                ...
                MYDWC_DEBUGPRINT("DWC_CloseAllConnectionsHard");
                DWC_CloseAllConnectionsHard( );
				_dWork->state = MDSTATE_DISCONNECT;
			}
			if( _dWork->isvchat )
...
{
	if( sync == 0 ){
        //OS_TPrintf(" mydwc_disconnect state %d \n",_dWork->state);
		switch( _dWork->state )	{
 ...
 */
//==============================================================================
int mydwc_startgame( int target,int maxnum )
{
	if( _dWork->state != MDSTATE_LOGIN ) return 0;
    ...
    if ( target < 0 ){
        GF_ASSERT(DWC_SetupGameServer(
	    	(u8)_dWork->maxConnectNum,
        ...
        GF_ASSERT(DWC_ConnectToGameServerAsync(target,
...
//				s_dwcstate = MYDWCSTATE_MATCH_CANCELFINISH;  
        ...
        MYDWC_DEBUGPRINT("Game server error occured. %d\n\n", error);

...
#define SCENE3_START_IN_SYNC	(30)//(120)		//シーン3開始ブラックインスピード
...
static void Scene1_Load(SCENE1_WORK *scene_wk)
{
	...
	//--コピーライトデータロード
	//キャラ
	ArcUtil_BgCharSet(ARC_TITLE_PL, NARC_titledemo_pl_cp_light_NCGR,
						scene_wk->bgl, COPY_LIGHT_FRAME, 0, 0, FALSE, HEAPID_OP_DEMO);
	//スクリーン
	ArcUtil_ScrnSet(ARC_TITLE_PL, NARC_titledemo_pl_cp_light_NSCR,
						scene_wk->bgl, COPY_LIGHT_FRAME, 0, 0, FALSE, HEAPID_OP_DEMO);
	//パレット
	ArcUtil_PalSet( ARC_TITLE_PL, NARC_titledemo_pl_cp_light_NCLR,
...
static BOOL Scene3_Main(SCENE3_WORK *scene_wk, const int inCounter)
{
	...
	case 0:	//ゆっくりブラックイン
		if (inCounter>=FM_TENGAN_IN){
			WIPE_SYS_Start(	WIPE_PATTERN_WMS, WIPE_TYPE_FADEIN,
								WIPE_TYPE_FADEIN,0x7fff,SCENE3_START_IN_SYNC,1,HEAPID_OP_DEMO );
			(*seq)++;
		}
		break;
	case 1:

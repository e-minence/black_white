...
static void setup_bg( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle );
static void cleanup_bg( ENDING_DEMO_WORK* wk );
static void setup_3d( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle );
...
static void setup_3d_model( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle );
...
static void setup_obj_graphics( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle );
static void setup_obj( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle );
...
PROC_RESULT EndingDemoProc_Init( PROC * proc, int * seq )
{
	...
	case 0:
		...
		p_handle = ArchiveDataHandleOpen( ARC_ENDING, HEAPID_ENDING_DEMO );
		if( wk )
		{
			...
			wk->proc = proc;
			setup_bg( wk, p_handle );
			setup_3d( wk, p_handle );
			setup_obj( wk, p_handle );
...
static void setup_bg( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle )
{
	...
	ArcUtil_HDL_PalSetEx( p_handle, NARC_ending_font_nclr, PALTYPE_MAIN_BG, COMMOM_FONT_PALPOS*0x20,
		COMMOM_FONT_PALPOS*0x20, 0x20, HEAPID_ENDING_DEMO);
	ArcUtil_HDL_PalSetEx( p_handle, NARC_ending_font_nclr, PALTYPE_SUB_BG,  COMMOM_FONT_PALPOS*0x20,
		COMMOM_FONT_PALPOS*0x20, 0x20, HEAPID_ENDING_DEMO);
...
static void setup_3d_model( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle )
{
	...
	grnd->modelLoadPtr = ArcUtil_HDL_LoadEx( p_handle, NARC_ending_end_grand02_nsbmd, FALSE,
	...
	if( grnd->modelLoadPtr )
	{
		...
		grnd->animLoadPtr = ArcUtil_HDL_LoadEx( p_handle, NARC_ending_end_grand01_nsbta, FALSE,
				HEAPID_ENDING_DEMO, ALLOC_TOP, &loadSize );
...
static void setup_obj_graphics( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle )
{
	...
	ArcUtil_HDL_CharSysLoad( p_handle, PlayerGraphicID[idx].ncg, FALSE, CHAR_MAP_1D,
			0, NNS_G2D_VRAM_TYPE_2DMAIN, OBJ_CHARPOS_HERO*0x20, HEAPID_ENDING_DEMO, &wk->heroImgProxy );

	ArcUtil_HDL_PalSysLoad( p_handle, PlayerGraphicID[idx].ncl, NNS_G2D_VRAM_TYPE_2DMAIN, 
...
static void setup_obj( ENDING_DEMO_WORK* wk, ARCHANDLE* p_handle )
{
	setup_obj_graphics( wk, p_handle );

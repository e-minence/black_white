...
static PROC_RESULT	EGG_ProcInit(PROC* proc, int* seq)
{
	...
	emwk->sys.handle = ArchiveDataHandleOpen( ARC_PM_EDIT, HEAPID_EGG_DEMO );
...
static PROC_RESULT	EGG_ProcMain(PROC* proc, int* seq)
{
	...
	case 0:
		...
		}
		{
			...
			PokeEditData_VoiceWaitSet( emwk->sys.handle, &emwk->voice_wait, monsno, CLIENT_TYPE_BB );
...
static PROC_RESULT	EGG_ProcExit(PROC* proc, int* seq)
{
	...
	ArchiveDataHandleClose( emwk->sys.handle );

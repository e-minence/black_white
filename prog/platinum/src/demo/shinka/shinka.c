							...
							int shinka_cond,int shinka_status_flag,int heapID)
{
	...
	sw->form_no=PokeParaGet(pp,ID_PARA_form_no,NULL);
	sw->after_mons_no=after_mons_no;
	sw->heapID=heapID;
	
	sw->handle = ArchiveDataHandleOpen( ARC_PM_EDIT, heapID );
	
	PokeEditData_VoiceWaitSet( sw->handle, &(sw->before_wait), sw->before_mons_no, CLIENT_TYPE_BB );
	PokeEditData_VoiceWaitSet( sw->handle, &(sw->after_wait),  sw->after_mons_no,  CLIENT_TYPE_BB );
...
void	ShinkaEnd(SHINKA_WORK *sw)
{
	...
	ArchiveDataHandleClose( sw->handle );
	...
	case SEQ_SHINKA_BEFORE_MSG_INIT:
		if(PaletteFadeCheck(sw->pfd)==0){
			...
			//	PokeAnmDataSet(&ssa[0],sw->before_mons_no)
				...
				PokeEditData_PrgAnimeSet( sw->handle,
										  sw->pasp,
										  sw->ss[0],
										  sw->before_mons_no,
										  PARA_FRONT,
										  PARA_HFLIP_OFF,
										  0 );
				PokeEditData_AnimeSet( sw->handle, &ssa[0], sw->before_mons_no, CLIENT_TYPE_BB );
				
				SoftSpriteAnimeDataSet(sw->ss[0],&ssa[0]);
				SoftSpriteAnimeSet(sw->ss[0],NULL);
			}
			Snd_PMVoiceWaitPlay(sw->before_mons_no, sw->before_wait, sw->form_no);
		...
		if((PaletteFadeCheck(sw->pfd)==0)&&(ShinkaParticle_Main(sw->shinka_pt)==FALSE)){
			...
			//	PokeAnmDataSet(&ssa[0],sw->after_mons_no);
				...
				PokeEditData_PrgAnimeSet( sw->handle,
										  ...
										  PARA_HFLIP_OFF,
										  0 );
				PokeEditData_AnimeSet( sw->handle, &ssa[0], sw->after_mons_no, CLIENT_TYPE_BB );

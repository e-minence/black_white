...
typedef struct {
	...
	u32							pokeMonsFormNumber[ TEMOTI_POKEMAX ];
...
static void setup_poke_actors( DEMO_WORK* wk, NNSG2dCellDataBank* cell, NNSG2dAnimBankData* anim, ARCHANDLE* p_handle );
...
PROC_RESULT  DemoDendouPROC_Init( PROC* proc, int* seq )
{
	...
	wk->pead_handle = ArchiveDataHandleOpen( ARC_PM_EDIT, HEAPID_DENDOU_DEMO );
	...
	case 0:
		{
			...
			ArchiveDataHandleClose( wk->pead_handle );
...
static void setup_bg( DEMO_WORK* wk )
{
	...
	p_handle = ArchiveDataHandleOpen( ARC_DENDOU_DEMO, HEAPID_DENDOU_DEMO );

	ArcUtil_HDL_BgCharSet(p_handle, NARC_dendou_demo_bg_poke_lz_ncgr, wk->bgl, GF_BGL_FRAME3_M, 0, 0, TRUE, HEAPID_DENDOU_DEMO );
	ArcUtil_HDL_ScrnSet(p_handle, NARC_dendou_demo_bg_poke_lz_nscr, wk->bgl, GF_BGL_FRAME3_M, 0, 0, TRUE, HEAPID_DENDOU_DEMO );
	ArcUtil_HDL_PalSet(p_handle, NARC_dendou_demo_bg_nclr, PALTYPE_MAIN_BG, 0, 0x60, HEAPID_DENDOU_DEMO );

	ArcUtil_HDL_ScrnSet(p_handle, NARC_dendou_demo_bg_mask_lz_nscr, wk->bgl, GF_BGL_FRAME2_M, 0, 0, TRUE, HEAPID_DENDOU_DEMO );
...
static void setup_actors( DEMO_WORK* wk )
{
	...
	p_handle = ArchiveDataHandleOpen( ARC_BATT_OBJ, HEAPID_DENDOU_DEMO );
	...
	wk->cellLoadPtr = ArcUtil_HDL_CellBankDataGet(p_handle, POKE_OAM_NCER, FALSE, &cell, HEAPID_DENDOU_DEMO);
	wk->animLoadPtr = ArcUtil_HDL_AnimBankDataGet(p_handle, POKE_OAM_NANR, FALSE, &anim, HEAPID_DENDOU_DEMO);

	setup_poke_actors( wk, cell, anim, p_handle );
...
static void setup_poke_actors( DEMO_WORK* wk, NNSG2dCellDataBank* cell, NNSG2dAnimBankData* anim, ARCHANDLE* p_handle )
{
	...
	cgxLoadPtr = ArcUtil_HDL_CharDataGet( p_handle, POKE_OAM_NCGR, FALSE, &charData, HEAPID_DENDOU_DEMO );
	palLoadPtr = ArcUtil_HDL_PalDataGet( p_handle, POKE_OAM_NCLR, &palData, HEAPID_DENDOU_DEMO );
	...
	for(i=0; i<wk->pokeCount; i++)
	{
		...
		wk->pokeMonsFormNumber[i] = PokeParaGet((POKEMON_PARAM*)pp, ID_PARA_form_no, NULL);
...
//		ChangesInto_OAM_from_PokeTex_RC( arc.arc_no, arc.index_chr, HEAPID_DENDOU_DEMO, &rect[1], wk->pokeCgxBuf );
	...
	//	PokeAnmDataSet( &wk->pokeAnim[i][0], wk->pokeMonsNumber[i] );
		...
		PokeEditData_AnimeSet( wk->pead_handle, &wk->pokeAnim[i][0], wk->pokeMonsNumber[i], CLIENT_TYPE_BB );
...
#else
	TrCLACTGraDataGet( ((MyStatus_GetMySex(wk->param->player)==PM_FEMALE)? TRTYPE_GIRL : TRTYPE_BOY),
					   PARA_FRONT, &tcg);
	ChangesInto_OAM_from_PokeTex_RC( tcg.arcID, tcg.ncbrID, HEAPID_DENDOU_DEMO, &rect[0], wk->pokeCgxBuf );
	DC_FlushRange( wk->pokeCgxBuf, POKE_ACT_CGX_BYTESIZE );
	GX_LoadOBJ( wk->pokeCgxBuf, PLAYER_OBJ_CHARPOS, POKE_ACT_CGX_BYTESIZE );
	ArcUtil_PalSet( tcg.arcID, tcg.nclrID, PALTYPE_MAIN_OBJ, PLAYER_OBJ_PALPOS, 0x20, HEAPID_DENDOU_DEMO );
#endif
...
static void StartPokeAnimTask( DEMO_WORK* wk, int pos, BOOL voicePlay, int taskID )
{
	...
	twk->formno = wk->pokeMonsFormNumber[ pos ];
...
//		Snd_PMVoicePlay( wk->monsno );
		Snd_PMVoicePlay( wk->monsno, wk->formno );
		wk->voicePlay = FALSE;
	}

...

			{
				...
				if( PokePasoParaGet((POKEMON_PASO_PARAM*)ppp, ID_PARA_tamago_flag, NULL) == FALSE)
				{
					...
					PokeEditData_VoiceWaitSet( wk->handle, &wait, 
											   DemoTrade_GetSendPokeNumber(wk->mainWork), CLIENT_TYPE_BB );
					
//					Snd_PMVoiceWaitPlay( DemoTrade_GetSendPokeNumber(wk->mainWork), wait );
					Snd_PMVoiceWaitPlay( DemoTrade_GetSendPokeNumber(wk->mainWork), wait, DemoTrade_GetSendPokeFormNumber(wk->mainWork) );
...
//	PokeAnmDataSet( wk->spriteAnim, DemoTrade_GetSendPokeNumber(wk->mainWork) );
	...
	PokeEditData_AnimeSet( wk->handle, wk->spriteAnim, DemoTrade_GetSendPokeNumber(wk->mainWork), CLIENT_TYPE_BB );

...
void* DemoTrade_Scene6Init( DEMO_TRADE_WORK* mainWork )
{
	...
	if( wk )
	{
		...
		wk->handle  = ArchiveDataHandleOpen( ARC_PM_EDIT, HEAPID_DEMO_TRADE_SUB );
	...
	case 3:
		if( wait_3dframe_flush_task( wk ) )
		{
			...
			if( PokePasoParaGet((POKEMON_PASO_PARAM*)ppp, ID_PARA_tamago_flag, NULL) == FALSE)
			{
				...
				PokeEditData_VoiceWaitSet( wk->handle, &wait, 
										   DemoTrade_GetRecvPokeNumber(wk->mainWork), CLIENT_TYPE_BB );
				
//				Snd_PMVoiceWaitPlay( DemoTrade_GetRecvPokeNumber(wk->mainWork), wait, 0 );
				Snd_PMVoiceWaitPlay( DemoTrade_GetRecvPokeNumber(wk->mainWork), wait, DemoTrade_GetRecvPokeFormNumber(wk->mainWork) );
...
//	PokeAnmDataSet( wk->spriteAnim, DemoTrade_GetRecvPokeNumber(wk->mainWork) );
	...
	PokeGraArcDataGetPPP( &arc, ppp, PARA_FRONT, FALSE );
//	PokeAnmDataSet( wk->spriteAnim, DemoTrade_GetRecvPokeNumber(wk->mainWork) );	
	PokeEditData_AnimeSet( wk->handle, wk->spriteAnim, DemoTrade_GetRecvPokeNumber(wk->mainWork), CLIENT_TYPE_BB );

	ypos = (SPOTLIGHT_DISP_Y - 20) + PokePasoParaHeightGet(ppp, PARA_FRONT, FALSE);

...
static void DanceDefaultOBJSet(DANCE_PROC_WORK *dpw, ARCHANDLE *hdl_obj);
static void DanceDefaultOBJDel(DANCE_PROC_WORK *dpw);
static void DanceDefaultOBJSet_Sub(DANCE_PROC_WORK *dpw, ARCHANDLE *hdl_obj);
static void DanceDefaultOBJDel_Sub(DANCE_PROC_WORK *dpw);
static void DanceDefaultBGSet(DANCE_PROC_WORK *dpw, ARCHANDLE *hdl_bg);
static void DanceDefaultBGDel(DANCE_PROC_WORK *dpw);
static void DanceDefaultBGSet_Sub(DANCE_PROC_WORK *dpw, ARCHANDLE *hdl_bg);
...
//	SetDanceWorkParameter(dpw, (NAMEIN_PARAM*)PROC_GetParentWork(proc));
...

	{
		...
		//ハンドルオープン
		hdl_bg  = ArchiveDataHandleOpen(ARC_CONTEST_BG,  HEAPID_DANCE); 
		hdl_obj = ArchiveDataHandleOpen(ARC_CONTEST_OBJ, HEAPID_DANCE);

		//常駐BGセット
		DanceDefaultBGSet(dpw, hdl_bg);
		DanceDefaultBGSet_Sub(dpw, hdl_bg);

		//BMPウィンドウ追加
		DanceSys_DefaultBmpWinAdd(dpw);

		//常駐OBJセット
		DanceDefaultOBJSet(dpw, hdl_obj);
		DanceDefaultOBJSet_Sub(dpw, hdl_obj);
...
{
	//常駐OBJパレットロード
	CATS_LoadResourcePlttWorkArcH(dpw->sys.pfd, FADE_MAIN_OBJ, dpw->sys.csp, dpw->sys.crp, 
		hdl_obj, CONTEST_DANCE_OBJ_NCLR, 0, 
		DANCE_COMMON_PAL_NUM, NNS_G2D_VRAM_TYPE_2DMAIN, D_PLTTID_OBJ_COMMON);
	
	DT_HeartResourceLoad(dpw->sys.csp, dpw->sys.crp);
	
	DT_OnpuResourceLoad(dpw->sys.csp, dpw->sys.crp, hdl_obj);
	
	DT_ReviewPanelResourceLoad(dpw->sys.csp, dpw->sys.crp, hdl_obj);
	DT_ReviewPanelActorSetAll(&dpw->sys, dpw->sys.csp, dpw->sys.crp);
	DT_ReviewFontOamCreateAll(&dpw->sys);

	DT_MyCursorResourceLoad(dpw->sys.csp, dpw->sys.crp, hdl_obj);
	DT_MyCursorActorSet(&dpw->sys, dpw->sys.csp, dpw->sys.crp);

	DT_PrinResourceLoad(dpw->sys.csp, dpw->sys.crp, hdl_obj);
	DT_PrinCreate(dpw, dpw->sys.csp, dpw->sys.crp);

	DT_ShadowResourceLoad(dpw->sys.csp, dpw->sys.crp, hdl_obj);
	DT_ShadowActorSetAll(&dpw->sys, dpw->sys.csp, dpw->sys.crp);

	DT_SpotLightResourceLoad(dpw->sys.csp, dpw->sys.crp, hdl_obj);
	DT_SpotLightActorSetAll(&dpw->sys, dpw->sys.csp, dpw->sys.crp);
	
	DT_StarResourceLoad(dpw->sys.csp, dpw->sys.crp, hdl_obj);
	DT_MusicLineResourceLoad(dpw->sys.csp, dpw->sys.crp, hdl_obj);

	DT_MusicLineActorSet(dpw);
}
...
static void DanceDefaultOBJDel(DANCE_PROC_WORK *dpw)
{
	...
	DT_MusicLineResourceFree(dpw->sys.crp);

	DT_MusicLineActorDelete(dpw);
...
static void DanceDefaultBGSet(DANCE_PROC_WORK *dpw, ARCHANDLE *hdl_bg)
{
	...
	//キャラ
	ArcUtil_HDL_BgCharSet(hdl_bg, CON_DANCE_BG_NCGR_BIN, dpw->sys.bgl, 
		DANCE_FRAME_BACKGROUND, 0, 0, 1, HEAPID_DANCE);
	
	//スクリーン
	{
		int rhythm_no = DanceDataNo[dpw->consys->c_game.rank][dpw->consys->c_game.type].rhythm_no;
		int scrn_index = RhythmDataGet(rhythm_no, RHYTHM_IDX_BG_SCRN_INDEX);
		ArcUtil_HDL_ScrnSet(hdl_bg, scrn_index, dpw->sys.bgl, 
			DANCE_FRAME_BACKGROUND, 0, 0, 1, HEAPID_DANCE);
	}
...
static int DanceSeq_MusicSelect(DANCE_PROC_WORK *dpw, DANCE_LOCAL_WORK *local)
{
	...
	case 0:
		if(dpw->consys->sio_flag == TRUE){
			...
			for(i = 0; i < dpw->consys->c_game.player_num; i++){
				my_status = CommInfoGetMyStatus(i);
				rom_code = MyStatus_GetRomCode(my_status);
				if(rom_code == POKEMON_DP_ROM_CODE){
					dp_sio_flag = TRUE;
					break;
				}
			}
		...
		if(dp_sio_flag == TRUE){	//DPが混ざった通信対戦
			data_no = DANCE_DATA_NO_DP_SIO;
			rhythm_no = RHYTHM_NO_4;
			OS_TPrintf("接続ユーザー：DPが混ざっている\n");
		}
		else{
			data_no = DanceDataNo[dpw->consys->c_game.rank][dpw->consys->c_game.type].musdata_no;
			rhythm_no = DanceDataNo[dpw->consys->c_game.rank][dpw->consys->c_game.type].rhythm_no;
			OS_TPrintf("接続ユーザー：プラチナのみ\n");
		}
		...
		all_frame = dpw->d_game.one_tempo_frame * RhythmDataGet(rhythm_no, RHYTHM_IDX_TEMPO_BASE_ONPU);
		//四捨五入して整数化
		...
		dpw->d_game.rhythm_no = rhythm_no;
 ...
 * @param   index			データ取得index
 *
 * @retval  値
 */
//--------------------------------------------------------------
int RhythmDataGet(int rhythm_no, int index)
{
	switch(index){
	case RHYTHM_IDX_BAR_LEN:
		return RhythmData[rhythm_no].bar_num * MUSIC_BAR_ONE_LEN;
	case RHYTHM_IDX_BAR_NUM:
		return RhythmData[rhythm_no].bar_num;
	case RHYTHM_IDX_TEMPO_BASE_ONPU:
		return RhythmData[rhythm_no].tempo_base_onpu;
	case RHYTHM_IDX_BEAT:
		return RhythmData[rhythm_no].tempo_base_onpu * 2;
	case RHYTHM_IDX_BG_SCRN_INDEX:
		return RhythmData[rhythm_no].bg_scrn_index;
	}

...
static void DOTCB_Dancing(TCB_PTR tcb, void *work)
{
	...
	case 0:
		...
		PrinReq_InMove(md->dpw->sys.prin_sys, md->dpw->d_game.rhythm_no);
		DT_MusicLineEnableSet(&md->dpw->sys.musline, CATS_ENABLE_TRUE);
...
//		if(Snd_BgmPlayCheck(SEQ_CO_JUNBI) == FALSE){
	...
	case 2:
		{//曲進行バー更新
			DT_MusicBarAnimeUpdateFrame(&md->dpw->sys, 
				...
				md->dpw->d_game.rhythm_no);
			PrinReq_FrameUpdate(md->dpw->sys.prin_sys, 
				md->dpw->music_vcount - md->dpw->music_vcount_start, md->dap.all_tempo_frame,
				md->dpw->d_game.rhythm_no);
			DT_MusicLineUpdateFrame(&md->dpw->sys.musline, 
				md->dpw->music_vcount - md->dpw->music_vcount_start, md->dap.all_tempo_frame,
				md->dpw->d_game.rhythm_no);
		}
			...
			if(dancer_type == DANCER_MAIN){
				...
				if(tp_ret != RECT_HIT_NONE){
					DancerReactionSet(md->dpw->consys->c_game.my_breeder_no, tp_ret, frame, 
						...
						md->touch_count, md->dap.musicdata.touch_count, md->dpw->d_game.rhythm_no);
	...
	case 3:
		if(DanceTouchCheck(&md->dtw, &tp_ret, TRUE, FALSE) == TRUE){
			...
			DT_MusicLineEnableSet(&md->dpw->sys.musline, CATS_ENABLE_FALSE);
			DT_FumenColorSet(&md->dpw->sys, FUMENCOLOR_NULL);
			DT_BGAlphaEffectSet(md->dpw, DANCE_BGALPHA_TYPE_NULL);
...
void DancerReactionSet(int breeder_no, int reaction, u32 frame, u32 one_tempo_frame, 
	...
	int rotation_pos, int touch_count, int max_touch, int rhythm_no)
{
	int dance_step, i;
	...
	if(maindancer_record != NULL){
		check_beat = hit_beat - RhythmDataGet(rhythm_no, RHYTHM_IDX_BEAT) / 2;
...
//		rtb->z = SoftSpriteParaGet(rtb->ss, SS_PARA_POS_Z) * 0x100;
	...
	case 1:
		...
		else if(rtb->x < DT_RotationPosGetX(rtb->next_rotation_pos) * 0x100){
			//プラチナでローテーション開始前の常駐アニメ終了待ちで、拡縮の終了だけ待って、
			//座標移動での待ちはしないようにしたので(タイミングによって移動終了待ちで次のダンス
			//が始まるまで待つ場合がある)、ここでそのオフセット分ローテーション位置を通り過ぎない
			//ように補正を入れている 2007.09.26(水) matsuda

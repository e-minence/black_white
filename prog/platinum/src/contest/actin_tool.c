...
static CATS_ACT_PTR AT_JudgeAdd(PALETTE_FADE_PTR pfd, CATS_SYS_PTR csp, CATS_RES_PTR crp, 
	const JUDGE_DATA *judge, int judge_no, ARCHANDLE *hdl_obj)
{
...
void AT_JudgeAddAll(ACTIN_SYSTEM_PARAM *sys, CONTEST_SYSTEM *consys, ARCHANDLE *hdl_obj)
{
	...
	for(i = 0; i < JUDGE_MAX; i++){
		...
		sys->judge_cap[i] = 
			AT_JudgeAdd(sys->pfd, sys->csp, sys->crp, &consys->c_game.jd[i], i, hdl_obj);
	}
}
 ...
 */
//--------------------------------------------------------------
void AT_JudgeReactionIconResourceLoad(PALETTE_FADE_PTR pfd, CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE *hdl_obj)
{
	u32 char_id, cell_id, anm_id;
...
void AT_SpecialHeartSet(ACTIN_SYSTEM_PARAM *sys, int judge_no, ARCHANDLE *hdl_obj)
{
	...
	CATS_LoadResourceCellArcH(sys->csp, sys->crp, hdl_obj, 
		HEART16_NCER_BIN, 1, 
...
void AT_NextIconResourceLoad(CATS_SYS_PTR csp, CATS_RES_PTR crp, ARCHANDLE *hdl_obj)
{
	...
	CATS_LoadResourceCellArcH(csp, crp, hdl_obj, NEXT_ICON_NCER_BIN, 1, CELLID_NEXT_ICON);
	CATS_LoadResourceCellAnmArcH(csp, crp, hdl_obj, NEXT_ICON_NANR_BIN, 

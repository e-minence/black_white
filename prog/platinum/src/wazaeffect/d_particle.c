...
//--------------------------------------------------------------
//	d_particle.cの機能を使用する場合はD_PARTICLE_ONのdefine定義を有効にする必要がある
//--------------------------------------------------------------
#ifdef PM_DEBUG
//	#define D_PARTICLE_ON		//d_particle.cの機能を使用する場合はこの定義を有効にする
	//グローバル関数の中身だけD_PARTICLE_ONで囲っています。
	//ローカル関数はファイル内で使用しているソースがなければ最適化で自動的に削除される為
	//static関数は囲っていません。
#endif

...
//	Debug_CB_Set(bes->bcp.pp);
	...
	if (bes->cb_id == 0){		///< カスタムボールIDが指定されてない
		...
		OS_Printf( " id = %2d, x = %3d, y = %3d \n", bes->core.cb_seal[0].seal_id, bes->core.cb_seal[0].x, bes->core.cb_seal[0].y );
	}
}
...
static void BMT_ClactInit(BMS_PTR bms)
{
	...
	///< 読み込み
	{
		...
		hdl = ArchiveDataHandleOpen( ARC_BATT_OBJ, bms->bmd.heap_id );
		
		CATS_LoadResourceCharArcH(bms->bmd.csp, bms->crp,
								 hdl, ncgr, 1, NNS_G2D_VRAM_TYPE_2DMAIN, bms->bmd.id + BALL_ARC_ID_BASE);

		CATS_LoadResourcePlttWorkArcH(bms->bmd.pfd, FADE_MAIN_OBJ,
									 bms->bmd.csp, bms->crp,
								 	 hdl, nclr, 0, NNS_G2D_VRAM_TYPE_2DMAIN, 1, bms->bmd.id + BALL_ARC_ID_BASE);

		CATS_LoadResourceCellArcH(bms->bmd.csp, bms->crp, hdl, ncer, 1, bms->bmd.id + BALL_ARC_ID_BASE);

		CATS_LoadResourceCellAnmArcH(bms->bmd.csp, bms->crp, hdl, nanr, 1, bms->bmd.id + BALL_ARC_ID_BASE);

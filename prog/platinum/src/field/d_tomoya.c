...
#include "application/wifi_lobby/lobby_news.h"
#include "application/wifi_lobby/lobby_news_data.h"
#include "application/wifi_lobby/wflby_def.h"

#include "application/wifi_lobby/wflby_room.h"
...
static void StartSprite_Start( DT_SPRITE_CHECK* data );
static void StartSprite_End( DT_SPRITE_CHECK* data );
static void StartSpriteContest_Start( DT_SPRITE_CHECK* data );
static void StartSpriteContest_End( DT_SPRITE_CHECK* data );
...
static void StartZukan_Check_End( DT_SPRITE_CHECK* data );
static void StartEvPokeSelect_Check_Start( DT_SPRITE_CHECK* data );
static void StartEvPokeSelect_Check_End( DT_SPRITE_CHECK* data );
static void StartWorldTimer_Check_Start( DT_SPRITE_CHECK* data );
static void StartWorldTimer_Check_End( DT_SPRITE_CHECK* data );

static void StartWFLBYROOM_Check( TCB_PTR tcb, void* work );
static void StartWFLBYROOM0_Check( TCB_PTR tcb, void* work );
static void StartWFLBYROOM1_Check( TCB_PTR tcb, void* work );
static void StartWFLBYROOM2_Check( TCB_PTR tcb, void* work );
static void StartWFLBYROOM_EVSPEED_Check( TCB_PTR tcb, void* work );
static void StartWFLBYROOM_ALLVIP_Check( TCB_PTR tcb, void* work );
static void StartWFLBYROOM_Check_Start( DT_SPRITE_CHECK* data );
static void StartWFLBYROOM_Check_End( DT_SPRITE_CHECK* data );
...
static void StartWeather18( TCB_PTR tcb,void* work );	// 凝れ美
static void StartWeather19( TCB_PTR tcb,void* work );	// 洞窟
static void StartWeather20( TCB_PTR tcb,void* work );	// 洞窟
static void StartWeather21( TCB_PTR tcb,void* work );	// ギンガ００
static void StartWeather22( TCB_PTR tcb,void* work );	// LostTower００
static void StartWeather23( TCB_PTR tcb,void* work );	// LostTower００
static void StartWeather24( TCB_PTR tcb,void* work );	// LostTower００
...
#else
	#define MAP_TYPE_LIST_COUNT_MAIN	(18)
#endif	// D_TOMOYA_MATERIAL_ON

#define MAP_TYPE_LIST_COUNT_NO_SUB	(15)		// サブメニューがないシステム
...
static const BMPLIST_DATA MainDebugList[] = {
	...
	{(void*)Msg01_29,(u32)StartWFLBYROOM0_Check},
	{(void*)Msg01_30,(u32)StartWFLBYROOM1_Check},
	{(void*)Msg01_31,(u32)StartWFLBYROOM2_Check},
	{(void*)Msg01_33,(u32)StartWFLBYROOM_ALLVIP_Check},
...
//	{(void*)Msg01_26,(u32)StartWorldTimer_Check},
...
#ifdef D_TOMOYA_MATERIAL_ON
	{(void*)Msg01_04,(u32)StartApplication},	// マテリアル
#endif	// D_TOMOYA_MATERIAL_ON
	{(void*)Msg01_05,(u32)StartApplication},	// フォグ
	{(void*)Msg01_07,(u32)StartApplication},	// てんき
//	{(void*)Msg01_10,(u32)StartApplication},	// エンカウント
...
static const BMPLIST_DATA SubWeatherDebugList[] = {	// 天気
	...
	{(void*)Msg07_20,(u32)StartWeather18},	// 凝れ美
	{(void*)Msg07_21,(u32)StartWeather19},	// 洞窟
	{(void*)Msg07_21,(u32)StartWeather20},	// 洞窟
	{(void*)Msg07_22,(u32)StartWeather21},	// ギンガ
	{(void*)Msg07_23,(u32)StartWeather22},	// LostTower
	{(void*)Msg07_24,(u32)StartWeather23},	// ハードマウンテン
	{(void*)Msg07_25,(u32)StartWeather24},	// 洞窟02
...
static void StartWorldTimer_Check( TCB_PTR tcb, void* work )
{
	...
	memset( p_data, 0, sizeof(WFLBY_WLDTIMER) );
	{
		...
		for( i=0; i<20; i++ ){
			...
			case 0:
				...
				p_data->data[i].outside = TRUE;
				break;
			case 1:
				p_data->data[i].nation = 219;
				p_data->data[i].area = 1;
				p_data->data[i].outside = FALSE;
				break;
			case 2:
				p_data->data[i].nation = 71;
				p_data->data[i].area = 8;
				p_data->data[i].outside = FALSE;
				break;
			...
			case 4:
				p_data->data[i].nation = 43;
				p_data->data[i].area = 17;
				p_data->data[i].outside = FALSE;
				break;
			case 5:
				p_data->data[i].nation = 43;
				p_data->data[i].area = 11;
				p_data->data[i].outside = FALSE;
				break;
			case 6:
				p_data->data[i].nation = 103;
				p_data->data[i].area = 18;
				p_data->data[i].outside = FALSE;
				break;
			case 7:
				p_data->data[i].nation = 103;
				p_data->data[i].area = 13;
				p_data->data[i].outside = FALSE;
				break;
			...
			case 8:
				...
				break;
			case 9:
				p_data->data[i].nation = 4;
				p_data->data[i].area = 0;
				p_data->data[i].outside = FALSE;
				break;
			case 10:
				p_data->data[i].nation = 208;
				p_data->data[i].area = 0;
				p_data->data[i].outside = FALSE;
				break;
			case 11:
				p_data->data[i].nation = 209;
				p_data->data[i].area = 0;
				p_data->data[i].outside = FALSE;
				break;
			...
			case 13:
				p_data->data[i].nation = 220;
				p_data->data[i].area = 47;
				p_data->data[i].outside = FALSE;
				break;
			case 14:
				p_data->data[i].nation = 36;
				p_data->data[i].area = 2;
				p_data->data[i].outside = FALSE;
				break;
			case 15:
				p_data->data[i].nation = 220;
				p_data->data[i].area = 28;
				p_data->data[i].outside = FALSE;
				break;
			...
			case 16:
				...
				break;
			case 17:
				p_data->data[i].nation = 98;
				p_data->data[i].area = 0;
				p_data->data[i].outside = FALSE;
				break;
			case 18:
				p_data->data[i].nation = 219;
				p_data->data[i].area = 6;
				p_data->data[i].outside = FALSE;
				break;
			case 19:
				p_data->data[i].nation = 41;
				p_data->data[i].area = 0;
				p_data->data[i].outside = FALSE;
				break;
			}
	...
	p_pearent->cp_data = p_data;
	p_pearent->p_save = D_TomoyaFldSys->savedata;
	p_pearent->worldtime.hour = 0;
	p_pearent->worldtime.minute = 0;
	p_pearent->worldtime.second = 0;
	DT_sprite->p_pearent = p_pearent;
	DT_sprite->p_fsys = D_TomoyaFldSys;
...
static void StartWorldTimer_Check_Start( DT_SPRITE_CHECK* data )
{
	...
	GameSystem_StartSubProc(data->p_fsys,  &Proc, data->p_pearent );
}
static void StartWorldTimer_Check_End( DT_SPRITE_CHECK* data )
{
	WLDTIMER_PARAM* p_pearent;
	...
	sys_FreeMemoryEz( (WFLBY_WLDTIMER*)p_pearent->cp_data );
	sys_FreeMemoryEz( p_pearent );
}
...
static void StartWFLBYROOM0_Check( TCB_PTR tcb, void* work )
{
	D_Tomoya_WiFiLobby_ChannelPrefix	= PPW_LOBBY_CHANNEL_PREFIX_DEBUG6;
	D_Tomoya_WiFiLobby_ChannelPrefixFlag= TRUE;
	StartWFLBYROOM_Check( tcb, work );
}
static void StartWFLBYROOM1_Check( TCB_PTR tcb, void* work )
{
	D_Tomoya_WiFiLobby_ChannelPrefix	= PPW_LOBBY_CHANNEL_PREFIX_DEBUG7;
	D_Tomoya_WiFiLobby_ChannelPrefixFlag= TRUE;
	StartWFLBYROOM_Check( tcb, work );
}
static void StartWFLBYROOM2_Check( TCB_PTR tcb, void* work )
{
	D_Tomoya_WiFiLobby_ChannelPrefix	= PPW_LOBBY_CHANNEL_PREFIX_DEBUG8;
	D_Tomoya_WiFiLobby_ChannelPrefixFlag= TRUE;
	StartWFLBYROOM_Check( tcb, work );
}
static void StartWFLBYROOM_EVSPEED_Check( TCB_PTR tcb, void* work )
...
static void StartWFLBYROOM_ALLVIP_Check( TCB_PTR tcb, void* work )
{
	D_Tomoya_WiFiLobby_ALLVip = TRUE;
	StartWFLBYROOM_Check( tcb, work );
}
...
static void StartWFLBYROOM_Check_End( DT_SPRITE_CHECK* data )
{
	D_Tomoya_WiFiLobby_DebugStart		= FALSE;
	D_Tomoya_WiFiLobby_ChannelPrefixFlag= DWC_LOBBY_CHANNEL_PREFIX_DEBUG;
	D_Tomoya_WiFiLobby_ChannelPrefix	= DWC_LOBBY_CHANNEL_PREFIX_DEBUG;
	D_Tomoya_WiFiLobby_ALLVip = FALSE;
	sys_FreeMemoryEz( data->p_pearent );
}
...
static void StartSprite_CheckTelevision_View( TCB_PTR tcb,void* work )
{
	...
	DT_sprite->pStartFunc = StartSpriteView_Start;
	DT_sprite->pEndFunc = StartSpriteView_End;
...
// LostTower
static void StartWeather22( TCB_PTR tcb,void* work )
{
...
//		このOBJIDがなくなったのでコメントアウトしました。(08.03.27)

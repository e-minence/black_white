...
typedef struct{
	...
	PTC_PTR			p_ptc;		// パーティクル
	void*			p_ptc_work;	// パーティクルに必要なワーク
	u32				tex_addr;	// テクスチャアドレス管理
	u32				plt_addr;	// パレットアドレス管理
} ENC_GPRINT_WK;

static ENC_GPRINT_WK* pENC_GPRINT_WK = NULL;

#define ENC_SET_GPRINT_VBLANK_TCB_PRI	( 1024 )
static void ENC_GPrint_VBlank( TCB_PTR tcb, void* p_work );
static void ENC_GPrint_TCB( TCB_PTR tcb, void* p_work );
...
static u32 ENC_GPrint_PTC_PltAlloc( u32 size, BOOL comp4pltt );
 ...
 */
//-----------------------------------------------------------------------------
void ENC_BG_SetNormalBG( ARCHANDLE* p_handle, u32 scrn_idx, u32 char_idx, u32 pltt_idx, u32 pltt_no, u32 pltt_num, GF_BGL_INI* p_bgl, u32 frame )
{
	void* buff;
...
static void ENC_BG_WndZiguzagu_HBlank(FIELD_HBLANK_OBJ* p_hobj, void* p_work)
{
	...
	if( v_count < 192 ){
		...
		G2_SetWnd0Position( set_x, 0, 255, 192 );
	}
}
...
//----------------------------------------------------------------------------
/**
 *	@brief	グラフィックプリント	新描画環境設定システム	初期化
 */
//-----------------------------------------------------------------------------
void ENC_GPrint_Init( FIELDSYS_WORK* p_fsys )
{
	GF_ASSERT( pENC_GPRINT_WK == NULL );

	pENC_GPRINT_WK = sys_AllocMemory( HEAPID_FIELD, sizeof(ENC_GPRINT_WK) );
	memset( pENC_GPRINT_WK, 0, sizeof(ENC_GPRINT_WK) );
	
	pENC_GPRINT_WK->seq		= 0;
	...
	GF_Disp_GX_VisibleControl(GX_PLANEMASK_BG3,VISIBLE_OFF);
	GX_ResetBankForBG();	// バンク設定を解除する
	
	// 次のフレームをキャプチャしてもらう
	MI_CpuClearFast( (void*)HW_LCDC_VRAM_C, HW_VRAM_C_SIZE );
	GX_SetCapture(
			GX_CAPTURE_SIZE_256x192,			// キャプチャサイズ
			GX_CAPTURE_MODE_AB,					// キャプチャモード
			GX_CAPTURE_SRCA_2D3D,				// キャプチャブレンドA
...
//----------------------------------------------------------------------------
/**
 *	@brief	３Dの表示から２D表示に切り替える
 */
//-----------------------------------------------------------------------------
void ENC_GPrint_SwitchOn(void)
{
	GF_ASSERT( pENC_GPRINT_WK );
	pENC_GPRINT_WK->draw_start = TRUE;
}
...
void ENC_GPrint_Exit(void)
{
	...
	sys_FreeMemoryEz( pENC_GPRINT_WK );
	pENC_GPRINT_WK = NULL;
}
...
void ENC_GPrint_PTC_Load(ARCHANDLE* p_handle, u32 idx)
{
	...
	pENC_GPRINT_WK->p_ptc = Particle_SystemCreate( 
			...
			pENC_GPRINT_WK->p_ptc_work, PARTICLE_LIB_HEAP_SIZE, 
			TRUE, HEAPID_FIELD );
 ...
 *	@brief	パーティクル	開始
 *	
 *	@param	emitter_num		エミッタ数
 */
//-----------------------------------------------------------------------------
void ENC_GPrint_PTC_Start(u32 emitter_num)
{
	int i;
	VecFx32 init_pos = {0,0,0};
...
void ENC_GPrint_PTC_Delete(void)
{
	...
	Particle_SystemExit( pENC_GPRINT_WK->p_ptc );
	sys_FreeMemoryEz( pENC_GPRINT_WK->p_ptc_work );
	pENC_GPRINT_WK->p_ptc		= NULL;
	pENC_GPRINT_WK->p_ptc_work	= NULL;
}
...
static void ENC_GPrint_SetUpBg( FIELDSYS_WORK* p_fsys )
{
	...
	// BGL再構築
	{
		...
		{	
			...
			GF_BGL_PrioritySet( GF_BGL_FRAME3_M, 3 );	
		...
		// エフェクト面を設定
		{
			GF_BGL_BGCNT_HEADER TextBgCntDat = {
				0, 0, 0x800, 0, GF_BGL_SCRSIZ_256x256, GX_BG_COLORMODE_16,
				GX_BG_SCRBASE_0xe800, GX_BG_CHARBASE_0x00000, GX_BG_EXTPLTT_23,
				0, 0, 0, FALSE
			};
			GF_BGL_BGControlSet( p_fsys->bgl, GF_BGL_FRAME2_M, &TextBgCntDat, GF_BGL_MODE_TEXT );
			GF_BGL_ClearCharSet( GF_BGL_FRAME2_M, 32, 0, HEAPID_FIELD );
			GF_BGL_ScrClear( p_fsys->bgl, GF_BGL_FRAME2_M );
		}
	}

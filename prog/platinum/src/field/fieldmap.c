...
//	定数定義
//============================================================================================
#define TEST_OBJCHAR_32K	//定義でOBJ1024キャラ化 kaga
//#define TEST_BGMODE_1		//定義でBGモード1へ
...
///fieldmap_subの下に配置するオーバーレイ
enum{
	FIELDMAP_SUBAPP_OVERLAY_GYM,		///<fieldmap_sub_gym をオーバーレイ
	FIELDMAP_SUBAPP_OVERLAY_FIELD,		///<fieldmap_sub_field をオーバーレイ
	FIELDMAP_SUBAPP_OVERLAY_HAKAI,		///<fieldmap_sub_hakai をオーバーレイ
};
...
//--------------------------------------------------------------
/**
 * @brief   fieldmap_subの下に付けるオーバーレイを判定
 *
 * @param   fsys		
 *
...
static BOOL FieldMap_ZoneChange(FIELDSYS_WORK * fsys)
{
	...
	//地名表示リクエスト
	{
		...
		area_win_id	 = ZoneData_GetAreaWindowID(new_zone_id);		///< area_win_id を取得して window を設定
		if (place_id_old != place_id_new){
			
			if ( area_win_id != 0 ){
				 area_win_id--;
			}			
			PlaceNameRequestEx(fsys->fldmap->place_name_cont, place_id_new, area_win_id);//地名表示
		}
	}
...
//------------------------------------------------------------------
/**
 * @brief	フィールド歩きでのゾーン切替　破れた世界専用
 * @param	fsys	フィールド制御ワークへのポインタ
 * @param	new_zone_id	新規のゾーンID
 * @return	nothing
 */
//------------------------------------------------------------------
...
static void FieldMap_Update(FIELDSYS_WORK * fsys, u8 flag)
{
	...
	if( SET_FUNC_CHECK( flag, FLD_SET_FIELD_3D_ANIME ) != 0 ){	//フィールド3Ｄアニメ
...
//----------------------------------------------------------------------------
/**
 *	@brief	3D表示の更新を行わなくするかを設定する
 *
 *	@param	fsys	フィールドシステム
 *	@param	flag	フラグ		TRUE：更新する	FALSE：更新しない
 */
//-----------------------------------------------------------------------------
...
static void FieldMapLoadSeqFeMoveObj(FIELDSYS_WORK * repw)
{
	...
	//フィールドOBJ、フィールドエフェクト使用レンダOBJ処理初期化
	...
	
	{
		...
		Player_DrawInit( repw->player, gimmick_id );	//自機描画初期化
	}

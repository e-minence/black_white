...
static u32 loadLightData( u32 lightidx, LIGHT_CONT_DATA** ppData );			// ライトデータ読み込み
...
{
	{"data/area00light.txt"},	// 野外
	{"data/area01light.txt"},	// 室内
	{"data/area03light.txt"},	// ダンジョン
	{"data/area02light.txt"},	// イアエア
};//*/
...
//			OS_TPrintf( "light=%d vec x[0x%x] y[0x%x] z[%x]\n", i, move->lightVec[i].x, move->lightVec[i].y, move->lightVec[i].z );
...
static u32 loadLightData( u32 lightidx, LIGHT_CONT_DATA** ppData )
{
	...
	tmp = ArcUtil_Load( ARC_FIELD_LIGHT, lightidx, FALSE, HEAPID_FIELD, ALLOC_TOP );		// ファイルオープン
	filep = tmp;				// 作業用変数に代入

...
//
//==============================================================================
BOOL UnionPokeFormReturn( VM_MACHINE * core )
{
	FIELDSYS_WORK* fsys	= core->fsys;
	POKEMON_PARAM* poke;
	POKEPARTY *party;
	...
	if(hakkin_num > 0){
		...
		if(ret == FALSE){
			return FALSE;	//ハッキンダマを手持ちに戻せなかった
		}
...
//==============================================================================
//
//	育て屋に預ける前にフォルムを戻す(UnionPokeFormReturnの単品バージョン)
//
//==============================================================================
BOOL SodateyaPokeFormReturn( VM_MACHINE * core )
{
	FIELDSYS_WORK* fsys	= core->fsys;
	POKEMON_PARAM* poke;
	POKEPARTY *party;
	...
	*ret_wk2 == 0xff;
	...
	for( i=0; i < temoti_max; i++ ){
		...
		if( (PokeParaGet(pp,ID_PARA_monsno,NULL) == MONSNO_PURAZUMA) &&
			(PokeParaGet(pp,ID_PARA_form_no,NULL) != FORMNO_ROTOMU_NORMAL) &&
...
BOOL EvCmdVillaListCheck( VM_MACHINE * core )
{
	...
	*ret_wk = 1;		//OK
	...
	case 15:
		//殿堂入りカウント20以上？
		if( RECORD_Get(rec,RECID_DENDOU_CNT) < 20 ){
			*ret_wk = 0;
		}
		break;
	...
	case 16:
		//再戦施設で戦闘した回数100以上？
		if( RECORD_Get(rec,RECID_LEADERHOUSE_BATTLE) < 100 ){
			*ret_wk = 0;
		}
		break;
	...
	case 17:
		//木の実を植えた回数100以上？
		if( RECORD_Get(rec,RECID_NUTS_PLANT) < 100 ){
			*ret_wk = 0;
		}
		break;
	...
	case 20:
		//歩数カウンター20000000以上？
		if( RECORD_Get(rec,RECID_WALK_COUNT) < 20000000 ){
			*ret_wk = 0;
		}
		break;

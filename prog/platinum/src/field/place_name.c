...
///< この定義を有効にすると、PLACE_WIN_NOTHING でも何かしらのウィンドウが表示される。
...
//#define WIN_DEAD_SPACE			(ONE_CHAR_SIZE*2-WIN_CAN_USE_SPACE)	///< デザインの関係上2キャラずれている
...
static void SetupPlaceNameWinEx(PNC_PTR outPlaceNameCont, const int inLen);
...
#ifdef PLACE_WIN_RONDOM
	ioPlaceNameCont->area_win_id = ( gf_rand() % 8 );
#endif
	
	win_id = ioPlaceNameCont->area_win_id * 2;
	
	ioPlaceNameCont->CharBuff = ArcUtil_CharDataGet( ARC_AREA_WIN_GRA, win_id, 0, &ioPlaceNameCont->CharDat, HEAPID_FIELD );
	GF_BGL_LoadCharacter(	ioPlaceNameCont->WinIni,
							frmnum,
							ioPlaceNameCont->CharDat->pRawData,
							ioPlaceNameCont->CharDat->szByte,
							cgx );
	buf = ArcUtil_PalDataGet( ARC_AREA_WIN_GRA, win_id+1, &pal_dat, HEAPID_FIELD );
#if 0
	ioPlaceNameCont->CharDat = GF_BGL_NTRCHR_CharGet(	&ioPlaceNameCont->CharBuff,
								HEAPID_FIELD,
								"/data/pl_areawin1.NCGR" );
	pal_dat = GF_BGL_NTRCHR_PalLoad(&buf,HEAPID_FIELD, "/data/pl_areawin1.NCLR");
#endif
	...
	//クリーン処理
	{
		...
		for ( i = 0; i < AREA_WIN_CHAR_NUM; i++ ){
			GF_BGL_BmpWinPrint(
					...
					i*ONE_CHAR_SIZE, 0, ONE_CHAR_SIZE, ONE_CHAR_SIZE,
		...
		sys_FreeMemoryEz( ioPlaceNameCont->CharBuff );
	}
}
...
static void SetupPlaceNameWinEx(PNC_PTR ioPlaceNameCont, const int inLen)
{
	...
	GF_BGL_BmpWinDataFill( &ioPlaceNameCont->Win, 0 );	//ＢＭＰウィンドウをクリア
	{
		...
		ArcUtil_ScrnSet( ARC_AREA_WIN_GRA, 0+2,
						 ioPlaceNameCont->WinIni, PLACE_NAME_FRAME, 0, 0, 0, HEAPID_FIELD);
						 
		ArcUtil_PalSet( ARC_AREA_WIN_GRA, 0+1, PALTYPE_MAIN_BG,
						0, 0, HEAPID_FIELD );
	}
	GF_BGL_BmpWinOn( &ioPlaceNameCont->Win );
}
...
void PlaceNameRequestEx(PNC_PTR ioPlaceNameCont,const int inPlaceID, const int inAreaWinID)
{
	STRCODE name[BUFLEN_PLACE_NAME];
	u32 len;	
	ioPlaceNameCont->NameID = inPlaceID;//ZoneData_GetPlaceNameID(inZoneID);
...
void PlaceNameRequestByFsys(FIELDSYS_WORK * fsys)
{
	...
	if ( !ZoneData_IsRoom(fsys->location->zone_id) ){
		...
		area_win_id = ZoneData_GetAreaWindowID(fsys->location->zone_id);
		
		if ( area_win_id != 0 ){
			 area_win_id--;
		}
		PlaceNameRequestEx(fsys->fldmap->place_name_cont, place_id, area_win_id); //地名表示
	}
}

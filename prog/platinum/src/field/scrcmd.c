...
static BOOL EvCmdInitPLGrassGym( VM_MACHINE * core );
static BOOL EvCmdInitVilla( VM_MACHINE * core );
...
static BOOL EvCmdLibraryTVSetProc( VM_MACHINE * core );
...
static BOOL	EvCmdClimax3DDemoCall(VM_MACHINE * core);		//クライマックスデモ
...
static BOOL EvCmdCheckMituhaniiComp( VM_MACHINE * core );
...
static BOOL EvCmdPLGrassGymTimeGain( VM_MACHINE * core );

static BOOL EvCmdGingaFogSet( VM_MACHINE * core );
static BOOL EvCmdGingaFogReset( VM_MACHINE * core );

static BOOL EvCmdExtraSaveInit( VM_MACHINE * core );
static BOOL EvCmdExtraSaveInitCheck( VM_MACHINE * core );
static BOOL EvCmdBtlPointShop( VM_MACHINE * core );
...
BOOL EvCmdGetCharCodeT06R0101( VM_MACHINE * core );
static u8 GetLeaderCharT06R0101( u16 c1, u16 c2, u16 c3, u16 c4 );
static u8 GetSevenCharT06R0101( u16 c1, u16 c2, u16 c3, u16 c4 );

static BOOL EvCmdGdsSetProc( VM_MACHINE * core );

static BOOL EvCmdWiFiLobbySetProc( VM_MACHINE * core );

static BOOL EvCmdTenganDemoEventCall( VM_MACHINE * core );
...
static BOOL EvCmdTwObjAdd( VM_MACHINE * core );
static BOOL EvCmdTwObjDel( VM_MACHINE * core );

static BOOL EvCmdHeapCheck( VM_MACHINE * core );

static BOOL EvCmdGirathinaSpWildBattle( VM_MACHINE * core );
...
const VM_CMD ScriptCmdTbl[] = {
	...
	EvCmdCheckMituhaniiComp,		// ミツハニーオスメスそろったか
...
static BOOL EvCmdBtlSearcherDirMvSet( VM_MACHINE * core )
{
	...
	if( *fldobj != NULL ){
		if( GYM_GimmickCodeCheck(core->fsys, FLD_GIMMICK_GHOST_GYM) == FALSE
				 || GYM_PlGhostGymTrainerMoveCodeChange(core->fsys,*fldobj) == FALSE ){
...
static BOOL EvCmdLibraryTVSetProc( VM_MACHINE * core )
{
	Field_LibraryTV_SetProc( core->fsys );
	VM_SetWait( core, EvCmdWaitSubProcEnd );
	return 1;
}
...
static BOOL EvCmdNaminori( VM_MACHINE * core )
{
	...
	
	{
		...
		if( Player_MoveBitCheck_Flat(core->fsys->player) == TRUE ){
			dir = Player_DirGet( core->fsys->player );
		}else{
			dir = Player_DirMoveGet( core->fsys->player );
		}
		
		EventCmd_NaminoriCall( core->event_work, dir, VMGetWorkValue(core) );
	}
...
//	FIELDSYS_WORK* fsys = core->fsys;
	...
	//もう使っていないので削除　2007.12.19(水) matsuda
...
static BOOL EvCmdFirstBattleSet( VM_MACHINE * core )
{
	...
	EventCmd_FirstBattle(core->event_work, tr_id_0, HEAPID_WORLD, win_flag);
	return 1;
}
...
static BOOL	EvCmdClimax3DDemoCall(VM_MACHINE * core)
{
	u8 id = VMGetU8(core);
	u16 *ret_wk = VMGetWork(core);
...
//----------------------------------------------------------------------------
/**
 *	@brief	ミツハニーのオスメスがポケパーティにそろっているかチェック
 *
 *	@param	core
 */
//-----------------------------------------------------------------------------
...
{
	POKEMON_PARAM* p_pp;
	int i,monsno,male_flag,female_flag,tamago,sex;
	u16* ret_wk			= VMGetWork( core );
	POKEPARTY* p_party	= SaveData_GetTemotiPokemon( core->fsys->savedata );
	int poke_count		= PokeParty_GetPokeCount( p_party );
...
static BOOL EvCmdIsHaihuEventEnable(VM_MACHINE *core)
{
	...
	GF_ASSERT( (id <= 4) && "SysWork_HaihuEventWorkCheck() id over\n" );
...
static BOOL EvCmdFrontierSystemCall( VM_MACHINE * core )
{
	...
	*buf = ex_param;
	...
	ex_param->fnote_data= core->fsys->fnote;
...
static BOOL EvCmdPLGrassGymTimeGain( VM_MACHINE * core )
{
	FIELDSYS_WORK *fsys = core->fsys;
	STRBUF **pBuf = GetEvScriptWorkMemberAdrs( fsys, ID_EVSCR_MSGBUF );
	GF_BGL_BMPWIN *win = GetEvScriptWorkMemberAdrs(fsys,ID_EVSCR_MSGWINDAT);
	GYM_PLGrass_TimeGainEventSet( fsys, win, core->msgman, *pBuf );
	return 1;
}
...
static void GingaFogSetTcb( TCB_PTR tcb, void* p_work )
{
	...
	if(fsys->fldmap->weather_data){
		WEATHER_Set( fsys->fldmap->weather_data, WEATHER_SYS_GINGA00 );
		TCB_Delete( tcb );
	}
}
...
{
	FIELDSYS_WORK *fsys = core->fsys;
	TCB_Add( GingaFogSetTcb, fsys, 128 );
	return 1;
}

//--------------------------------------------------------------------------------------------
/**
 * @brief		スクリプトコマンド：銀河団のFOGを消す
 * @param		core
 * @return		"1"
 */
//--------------------------------------------------------------------------------------------
...
{
	FIELDSYS_WORK *fsys = core->fsys;
	u16 no = Situation_GetWeatherID(SaveData_GetSituation(fsys->savedata));
	WEATHER_Set( fsys->fldmap->weather_data, no );
	return 1;
}

//--------------------------------------------------------------------------------------------
/**
 * @brief		スクリプトコマンド：バトルポイントショップ
 * @param		core
 * @return		"1"
 */
//--------------------------------------------------------------------------------------------
...
static BOOL EvCmdBtlPointShop( VM_MACHINE * core )
{
	...
	static const u16* goods_data[] = {
		goods_list01,
		goods_list02,
	};
	
	OS_Printf( "shop type = %2d\n", type );
	
	ShopInit( core->event_work, core->fsys, (u16*)goods_data[ type ], SHOP_TYPE_BP, FALSE );
...
#define T06R0101_LEADER_MAX		( NELEMS(t06r0101_leader_char_tbl) )

static const u16 t06r0101_seven_char_tbl[] = {
	SEVEN1,
	SEVEN2,
	SEVEN3,
	SEVEN4,
...
#define T06R0101_SEVEN_MAX		( NELEMS(t06r0101_seven_char_tbl) )
...
{
	u16 r,count;
	u16* set_wk1 = VMGetWork( core );
	u16* set_wk2 = VMGetWork( core );
	...
	*set_wk4 = 0;
	...
	r = ( gf_rand() % T06R0101_LEADER_MAX );
	*set_wk1 = t06r0101_leader_char_tbl[r];		/*1人目は必ずいる*/

	r = GetLeaderCharT06R0101( *set_wk1, *set_wk2, *set_wk3, *set_wk4 );
	if( r != T06R0101_LEADER_MAX ){
		*set_wk2 = t06r0101_leader_char_tbl[r];	/*2人目*/
	}

	r = GetLeaderCharT06R0101( *set_wk1, *set_wk2, *set_wk3, *set_wk4 );
	if( r != T06R0101_LEADER_MAX ){
		*set_wk3 = t06r0101_leader_char_tbl[r];	/*3人目*/
	}

	//5人衆はワークに値はセットするが、スクリプトでイベントチェックして表示されないことがある*/
	r = GetSevenCharT06R0101( *set_wk1, *set_wk2, *set_wk3, *set_wk4 );
	if( r != T06R0101_SEVEN_MAX ){
		*set_wk4 = t06r0101_seven_char_tbl[r];	/*4人目*/
	}
...
static u8 GetLeaderCharT06R0101( u16 c1, u16 c2, u16 c3, u16 c4 )
{
	...
	while( 1 ){
		r = ( gf_rand() % (T06R0101_LEADER_MAX+1) );	//8だったら、いないことにする
		if( r == T06R0101_LEADER_MAX ){
			break;
		}
		...
		count++;
		if( count >= 8 ){
			r = T06R0101_LEADER_MAX;
			break;
		}

		//被っていないかチェック
		if( (t06r0101_leader_char_tbl[r] == c1) && (t06r0101_leader_char_tbl[r] == c2) &&
			(t06r0101_leader_char_tbl[r] == c3) && (t06r0101_leader_char_tbl[r] == c4) ){
			break;
		}
	}
...
static u8 GetSevenCharT06R0101( u16 c1, u16 c2, u16 c3, u16 c4 )
{
	...
	while( 1 ){
		r = ( gf_rand() % (T06R0101_SEVEN_MAX+1) );	//5だったら、いないことにする
		if( r == T06R0101_SEVEN_MAX ){
			break;
		}
		...
		count++;
		if( count >= 8 ){
			r = T06R0101_SEVEN_MAX;
			break;
		}

		//被っていないかチェック
		if( (t06r0101_seven_char_tbl[r] == c1) && (t06r0101_seven_char_tbl[r] == c2) &&
			(t06r0101_seven_char_tbl[r] == c3) && (t06r0101_seven_char_tbl[r] == c4) ){
			break;
		}
	}
...
static BOOL EvCmdTenganDemoEventCall( VM_MACHINE * core )
{
	EventCmd_DemoTanganProc( core->event_work, core->fsys->savedata );
	return 1;
}
...
static BOOL EvCmdTwObjAdd( VM_MACHINE * core )
{
	FIELDSYS_WORK *fsys = core->fsys;
	u16 id				= VMGetWorkValue(core);
	FldTornWorld_EvFldOBJAdd( fsys, id );
	return 0;
}
...
static BOOL EvCmdTwObjDel( VM_MACHINE * core )
{
	FIELDSYS_WORK *fsys = core->fsys;
	u16 id				= VMGetWorkValue(core);
	FldTornWorld_EvFldOBJDelete( fsys, id );
	return 0;
}

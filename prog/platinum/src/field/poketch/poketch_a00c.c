...
static BOOL SetupWork( APPWORK *wk, CONTACT_WORK* conwk, GF_BGL_INI* bgl, u32 param )
{
	...
	// 時間取得。ヘンな値が入ってきた時のために丸めておく
		...
		if( wk->vpara.time.minute >= 60 ){
			...
			wk->vpara.time.minute %= 60;	// たぶんminuteをしなくちゃいけないと思ったので
											// 追加しました tomoya takahashi
		}
		wk->rtc_prev_minute = wk->vpara.time.minute;
#if PLFIX_S2414
		wk->rtc_prev_hour = wk->vpara.time.hour;
#endif
...
#if PLFIX_S2414
		// 1時間後にスリープ解除すると
		// 分しか見ていないときは、変化がないので表示を更新しない。
		// それを回避する為に時間もみるようにしました。
		// 日にちが変わっても時間しか表示してないからもう大丈夫！
		wk->rtc_prev_minute = wk->vpara.time.minute;
		wk->rtc_prev_hour = wk->vpara.time.hour;
		GF_RTC_GetTime( &(wk->vpara.time) );

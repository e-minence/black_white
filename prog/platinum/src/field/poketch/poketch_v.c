...
enum {
	...
	CHARNO_BUTTON_FREETOP = 0x8,
	CHARNO_BUTTON_PRESSTOP = 0xc,
	CHARNO_BUTTON_HOLDTOP = 0x10,

	CHARNO_BUTTON_FREETOP_DOWN = 0x14,
	CHARNO_BUTTON_PRESSTOP_DOWN = 0x18,
	CHARNO_BUTTON_HOLDTOP_DOWN = 0x1c,
...
enum {
	...
	SCRN_BUTTON_DOWN_XPOS = SCRN_BUTTON_XPOS,
	SCRN_BUTTON_DOWN_YPOS = 12,

	SCRN_BUTTON_WIDTH = 4,
	SCRN_BUTTON_HEIGHT = 8,
	
	SCRN_BUTTON_HEIGHT_SAME_SCRN_START = 2,
	SCRN_BUTTON_HEIGHT_SAME_SCRN_END = 4,
};

#define SCRN_BUTTON_SIZE	(SCRN_BUTTON_WIDTH * SCRN_BUTTON_HEIGHT)
#define BOARD_CHARDATA_WIDTH		(32)
...
struct _POKETCH_VIEW_WORK {
	...
	u16   scrn_button_down_free[ SCRN_BUTTON_SIZE ];
	u16   scrn_button_down_press[ SCRN_BUTTON_SIZE ];
	u16   scrn_button_down_hold[ SCRN_BUTTON_SIZE ];
...
static void CmdButton_Common(void *wk_adrs, u16 *scrnbuf, int up_down);
static void CmdButtonLock_Up(TCB_PTR tcb, void *wk_adrs);
static void CmdButtonLock_Down(TCB_PTR tcb, void *wk_adrs);
static void CmdButtonFree_Up(TCB_PTR tcb, void *wk_adrs);
static void CmdButtonFree_Down(TCB_PTR tcb, void *wk_adrs);
static void CmdButtonPush_Up(TCB_PTR tcb, void *wk_adrs);
static void CmdButtonPush_Down(TCB_PTR tcb, void *wk_adrs);
...
BOOL  Poketch_MAIN_SetViewWork(POKETCH_VIEW_WORK** wk_ptr, const POKETCH_MAIN_VPARAM *vpara, NNSG2dOamManagerInstance* oamm, CONTACT_WORK* cwk )
{
	*wk_ptr = sys_AllocMemory(POKETCH_MAIN_VIEW_HEAPID, sizeof(POKETCH_VIEW_WORK));
	...
	if(*wk_ptr != NULL)
	{
		...
		MakeButtonScrn( (*wk_ptr)->scrn_button_down_free, 
			BOARD_CHAROFS+CHARNO_BUTTON_FREETOP_DOWN );
		MakeButtonScrn( (*wk_ptr)->scrn_button_down_press, 
			BOARD_CHAROFS+CHARNO_BUTTON_PRESSTOP_DOWN );
		MakeButtonScrn( (*wk_ptr)->scrn_button_down_hold, 
			BOARD_CHAROFS+CHARNO_BUTTON_HOLDTOP_DOWN );
...
static void MakeButtonScrn( u16 *buf, u32 charno_top )
{
	...
	i = 0;
	ypos = 0;
	for(y = 0; y < SCRN_BUTTON_HEIGHT; y++){
		for(x = 0; x < SCRN_BUTTON_WIDTH; x++){
			buf[i] = (BOARD_PALNO<<12) | (charno_top + ypos + x);
			i++;
		}
		if(y >= SCRN_BUTTON_HEIGHT_SAME_SCRN_START && y < SCRN_BUTTON_HEIGHT_SAME_SCRN_END){
			;
		}
		else{
			ypos += BOARD_CHARDATA_WIDTH;
		}
	}
}
...
static const POKETCH_VIEW_CMD CmdTbl[] = {
	...
	{ CMD_MAIN_BUTTONLOCK,	CmdButtonLock_Up,	0 },
	{ CMD_MAIN_BUTTONFREE,	CmdButtonFree_Up,	0 },
	{ CMD_MAIN_BUTTONPUSH,	CmdButtonPush_Up,	0 },
	{ CMD_MAIN_BUTTONLOCK_DOWN,	CmdButtonLock_Down,	0 },
	{ CMD_MAIN_BUTTONFREE_DOWN,	CmdButtonFree_Down,	0 },
	{ CMD_MAIN_BUTTONPUSH_DOWN,	CmdButtonPush_Down,	0 },
 ...
 * @param   tcb		
 * @param   wk_adrs		
 * @param   up_down		BUTTON_SCRN_UP or BUTTON_SCRN_DOWN
 *
 */
//------------------------------------------------------------------
static void CmdButton_Common(void *wk_adrs, u16 *scrnbuf, int up_down)
{
	...
	if(up_down == BUTTON_SCRN_UP){
		x = SCRN_BUTTON_XPOS;
		y = SCRN_BUTTON_YPOS;
	}
	else{
		x = SCRN_BUTTON_DOWN_XPOS;
		y = SCRN_BUTTON_DOWN_YPOS;
	}
	
	GF_BGL_ScrWrite( vwk->bgl, GF_BGL_FRAME0_S, scrnbuf, 
			x, y, SCRN_BUTTON_WIDTH, SCRN_BUTTON_HEIGHT );
...
{
	POKETCH_VIEW_WORK* vwk = PoketchViewTool_GetViewWork(wk_adrs);
	CmdButton_Common(wk_adrs, vwk->scrn_button_press, BUTTON_SCRN_UP);
	Snd_SePlay( POKETCH_SE_BUTTON_ROCK );
}
//------------------------------------------------------------------
/**
 * コマンド：ボタンロック状態へ変更(下ボタン)
 *
 * @param   tcb		
 * @param   wk_adrs		
 *
...
{
	POKETCH_VIEW_WORK* vwk = PoketchViewTool_GetViewWork(wk_adrs);
	CmdButton_Common(wk_adrs, vwk->scrn_button_down_press, BUTTON_SCRN_DOWN);
	Snd_SePlay( POKETCH_SE_BUTTON_ROCK );
}

//------------------------------------------------------------------
/**
 * コマンド：ボタン離し状態へ変更(上ボタン)
 *
 * @param   tcb		
 * @param   wk_adrs		
 *
...
{
	POKETCH_VIEW_WORK* vwk = PoketchViewTool_GetViewWork(wk_adrs);
	CmdButton_Common(wk_adrs, vwk->scrn_button_free, BUTTON_SCRN_UP);
}
//------------------------------------------------------------------
/**
 * コマンド：ボタン離し状態へ変更(下ボタン)
 *
 * @param   tcb		
 * @param   wk_adrs		
 *
...
{
	POKETCH_VIEW_WORK* vwk = PoketchViewTool_GetViewWork(wk_adrs);
	CmdButton_Common(wk_adrs, vwk->scrn_button_down_free, BUTTON_SCRN_DOWN);
}

//------------------------------------------------------------------
/**
 * コマンド：ボタン押し込み状態へ変更(上ボタン)
 *
 * @param   tcb		
 * @param   wk_adrs		
 *
...
{
	POKETCH_VIEW_WORK* vwk = PoketchViewTool_GetViewWork(wk_adrs);
	CmdButton_Common(wk_adrs, vwk->scrn_button_hold, BUTTON_SCRN_UP);
	Snd_SePlay( POKETCH_SE_BUTTON_PUSH );
}
//------------------------------------------------------------------
/**
 * コマンド：ボタン押し込み状態へ変更(下ボタン)
 *
 * @param   tcb		
 * @param   wk_adrs		
 *
...
{
	POKETCH_VIEW_WORK* vwk = PoketchViewTool_GetViewWork(wk_adrs);
	CmdButton_Common(wk_adrs, vwk->scrn_button_down_hold, BUTTON_SCRN_DOWN);
	Snd_SePlay( POKETCH_SE_BUTTON_PUSH );
}
...
#if (AFTER_MASTER_070719_POKETCH_APPNUM_PAL==0)

...
#define MENU_X_CHAR_SIZE (12)		//デバッグメニュー横幅　キャラサイズ
...
#define D_GHOSTGYM_CHAR_SIZE_Y (5*MENU_Y_CHAR_ONE)
...
static BOOL D_TakinoboriEvent( GMEVENT_CONTROL *ev );

//--------------------------------------------------------------
//	秘伝技たきのぼり
//--------------------------------------------------------------
static void D_TakinoboriInit( D_KAGA_WORK *kaga )
{
	D_TAKI_WORK *work;
	
	work = D_KagaAllocMemory( sizeof(D_TAKI_WORK) );
	work->fsys = kaga->fsys;
	FieldEvent_Set( work->fsys, D_TakinoboriEvent, work );
}
...
static BOOL D_TakinoboriEvent( GMEVENT_CONTROL *ev )
{
	D_TAKI_WORK *work = FieldEvent_GetSpecialWork( ev );
	
	switch( work->seq_no ){
	case 0:
		{
			int dir = Player_DirGet( work->fsys->player );
			EventCmd_TakiNoboriCall( ev, dir, 0 );
			work->seq_no++;
		}
		break;
	case 1:
...
static const DEBUG_MENU_LIST DATA_MenuList[MENU_LIST_MAX] =
{
	...
	{ debug_kaga_msg_24, (u32)D_TakinoboriInit },
};

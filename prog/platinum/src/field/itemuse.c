...
#include "msgdata/msg_place_name.h"		// 改訂版で追加 2006/10/23 by nakahiro
...
static u32 ItemUseCheck_Tankenset( const ITEMCHECK_WORK * icwk )
{
	// 謎の場所（改訂版で追加 2006/10/23 by nakahiro）
...
static u32 ItemUseCheck_Cycle( const ITEMCHECK_WORK * icwk )
{
	...
	//サイクリングロード"ゲート"内の半分を通過したかチェック(プラチナで追加)
...
static BOOL ItemCnv_BattleRec( CONV_WORK * wk )
{
	ConvFuncCallSet( wk, ConvCall_BattleRec );
	return TRUE;
}
...
static BOOL ItemCnv_Grashidea( CONV_WORK * wk )
{
	ConvFuncCallSet( wk, ConvCall_Grashidea );
	return TRUE;
}

...
#define	WIN_SHOP_MENU_SX_BP	( 6 )		// BT用のはいいいえ plで追加
...
#define	WIN_SHOP_GOLD_SX	( 9 )		///< 小さくした
...
static u16 ShopBuyItemPriceBPGet( SHOP_WORK * wk, u16 item );

static u32	 MyBPGoldGet( SHOP_WORK * wk );
...
void ShopInit( GMEVENT_CONTROL * event, FIELDSYS_WORK * repw, u16 * item, u8 type, BOOL dp_flg )
{
	...
	///< 8x8 num font
	wk->num_font = NUMFONT_Create( FBMP_COL_BLACK, FBMP_COL_BLK_SDW, FBMP_COL_NULL, HEAPID_WORLD );
...
static void ShopMenuSet( SHOP_WORK * wk )
{
	...
	if( wk->shop_type == SHOP_TYPE_ITEM ){
		...
		max = 2;
		wk->md = BMP_MENULIST_Create( max, HEAPID_WORLD );
		BMP_MENULIST_AddArchiveString( wk->md, wk->mman, mes_shop_11_02, SHOP_SEQ_BUY_INIT );
		BMP_MENULIST_AddArchiveString( wk->md, wk->mman, mes_shop_11_03, BMPMENU_CANCEL );

		GF_BGL_BmpWinAdd(
			wk->bgl, &wk->win[WIN_IDX_NENU], FLD_MBGFRM_FONT,
			WIN_SHOP_YESNO_PX, WIN_SHOP_YESNO_PY,
			WIN_SHOP_YESNO_SX, WIN_SHOP_YESNO_SY, WIN_SHOP_MENU_PAL, WIN_SHOP_MENU_CGX );
	}
	else{
		max = 2;
		wk->md = BMP_MENULIST_Create( max, HEAPID_WORLD );
...
static void ShopBuyWinAdd( SHOP_WORK * wk )
{
	...
	for( i=0; i<WIN_IDX_MAX; i++ ){
		if( wk->shop_type != SHOP_TYPE_ITEM 
		&&	wk->shop_type != SHOP_TYPE_BP && i == WIN_IDX_INFO ){
			GF_BGL_BmpWinAddEx( wk->bgl, &wk->win[i], &ShopBuyGoodsInfoWin );
		}
		if ( wk->shop_type == SHOP_TYPE_BP && i == WIN_IDX_GOLD ){
			GF_BGL_BmpWinAddEx( wk->bgl, &wk->win[i], &ShopBuyBPWin );
		}
		else{
...
static void ShopBuyGraphicSet( SHOP_WORK * wk )
{
	...
	p_handle = ArchiveDataHandleOpen( ARC_SHOP_GRA, HEAPID_WORLD );
	
	ArcUtil_HDL_BgCharSet(
		p_handle, NARC_shop_gra_shop_bg_NCGR,
		wk->bgl, SHOP_FRM_BG, 0, 0, 0, HEAPID_WORLD );
		
	if( wk->shop_type == SHOP_TYPE_ITEM
	||	wk->shop_type == SHOP_TYPE_BP ){
		ArcUtil_HDL_ScrnSet(
			p_handle, NARC_shop_gra_shop_bg1_NSCR,
			wk->bgl, SHOP_FRM_BG, 0, 0, 0, HEAPID_WORLD );
	}else{
		ArcUtil_HDL_ScrnSet(
			p_handle, NARC_shop_gra_shop_bg2_NSCR,
			wk->bgl, SHOP_FRM_BG, 0, 0, 0, HEAPID_WORLD );
	}
	
	if ( wk->shop_type == SHOP_TYPE_BP ){
		ArcUtil_HDL_PalSet(
			p_handle, NARC_shop_gra_shop_bg_bt_NCLR, PALTYPE_MAIN_BG, 0, 32, HEAPID_WORLD );
	}
	else {
		ArcUtil_HDL_PalSet(
...
static void ShopBuyItemListMake( SHOP_WORK * wk )
{
	...
	BOOL waza_machine = FALSE;

	if( wk->shop_type == SHOP_TYPE_ITEM
	||  wk->shop_type == SHOP_TYPE_BP ){
		man = MSGMAN_Create( MSGMAN_TYPE_NORMAL, ARC_MSG, NARC_msg_itemname_dat, HEAPID_WORLD );
		waza_man = MSGMAN_Create( MSGMAN_TYPE_NORMAL, ARC_MSG, NARC_msg_wazaname_dat, HEAPID_WORLD );
	}
	else if( wk->shop_type == SHOP_TYPE_GOODS ){
	...
	for( i=0; i<wk->max; i++ ){
		...
		&&	 id >= ITEM_WAZAMASIN01 ){	///< 技マシンは、アイテム名からNoXX＋技名に変更する
...
//	BMP_MENULIST_AddArchiveString( wk->ld, wk->mman, mes_shop_02_06, BMPLIST_RABEL );
...
static void CB_ItemListMove( BMPLIST_WORK * work, u32 param, u8 mode )
{
...

	{
		...
		//	BmpListParamSet( work, BMPLIST_ID_DATA_X, ITEM_LIST_DX_NORMAL );
...
static void CB_ItemListWrite( BMPLIST_WORK * work, u32 param, u8 y )
{
	...
	if( param != BMPLIST_CANCEL ){
		...
		if( wk->shop_type == SHOP_TYPE_BP ){
			strb  = MSGMAN_AllocString( wk->mman, mes_shop_12_01 );
		}
		else {
	...
	if( param == BMPLIST_CANCEL ){
		STRBUF * str = MSGMAN_AllocString( wk->mman, mes_shop_02_06 );

		GF_BGL_BmpWinFill( &wk->win[ WIN_IDX_LIST ], 15, 0, y, WIN_SHOP_LIST_SX*8, 16 );

		GF_STR_PrintColor(
			&wk->win[ WIN_IDX_LIST ], FONT_SYSTEM, str,
			0, y, MSG_NO_PUT, SHOPCOL_N_BLACK, NULL );
...
static void ShopBuyGoldWinPut( SHOP_WORK * wk, u8 flg )
{
	...
	if( wk->shop_type == SHOP_TYPE_BP ){			
		if( flg == 0 ){
			...
			GF_BGL_BmpWinFill( &wk->win[WIN_IDX_GOLD], 15, 0, 0, GOLD_EX, 16 );
		}

		expb = STRBUF_Create( GOLD_BUFLEN, HEAPID_WORLD );
		strb = MSGMAN_AllocString( wk->mman, mes_shop_13_01 );
	...
	//	gold = MyStatus_GetGold( wk->my );
		...
		siz = FontProc_GetPrintStrWidth( FONT_SYSTEM, expb, 0 );
		GF_STR_PrintSimple(
			&wk->win[WIN_IDX_GOLD], FONT_SYSTEM, expb, GOLD_EX-siz-8, 0, MSG_NO_PUT, NULL );
	}
	else {			
		if( flg == 0 ){
			GF_BGL_BmpWinDataFill( &wk->win[WIN_IDX_GOLD], 15 );
	...
	default:
		...
		//	gold = MyStatus_GetGold( wk->my );
			...
			if( gold < wk->sel_price ){
				...
				if( wk->shop_type == SHOP_TYPE_BP ){
					str = MSGMAN_AllocString( wk->mman, mes_shop_12_06 );
				}
				else {
...
//			WORDSET_RegisterItemName( wk->wset, 0, wk->sel_item );
			...
			if( wk->shop_type == SHOP_TYPE_BP ){
				str = MSGMAN_AllocString( wk->mman, mes_shop_12_02 );
			}
			else {
...
//	WORDSET_RegisterItemName( wk->wset, 0, wk->sel_item );
	...
	if ( wk->sel_item <= ITEM_HIDENMASIN01 && wk->sel_item >= ITEM_WAZAMASIN01 ){
		...
		if ( wk->shop_type == SHOP_TYPE_BP ){						///< BP用
			str = MSGMAN_AllocString( wk->mman, mes_shop_12_05 );
		}
		else {
			str = MSGMAN_AllocString( wk->mman, mes_shop_10_02 );
		}
	}
	else {
		if ( wk->shop_type == SHOP_TYPE_BP ){
			str = MSGMAN_AllocString( wk->mman, mes_shop_12_04 );
		}
		else {
			str = MSGMAN_AllocString( wk->mman, mes_shop_02_03 );
		}
...
static void NumSelWinPut( SHOP_WORK * wk, u8 flg )
{
	...
	if( wk->shop_type == SHOP_TYPE_BP ){
		strb = MSGMAN_AllocString( wk->mman, mes_shop_12_03 );
	}
	else {
...
static u8 ShopBuyCmpSeq( SHOP_WORK * wk )
{
	...
	MyBPGoldSub( wk, wk->sel_price*wk->sel_num );
...
static u8 ShopBuyCmpWaitSeq( SHOP_WORK * wk )
{
	...
	if( sys.trg & (PAD_BUTTON_A|PAD_BUTTON_B) ){
		...
		if( ( wk->shop_type == SHOP_TYPE_ITEM || wk->shop_type == SHOP_TYPE_BP ) &&
...
static u32 ShopBuyItemPriceGet( SHOP_WORK * wk, u16 item )
{
	if( wk->shop_type == SHOP_TYPE_ITEM ){
		...
		return ShopBuyItemPriceBPGet( wk, item );
	}
	else if( wk->shop_type == SHOP_TYPE_GOODS ){
...
static u16 ShopBuyItemPriceBPGet( SHOP_WORK * wk, u16 item )
{
	...
	#define BP_ITEM_MAX		( NELEMS( bpgift_list ) )
	
	for ( i = 0; i < BP_ITEM_MAX; i++ ){
		if ( bpgift_list[ i ][ 0 ] == item ){
			
			return bpgift_list[ i ][ 1 ];
		}
	}	
 ...
 */
//--------------------------------------------------------------
u32 MyBPGoldGet( SHOP_WORK * wk )
{
	if ( wk->shop_type == SHOP_TYPE_BP ){
		
		return BattlePoint_Get( wk->sv );
	}
	else {
 ...
 */
//--------------------------------------------------------------
void  MyBPGoldSub( SHOP_WORK * wk, u32 sub )
{
	if ( wk->shop_type == SHOP_TYPE_BP ){
		BattlePoint_Sub( wk->sv, (u16)sub );
	}
	else {
		Mystatus_SubGold( wk->my, sub );
	}
}
...
static void ShopExitSeq( FIELDSYS_WORK * fsys, SHOP_WORK * wk )
{
	...
	if( wk->shop_type == SHOP_TYPE_BP ){
		str = MSGMAN_AllocString( wk->mman, mes_shop_11_04 );
	}
	else {
		str = MSGMAN_AllocString( wk->mman, mes_shop_01_02 );
	}

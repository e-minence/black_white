...
#define	PROG_TOPIC_MAX		9			//★

#define	TOPIC_ARRAY_MAX		16			//配列
 ...
 *
 * topic_typeが１バイト、topic_id１バイトがPROG_TOPIC_MAX個の合計10バイトで構成されています。
 * 10バイトｘ9番組＝90バイトのバイナリデータから取得しています。★
 */
//--------------------------------------------------------------------
...
static TV_PROGRAM_ID GetNowProgramID(FIELDSYS_WORK * fsys)
{
	...
	int week, hour, min;
	int ofs;
	week = (EVTIME_GetWeek(fsys) + 6) % 7;		//番組表データが月曜からの並びのため
	hour = EVTIME_GetHour(fsys);
	min = EVTIME_GetMinute(fsys);

	//19=番組表データが5時からの並びのため
	//15=番組データが15分刻みなので
...
//--------------------------------------------------------------------
/**
 * @brief	トピック種別の取得(質問、監視、ギネスなど)
 * @param	prog_id
 * @return	TVTOPICTYPE
 *
 * NARC_tv_contents_binがプラチナ用で変更された★
 */
//--------------------------------------------------------------------
...
int TV_ChooseBroadcastTopic(FIELDSYS_WORK * fsys, BOOL mine, BOOL see)
{
	...
	//番組IDを取得して、
	//その番組のトピック種類を取得して、
	//トピックIDをtype_arrayに格納
	//
	//★新規で追加されたトピックIDは番組には書かれていないため、DPでははじかれるので大丈夫
	//
	//DPでトピックIDは存在するがデータが作られない、PLでは復活したトピックIDは？
	//
	//BOOL TVTOPIC_CheckLimit(TVTOPICTYPE topic_type, FIELDSYS_WORK * fsys, TVBCD * bcd)
	//この中で、生成関数がない場合は、無条件で放送しない
	//
	//★生成関数が登録されていないものは、はじかれるので大丈夫
	//
	//なので、混ぜても平気なはず。
#endif

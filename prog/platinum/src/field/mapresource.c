...
#define BUILD_MODEL_MAX	(768)	//(550)		プラチナで増やした
...
//		outMapResource->model[0] = ArchiveDataLoadMalloc( ARC_3D_STRUCT, 0, HEAPID_FIELD );
	...
	ArchiveDataHandleClose( model_handle );
		...
		if ( MAPRES_IsValidLight(outMapResource) == FALSE) {
			G3X_SetEdgeColorTable(gEdgeColorField);	// 室内
		} else {
			G3X_SetEdgeColorTable(gEdgeColorOther);	// 野外
		}
#endif
...
{
	u8 light_idx; 
	light_idx = mapresource->ResourceParam.light;
 ...
 * モデルファイルからOBJIDを取得
 *
 * @param   inMapresource		マップリソースのポインタ

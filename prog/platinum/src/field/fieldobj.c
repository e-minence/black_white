...
#elif 1
		if( head->obj_code == BIGMAN ){
			head->obj_code = DEBUG_NEWOBJCODE_0;
		}else if( head->obj_code == BOY1 ){
			head->obj_code = DEBUG_NEWOBJCODE_1;
		}
#endif
	}
#endif	
 ...
 */
//--------------------------------------------------------------
void FieldOBJ_MoveBitSet_AttrGetOFF( FIELD_OBJ_PTR fldobj, int flag )
{
	if( flag == TRUE ){
		FieldOBJ_MoveBit_ON( fldobj, FLDOBJ_MOVE_BIT_ATTR_GET_OFF );
	}else{
		FieldOBJ_MoveBit_OFF( fldobj, FLDOBJ_MOVE_BIT_ATTR_GET_OFF );
	}
}
...
{
	if( FieldOBJ_MoveBit_Check(fldobj,FLDOBJ_MOVE_BIT_ATTR_GET_OFF) ){
		return( TRUE );
	}
	...
	CONST_FIELD_OBJ_SYS_PTR sys, int x, int z, int old_hit )
{
	...
	do{
		if( FieldOBJ_StatusBit_Check(fldobj,FLDOBJ_STA_BIT_USE) ){
			if( old_hit ){
				if( FieldOBJ_OldPosGX_Get(fldobj) == x &&
					FieldOBJ_OldPosGZ_Get(fldobj) == z ){
					return( fldobj );
				}
			}
			
			if( FieldOBJ_NowPosGX_Get(fldobj) == x &&
				FieldOBJ_NowPosGZ_Get(fldobj) == z ){
				return( fldobj );
			}
		}

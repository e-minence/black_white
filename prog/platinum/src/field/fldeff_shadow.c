...
static void Shadow_GraphicInit( FE_SHADOW_PTR sd )
{
	...
	for( i = 0; i < SHADOW_MAX; i++ ){
		FE_FROMdl_ResSetArcLoadTexTrans(
			sd->fes, &sd->rmdl[i], 0, DATA_ShadowArcIdx[i], ALLOC_FR );
		FRO_OBJ_InitInMdl( &sd->robj[i], &sd->rmdl[i] );
	}
}
...
static void Shadow_GraphicAlphaSet( FE_SHADOW_PTR sd, int alpha )
{
	NNS_G3dMdlUseMdlAlpha( sd->rmdl[SHADOW_BLACK].pResMdl );
	NNS_G3dMdlSetMdlAlphaAll( sd->rmdl[SHADOW_BLACK].pResMdl, alpha );
}

...
#if 1
	val = 1;
	poke = PokeParty_GetMemberPointer( btlprm->poke_party[POKEPARTY_ENEMY], 0 );
	PokeParaPut( poke, ID_PARA_event_get_flag, &val );
#endif
...
void EventCmd_FirstBattle(GMEVENT_CONTROL * event, int tr_id_0, int heapID, int * winlose_flag)
{
...
#if 0
	{
		...
		PokeParaPut( poke, ID_PARA_hp, &value );
		poke = PokeParty_GetMemberPointer( SaveData_GetTemotiPokemon(fsys->savedata), 1);
		PokeParaPut( poke, ID_PARA_hp, &value );
	}
#endif
...
//-----------------------------------------------------------------------------
/**
 * @brief	WIFI通信対戦イベントセット（WifiAIマルチ実験）
 * @param	event
 */
//-----------------------------------------------------------------------------
void EventCmd_WifiBattle(GMEVENT_CONTROL * event,int targetID, int lvLimit,WIFI_BATTLEFLAG bflag)
{
	FIELDSYS_WORK * fsys = FieldEvent_GetFieldSysWork(event);
	EV_BATTLE_WORK * ebw;
	BATTLE_PARAM *btlprm;
...
void EventCmd_GirathinaWildBattle( GMEVENT_CONTROL * event,
		u16 monsno, u8 level, int *winlose_flag, BOOL densetsu)
{
	EV_BATTLE_WORK * ebw;
	BATTLE_PARAM *btlprm;
	FIELDSYS_WORK * fsys;
	...
	//ギラティナ　オリジンフォームセット
	{
		POKEMON_PARAM *pp;
		pp = PokeParty_GetMemberPointer(
				btlprm->poke_party[POKEPARTY_ENEMY], 0 );
	...
	*/
	...
	//破れた世界のギラティナ専用の登場エフェクトを起動する
	btlprm->battle_status_flag |= BATTLE_STATUS_FLAG_HAKAI_GIRA_BATTLE;
	/*
		持たせるアイテムの指定が必要かも？
	*/

...
#ifdef PM_DEBUG
//----
#define TW_DEBUG				//デバッグ機能有効

//#define DEBUG_TW_FOG_OFF		//定義でフォグ無効

#ifdef DEBUG_ONLY_FOR_kagaya	//kagayaのみ
#define DEBUG_TW_PRINTF			//プリント出力有効
#define DEBUG_TW_OBJ_COUNT		//OBJ数カウント
#define DEBUG_TW_BUF_OBJ_ASSERT	//OBJアサート有効
//#define DEBUG_GIRAS				//ギラティナ影デバッグ
#define DEBUG_TW_FLAG_INIT	//定義でデバッグフラグ初期化有効
#define DEBUG_TW_GIRAS_FLAG_OFF	//定義でギラ影オフ
#endif

#define DEBUG_TW_PRINTF_FORCE	//他の環境でも出したいプリント出力はこちら
//----
#endif
//----

#define TORNWORLD_PLTT_OFF
...
///	フォグ
//--------------------------------------------------------------
#define TW_FOG_OFFS		(0x76a0)	//オフセット 低い程FOG適用Z値が低くなる
#define TW_FOG_COLOR	(GX_RGB(0,0,0))
//#define TW_FOG_ALPHA	(24)		//0(2D面と融合)～31(2D面との融合無し)
#define TW_FOG_ALPHA	(16)		//0(2D面と融合)～31(2D面との融合無し)
#define TW_FOG_MAX	(72)
...
enum
{
	ZCHG_MID_Z0101_0,		//1->2
	ZCHG_MID_Z0102_0,		//2->1
	ZCHG_MID_Z0102_1,		//2->3
	ZCHG_MID_Z0103_0,		//3->2
	ZCHG_MID_Z0103_1,		//3->4
	ZCHG_MID_Z0104_0,		//4->3
	ZCHG_MID_Z0104_1,		//4->5A
	ZCHG_MID_Z0104_2,		//4->5B
	ZCHG_MID_Z0104_3,		//4->6A その0
	ZCHG_MID_Z0104_4,		//4->6A その1
	ZCHG_MID_Z0105_0,
	ZCHG_MID_Z0105_1,
	ZCHG_MID_Z0105_2,
	ZCHG_MID_Z0107_0,
...
typedef enum
{
	...
	TWSCRCODE_FLDOBJADD,
	TWSCRCODE_FLDOBJDEL,
	TWSCRCODE_TAKINOBORI,
	TWSCRCODE_SCRCALL,
	TWSCRCODE_EVSEQSET,
	TWSCRCODE_GIRAS,
	TWSCRCODE_GIRASFLAGSET,
	TWSCRCODE_TWEVFLAG_SET,
	TWSCRCODE_TAKIKUDARI,
	TWSCRCODE_MAX,
}TWSCRCODE;
...
enum
{
	...
	TWEVFLAGNO_FALLROCK0_7 = 0,			///<岩落とし0 7層に存在
	TWEVFLAGNO_FALLROCK1_7,				///<岩落とし1 7層に存在
	TWEVFLAGNO_FALLROCK2_7,				///<岩落とし2 7層に存在
	TWEVFLAGNO_FALLROCK0_8,				///<岩落とし0 8層に存在
	TWEVFLAGNO_FALLROCK1_8,				///<岩落とし1 8層に存在
	TWEVFLAGNO_FALLROCK2_8,				///<岩落とし2 8層に存在
	TWEVFLAGNO_FALLROCK0_OK,			///<岩落とし 0 OK
	TWEVFLAGNO_FALLROCK1_OK,			///<岩落とし 1 OK
	TWEVFLAGNO_FALLROCK2_OK,			///<岩落とし 2 OK
	TWEVFLAGNO_GIRA_BARK,				///<ギラティナ吠える
		
	TWEVFLAGNO_MAX,						///<最大,無効判定用
};

///岩落とし 初期配置フラグまとめ
#define TWEVFLAG_FALLROCK_7_ALL \
...
enum
{
	...
	TWFLAGTYPE_SYSWORK_EQ,		//破れた世界SYSWORK イコール
	TWFLAGTYPE_SYSWORK_LOW,		//破れた世界SYSWORK 低い
	TWFLAGTYPE_SYSWORK_HI,		//破れた世界SYSWORK 高い
...
//clact
static void tw_ClActInit( TW_CLACT_SETDATA *set, ARCHANDLE *hd );
...
//eoa cloud
static void tw_EoaCloudSet( FE_SYS *fes, TW_CLACT_SETDATA *set );
...
static void tw_DmyMapZoneChgDown( TORNWORLD_WORK *tw );
static void tw_DmyMapZoneChgUp( TORNWORLD_WORK *tw );
...
static void tw_FldOBJEventAdd( TORNWORLD_WORK *tw, u32 zone_id, u16 id );
...
void FldTornWorld_Setup( FIELDSYS_WORK *fsys )
{
	...
	tw_PlttInit( work );
	tw_FogInit( work );
	tw_ClActInit( &work->clact_set, work->arc_h_etc );
	tw_CameraInit( work );
	tw_EventInit( work );
	tw_JikiInit( work );
	tw_FldOBJInit( work );
	...
	//その他
	tw_EoaCloudSet( work->fsys->fes, &work->clact_set );
...
static void tw_SaveDataEventFlagInit( TORNWORLD_WORK *tw )
{
	...
	if( SysFlag_TornWorldRockFinish(ev,SYSFLAG_MODE_CHECK) == FALSE ){
		gmc_tw->event_flag |= TWEVFLAG_FALLROCK_7_ALL;
	}
	
#ifdef DEBUG_TW_GIRAS_FLAG_OFF
	{
		EVENTWORK *ev = SaveData_GetEventWork( tw->fsys->savedata );
...
#ifdef DEBUG_TW_FLAG_INIT	//デバッグ用初期化
	{
		...
		gmc_tw->event_flag |=
			(1<<TWEVFLAGNO_FALLROCK0_8) |
			(1<<TWEVFLAGNO_FALLROCK1_8) |
			(1<<TWEVFLAGNO_FALLROCK2_8);
		#endif
	...
	const fx32 inHeight, BOOL *outHit )
{
	...
	*outHit = FALSE;
	...
	if( twNowZoneID(tw) == ZONE_ID_D34R0110 ){
		...
		SysWork_TornWorldEventSeqNoGet( ){
		if( 15 == inGridZ && 15 == inGridX ){
			*outHit = TRUE;
			return( TRUE );
		}
	}
...
static void tw_ProcInit( TORNWORLD_WORK *work )
{
	...
	pwk->tcb_proc = TCB_Add( tw_ProcTcb, work, TW_TCBPRI_PROC );
}
...
static void tw_JikiInit( TORNWORLD_WORK *tw )
{
	...
	tw_OBJRotate_RegOBJ( tw, fldobj, rot[type] );	//地形を見て角度を決める
	...
	case 1:	//向きを壁へ向ける
		if( FieldOBJ_AcmdSetCheck(fldobj) == FALSE ){
			...
			case TW_GNDTYPE_ROOF:
				{
					int tbl[] = {DIR_DOWN,DIR_UP,DIR_LEFT,DIR_RIGHT};
					dir = tbl[dir];
				}
				break;
			}
...
static int tw_EoaCloudInit( EOA_PTR eoa, void *wk )
{
	...
	CLACT_SetScaleAffine( work->clact, &work->head.data.scale, CLACT_AFFINE_DOUBLE );
	return( TRUE );
}
...
static void tw_EoaCloudMove( EOA_PTR eoa, void *wk )
{
	...
	twAngleAdd360Fx( &work->angle, work->head.data.speed );
	angle = work->angle;
	twAngleAdd360Fx( &angle, work->head.data.offs_angle );
	
	pos = work->head.data.c_pos;
	pos.x += Cos360( FX32_NUM(work->angle) ) * work->head.data.radius;
	pos.y += Sin360( FX32_NUM(work->angle) ) * work->head.data.radius;
...
static void tw_FogInit( TORNWORLD_WORK *tw )
{
	...
	SetFogData( fog, FOG_SYS_ALL, TRUE,
		GX_FOGBLEND_COLOR_ALPHA,
		GX_FOGSLOPE_0x0200, TW_FOG_OFFS );
	...
	
	{
		int i = 0;
		char fog_tbl[32];
		do{ fog_tbl[i]=i*(TW_FOG_MAX/32); i++; }while( i < 32 );
...
static void tw_MapDataInit( TORNWORLD_WORK *tw )
{
	...
	{	//自機現地形データセット
		int gx,gy,gz;
		twJikiGridPosGet( tw, &gx, &gy, &gz );
		tw_MapDataGndDataPosDataSet( tw, gx, gy, gz );
	}
}
...
static void tw_EventSet_ZoneChange(
		TORNWORLD_WORK *tw, TW_LINKPOS_ACTOR *lact )
{
	TW_ZONECHG_WORK *work;
	
	work = tw_EventWorkInit( tw, sizeof(TW_ZONECHG_WORK) );
	work->lact = lact;
...
static BOOL tw_GMEVENT_ZoneChange( GMEVENT_CONTROL *event )
{
	PROCRET ret;
	TORNWORLD_WORK *tw = FieldEvent_GetSpecialWork( event );
	TW_ZONECHG_WORK *work = tw_EventWorkGet( tw );
 ...
 */
//--------------------------------------------------------------
static int tw_ZoneChangeMoveCount( TW_ZONECHG_WORK *work )
{
 ...
 */
//--------------------------------------------------------------
static PROCRET EvZoneChg_Init( TORNWORLD_WORK *tw, TW_ZONECHG_WORK *work )
{
	u32 zone_id;
	FIELD_OBJ_PTR fldobj;
	const TW_ZONE_LINKDATA *link;
	...
	if( work->zone_chg_type == LPOSMOVE_ZONE_DOWN ){
		...
		}else if( zone_id == ZONE_ID_D34R0108 && ev_seq_no == TWEVSEQ_07_00 ){
			work->fldobj_pair = FieldOBJSys_OBJIDSearch(
	...
	mdata_p = tw_ZoneLinkPosZoneChgMoveDataGet( work->lact->ldata.mdata_id );
 ...
 */
//--------------------------------------------------------------
static PROCRET EvZoneChg_Start( TORNWORLD_WORK *tw, TW_ZONECHG_WORK *work )
{
	int grid;
	PLAYER_STATE_PTR jiki = tw->fsys->player;
	FIELD_OBJ_PTR fldobj = Player_FieldOBJGet( jiki );
	...
	if( work->fldobj_pair != NULL ){
		...
		FieldOBJ_VecPosSet( work->fldobj_pair, &pos );
	}
 ...
 */
//--------------------------------------------------------------
static PROCRET EvZoneChg_Change( TORNWORLD_WORK *tw, TW_ZONECHG_WORK *work )
{
 ...
 */
//--------------------------------------------------------------
static PROCRET EvZoneChg_Move( TORNWORLD_WORK *tw, TW_ZONECHG_WORK *work )
{
	int grid;
	PLAYER_STATE_PTR jiki = tw->fsys->player;
	FIELD_OBJ_PTR fldobj = Player_FieldOBJGet( jiki );
 ...
 */
//--------------------------------------------------------------
static PROCRET EvZoneChg_End( TORNWORLD_WORK *tw, TW_ZONECHG_WORK *work )
{
	FIELD_OBJ_PTR fldobj = Player_FieldOBJGet( tw->fsys->player );
	FieldOBJ_NowPosGX_Set( fldobj, work->end_gx );
	FieldOBJ_NowPosGY_Set( fldobj, work->end_gy );
...
///	ゾーン切り替え処理テーブル
//--------------------------------------------------------------
static PROCRET (* const DATA_EvZoneChgProcTbl[SEQNO_ZCHG_MAX])( TORNWORLD_WORK *tw, TW_ZONECHG_WORK *work ) =
{
	EvZoneChg_Init,
	EvZoneChg_Start,
	EvZoneChg_Change,
...
//	pid &= 0x1f;
//	NNS_G3dMdlSetMdlPolygonIDAll( fstone->rmdl[type].pResMdl, pid );
}
...
static void tw_EoaLinkOBJDraw( EOA_PTR eoa, void *wk )
{
	...
	if( work->vanish == FALSE ){
		if( work->alpha_on == TRUE ){
			...
			tw_OBJResDataMdlAlphaSet( head->tw, head->objtype, work->alpha );
			FRO_OBJ_DrawPos( &work->objres_p->robj, &work->pos );
...
static PROCRET EvLiftMove_Move( TORNWORLD_WORK *tw, TW_LIFTMOVE_WORK *work )
{
	...
	FieldOBJ_VecPosSet( fldobj, &j_pos );
...
static PROCRET EvLiftMove_End( TORNWORLD_WORK *tw, TW_LIFTMOVE_WORK *work )
{
	...
	tw_MapDataGndDataPosDataSet(
		tw, work->gx_end, work->gy_end, work->gz_end );
...
{
	VecFx32 pos;
	int dir_r[DIR_4_MAX] = {0,180,90,270};
	
	EOA_GIRAS_WORK *work = wk;
	const EOA_GIRAS_HEAD *head = EOA_AddPtrGet( eoa );
	const SCRDATA_GIRAS *data_p = head->data_p;
	
	work->head = *head;
	work->vanish = TRUE;
	work->rotate.y = dir_r[data_p->dir];
...
#define GIRAS_MOVE_VALUE (FX32_ONE*48)
...
static void tw_EoaGirasActMove( EOA_PTR eoa, void *wk )
{
	...
	case 0:
		{
			VecFx32 pos;
			const VecFx32 *move = &work->head.data_p->move_val;
			...
			if( work->frame >= work->head.data_p->frame ){
				work->seq_no++;
				work->end_flag = TRUE;
				work->vanish = TRUE;
			}
...
static void tw_EoaGirasActDraw( EOA_PTR eoa, void *wk )
{
	...
	if( work->vanish == FALSE ){
		...
		FRO_OBJ_DrawRotate(
			&work->head.objres_p->robj, &pos, &work->rotate );
	}
}
...
static BOOL FallRockEvent_7_8( EV_FALLROCK_WORK *work )
{
	...
	{	//フラグ立て
		...
		case TW_OBJID_0107_ROCK_0:
			flag_on = TWEVFLAGNO_FALLROCK0_8;
			flag_off = TWEVFLAGNO_FALLROCK0_7;
			break;
		case TW_OBJID_0107_ROCK_1:
			flag_on = TWEVFLAGNO_FALLROCK1_8;
			flag_off = TWEVFLAGNO_FALLROCK1_7;
			break;
		case TW_OBJID_0107_ROCK_2:
			flag_on = TWEVFLAGNO_FALLROCK2_8;
			flag_off = TWEVFLAGNO_FALLROCK2_7;
			break;
		}
	...
	case 0:
		{
			...
			if( work->frame < FROCK_FALL_FRAME ){
				return( FALSE );
			}
			...
			case TW_OBJID_0108_ROCK_0:
				num = TWEVFLAGNO_FALLROCK0_8;
				break;
			case TW_OBJID_0108_ROCK_1:
				num = TWEVFLAGNO_FALLROCK1_8;
				break;
			case TW_OBJID_0108_ROCK_2:
				num = TWEVFLAGNO_FALLROCK2_8;
				break;
			default:
				GF_ASSERT( 0 );
			}
	...
	case 0:
		{
			...
			switch( FieldOBJ_OBJIDGet(fldobj) ){
			case TW_OBJID_0108_ROCK_0:
				on = TWEVFLAGNO_FALLROCK0_7;
				off = TWEVFLAGNO_FALLROCK0_8;
				break;
			case TW_OBJID_0108_ROCK_1:
				on = TWEVFLAGNO_FALLROCK1_7;
				off = TWEVFLAGNO_FALLROCK1_8;
				break;
			case TW_OBJID_0108_ROCK_2:
				on = TWEVFLAGNO_FALLROCK2_7;
				off = TWEVFLAGNO_FALLROCK2_8;
				break;
			default:
				GF_ASSERT( 0 );
			}
			
			tw_SaveDataEventFlagOFF( work->tw, on );
...
#define TW_BG_BLACK (RGB16(2,2,4))
...
#define TW_TOP_BOTTOM_EVY_SIZE (TW_TOP_BOTTOM_SIZE/16)
 ...
 */
//--------------------------------------------------------------
static void TakiKudari_CmActProc(
	...
	TORNWORLD_WORK *tw, GMEVENT_CONTROL *ev, u16 *seq_no, const void *data )
{
	...
	if( work->frame == 0 ){
		...
		
		{
			...
			work->np_offs_val.x = 0;
			work->np_offs_val.y = 0;
			work->np_offs_val.z = NUM_FX32( -10 ) / 64; //カメラ0と同一
		}
		...
		{	//波乗りパラメタ初期化
			{
				...
				np_offs_p->x = 0; np_offs_p->y = 0; np_offs_p->z = 0;
...
///	スクリプトテーブル　滝登り
//--------------------------------------------------------------
static const TW_SCR_PROC DATA_ScrTbl_TakiNobori[1] =
{
	NULL,
};
...
{
	TORNWORLD_WORK *tw = fsys->fldmap->Work;
	return( tw_MapDataGndDataPosExistCheck(tw,gx,gy,gz) );
}
	...
	FIELDSYS_WORK *fsys, int gx, int gy, int gz )
{
	...
	if( tw_MapDataGndDataPosExistCheck(tw,gx,gy,gz) == TRUE ){
		tw_MapDataGndDataPosDataSet( tw, gx, gy, gz );
		return;
	}
...
static BOOL twFlagCheck( TORNWORLD_WORK *tw, u32 type, u32 flag )
{
	...
	case TWFLAGTYPE_EV_ON:			//ローカルイベント ON
		if( tw_SaveDataEventFlagCheck(tw,flag) == FALSE ){
			return( FALSE );
		}
		break;
	case TWFLAGTYPE_SYSWORK_EQ:		//TWEVSEQ==flag
		if( SysWork_TornWorldEventSeqNoGet(ev) != flag ){
			return( FALSE );
		}
		break;
	case TWFLAGTYPE_SYSWORK_LOW:	//TWEVSEQ > flag
		if( SysWork_TornWorldEventSeqNoGet(ev) > flag ){
			return( FALSE );
		}
		break;
	case TWFLAGTYPE_SYSWORK_HI:		//TWEVSEQ < flag
		if( SysWork_TornWorldEventSeqNoGet(ev) < flag ){
			return( FALSE );
		}
		break;
	case TWFLAGTYPE_GIRASFLAG_OFF:	//sysflag GIRAS OFF
		if( SysFlag_TornWorldGirasFlag(
			ev,SYSFLAG_MODE_CHECK,flag) == TRUE ){
			return( FALSE );
		}
		break;
	}
...
{
	{
		CLOUD_6,NUM_FX32(0),NUM_FX32(135),104,0x1400,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y_BIG),0},
		{FX32_ONE*2,FX32_ONE*2,FX32_ONE*2}
	},
	{
		CLOUD_6,NUM_FX32(180),NUM_FX32(135),104,0x1400,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y_BIG),0},
		{FX32_ONE*2,FX32_ONE*2,FX32_ONE*2}
	},
	{
		CLOUD_6,NUM_FX32(90),NUM_FX32(135),104,0x1400,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y_BIG),0},
		{FX32_ONE*2,FX32_ONE*2,FX32_ONE*2}
	},
	{
		CLOUD_6,NUM_FX32(270),NUM_FX32(135),104,0x1400,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y_BIG),0},
		{FX32_ONE*2,FX32_ONE*2,FX32_ONE*2}
	},
	{
		CLOUD_1,NUM_FX32(0),NUM_FX32(90),60,0x1000,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y),0},
		{FX32_ONE,FX32_ONE,FX32_ONE}
	},
	{
		CLOUD_2,NUM_FX32(180),NUM_FX32(90),60,0x1000,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y),0},
		{FX32_ONE,FX32_ONE,FX32_ONE}
	},
	{
		CLOUD_3,NUM_FX32(315),NUM_FX32(90),42,0x0c00,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y),0},
		{FX32_ONE,FX32_ONE,FX32_ONE}
	},
	{
		CLOUD_4,NUM_FX32(135),NUM_FX32(90),42,0x0c00,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y),0},
		{FX32_ONE,FX32_ONE,FX32_ONE}
	},
 	{
		CLOUD_5,NUM_FX32(0),NUM_FX32(90),0,0x0800,
		{NUM_FX32(128),NUM_FX32(96+TW_CLOUD_OFFS_Y),0},
		{FX32_ONE,FX32_ONE,FX32_ONE}
	},
};
...
static const LINKPOS_DATA DATA_LinkPosZ0101_0 =		//->2
{
	0,						//id
	40, 289, 54,			//x,y,z
	ZCHG_MID_Z0101_0, LPOSMOVE_ZONE_DOWN,	//move no, move type
	0,						//ゾーンチェンジ後のID
...
static const LINKPOS_DATA DATA_LinkPosZ0102_0 =		//->1
{
	0,						//id
	40, 257, 54,			//x,y,z
	ZCHG_MID_Z0102_0, LPOSMOVE_ZONE_UP,	//move no, move type
	0,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP3A,		//draw obj
	ZLINKMFLAG_0102,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0102_1 =		//->3
{
	1,						//id
	33, 257, 45,			//x,y,z
	ZCHG_MID_Z0102_1, LPOSMOVE_ZONE_DOWN,	//move no, move type
	0,						//ゾーンチェンジ後のID
...
static const LINKPOS_DATA DATA_LinkPosZ0103_0 =		//->2
{
	...
	33, 225, 45,			//x,y,z
	ZCHG_MID_Z0103_0, LPOSMOVE_ZONE_UP,	//move no, move type
	1,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP3B,		//draw obj
	ZLINKMFLAG_0203,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0103_8 =		//lift 6
{
	...
	65, 225, 31,			//x,y,z
	ZCHG_MID_Z0103_1, LPOSMOVE_ZONE_DOWN,	//move no, move type
	0,						//ゾーンチェンジ後のID
...
static const LINKPOS_DATA DATA_LinkPosZ0104_0 =		//->3
{
	0,						//id
	65, 193, 31,			//x,y,z
	ZCHG_MID_Z0104_0, LPOSMOVE_ZONE_UP,	//move no, move type
...
static const LINKPOS_DATA DATA_LinkPosZ0104_1 =		//4A->5A
{
	1,						//id
	79, 193, 62,			//x,y,z
	ZCHG_MID_Z0104_1, LPOSMOVE_ZONE_DOWN,	//move no, move type
	0,						//ゾーンチェンジ後のID
...
static const LINKPOS_DATA DATA_LinkPosZ0104_2 =		//4B->5B
{
	2,						//id
	95, 193, 70,			//x,y,z
	ZCHG_MID_Z0104_2, LPOSMOVE_ZONE_DOWN,	//move no, move type
	1,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP8,		//draw obj
	ZLINKMFLAG_MAX,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0104_3 =		//4A->6A
{
	3,						//id
	96, 193, 43,			//x,y,z
	ZCHG_MID_Z0104_3, LPOSMOVE_ZONE_DOWN,	//move no, move type
	2,						//ゾーンチェンジ後のID
...
static const LINKPOS_DATA DATA_LinkPosZ0105_0 =		//5A->4A
{
	0,						//id
	79, 161, 62,			//x,y,z
	ZCHG_MID_Z0105_0, LPOSMOVE_ZONE_UP,	//move no, move type
	1,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP8,		//draw obj
	ZLINKMFLAG_0405A,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0105_1 =		//5B->4A
{
	1,						//id
	95, 161, 70,			//x,y,z
	ZCHG_MID_Z0105_1, LPOSMOVE_ZONE_UP,	//move no, move type
	2,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP8,		//draw obj
	ZLINKMFLAG_0405B,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0105_2 =		//->6
{
	2,						//id
	78, 161, 77,			//x,y,z
	ZCHG_MID_Z0105_2, LPOSMOVE_ZONE_DOWN,	//move no, move type
	0,						//ゾーンチェンジ後のID
...
static const LINKPOS_DATA DATA_LinkPosZ0107_0 =		//->5
{
	0,						//id
	78, 129, 77,			//x,y,z
	ZCHG_MID_Z0107_0, LPOSMOVE_ZONE_UP,	//move no, move type
	2,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP15,		//draw obj
	ZLINKMFLAG_0506B,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0107_1 =		//->7
{
	1,						//id
	87, 129, 67,			//x,y,z
	ZCHG_MID_Z0107_1, LPOSMOVE_ZONE_DOWN,	//move no, move type
	0,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP15,		//draw obj
	ZLINKMFLAG_MAX,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0107_2 =		//->4
{
	2,						//id
	96, 129, 43,			//x,y,z
	ZCHG_MID_Z0107_2, LPOSMOVE_ZONE_UP,	//move no, move type
	3,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP7,		//draw obj
	ZLINKMFLAG_0406A,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0108_0 =		//->6
{
	0,						//id
	87, 115, 67,			//x,y,z
	ZCHG_MID_Z0108_0, LPOSMOVE_ZONE_UP,	//move no, move type
	1,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP15,		//draw obj
	ZLINKMFLAG_0607,
};
...
static const LINKPOS_DATA DATA_LinkPosZ0108_1 =		//->8
{
	1,						//id
	85, 115, 86,			//x,y,z
	ZCHG_MID_Z0108_1, LPOSMOVE_ZONE_DOWN,	//move no, move type
	0,						//ゾーンチェンジ後のID
...
static const LINKPOS_DATA DATA_LinkPosZ0109_0 =		//->7
{
	0,					//id
	85, 65, 86,			//x,y,z
	ZCHG_MID_Z0109_0,LPOSMOVE_ZONE_UP,	//move no, move type
	1,						//ゾーンチェンジ後のID
	TWOBJTYPE_STEP18,		//draw obj
	ZLINKMFLAG_0708,
};
...
{
 {
	 ZCHG_MID_Z0101_0,ZCHG_MID_MAX,0,-32,0, 0,-14,0, {0,NUM_FX32(-4),0},
	 ZLINKMFLAG_0102,ZLINKMFLAG_MAX,
 },
 {
	 ZCHG_MID_Z0102_0,ZCHG_MID_MAX,0,32,0, 0,18,0, {0,NUM_FX32(4),0},
	 ZLINKMFLAG_MAX,ZLINKMFLAG_0102,
 },
 {
	 ZCHG_MID_Z0102_1,ZCHG_MID_MAX,0,-32,0, 0,-14,0, {0,NUM_FX32(-4),0},
	 ZLINKMFLAG_0203,ZLINKMFLAG_MAX,
 },
 {
	 ZCHG_MID_Z0103_0,ZCHG_MID_MAX,0,32,0, 0,18,0, {0,NUM_FX32(4),0},
	 ZLINKMFLAG_MAX,ZLINKMFLAG_0203,
 },
 {
	 ZCHG_MID_Z0103_1,ZCHG_MID_MAX,0,-32,0, 0,-14,0, {0,NUM_FX32(-4),0},
	 ZLINKMFLAG_0304,ZLINKMFLAG_MAX,
 },
 {
	 ZCHG_MID_Z0104_0,ZCHG_MID_MAX,0,32,0, 0,18,0, {0,NUM_FX32(4),0},
	 ZLINKMFLAG_MAX,ZLINKMFLAG_0304,
 },
 {
	 ZCHG_MID_Z0104_1,ZCHG_MID_MAX,0,-32,0, 0,-14,0, {0,NUM_FX32(-4),0},
	 ZLINKMFLAG_0405A,ZLINKMFLAG_MAX,
 },
 {
	 ZCHG_MID_Z0104_2,ZCHG_MID_MAX,0,-32,0, 0,-14,0, {0,NUM_FX32(-4),0},
	 ZLINKMFLAG_0405B,ZLINKMFLAG_MAX,
 },
 {
	ZCHG_MID_Z0104_3,ZCHG_MID_Z0104_4,0,-32,0, 0,-14,0, {0,NUM_FX32(-4),0},
	ZLINKMFLAG_0406A,ZLINKMFLAG_MAX,
 },
 {
	ZCHG_MID_Z0104_4,ZCHG_MID_MAX,0,-32,0, 0,-14,0, {0,NUM_FX32(-4),0},
	ZLINKMFLAG_MAX,ZLINKMFLAG_MAX,
 },
 {
	ZCHG_MID_Z0105_0,ZCHG_MID_MAX,0,32,0, 0,18,0, {0,NUM_FX32(4),0},
	ZLINKMFLAG_MAX,ZLINKMFLAG_0405A,
 },
 {
	ZCHG_MID_Z0105_1,ZCHG_MID_MAX,0,32,0, 0,18,0, {0,NUM_FX32(4),0},
	ZLINKMFLAG_MAX,ZLINKMFLAG_0405B,
 },
 {
	ZCHG_MID_Z0105_2,ZCHG_MID_MAX,0,-32,0, 0,-14,0, {0,NUM_FX32(-4),0},
	ZLINKMFLAG_0506B,ZLINKMFLAG_MAX,
 },
 {
	ZCHG_MID_Z0107_0,ZCHG_MID_MAX,0,32,0, 0,18,0, {0,NUM_FX32(4),0},
	ZLINKMFLAG_MAX,ZLINKMFLAG_0506B,
 },
 {
	ZCHG_MID_Z0107_1,ZCHG_MID_MAX,0,-14,0, 0,-12,0, {0,NUM_FX32(-4),0},
	ZLINKMFLAG_0607,ZLINKMFLAG_MAX,
 },
 {
	ZCHG_MID_Z0107_2,ZCHG_MID_Z0107_3,0,32,0, 0,18,0, {0,NUM_FX32(4),0},
	ZLINKMFLAG_MAX,ZLINKMFLAG_0406A,
 },
 {
	ZCHG_MID_Z0107_3,ZCHG_MID_MAX,0,32,0, 0,18,0, {0,NUM_FX32(4),0},
	ZLINKMFLAG_MAX,ZLINKMFLAG_MAX,
 },
 {
	ZCHG_MID_Z0108_0,ZCHG_MID_MAX,0,14,0, 0,2,0, {0,NUM_FX32(4),0},
	ZLINKMFLAG_MAX,ZLINKMFLAG_0607,
 },
 {
	ZCHG_MID_Z0108_1,ZCHG_MID_MAX,0,-50,0, 0,-17,0, {0,NUM_FX32(-4),0},
	ZLINKMFLAG_0708,ZLINKMFLAG_MAX,
 },
 {
	ZCHG_MID_Z0109_0,ZCHG_MID_MAX,0,50,0, 0,18,0, {0,NUM_FX32(4),0},
...
{
	{40,289,52,TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_01_01,DATA_Scr0101_00},
	{0,0,0,0,NULL},
};
...
{SCRID_D34R0102_MES01};
static const SCRDATA_EVSEQ DataScr0102_01 =
{TWEVSEQ_03_00};
static const SCRDATA_FLDOBJADD DataScr0102_02 =
...
{
	{15,257,58,TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_02_00,DATA_Scr0102_00},
	{0,0,0,0,NULL},
};
...
{LPOSID0103_LIFT201,FALSE,HEROTWTYPE_GROUND,9,0,22,{NUM_FX32(4),0,0}};
...
static const SCRPOSDATA DATA_ScrPos0103[] =
{
	...
	{63,225,31,TWFLAGTYPE_NON,0,DATA_Scr0103_Lift09},	//8-5-5
	...
	{38,233,31,TWFLAGTYPE_NON,0,Data_Scr0103_2_Lift13},	//2 9-8-8
...
{
	{65,193,41,TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_03_01,DATA_Scr0104_0},
	{0,0,0,0,NULL},
};
...
{79,161+4,64-24,DIR_DOWN,{0,0,GIRAS_MOVE_VALUE},32};
...
{98-24,161-24,56-24,DIR_RIGHT,{GIRAS_MOVE_VALUE,0,0},32};
...
static const SCRPOSDATA DATA_ScrPos0105[] =
{
	...
	{78,161,64,TWFLAGTYPE_GIRASFLAG_OFF,GIRASFLAG_0500,DATA_Scr0105_Giras00},
	{79,161,64,TWFLAGTYPE_GIRASFLAG_OFF,GIRASFLAG_0500,DATA_Scr0105_Giras00},
...
{ LPOSMOVE_ZONE_UP, 0,42,0, 0,20,0, {0,NUM_FX32(4),0} };

static const TW_SCRDATA Data_Scr0107_0[] =
{
...
{
	{104,128,75,TWFLAGTYPE_NON,0,Data_Scr0107_0},	//0
	{104,128,76,TWFLAGTYPE_NON,0,Data_Scr0107_0},
	{104,128,77,TWFLAGTYPE_NON,0,Data_Scr0107_0},
	{104,128,78,TWFLAGTYPE_NON,0,Data_Scr0107_0},
	{105,128,79,TWFLAGTYPE_NON,0,Data_Scr0107_0},
	{0,0,0,0,NULL}, //終端
};
#endif
...
{
	{84,65,75,TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_08_00,DATA_Scr0109_0},
	{85,65,75,TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_08_00,DATA_Scr0109_0},
	{86,65,75,TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_08_00,DATA_Scr0109_0},
//	{89,65,57,TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_08_02,DATA_Scr0109_1},
	{0,0,0,0,NULL},
};
...
///	ギラティナ吠えるスクリプト呼び出し
//--------------------------------------------------------------
static const SCRDATA_SCRCALL DataScr0110_00 =
{SCRID_D34R0110_GIRA_BARK};
static const SCRDATA_TWEVFLAG DataScr0110_01 =
{TWEVFLAGNO_GIRA_BARK};

static const TW_SCRDATA DATA_Scr0110_0[] =
{
	{TWSCRCODE_SCRCALL,&DataScr0110_00},
	{TWSCRCODE_TWEVFLAG_SET,&DataScr0110_01},
	{TWSCRCODE_MAX,NULL},
};

static const SCRPOSDATA DATA_ScrPos0110[] =
{
	{15,1,24,TWFLAGTYPE_EV_OFF,TWEVFLAGNO_GIRA_BARK,DATA_Scr0110_0},
	{0,0,0,0,NULL},
};
...
{
	{0,TWOBJTYPE_ANA_OUT,15,1,13,TWFLAGTYPE_SYSWORK_HI,TWEVSEQ_08_02},
	{0,TWOBJTYPE_MAX,0,0,0,0,0},
};
...
{
	TWFLAGTYPE_SYSWORK_LOW,TWEVSEQ_01_02,
	TWFLDOBJ_ROT_OFF,0,
	{
...
{
	TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_03_00,
	TWFLDOBJ_ROT_ON,90,
	{
...
{
	TWFLAGTYPE_SYSWORK_EQ,TWEVSEQ_03_01,
	TWFLDOBJ_ROT_ON,90,
	{
		TW_OBJID_0103_SIRONA,
		CHAMPION,
...
///	第八階層
//--------------------------------------------------------------
static const TW_FLDOBJ_HEAD DATA_FldOBJHead_0109_0 =	//シロナ
{
	TWFLAGTYPE_SYSWORK_EQ,
	TWEVSEQ_08_00,
	TWFLDOBJ_ROT_OFF,
...
static const TW_FLDOBJ_HEAD DATA_FldOBJHead_0109_1 =	//シロナその２
{
	TWFLAGTYPE_SYSWORK_EQ,
	TWEVSEQ_08_02,
	TWFLDOBJ_ROT_OFF,
...
static const TW_FLDOBJ_HEAD DATA_FldOBJHead_0109_2 =	//アカギ
{
	TWFLAGTYPE_SYSWORK_LOW,
	TWEVSEQ_08_00,
	TWFLDOBJ_ROT_OFF,
	0,
	{

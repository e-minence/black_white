...
static void Debug_frontier_jump(TCB_PTR tcb, void* work)
{
	FLD_DEB_DAT	* wk = (FLD_DEB_DAT *)work;
	FieldDebugExit(tcb,work);
	EventSet_EasyMapChange(wk->fsys, ZONE_ID_D32, DOOR_ID_JUMP_CODE, 48, 36, DIR_DOWN);
}
...
static void Debug_btower_jump(TCB_PTR tcb, void* work)
{
	FLD_DEB_DAT	* wk = (FLD_DEB_DAT *)work;
	FieldDebugExit(tcb,work);
	EventSet_EasyMapChange(wk->fsys, ZONE_ID_D31R0201, DOOR_ID_JUMP_CODE, 15, 6, DIR_UP);
}
...
static void Debug_bfactory_jump(TCB_PTR tcb, void* work)
{
	FLD_DEB_DAT	* wk = (FLD_DEB_DAT *)work;
	FieldDebugExit(tcb,work);
	EventSet_EasyMapChange(wk->fsys, ZONE_ID_D32R0301, DOOR_ID_JUMP_CODE, 21, 7, DIR_UP);
}
...
static void Debug_bstage_jump(TCB_PTR tcb, void* work)
{
	FLD_DEB_DAT	* wk = (FLD_DEB_DAT *)work;
	FieldDebugExit(tcb,work);
	EventSet_EasyMapChange(wk->fsys, ZONE_ID_D32R0401, DOOR_ID_JUMP_CODE, 7, 9, DIR_LEFT);
}
...
static void Debug_bcastle_jump(TCB_PTR tcb, void* work)
{
	FLD_DEB_DAT	* wk = (FLD_DEB_DAT *)work;
	FieldDebugExit(tcb,work);
	EventSet_EasyMapChange(wk->fsys, ZONE_ID_D32R0501, DOOR_ID_JUMP_CODE, 6, 9, DIR_LEFT);
}
...
static void Debug_broulette_jump(TCB_PTR tcb, void* work)
{
	FLD_DEB_DAT	* wk = (FLD_DEB_DAT *)work;
	FieldDebugExit(tcb,work);
	EventSet_EasyMapChange(wk->fsys, ZONE_ID_D32R0601, DOOR_ID_JUMP_CODE, 14, 6, DIR_UP);
}
...
static void Debug_pcug_jump(TCB_PTR tcb, void* work)
{
	FLD_DEB_DAT	* wk = (FLD_DEB_DAT *)work;
	FieldDebugExit(tcb,work);
	EventSet_EasyMapChange(wk->fsys, ZONE_ID_C01PC0103, DOOR_ID_JUMP_CODE, 9, 6, DIR_UP);
}
...
static void Debug_staff_sogabe(TCB_PTR tcb,void* work)
{
	...
	DebugBattleFlag^=DBF_SPEEDUP;

	if(DebugBattleFlag&DBF_SPEEDUP){
		Snd_SePlay(SEQ_SE_DP_PINPON);
	}
	else{
...
static void DebugMapJumpPosTornWorld( u32 zone_id, u16 *x, u16 *z )
{
	...
	for( i = 0; i < DEBUG_DATA_FIELD_TORNWORLD_ZONE_BLOCK_MAX; i++ ){
		if( DEBUG_DATA_FieldtornWorldZoneBlockPosTable[i][0] == zone_id ){
			if( zone_id == ZONE_ID_D34R0101 ){
				*x = 55;
				*z = 40;
			}else{
				*x += DEBUG_DATA_FieldtornWorldZoneBlockPosTable[i][1];
				*z += DEBUG_DATA_FieldtornWorldZoneBlockPosTable[i][3];
			}
			return;
		}
	}
...
static void FieldDebugListJumpMenuTask(TCB_PTR tcb,void* work)
{
	...
	if(sys.trg & PAD_BUTTON_A){
		if(ret_code){
			...
			x = x * 32 + 16;
			z = z * 32 + 16;
			DebugMapJumpPosTornWorld( ret_code, &x, &z );
			EventSet_EasyMapChange(wp->fsys,
				ret_code, DOOR_ID_JUMP_CODE, x, z, DIR_DOWN);
#endif
		}
		FieldDebugExit(tcb,work);
		return;
...
static const GMM_MENU_PARAM FieldDebugListJumpMenuHakai[] = {
	{ msg_label_listjump_hakai,	BMPLIST_RABEL},			//ラベル01
	{ msg_d34_00_00,	ZONE_ID_D34R0101},	//
	{ msg_d34_00_01,	ZONE_ID_D34R0102},	//
	{ msg_d34_00_02,	ZONE_ID_D34R0103},	//
	{ msg_d34_00_03,	ZONE_ID_D34R0104},	//
	{ msg_d34_00_04,	ZONE_ID_D34R0105},	//
//	{ msg_d34_00_05,	ZONE_ID_D34R0106},	//
	{ msg_d34_00_06,	ZONE_ID_D34R0107},	//
	{ msg_d34_00_07,	ZONE_ID_D34R0108},	//
...
static void Debug_yaburew_jump(TCB_PTR tcb, void* work)
{
	...
	FLD_DEB_DAT	* wk = (FLD_DEB_DAT *)work;
	FieldDebugExit(tcb,work);
	EventSet_EasyMapChange(wk->fsys, ZONE_ID_D34R0111, DOOR_ID_JUMP_CODE, 16, 16, DIR_UP);
#else
	FLD_DEB_DAT * wp = (FLD_DEB_DAT *)work;
	FIELDSYS_WORK * fsys = wp->fsys;
	FieldDebugExitCall(tcb, work);
	...
	wp = FieldDebugListCreateGMM(fsys, NARC_msg_debug_list_jump_dat,
			FieldDebugListJumpMenuHakai, NELEMS(FieldDebugListJumpMenuHakai),
...
static const GMM_MENU_PARAM FieldDebugListTop[] = {
	...
	{DF_BFRONTIER,(u32)Debug_frontier_jump},
	{DF_BTOWER,(u32)Debug_btower_jump},
	{DF_BFACTORY,(u32)Debug_bfactory_jump},
	{DF_BSTAGE,(u32)Debug_bstage_jump},
	{DF_BCASTLE,(u32)Debug_bcastle_jump},
	{DF_BROULETTE,(u32)Debug_broulette_jump},
	{DF_PCUG,(u32)Debug_pcug_jump},
	{DF_YABUREW,(u32)Debug_yaburew_jump},
...
//--------------------------------------------------------------------
//--------------------------------------------------------------------
static void Debug_flag_haihu04_on(TCB_PTR tcb, void * work)
{
	HaihuOnCommon(work, HAIHUEVENT_ID_C04, 0);
}
//--------------------------------------------------------------------
//--------------------------------------------------------------------
static void Debug_flag_haihu04_off(TCB_PTR tcb, void * work)
{
	HaihuOffCommon(work, HAIHUEVENT_ID_C04, 0);
}
...
//	{DF_FLAG_GAMECLEAR_OFF,	(u32)Debug_flag_gameclear_off},
	...
	{DF_FLAG_HAIHU04_ON,	(u32)Debug_flag_haihu04_on},
	{DF_FLAG_HAIHU04_OFF,	(u32)Debug_flag_haihu04_off},
};
...
void DebugBgPrint(GF_BGL_INI *ini, FIELDSYS_WORK *fsys)
{
	...
	#else
	{
		...
		int gy = H_GRID_G_GRID( FieldOBJ_NowPosGY_Get(fldobj) );
		int gz = FieldOBJ_NowPosGZ_Get( fldobj );
		sprintf(str,"GRID X = %03d, Z = %03d, Y = %03d  ", gx, gz, gy );
		DebugStrPut(ini, str,1,1);
	}
	#endif
...
//-----------------------------------------------------------------------------
/**
 *	ここから		2007.10.25	銃中領域を増やすために　gflib/fntsysから移動
 */
//-----------------------------------------------------------------------------
//===============================================
...
//-----------------------------------------------------------------------------
/**
 *			ここまで　2007.10.25	銃中領域を増やすために　gflib/fntsysから移動
 */
//-----------------------------------------------------------------------------

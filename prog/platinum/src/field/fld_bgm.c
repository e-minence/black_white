...
u16 Snd_FieldBgmNoGet( FIELDSYS_WORK* fsys, int zone_id )
{
	...
	if( status == HERO_FORM_SWIM ){
		...
		case ZONE_ID_D34R0103:
		case ZONE_ID_D34R0104:
		case ZONE_ID_D34R0105:
		case ZONE_ID_D34R0107:
		case ZONE_ID_D34R0108:
		case ZONE_ID_D34R0109:
		case ZONE_ID_D34R0110:
		case ZONE_ID_D34R0111:
			break;
		default:
			return SEQ_NAMINORI;
		}
...
static u16 Snd_CyclingBgmNoGet( FIELDSYS_WORK* fsys, int zone_id )
{
	...
	if( (before->zone_id == ZONE_ID_C04R0401) || (before->zone_id == ZONE_ID_R206R0101) ){

		if( x < SCR_CYCLING_X_L ){
			return 0;
		}

		if( x > SCR_CYCLING_X_R ){
			return 0;
		}

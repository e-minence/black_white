...
static void RockRide_GraphicInit( FE_ROCKRIDE_PTR rockride )
{
	if( rockride->draw_init == FALSE ){
		FE_FROMdl_ResSetArcLoadTexTrans(
			rockride->fes, &rockride->rmdl, 0, NARC_fldeff_rock_ride_nsbmd, ALLOC_LO );
		FRO_OBJ_InitInMdl( &rockride->robj, &rockride->rmdl );
...
static void RockRide_GraphicDelete( FE_ROCKRIDE_PTR rockride )
{
	if( rockride->draw_init == TRUE ){
		...
		FRO_MDL_DeleteAll( &rockride->rmdl );
	}
}
...
static void EoaRockRide_Draw( EOA_PTR eoa, void *wk )
{
	...
	
	{
		u32 y = 0;
		MtxFx33 rot;
		VecFx32 pos,scale = { FX32_ONE, FX32_ONE, FX32_ONE };
		FRO_OBJ *robj = &work->head.rockride->robj;
		
		switch( work->dir ){
		case DIR_UP:	y = 180; break;
		case DIR_DOWN:	break;
		case DIR_LEFT:	y = 270; break;
		case DIR_RIGHT:	y = 90; break;
		}
		
		Rot360_to_MtxFx33( &rot, 0, y, 0 );

...
typedef const fx32 (*GET_HEIGHT_FUNC)(	const FIELDSYS_WORK *fsys,
		const fx32 inNowHeight, const fx32 inX, const fx32 inZ,	u8 *outCode);
 ...
 * @param	inNowHeight	現在高さ
 * @param	inX			Ｘ座標
 * @param	inZ			Ｚ座標
 * @param	outCode		高さタイプ（通常・拡張）
 *
 * @retval  fx32		取得高さ
 */
//==============================================================================
static const fx32 GetGroundMapHeight(
			const FIELDSYS_WORK *fsys,
			const fx32 inNowHeight, const fx32 inX, const fx32 inZ,
			u8 *outCode)
{
	...
	VecFx32 org_pos;
	DMC_CONST_PTR dmc = fsys->map_cont_dat;
	GetDivMapOriginPos( dmc, &org_pos );
	inHeight = inNowHeight - org_pos.y;
	
	ex_rc = FALSE;
	normal_rc = FALSE;
	
	height = 0;
	target.x =inX - org_pos.x;
	target.z =inZ - org_pos.z;
	target.y =0;
	
	{
		...
		if (local_index>3){
			...
			//高さ取得
			{
				...
				normal_rc = GetHeightForBlock(
					inHeight, target.x, target.z, height_info, &target.y);
			}
		}
	}
...
///				ioPos->y = target.y;
				...
				sub1 = GetSubVal(target.y, inHeight);	//通常高さと現在値の差
				sub2 = GetSubVal(ex_height, inHeight);	//拡張高さと現在値の差
 ...
 * @param	inX			グリッドX座標
 * @param	inZ			グリッドZ座標
 * @param	outAttr		アトリビュート
 *
 * @retval  BOOL		TRUE:取得成功		FALSE:取得失敗
 */
//==============================================================================
static BOOL GetGroundAttribute(
	const FIELDSYS_WORK *fsys,
	const int inX, const int inZ, u16 *outAttr )
{
	BOOL rc;
	u32 block_x,block_z;
	u32 block_index;
	...
	inGridX = inX - GetDivMapOriginGridX( map_data_ptr );
	inGridZ = inZ - GetDivMapOriginGridZ( map_data_ptr );
 ...
 * マップツールリストのセット
 *
 * @param   outMapToolList	マップツールリストポインタ
 * @param	inMapMode		マップモード
 *
 * @retval  none
 */
//==============================================================================
 ...
 * @param	inGridX		指定グリッドX座標
 * @param	inGridZ		指定グリッドZ座標
 * @param	inOfsX		基点からのオフセット　矩形X座標（左開始）
 * @param	inOfsZ		基点からのオフセット　矩形Z座標（奥開始）
 * @param	inW			矩形横幅
 * @param	inH			矩形縦幅
 * @param	outRect		矩形データ
 *

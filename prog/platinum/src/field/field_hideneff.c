...
static void FHE_objDirGridMatSet( FIELD_OBJ_SYS_PTR fldobjsys, u32 objID, D3DOBJ* p_obj )
{
	...
	D3DOBJ_SetMatrix( p_obj, GRID_SIZE_FX32(grid_x)+GRID_HALF_FX32, pos.y, GRID_SIZE_FX32(grid_z)+GRID_HALF_FX32 );
}
...
/*	int grid_x, grid_z, grid_y;
	...
	D3DOBJ_SetMatrix( p_obj, GRID_SIZE_FX32(grid_x)+GRID_HALF_FX32, 
...
typedef struct {
	D3DOBJ		obj;
	D3DOBJ_MDL	mdl;
	D3DOBJ_ANM	anm[FHE_COMMON_ANM_NUM];
	u32				anm_num;
} FHE_COMMON_OBJ;
...
static BOOL Fhe_CommonObj_NoLoopAnime( FHE_COMMON_OBJ* p_obj )
{
	...
	check = TRUE;
	for( i=0; i<p_obj->anm_num; i++ ){
		check &= D3DOBJ_AnmNoLoop( &p_obj->anm[i], FX32_ONE );
	}
	return check;
}
...
static void Fhe_CommonObj_LoopAnime( FHE_COMMON_OBJ* p_obj )
{
	...
	for( i=0; i<p_obj->anm_num; i++ ){
		D3DOBJ_AnmLoop( &p_obj->anm[i], FX32_ONE );
	}
}
...
typedef struct {
	D3DOBJ		obj;
	D3DOBJ_ANM	anm[FHE_ROCKCLIMB_ANM_NUM];
	u32 status;
...
static void FHE_Iaigiri_Main(FLDMAPFUNC_WORK * p_fwk, FIELDSYS_WORK * p_fsys, void * p_work)
{
	...
	case FHE_IAIGIRI_SEQ_MOVE:
		...
		if( check == TRUE ){
			D3DOBJ_SetDraw( &p_fhe_w->obj.obj, FALSE );
			p_fhe_w->seq ++;
		}
		break;
...
#else
	Fhe_CommonObjInit( &p_fhe_w->obj[FHE_TAKINOBORI_OBJ_START], p_handle, 
...
static void FHE_Takinobori_Main(FLDMAPFUNC_WORK * p_fwk, FIELDSYS_WORK * p_fsys, void * p_work)
{
	...
	switch( p_fhe_w->seq ){
	case FHE_TAKINOBORI_SEQ_LOADSEQ:	// もうひとつのモデル読み込み　１つのテクスチャが多きため分割して読み込む
		p_handle = ArchiveDataHandleOpen( ARC_HIDEN_EFFECT, HEAPID_FIELD );
	...
	case FHE_TAKINOBORI_SEQ_START:
		...
		FHE_playerVecMatSet( p_fsys->player, &p_fhe_w->obj[FHE_TAKINOBORI_OBJ_START].obj );
		if( check == TRUE ){
			D3DOBJ_SetDraw( &p_fhe_w->obj[ FHE_TAKINOBORI_OBJ_LOOP ].obj, TRUE );
			D3DOBJ_SetDraw( &p_fhe_w->obj[ FHE_TAKINOBORI_OBJ_START ].obj, FALSE );
...
static void Fhe_RockClime_ObjMain( FLD_HIDENEFF_ROCKCLIMB_OBJ* p_obj )
{
	...
	result = TRUE;
	for( i=0; i<FHE_ROCKCLIMB_ANM_NUM; i++ ){
		result &= D3DOBJ_AnmNoLoop( &p_obj->anm[i], FX32_ONE );
	}
...
static void Fhe_RockClime_ObjInit( FLD_HIDENEFF_ROCKCLIMB_OBJ* p_obj, D3DOBJ_MDL* p_mdl, NNSFndAllocator* p_allocator, void** pp_anmbuff )
{
	...
	D3DOBJ_Init( &p_obj->obj, p_mdl );
	for( i=0; i<FHE_ROCKCLIMB_ANM_NUM; i++ ){
		D3DOBJ_AnmLoad_Data( &p_obj->anm[i], p_mdl, pp_anmbuff[i],
				p_allocator );
		D3DOBJ_AddAnm( &p_obj->obj, &p_obj->anm[i] );
	}

...
#include "src/fielddata/eventdata/zone_d05r0115evc.h"
...
#define FLDMAP_DEMO_LEGEND_OBJID	( 0 )	// 出現させるイアエアの管理ID
//		このOBJIDがなくなったので"0"にしました。(08.03.27)

#define FLDMAP_DEMO_LEGEND_EFFSTART	( 240 )		// エフェクト開始
#define FLDMAP_DEMO_BOSS_OBJID		( D05R0114_GINGABOSS )
...
static BOOL FldDemo_KusariAnime( FLD_DEMO_LEGENDPOKE_APP* p_data );
static BOOL FldDemo_RedFade( FLD_DEMO_LEGENDPOKE_APP* p_data, int type, int mode );
...
static void FLDMAP_DEMO_LegendPokeApp_Main(FLDMAPFUNC_WORK * p_fwk, FIELDSYS_WORK * p_fsys, void * p_work)
{
	...
	case FLDMAP_DEMO_LEGEND_START:
		...
		p_lgd->count = 0;//FLDMAP_DEMO_LEGEND_EFFSTART;
		p_lgd->wait = 0;
		p_lgd->seq ++;
		break;
	...
	case FLDMAP_DEMO_LEGEND_EFF_WAIT:
		{
			...
			if ( p_lgd->count != 12 ){
				if ( (++p_lgd->wait) >= 8 ){
					result = FldDemo_RedFade( p_lgd->p_legend, p_lgd->count % 2, p_lgd->count );			
					if ( result ){
						p_lgd->wait = 0;
						p_lgd->count++;
					}
			...
			if ( FldDemo_KusariAnime( p_lgd->p_legend ) && result ){
				p_lgd->wait = 0;
				p_lgd->seq = FLDMAP_DEMO_LEGEND_END_WAIT;
			}
		}
		break;

	case FLDMAP_DEMO_LEGEND_END_WAIT:
		if ( FldDemo_RedFade( p_lgd->p_legend, 0, 60 ) ){
			p_lgd->wait = 0;
			p_lgd->seq = FLDMAP_DEMO_LEGEND_END;
		}
		break;
...
#define FLDMAP_DEMO_BLACKRING_OBJID	( 0 )	// 出現させるイアエアの管理ID
//		このOBJIDがなくなったので"0"にしました。(08.03.27)
...
static void FLDMAP_DEMO_AntEggCut01_Init(FLDMAPFUNC_WORK * p_fwk, FIELDSYS_WORK * p_fsys, void * p_work)
{
	...
	p_fsys->light_cont_data = InitLightCont( p_fsys->glst_data, LIGHT_TYPEIAEA );
...
//	FLD_DEMO_DARKLIGHT* p_dark;	// ダークライト
    ...
    FLD_DEMO_MOVE_PARAM_FX	move_z;
    
    FIELDSYS_WORK*			fsys;
    FIELD_OBJ_PTR 			fldobj;
    FIELD_OBJ_PTR 			fldobj_s;
    FIELD_OBJ_PTR 			fldobj_p;
    PLAYER_STATE_PTR		player;
	
} FLDMAP_DEMO_GINGA;

#define FLDMAP_DEMO_POKE_M_ID	( D05R0115_SPPOKE2 )	// 出現させるイアエアの管理ID

static void DEMO_UMA_Load( FLDMAP_DEMO_GINGA* wk )
{
	int mdl_id;
	int anm_id;
	ARCHANDLE* hdl;
	...
	static const int data[ ][ 2 ] = {
		{ NARC_demo_tengan_gra_demo_tama_u_nsbmd, NARC_demo_tengan_gra_demo_tama_u_nsbca },
		{ NARC_demo_tengan_gra_demo_tama_a_nsbmd, NARC_demo_tengan_gra_demo_tama_a_nsbca },
		{ NARC_demo_tengan_gra_demo_tama_m_nsbmd, NARC_demo_tengan_gra_demo_tama_m_nsbca },
	};
	
	hdl = ArchiveDataHandleOpen( ARC_DEMO_TENGAN_GRA, USE_HEAPID );
	
	mdl_id = data[ wk->poke_type ][ 0 ];
	anm_id = data[ wk->poke_type ][ 1 ];
	
	D3DOBJ_MdlLoadH( &wk->mdl, hdl, mdl_id, USE_HEAPID );
	D3DOBJ_Init( &wk->obj, &wk->mdl );
	D3DOBJ_SetMatrix( &wk->obj, 0, 0, 0 );
    D3DOBJ_SetScale( &wk->obj, FX32_CONST(1.00f), FX32_CONST(1.00f), FX32_CONST(1.00f) );
    D3DOBJ_SetDraw( &wk->obj, TRUE );

	D3DOBJ_AnmLoadH( &wk->anm, &wk->mdl, hdl, anm_id, USE_HEAPID, &wk->allocator );
	D3DOBJ_AnmSet( &wk->anm, 0 );
	D3DOBJ_AddAnm( &wk->obj, &wk->anm );	
	
	ArchiveDataHandleClose( hdl );
}

static void DEMO_UMA_Delete( FLDMAP_DEMO_GINGA* wk )
{
	D3DOBJ_MdlDelete( &wk->mdl );	
	D3DOBJ_AnmDelete( &wk->anm, &wk->allocator );
}
...
static void FLDMAP_DEMO_Ginga_Init(FLDMAPFUNC_WORK * p_fwk, FIELDSYS_WORK * p_fsys, void * p_work)
{
	...
	p_dwk->sub_seq	 = 0;
	p_dwk->move_seq	 = 0;
	p_dwk->count	 = 30;		///< 最初余裕を持たせる
	p_dwk->fsys		 = p_fsys;	
	p_dwk->fldobj	 = FieldOBJSys_OBJIDSearch( p_dwk->fsys->fldobjsys, FLDMAP_DEMO_POKE_M_ID );
	p_dwk->fldobj_s	 = FieldOBJSys_OBJIDSearch( p_dwk->fsys->fldobjsys, D05R0115_CHAMP );
	p_dwk->fldobj_p	 = Player_FieldOBJGet( Player_FieldSysWorkPlayerGet( p_dwk->fsys ) );//FieldOBJSys_OBJIDSearch( p_dwk->fsys->fldobjsys, FLDOBJ_ID_PLAYER );
//	p_dwk->player	 = Player_FieldSysWorkPlayerGet( p_dwk->fsys );
	
	sys_InitAllocator( &p_dwk->allocator, USE_HEAPID, 32 );
	DEMO_UMA_Load( p_dwk );
	GX_SetMasterBrightness( p_dwk->brightness );
...
//static void FLD_DEMO_MoveInit_FX( FLD_DEMO_MOVE_PARAM_FX* p_data, fx32 s_num, fx32 e_num, s32 count );
//static BOOL FLD_DEMO_MoveMain_FX( FLD_DEMO_MOVE_PARAM_FX* p_data, s32 count );
#define UMA_MOVE_FRAME_U		( 20 )
#define UMA_MOVE_FRAME_M		( 32 )
#define UMA_MOVE_FRAME_A		(  8 )
#define UMA_MOVE_FRAME_LAST		(  6 )
#define UMA_FINISH_SEQ			( 99 )
#define DEMO_MAP_OFS			(  30 )
#define UMA_DEMO_SE_01			( SEQ_SE_DP_CLIMAX10 )	///< 穴に突入
//Snd_SePlay( UMA_DEMO_SE_01 );
static const fx32 t_tbl[][2] = {
	{ 504, 330 },
	{ 579, 330 },
	{ 504, 330 },
};
static BOOL UMA_Move_U( FLDMAP_DEMO_GINGA* wk )
{
	BOOL result[ 2 ];
	fx32 x, y, z;
	...
	case 0:
		wk->count = 0;
		wk->target_x = FX32_CONST( 573 );
		wk->target_z = FX32_CONST( 330 + DEMO_MAP_OFS );		
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );		
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_U );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_U );
		wk->move_seq++;
		break;
	case 1:
		wk->count++;
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );			
		result[ 0 ] = FLD_DEMO_MoveMain_FX( &wk->move_x, wk->count );
		result[ 1 ] = FLD_DEMO_MoveMain_FX( &wk->move_z, wk->count );
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data, y, wk->move_z.data );			
		if ( result[ 0 ] && result[ 1 ] ){
			Snd_PMVoicePlayEx( PV_NORMAL, MONSNO_REI, 0, 100, USE_HEAPID, 0 );				
			wk->move_seq++;
		}
		break;
	...
	case 3:					
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );			
		result[ 0 ] = FLD_DEMO_MoveMain_FX( &wk->move_x, wk->count );
		wk->target_z += ( Cos360R( wk->count * ( 180 / UMA_MOVE_FRAME_U ) ) * 3 );
		wk->count++;
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data, y, wk->target_z );			
		if ( result[ 0 ] ){				
			wk->move_seq++;
		}
		break;
	...
	case 5:					
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );			
		result[ 0 ] = FLD_DEMO_MoveMain_FX( &wk->move_x, wk->count );
		wk->target_z -= ( Cos360R( wk->count * ( 90 / UMA_MOVE_FRAME_U ) ) * 3 );
		wk->count++;
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data, y, wk->target_z );			
		if ( result[ 0 ] ){				
			wk->move_seq = UMA_FINISH_SEQ;
		}
		break;
...
static BOOL UMA_Move_M( FLDMAP_DEMO_GINGA* wk )
{
	BOOL result[ 2 ];
	fx32 x, y, z;
	...
	case 0:
		wk->count = 0;
		wk->target_x = FX32_CONST( 555 );
		wk->target_z = FX32_CONST( 317 + DEMO_MAP_OFS );
		wk->curve_z	 = FX32_CONST( 0 );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_M );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_M );
		wk->move_seq++;
		break;
	case 1:
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		...
		result[ 1 ] = FLD_DEMO_MoveMain_FX( &wk->move_z, wk->count );
		wk->count++;
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data, y, wk->move_z.data + wk->curve_z );			
		if ( result[ 0 ] && result[ 1 ] ){
			wk->move_seq++;
		}
		break;
	...
	case 3:				
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );			
		result[ 0 ] = FLD_DEMO_MoveMain_FX( &wk->move_x, wk->count );
		wk->target_z += ( Cos360R( wk->count * ( 180 / UMA_MOVE_FRAME_M ) ) * 3 );
		wk->count++;
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data, y, wk->target_z );			
		if ( result[ 0 ] ){				
			wk->move_seq++;
	...
	case 4:
		wk->count = 0;
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &wk->target_z );
		wk->move_seq++;
		
	case 5:
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		wk->target_z -= ( Cos360R( wk->count * ( 360 / UMA_MOVE_FRAME_M ) ) * 2 );
		wk->count++;
		D3DOBJ_SetMatrix( &wk->obj, x, y, wk->target_z );			
		if ( wk->count >= ( UMA_MOVE_FRAME_M * 2 )  ){
			wk->count = 0;
			wk->move_seq++;
		}
		break;
	...
	case 6:
		wk->count = 0;
		wk->target_x = FX32_CONST( 465 );
		wk->target_z = FX32_CONST( 468 + DEMO_MAP_OFS );
		wk->curve_x	 = FX32_CONST( 0 );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_M );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_M );
		wk->move_seq++;
	...
	case 7:
		...
		result[ 1 ] = FLD_DEMO_MoveMain_FX( &wk->move_z, wk->count );
		wk->count++;
		wk->curve_x -= ( Cos360R( ( wk->count + 1 ) * ( 180 / ( UMA_MOVE_FRAME_M ) ) ) * 4 );
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data + wk->curve_x, y, wk->move_z.data );			
		if ( result[ 0 ] && result[ 1 ] ){ wk->move_seq++; }
		break;

	///< 主人公に訴えかける上下
	case 8:
		wk->count = 0;
		FieldOBJ_DirDispSetForce( wk->fldobj_s, DIR_LEFT );
		FieldOBJ_AcmdSet( wk->fldobj_p, AC_DIR_L );
	//	Player_GPosAddDir( wk->player, DIR_LEFT );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &wk->target_z );
		wk->move_seq++;
		
	case 9:
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		wk->target_z += ( Cos360R( wk->count * ( 360 / UMA_MOVE_FRAME_M ) ) * 1 );
		wk->count++;
		D3DOBJ_SetMatrix( &wk->obj, x, y, wk->target_z );			
		if ( wk->count >= ( UMA_MOVE_FRAME_M * 1 )  ){
			wk->timer = 1;
			wk->count = 0;
			wk->move_seq++;
		}
	...
	///< 周る　SinCosより計算されてない感じでまわす。
	case 10:
		wk->count = 0;
		wk->target_x = FX32_CONST( 503 );
		wk->target_z = FX32_CONST( 475 + DEMO_MAP_OFS );
		wk->curve_z	 = FX32_CONST( 0 );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_M );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_M );
	...
	case 11:
		...
		result[ 0 ] = FLD_DEMO_MoveMain_FX( &wk->move_x, wk->count );
		result[ 1 ] = FLD_DEMO_MoveMain_FX( &wk->move_z, wk->count );
		wk->curve_z += ( Cos360R( wk->count * ( 180 / ( UMA_MOVE_FRAME_M ) ) ) * 2 );
		wk->count++;
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data, y, wk->move_z.data + wk->curve_z );			
		if ( result[ 0 ] && result[ 1 ] ){ wk->move_seq++; }
		break;	
	...
	case 12:
		wk->count = 0;
		wk->target_x = FX32_CONST( 503 );
		wk->target_z = FX32_CONST( 454 + DEMO_MAP_OFS );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_M / 4 );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_M / 4 );
		wk->move_seq++;
		break;
	case 13:
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		...
		if ( wk->count >= 4 ){
			if ( wk->brightness < 16 ){
				wk->brightness += 2;
				GX_SetMasterBrightness( wk->brightness );
			}
		}
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data, y, wk->move_z.data );			
		if ( result[ 0 ] && result[ 1 ] ){ wk->move_seq++; }
	...
	case 14:
		...
		FieldOBJ_StatusBitSet_Vanish( wk->fldobj, FALSE );
		D3DOBJ_SetDraw( &wk->obj, FALSE );
		wk->move_seq++;
		break;
	...
	case 15:
		...
		wk->count = 0;
		Snd_PMVoicePlayEx( PV_NORMAL, MONSNO_AI, 0, 100, USE_HEAPID, 0 );
		wk->move_seq++;
		break;
	
	case 16:
		if ( (++wk->count) < 60 ){ break; }
		wk->move_seq++;
		break;
	...
	case 17:
		...
		FieldOBJ_StatusBitSet_Vanish( wk->fldobj, TRUE );
		D3DOBJ_SetDraw( &wk->obj, TRUE );
		wk->move_seq++;
	//	break;
	...
	case 19:
		wk->count = 0;
		wk->target_z = FX32_CONST( 319 + DEMO_MAP_OFS );
		wk->curve_x	 = FX32_CONST( 0 );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		wk->target_x = x;
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_M );
	...
	case 20:
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
		result[ 0 ] = FLD_DEMO_MoveMain_FX( &wk->move_z, wk->count );
		wk->count++;
		if ( wk->brightness > 0 ){
			wk->brightness -= 1;
			GX_SetMasterBrightness( wk->brightness );
		}
		wk->curve_x += ( Cos360R( ( wk->count + 1 ) * ( 180 / ( UMA_MOVE_FRAME_M ) ) ) * 8 );
		D3DOBJ_SetMatrix( &wk->obj, wk->target_x + wk->curve_x, y, wk->move_z.data );			
		if ( result[ 0 ] ){ wk->move_seq = UMA_FINISH_SEQ; }
		break;

	case UMA_FINISH_SEQ:
		wk->count = 0;			
		wk->target_x = FX32_CONST( t_tbl[ wk->poke_type ][ 0 ] );
		wk->target_z = FX32_CONST( t_tbl[ wk->poke_type ][ 1 ] );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );			
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_U );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_U );
		wk->move_seq++;
	...
	if ( wk->timer != 0 ){		
		if ( wk->timer == 20 ){
			FieldOBJ_DirDispSetForce( wk->fldobj_s, DIR_DOWN );
		//	FieldOBJ_DirDispSetForce( wk->fldobj_p, DIR_DOWN );			
			FieldOBJ_AcmdSet( wk->fldobj_p, AC_DIR_D );
		//	Player_DirSet( wk->player, DIR_DOWN );
		}	
		if ( wk->timer == 40 ){
			FieldOBJ_DirDispSetForce( wk->fldobj_s, DIR_LEFT );
		//	FieldOBJ_DirDispSetForce( wk->fldobj_p, DIR_RIGHT );		
			FieldOBJ_AcmdSet( wk->fldobj_p, AC_DIR_R );	
		//	Player_DirSet( wk->player, DIR_RIGHT );
		}
		if ( wk->timer == 50 ){
			FieldOBJ_DirDispSetForce( wk->fldobj_s, DIR_UP );
		//	FieldOBJ_DirDispSetForce( wk->fldobj_p, DIR_UP );			
			FieldOBJ_AcmdSet( wk->fldobj_p, AC_DIR_U );	
		//	Player_DirSet( wk->player, DIR_UP );
		}		
		wk->timer++;		
	}
...
static BOOL UMA_Move_A( FLDMAP_DEMO_GINGA* wk )
{
	BOOL result[ 2 ];
	fx32 x, y, z;
	...
	case 0:
		wk->count = 0;
		wk->target_x = FX32_CONST( 606 );
		wk->target_z = FX32_CONST( 340 + DEMO_MAP_OFS );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );			
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_A );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_A );
		wk->move_seq++;
		break;
	case 1:
		wk->count++;
	...
	case 2:
		wk->count = 0;
		wk->target_x = FX32_CONST( 438 );
		wk->target_z = FX32_CONST( 412 + DEMO_MAP_OFS );
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );			
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_A );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_A );
		wk->move_seq++;
	...
	case 3:					
		...
		D3DOBJ_SetMatrix( &wk->obj, wk->move_x.data, y, wk->move_z.data );			
		if ( result[ 0 ] && result[ 1 ] ){
			Snd_PMVoicePlayEx( PV_NORMAL, MONSNO_HAI, 0, 100, USE_HEAPID, 0 );
			wk->move_seq++;
		}
		break;
	...
	case 4:
		wk->count = 0;
		wk->target_x = FX32_CONST( 504 );
		wk->target_z = FX32_CONST( 307 + DEMO_MAP_OFS );			
		D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );			
		FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_A );
		FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_A );
		wk->move_seq++;
...
static BOOL UMA_Move( FLDMAP_DEMO_GINGA* wk )
{
	BOOL result[ 2 ];
	fx32 x, y, z;
	...
	case 0:
		switch ( wk->poke_type ){
		case 0:
			result[ 0 ] = UMA_Move_U( wk );
			break;
		case 1:
			result[ 0 ] = UMA_Move_A( wk );
			break;
		case 2:
			result[ 0 ] = UMA_Move_M( wk );
			break;
		}
		if ( result[ 0 ] ){ wk->sub_seq++; }
		break;
	
	case 1:
		{
			wk->target_x = FX32_CONST( 504 );
			wk->target_z = FX32_CONST( 439 );
			wk->count = 0;
			
			D3DOBJ_GetMatrix( &wk->obj, &x, &y, &z );
			
			FLD_DEMO_MoveInit_FX( &wk->move_x, x, wk->target_x, UMA_MOVE_FRAME_LAST );
			FLD_DEMO_MoveInit_FX( &wk->move_z, z, wk->target_z, UMA_MOVE_FRAME_LAST );
			wk->sub_seq++;
		}
		break;
 ...
 */
//-----------------------------------------------------------------------------
//static int flag_sw = 0;
static void FLDMAP_DEMO_Ginga_Main(FLDMAPFUNC_WORK * p_fwk, FIELDSYS_WORK * p_fsys, void * p_work)
{
	FLDMAP_DEMO_GINGA* wk = p_work;
	u32 result[2];
	VecFx32 matrix;
	int wait_tbl[] = { 68, 38, 390 };
	
	switch ( wk->seq ){
	case 0:
		if ( wk->count != 0 ){		///< 白い状態を維持するため
			wk->count--;
			break;
		}
		if ( wk->brightness > 0 ){	///< 白戻す
			wk->brightness -= 1;
			GX_SetMasterBrightness( wk->brightness );
			break;
		}
		matrix = GFC_GetLookTarget( p_fsys->camera_ptr );
		{
			...
			D3DOBJ_SetMatrix( &wk->obj, FX32_CONST( ini_pos[ wk->poke_type ] ), FX32_CONST( 99 ), FX32_CONST( 290 + DEMO_MAP_OFS ) );
		}
	//	FLD_DEMO_GINGA_Init( wk->p_ginga, matrix.x, matrix.y + FLDMAP_DEMO_GINGA_Y_OFS, matrix.z );
		wk->white_count = 0;
		wk->seq++;
		break;
	
	case 1:
		if ( UMA_Move( wk ) ){ 			///< 各種動作メイン
			OS_Printf( "■■■■■■■white = %d\n",wk->white_count);	///< この値-6がwait_tblの値でいい
			Snd_SePlay( UMA_DEMO_SE_01 );
			wk->white_count = 0;
			wk->seq++;
		}
			...
			wk->white_count++;
			if ( wait_tbl[ wk->poke_type ] < wk->white_count ){
				if ( wk->brightness != 16 ){	///< 終了タイミングでホワイトアウト
					wk->brightness += 2;
					GX_SetMasterBrightness( wk->brightness );
				}
				else {
	...
	default:
		...
		wk->seq = FLDMAP_DEMO_GINGA_SEQ_END;
		break;
	}
	if ( wk->poke_type != 3 ){
		G3_Identity();
		D3DOBJ_AnmLoop( &wk->anm, FX32_ONE );
		NNS_G3dGePushMtx();
		D3DOBJ_Draw( &wk->obj );
		NNS_G3dGePopMtx( 1 );
	}
...
// モデル
static void FLD_DEMO_3DObjMdlLoad( D3DOBJ_MDL* p_mdl, u32 arc_idx, u32 data_idx, u32 heap );
static void FLD_DEMO_3DObjMdlDelete( D3DOBJ_MDL* p_mdl );

// アニメ
static void FLD_DEMO_3DObjAnmLoad( D3DOBJ_ANM* p_anm, const D3DOBJ_MDL* cp_mdl, u32 arc_idx, u32 data_idx, u32 heap, NNSFndAllocator* pallocator );
static void FLD_DEMO_3DObjAnmDelete( D3DOBJ_ANM* p_anm, NNSFndAllocator* pallocator );
static BOOL FLD_DEMO_3DObjAnmNoLoop( D3DOBJ_ANM* p_anm );
static void FLD_DEMO_3DObjAnmLoop( D3DOBJ_ANM* p_anm );
static void FLD_DEMO_3DObjAnmLoopAdd( D3DOBJ_ANM* p_anm, fx32 add );
static void FLD_DEMO_3DObjAnmSet( D3DOBJ_ANM* p_anm, fx32 num );
static fx32 FLD_DEMO_3DObjAnmGet( const D3DOBJ_ANM* cp_anm );

// 描画オブジェ
static void FLD_DEMO_3DObjDrawInit( D3DOBJ* p_draw, D3DOBJ_MDL* cp_mdl );
static void FLD_DEMO_3DObjDrawAddAnm( D3DOBJ* p_draw, D3DOBJ_ANM* p_anm );
static void FLD_DEMO_3DObjDrawDraw( D3DOBJ* p_draw );
static void FLD_DEMO_3DObjDrawSetDraw( D3DOBJ* p_draw, BOOL flag );
static BOOL FLD_DEMO_3DObjDrawGetDraw( const D3DOBJ* cp_draw );
static void FLD_DEMO_3DObjDrawSetMatrix( D3DOBJ* p_draw, fx32 x, fx32 y, fx32 z );
static void FLD_DEMO_3DObjDrawGetMatrix( D3DOBJ* p_draw, fx32* p_x, fx32* p_y, fx32* p_z );
static void FLD_DEMO_3DObjDrawSetScale( D3DOBJ* p_draw, fx32 x, fx32 y, fx32 z );
static void FLD_DEMO_3DObjDrawGetScale( D3DOBJ* p_draw, fx32* p_x, fx32* p_y, fx32* p_z );
static void FLD_DEMO_3DObjDrawSetRota( D3DOBJ* p_draw, u16 rota, u32 way );
static u16 FLD_DEMO_3DObjDrawGetRota( D3DOBJ* p_draw, u32 way );
...
static void FLD_DEMO_3DObjMdlLoad( D3DOBJ_MDL* p_mdl, u32 arc_idx, u32 data_idx, u32 heap )
{
	D3DOBJ_MdlLoad( p_mdl, arc_idx, data_idx, heap );
}
 ...
 */
//-----------------------------------------------------------------------------
static void FLD_DEMO_3DObjMdlLoadH( D3DOBJ_MDL* p_mdl, ARCHANDLE* p_handle, u32 data_idx, u32 heap )
{
	D3DOBJ_MdlLoadH( p_mdl, p_handle, data_idx, heap );
}
...
static void FLD_DEMO_3DObjAnmLoad( D3DOBJ_ANM* p_anm, const D3DOBJ_MDL* cp_mdl, u32 arc_idx, u32 data_idx, u32 heap, NNSFndAllocator* pallocator )
{
	D3DOBJ_AnmLoad( p_anm, cp_mdl, arc_idx, data_idx, heap, pallocator );
}
 ...
 */
//-----------------------------------------------------------------------------
static void FLD_DEMO_3DObjAnmLoadH( D3DOBJ_ANM* p_anm, const D3DOBJ_MDL* cp_mdl, ARCHANDLE* p_handle, u32 data_idx, u32 heap, NNSFndAllocator* pallocator )
{
	D3DOBJ_AnmLoadH( p_anm, cp_mdl, p_handle, data_idx, heap, pallocator );
}
...
static void FLD_DEMO_3DObjAnmDelete( D3DOBJ_ANM* p_anm, NNSFndAllocator* pallocator )
{
	D3DOBJ_AnmDelete( p_anm, pallocator );
}
...
static BOOL FLD_DEMO_3DObjAnmNoLoop( D3DOBJ_ANM* p_anm )
{
	return D3DOBJ_AnmNoLoop( p_anm, FX32_ONE );
}
 ...
 */
//-----------------------------------------------------------------------------
static void FLD_DEMO_3DObjDrawInit( D3DOBJ* p_draw, D3DOBJ_MDL* p_mdl )
{
	D3DOBJ_Init( p_draw, p_mdl );
}
...
static void FLD_DEMO_3DObjDrawGetMatrix( D3DOBJ* p_draw, fx32* p_x, fx32* p_y, fx32* p_z )
{
	D3DOBJ_GetMatrix( p_draw, p_x, p_y, p_z );
}
...
static void FLD_DEMO_3DObjDrawGetScale( D3DOBJ* p_draw, fx32* p_x, fx32* p_y, fx32* p_z )
{
	D3DOBJ_GetScale( p_draw, p_x, p_y, p_z );
}
...
static void FLD_DEMO_3DObjDrawSetRota( D3DOBJ* p_draw, u16 rota, u32 way )
{
	D3DOBJ_SetRota( p_draw, rota, way );
}
...
static u16 FLD_DEMO_3DObjDrawGetRota( D3DOBJ* p_draw, u32 way )
{
	return D3DOBJ_GetRota( p_draw, way );
}
...
//=====================================
typedef struct {
	D3DOBJ		draw;		// 描画オブジェクト
	D3DOBJ_MDL	mdl;		// モデル
	D3DOBJ_ANM	anm[FLD_DEMO_GINGA_ANM_NUM];		// アニメ
...
static void FLD_DEMO_CLM_GingaObjInit( FLD_DEMO_CLM_GINGA* p_obj, NNSFndAllocator* pallocator, u32 heapID, fx32 x, fx32 y, fx32 z )
{
	...
	// ハンドルopen
	p_handle = ArchiveDataHandleOpen( ARC_FIELD_DEMO_CLI_GRA, heapID );

	// フラッシュオブジェクト作成
	p_obj->p_flash = FLD_DEMO_Flash_Alloc( USE_HEAPID );
	
	// モデルデータ読み込み
	FLD_DEMO_3DObjMdlLoadH( &p_obj->mdl, p_handle, NARC_demo_climax_ginga_nsbmd, heapID );

	for( i=0; i<FLD_DEMO_GINGA_ANM_NUM; i++ ){
		// アニメ読み込み
		FLD_DEMO_3DObjAnmLoadH( &p_obj->anm[i], &p_obj->mdl, p_handle, NARC_demo_climax_ginga_nsbca+i, heapID, pallocator );
	}
	// 描画モデル初期化
...
typedef struct {
	...
	D3DOBJ			draw;		// 描画オブジェクト
	D3DOBJ_MDL		mdl;		// モデル
	D3DOBJ_ANM		anm[FLD_DEMO_LEGENDEFF_ANMOBJ_NUM];	// アニメ
#endif
	int			eva;
	int			evb;
	D3DOBJ		kusari_draw;
	D3DOBJ_MDL	kusari_mdl;
	D3DOBJ_ANM	kusari_anm[ FLD_DEMO_KUSARI_MAX ];
...
FLD_DEMO_LEGENDPOKE_APP* FLD_DEMO_LegendPokeApp_Alloc( u32 heapID, FIELDSYS_WORK * fsys )
{
	...
	// BG FADE 用 のスクリーン転送
	{
		GF_BGL_INI* ini = FieldBglIniGet( p_data->fsys );
		ArcUtil_BgCharSet( ARC_DEMO_TENGAN_GRA, NARC_demo_tengan_gra_bg_fade_NCGR, ini, GF_BGL_FRAME2_M, 0, 0, 0, heapID );	//キャラ
		ArcUtil_ScrnSet(   ARC_DEMO_TENGAN_GRA, NARC_demo_tengan_gra_bg_fade_NSCR, ini, GF_BGL_FRAME2_M, 0, 0, 0, heapID );
		ArcUtil_PalSet(    ARC_DEMO_TENGAN_GRA, NARC_demo_tengan_gra_bg_fade_NCLR, PALTYPE_MAIN_BG, 0x20 * 6, 0x20,  heapID );

		GF_BGL_PrioritySet( GF_BGL_FRAME2_M, 1 );

		p_data->legend_eff.eva =  0;
		p_data->legend_eff.evb = 31;
		G2_SetBlendAlpha( GX_BLEND_PLANEMASK_BG2, GX_BLEND_PLANEMASK_BG0, p_data->legend_eff.eva, p_data->legend_eff.evb );
	}
...
void FLD_DEMO_LegendPokeApp_Start( FLD_DEMO_LEGENDPOKE_APP* p_data, u32 legendpoke_id )
{
	...
	FldDemo_LegendPokeObjInit( &p_data->legend_poke, p_data->fsys, FLDMAP_DEMO_BOSS_OBJID );
...
static void FldDemo_LegendEffObjAlloc( FLD_DEMO_LEGEND3D_EFF* p_eff, u32 heapID, NNSFndAllocator* p_allocator )
{
	...
	p_handle = ArchiveDataHandleOpen( ARC_DEMO_TENGAN_GRA, heapID );
	
	// モデル読み込み
	FLD_DEMO_3DObjMdlLoadH( &p_eff->kusari_mdl, p_handle, NARC_demo_tengan_gra_demo_kusari_nsbmd, heapID );	

	// アニメ読み込み
	for( i = 0; i < FLD_DEMO_KUSARI_MAX; i++ ){
		FLD_DEMO_3DObjAnmLoadH( &p_eff->kusari_anm[i],
				&p_eff->kusari_mdl, p_handle,
				NARC_demo_tengan_gra_demo_kusari_nsbca + i, 
				heapID, p_allocator );
	}

	// 描画オブジェクト作成
	FLD_DEMO_3DObjDrawInit( &p_eff->kusari_draw, &p_eff->kusari_mdl );
	for( i=0; i<FLD_DEMO_KUSARI_MAX; i++ ){
		FLD_DEMO_3DObjDrawAddAnm( &p_eff->kusari_draw, &p_eff->kusari_anm[i] );
	}

	// 表示OFF
	FLD_DEMO_3DObjDrawSetDraw( &p_eff->kusari_draw, FALSE );
	
#if 0
	// モデル読み込み
	FLD_DEMO_3DObjMdlLoadH( &p_eff->mdl, p_handle, NARC_demo_climax_kurotama_nsbmd, heapID );	

	// アニメ読み込み
	for( i=0; i<FLD_DEMO_LEGENDEFF_ANMOBJ_NUM; i++ ){
		FLD_DEMO_3DObjAnmLoadH( &p_eff->anm[i],
				&p_eff->mdl, p_handle,
...
static void FldDemo_LegendEffObjInit( FLD_DEMO_LEGEND3D_EFF* p_eff, fx32 x, fx32 y, fx32 z )
{
	...
	// 動作設定
	FLD_DEMO_3DObjDrawSetMatrix( &p_eff->kusari_draw, x, y, z );

	FLD_DEMO_3DObjDrawSetScale( &p_eff->kusari_draw, FX32_ONE, FX32_ONE, FX32_ONE );

	// フレーム数設定
	for( i=0; i<FLD_DEMO_KUSARI_MAX; i++ ){
		FLD_DEMO_3DObjAnmSet( &p_eff->kusari_anm[i], 0 );
	}
	p_eff->count = 0;
	p_eff->seq = 0;
	...
	// 表示ON
	FLD_DEMO_3DObjDrawSetDraw( &p_eff->kusari_draw, TRUE );
}
...
static void FldDemo_LegendEffObjDelete( FLD_DEMO_LEGEND3D_EFF* p_eff, NNSFndAllocator* p_allocator )
{
	int i;
	// モデル破棄
	FLD_DEMO_3DObjMdlDelete( &p_eff->kusari_mdl );

	// アニメ破棄
	for( i=0; i<FLD_DEMO_KUSARI_MAX; i++ ){
		FLD_DEMO_3DObjAnmDelete( &p_eff->kusari_anm[ i ], p_allocator );
	}
}
...
{
	BOOL result = FALSE;
	BOOL bEnd[ 2 ] = { FALSE, FALSE };
	
	FLD_DEMO_LEGEND3D_EFF* p_eff = &p_data->legend_eff;
	...
	case 4:
		if ( type ){
			if ( p_eff->eva <  8 ){ p_eff->eva++; } else { bEnd[ 0 ] = TRUE; };
			if ( p_eff->evb >  8 ){ p_eff->evb--; } else { bEnd[ 1 ] = TRUE; };
		}
		else {		
			if ( p_eff->eva >  2 ){ p_eff->eva--; } else { bEnd[ 0 ] = TRUE; };
			if ( p_eff->evb < 14 ){ p_eff->evb++; } else { bEnd[ 1 ] = TRUE; };
		}		
		break;
	...
	case 8:
		if ( type ){
			...
			if ( p_eff->eva >  3 ){ p_eff->eva--; } else { bEnd[ 0 ] = TRUE; };
			if ( p_eff->evb < 13 ){ p_eff->evb++; } else { bEnd[ 1 ] = TRUE; };
		}		
		break;
	...
	case 10:
		if ( type ){
			...
			if ( p_eff->eva >  4 ){ p_eff->eva--; } else { bEnd[ 0 ] = TRUE; };
			if ( p_eff->evb < 12 ){ p_eff->evb++; } else { bEnd[ 1 ] = TRUE; };
		}		
		break;
	...
	case 12:
		if ( type ){
			...
			if ( p_eff->eva >  5 ){ p_eff->eva--; } else { bEnd[ 0 ] = TRUE; };
			if ( p_eff->evb < 11 ){ p_eff->evb++; } else { bEnd[ 1 ] = TRUE; };
		}		
		break;
	...
	case 14:
		if ( type ){
			...
			if ( p_eff->eva >  6 ){ p_eff->eva--; } else { bEnd[ 0 ] = TRUE; };
			if ( p_eff->evb < 10 ){ p_eff->evb++; } else { bEnd[ 1 ] = TRUE; };
		}		
		break;
#if 0	
	case 11:
		if ( p_eff->evb >  0 ){ p_eff->evb--; } else { bEnd[ 0 ] = bEnd[ 1 ] = TRUE; };
		break;

	case 12:	
		if ( p_eff->eva >  0 ){ p_eff->eva--; } else { bEnd[ 0 ] = bEnd[ 1 ] = TRUE; };		
		break;
#endif	
	default:
		if ( p_eff->eva >  0 ){ p_eff->eva--; } else { bEnd[ 0 ] = TRUE; };
		if ( p_eff->evb >  0 ){ p_eff->evb--; } else { bEnd[ 1 ] = TRUE; };
#if 0
		if ( type ){
			if ( p_eff->eva <  8 ){ p_eff->eva++; } else { bEnd[ 0 ] = TRUE; };
			if ( p_eff->evb >  8 ){ p_eff->evb--; } else { bEnd[ 1 ] = TRUE; };
			...
			if ( p_eff->eva >  0 ){ p_eff->eva--; } else { bEnd[ 0 ] = TRUE; };
			if ( p_eff->evb < 16 ){ p_eff->evb++; } else { bEnd[ 1 ] = TRUE; };
		}
#endif
		break;
	}
	...
	case 0:
		if ( type == 0 ){
			if ( p_eff->eva <  8 ){	 p_eff->eva += 1; } else { p_eff->eva =  8; bEnd[ 0 ] = TRUE; }
			if ( p_eff->evb > 23 ){	 p_eff->evb -= 1; } else { p_eff->evb = 23; bEnd[ 1 ] = TRUE; }
		}
		else {
			if ( p_eff->eva >  0 ){	 p_eff->eva -= 1; } else { p_eff->eva =  0; bEnd[ 0 ] = TRUE; }
			if ( p_eff->evb < 31 ){	 p_eff->evb += 1; } else { p_eff->evb < 31; bEnd[ 1 ] = TRUE; }
		}
		break;
		
	case 1:
		if ( type == 0 ){
			if ( p_eff->eva < 16 ){	 p_eff->eva += 1; } else { p_eff->eva = 16; bEnd[ 0 ] = TRUE; }
			if ( p_eff->evb > 15 ){	 p_eff->evb -= 1; } else { p_eff->evb = 15; bEnd[ 1 ] = TRUE; }
		//	if ( p_eff->eva < 16 ){	 p_eff->eva += 1; } else { p_eff->eva = 16; result = TRUE; }
		//	if ( p_eff->evb > 15 ){	 p_eff->evb -= 1; } else { p_eff->evb = 15; result = TRUE; }
		}
		else {
			if ( p_eff->eva >  0 ){	 p_eff->eva -= 1; } else { p_eff->eva =  0; bEnd[ 0 ] = TRUE; }
			if ( p_eff->evb < 31 ){	 p_eff->evb += 1; } else { p_eff->evb < 31; bEnd[ 1 ] = TRUE; }
		//	if ( p_eff->eva >  0 ){	 p_eff->eva -= 1; } else { p_eff->eva =  0; result = TRUE; }
		//	if ( p_eff->evb < 31 ){	 p_eff->evb += 1; } else { p_eff->evb < 31; result = TRUE; }
		}
		break;
		
	case 2:
		if ( type == 0 ){
			if ( p_eff->eva < 31 ){	 p_eff->eva += 1; } else { p_eff->eva = 31; bEnd[ 0 ] = TRUE; }
			if ( p_eff->evb >  0 ){	 p_eff->evb -= 1; } else { p_eff->evb =  0; bEnd[ 1 ] = TRUE; }
		}
		else {
			if ( p_eff->eva >  0 ){	 p_eff->eva -= 1; } else { p_eff->eva =  0; bEnd[ 0 ] = TRUE; }
			if ( p_eff->evb < 31 ){	 p_eff->evb += 1; } else { p_eff->evb < 31; bEnd[ 1 ] = TRUE; }
...
#endif	
	G2_ChangeBlendAlpha( p_eff->eva, p_eff->evb );	
...
static BOOL FldDemo_KusariAnime( FLD_DEMO_LEGENDPOKE_APP* p_data )
{
	...
	for( i = 0; i < FLD_DEMO_KUSARI_MAX; i++ ){
		result = FLD_DEMO_3DObjAnmNoLoop( &p_eff->kusari_anm[ i ] );
	}
...
static u32 FldDemo_LegendEffObjMain( FLD_DEMO_LEGEND3D_EFF* p_eff )
{
	...
	case FLD_DEMO_LEGENDEFF_SEQ_FADEOUT:	// フェードアウト
		...
		FLD_DEMO_3DObjDrawSetScale( &p_eff->kusari_draw, p_eff->scale.data, p_eff->scale.data, p_eff->scale.data );
		if( result == TRUE ){
			p_eff->seq++;
			// 表示OFF
			FLD_DEMO_3DObjDrawSetDraw( &p_eff->kusari_draw, FALSE );
...
static void FldDemo_LegendEffObjMoveY( FLD_DEMO_LEGEND3D_EFF* p_eff, fx32 y )
{
	...
	// 座標取得
	FLD_DEMO_3DObjDrawGetMatrix( &p_eff->kusari_draw, &matrix.x,  &matrix.y, &matrix.z );
	// 設定
	matrix.y = y;
	FLD_DEMO_3DObjDrawSetMatrix( &p_eff->kusari_draw, matrix.x, matrix.y, matrix.z );
}
...
static void FldDemo_LegendEffObjDraw( FLD_DEMO_LEGEND3D_EFF* p_eff )
{
	FLD_DEMO_3DObjDrawDraw( &p_eff->kusari_draw );
}
...
static void FLD_DEMO_BlackRingObj_Init( FLD_DEMO_BLACK_RING_OBJ* p_obj, D3DOBJ_MDL* p_mdl );
...
static void FLD_DEMO_BlackRingObj_Start( FLD_DEMO_BLACK_RING_OBJ* p_obj, fx32 s_scale, fx32 e_scale, int sync, fx32 x, fx32 y, fx32 z, u16 rota_x, u16 rota_y, u16 rota_z )
{
	...
	FLD_DEMO_3DObjDrawSetRota( &p_obj->draw, rota_x, D3DOBJ_ROTA_WAY_X );
	FLD_DEMO_3DObjDrawSetRota( &p_obj->draw, rota_y, D3DOBJ_ROTA_WAY_Y );
	FLD_DEMO_3DObjDrawSetRota( &p_obj->draw, rota_z, D3DOBJ_ROTA_WAY_Z );
...
typedef struct _FLD_DEMO_ANTEGG {
	D3DOBJ draw;		// 描画ワーク
	D3DOBJ_MDL mdl;		// モデル
	D3DOBJ_ANM anm[FLD_DEMO_ANTEGG_ANM_NUM];	// アニメ
...
static FLD_DEMO_ANTEGG* FLD_DEMO_AntEgg_Alloc( u32 heapID, u32 dataIdx )
{
	...
	// モデル読み込み
	FLD_DEMO_3DObjMdlLoadH( &p_data->mdl, p_handle, dataIdx, heapID );
	for( i=0; i<FLD_DEMO_ANTEGG_ANM_NUM; i++ ){
		FLD_DEMO_3DObjAnmLoadH( &p_data->anm[i], &p_data->mdl, p_handle, NARC_demo_climax_aritama_nsbta + i, heapID, &p_data->allocator );
	}
...
static void FLD_DEMO_AntEgg_Start( FLD_DEMO_ANTEGG* p_data, fx32 x, fx32 y, fx32 z, u16 rota_x, u16 rota_y, u16 rota_z )
{
	...
	FLD_DEMO_3DObjDrawSetRota( &p_data->draw, rota_x, D3DOBJ_ROTA_WAY_X );
	FLD_DEMO_3DObjDrawSetRota( &p_data->draw, rota_z, D3DOBJ_ROTA_WAY_Z );
	FLD_DEMO_3DObjDrawSetRota( &p_data->draw, rota_y, D3DOBJ_ROTA_WAY_Y );
...
void FLD_DEMO_AntEgg_Cut_Main( FLD_DEMO_ANTEGG* p_data )
{
	...
	case 0:	
		if( sys.cont & PAD_BUTTON_R ){
			...
			FLD_DEMO_3DObjDrawSetRota( &p_data->draw, FLD_DEMO_ANTEGG_CUT00_ROTA_Y, D3DOBJ_ROTA_WAY_Y );
		}
		...
		if( sys.cont & PAD_BUTTON_L ){
			...
			// ｚ軸１８０度回転
			FLD_DEMO_3DObjDrawSetRota( &p_data->draw, FLD_DEMO_ANTEGG_CUT00_ROTA_Z, D3DOBJ_ROTA_WAY_Z );
		}
		...
		if( sys.cont & PAD_BUTTON_START ){
			...
			FLD_DEMO_3DObjDrawSetRota( &p_data->draw, FLD_DEMO_ANTEGG_CUT00_ROTA_X, D3DOBJ_ROTA_WAY_X );
		}
		break;
...
typedef struct _FLD_DEMO_ARITAMA_START{
	D3DOBJ draw;		// 描画ワーク
	D3DOBJ_MDL mdl;		// モデル
	D3DOBJ_ANM anm[FLD_DEMO_ARITAMA_ANM_NUM];	// アニメ
	FIELDSYS_WORK* p_fsys;	// フィールドシステム
	NNSFndAllocator allocator;	// アロケータ
};
...
FLD_DEMO_ARITAMA_START* FLD_DEMO_AriTamaStart_Alloc( u32 heapID, FIELDSYS_WORK* p_fsys )
{
	...
	// モデル読み込み
	FLD_DEMO_3DObjMdlLoadH( &p_work->mdl, p_handle, NARC_demo_climax_ari_start_nsbmd, heapID );
	for( i=0; i<FLD_DEMO_ARITAMA_ANM_NUM; i++ ){
		FLD_DEMO_3DObjAnmLoadH( &p_work->anm[i], &p_work->mdl, p_handle, NARC_demo_climax_ari_start_nsbca + i, heapID, &p_work->allocator );
	}

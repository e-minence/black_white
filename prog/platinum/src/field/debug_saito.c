...
static const ID_PARAM DebugCameraMenuList[] = {
	...
	{ D_SAITO_MSG08_01, (u32)SetupCameraListZoneWindow },
};
...
static void CameraListDispTask(TCB_PTR tcb,void* work)
{
	...
	if(d_work->fld_camera == TRUE){
		camera_tbl_num = Debug_FieldCameraTblNumGet();
	}
	else{
		camera_tbl_num = NELEMS(TestOrgCamera);
	}
	
	flg = FALSE;
	if ( sys.trg & PAD_KEY_RIGHT ){
		flg = TRUE;
		if (d_work->camera_tbl_no == camera_tbl_num-1){
		...
		if (d_work->camera_tbl_no == 0){
			d_work->camera_tbl_no = camera_tbl_num-1;
		}else{
			d_work->camera_tbl_no--;
		}
	}
		...
		//カメラセット
		{
			if(d_work->fld_camera == TRUE){
				Debug_FieldCameraTblPtrGet(
					...
					d_work->camera, d_work->camera_tbl_no);
			}
			else{
				GFC_InitCameraTDA(
					Player_VecPosPtrGet(d_work->fsys->player),
...
//カメラリスト用ウィンドウセット
//	SetupCameraListWindowとの違いは、こちらはフィールドカメラ(ゾーンデータ表で設定出来るカメラ)
//	を使用すること
static void SetupCameraListZoneWindow(TCB_PTR tcb,void *work)
{
	struct _FIELDSYS_WORK * fsys;

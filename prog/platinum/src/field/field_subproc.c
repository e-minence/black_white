...
//frontier_systemから呼び出すためにstatic外しました
...
//------------------------------------------------------------------
/**
 * @brief	ぐるぐる交換ポケモンリスト呼び出し
 * @param	fsys	フィールド制御ワークへのポインタ
 * @return	void *	ポケモンリスト用パラメータのポインタ
 */
//------------------------------------------------------------------
void * Guru2ListEvent_SetProc( FIELDSYS_WORK * fsys )
{
	PLIST_DATA * pld = PLISTDATA_Create(HEAPID_WORLD, fsys, PL_TYPE_SINGLE, PL_MODE_GURU2 );
	GameSystem_StartSubProc( fsys, &PokeListProcData, pld );
	return pld;
}
 ...
 */
//------------------------------------------------------------------
PSTATUS_DATA * PSTATUS_Temoti_Create(FIELDSYS_WORK* fsys,int heapID, int mode)
{
	PSTATUS_DATA* psd;
	SAVEDATA	*sv;
...
//		EventCmd_StartFieldMap(event);
	...
	case 7:
...

		{
			...
			if(CommStateIsWifiConnect()){
				RECORD_Inc( record, RECID_WIFICLUB_TRADE );
			}
		}
 ...
 */
//--------------------------------------------------------------
void BattleRecorder_SetProc( FIELDSYS_WORK *fsys,SAVEDATA* pSaveData )
{
	GameSystem_StartSubProc( fsys, &BattleRecorder_ManagerProcData, fsys ); 
}
...
void ShipDemoGarinko_ProcSet( FIELDSYS_WORK * fsys , void * wk )
{
	...
	const PROC_DATA ProcData = {
		...
		ShipDemoGarinkoProc_Main,
		ShipDemoGarinkoProc_End,
		FS_OVERLAY_ID(ship_demo)
	};
 ...
 *
 *	@param		fsys		フィールドワーク
 *	@param		type		シングル、ダブルなど(src/frontier/stage_def.h参照)
 *	@param		fr_no		施設ナンバー
 *	@param		heapID		ヒープID
 *
 *	@retval		"FRONTIER_MONITOR_CALL型のポインタ"
 */
//-----------------------------------------------------------------------------
#include "application/frontier_monitor.h"
void* FrontierMonitor_SetProc( FIELDSYS_WORK * fsys, u8 type, u8 fr_no, u16 monsno, int heapID )
{
	...
	static PROC_DATA FrontierMonitorProc = {
		...
		FrontierMonitorProc_End,
		FS_OVERLAY_ID(frontier_monitor),
	};

	bsm_call_wk = sys_AllocMemory( heapID, sizeof(FRONTIER_MONITOR_CALL) );

	bsm_call_wk->type	= type;
	bsm_call_wk->fr_no	= fr_no;
	bsm_call_wk->monsno	= monsno;
	bsm_call_wk->sv		= fsys->savedata;

	GameSystem_StartSubProc( fsys,  &FrontierMonitorProc, bsm_call_wk );
...
//--------------------------------------------------------------
/**
 * @brief   フォルムチェンジする為のポケモンリスト画面：サブプロセス登録
 *
 * @param   fsys		
 * @param   heapID		
 * @param   item		
 ...
 */
//--------------------------------------------------------------
PLIST_DATA * FieldFormChangePokeList_SetProc(FIELDSYS_WORK* fsys,int heapID, int item)
{
	PLIST_DATA * pld;

	pld  = sys_AllocMemory( heapID, sizeof(PLIST_DATA) );
	...
	pld->scwk = NULL;//&mwk->skill_check;
	pld->type = PL_TYPE_SINGLE;
	pld->mode = PL_MODE_ITEMUSE;
	pld->item = item;
	pld->ret_sel = 0;//iuwk->use_poke;
	GameSystem_StartSubProc( fsys, &PokeListProcData, pld );
...
//--------------------------------------------------------------
#if 0
FORMDEMO_CALL_DATA * FieldFormChangeDemo_SetProc(FIELDSYS_WORK* fsys,int pos, int after_form_no)
{
	POKEPARTY * party;
	POKEMON_PARAM * pp;
	...
	fcd = sys_AllocMemory(HEAPID_WORLD, sizeof(FORMDEMO_CALL_DATA));
	MI_CpuClear8(fcd, sizeof(FORMDEMO_CALL_DATA));
	
	fcd->pp = pp;
	fcd->after_form_no = after_form_no;
	fcd->config = SaveData_GetConfig(fsys->savedata);
	fcd->zw = SaveData_GetZukanWork(fsys->savedata);
	fcd->pos = pos;
	GameSystem_StartSubProc( fsys,  &FormDemoProc, fcd );
 ...
 *	@param		heapID		ヒープID
 *
 *	@retval		"SCRATCH_CALL_WORK型のポインタ"
 */
//-----------------------------------------------------------------------------
#include "application/scratch.h"
void* Scratch_SetProc( FIELDSYS_WORK * fsys, int heapID )
{
	...
	static PROC_DATA ScratchProc = {
		...
		ScratchProc_Main,
		ScratchProc_End,
		FS_OVERLAY_ID(scratch),
	};

	scratch_call_wk = sys_AllocMemory( heapID, sizeof(SCRATCH_CALL_WORK) );
	memset( scratch_call_wk, 0, sizeof( SCRATCH_CALL_WORK ) );

	//scratch_call_wk->type	= type;
	//scratch_call_wk->fr_no	= fr_no;
	scratch_call_wk->sv		= fsys->savedata;

	GameSystem_StartSubProc( fsys,  &ScratchProc, scratch_call_wk );

	return scratch_call_wk;
}
...
void* WazaOshiePokeStatusEvent_SetProc( int heap, FIELDSYS_WORK* fsys, u16 pos ,u16 waza )
{
	...
	psd				= sys_AllocMemory( HEAPID_WORLD, sizeof(PSTATUS_DATA) );
	//psd				= sys_AllocMemory( HEAPID_EVENT, sizeof(PSTATUS_DATA) );
	psd->ppd		= PokeParty_GetMemberPointer( SaveData_GetTemotiPokemon(fsys->savedata), pos );
	psd->cfg		= SaveData_GetConfig( fsys->savedata );
	psd->ppt		= PST_PP_TYPE_POKEPARAM;
	psd->pos		= 0;
	psd->max		= 1;
	psd->waza		= waza;
	psd->mode		= PST_MODE_WAZAADD;
	...
	PokeStatus_PageSet( psd, waza_oshie_page );
	PokeStatus_PlayerSet( psd, SaveData_GetMyStatus(fsys->savedata) );
	GameSystem_StartSubProc( fsys, &PokeStatusProcData, psd );
	return psd;
}
...
//--------------------------------------------------------------
/**
 * @brief   バトルレコーダー(GDSモード)を呼び出す
 *
 * @param   fsys		
 * @param   connect		初回接続かどうか(0:2度目以降、0以外:初回接続)
 * @param   gds_mode	呼び出すGDS機能(BR_MODE_GDS_???)
 *
 * @retval  
 *
 * connectフラグはネット接続時の「はい / いいえ」を出すかどうかの判定で使用。
 * 初回接続の場合は、ここに来る前にGSプロファイルID取得で、
 * ネットに接続するか「はい / いいえ」選択をしているので、バトルレコーダー側では
 * 選択は出さないようにする。
 */
//--------------------------------------------------------------
...
*/
	...
	param->savedata = fsys->savedata;
	param->connect = connect;
	param->gds_mode = gds_mode;
	
	GameSystem_StartSubProc(fsys,  &GdsMainProcData, param );
}
...
//
//============================================================================================
#include "application/library_tv.h"

FS_EXTERN_OVERLAY(library_tv);
//-----------------------------------------------------------------------------
/**
 * @brief	図書館TV用PROC定義データ
 */
//-----------------------------------------------------------------------------
...
{
	GameSystem_StartSubProc(fsys, &LibraryTV_proc_data, fsys->savedata);
}

...
BATTLE_PARAM * BattleParam_Create(int heapID, u32 fight_type)
{
	...
	//録画データ生成のため、ランダムの種を固定しておく
	{
		RTCDate date;
		RTCTime time;

		GF_RTC_GetDateTime(&date, &time);
		bp->rand = date.year + date.month * 0x100 * date.day * 0x10000
...
void BattleParam_SetParamByGameDataCore(BATTLE_PARAM* bp, const FIELDSYS_WORK* fsys, SAVEDATA* savedata, int zone_id, FNOTE_DATA* fnote, BAG_CURSOR* bag_cursor, BATTLE_CURSOR_DISP* battle_cursor )
{
	...
	POKEPARTY * temoti = SaveData_GetTemotiPokemon(savedata);
	MYITEM * item = SaveData_GetMyItem(savedata);
	ZUKAN_WORK * zw = SaveData_GetZukanWork(savedata);
	PERAPVOICE * pv = SaveData_GetPerapVoice(savedata);
	CONFIG * cfg = SaveData_GetConfig(savedata);
	...
	if( fsys != NULL ){
		...
		{
			GMTIME * tm = SaveData_GetGameTime(savedata);
			bp->time_zone = GF_RTC_ConvertHourToTimeZone(tm->sv_time.hour);
		}
	}
	...
	bp->shinka_place_mode = ZoneData_GetShinkaPlaceID(zone_id);
	bp->contest_see_flag = PokeStatus_ContestFlagGet(savedata);
	bp->mizuki_flag = SysFlag_MizukiCheck(SaveData_GetEventWork(savedata));
	bp->weather = Situation_GetWeatherID(sit);

	bp->bag_cursor = bag_cursor;
	bp->battle_cursor = battle_cursor;

	bp->poketch_data = SaveData_GetPoketchData(savedata);

	bp->wifihistory = SaveData_GetWifiHistory(savedata);

	bp->record = SaveData_GetRecord(savedata);

	bp->fnote_data = fnote;

	bp->friendlist = SaveData_GetFriendList( savedata );

	bp->zone_id = zone_id;
	
	bp->savedata = savedata;
}
#endif
...
void BattleParam_SetParamByGameData(BATTLE_PARAM * bp, const FIELDSYS_WORK * fsys)
{
	...
	BattleParam_SetParamByGameDataCore(bp,fsys,fsys->savedata,
...
void BattleParam_SetParamByWiFiGameData(BATTLE_PARAM * bp, const FIELDSYS_WORK * fsys, int lvLimit)
{
	...
	bp->time_zone = EVTIME_GetTimeZone(fsys);
	bp->bag_cursor = fsys->bag_cursor;
	bp->battle_cursor = fsys->battle_cursor;
	bp->poketch_data = SaveData_GetPoketchData(fsys->savedata);
	bp->wifihistory = SaveData_GetWifiHistory(fsys->savedata);
	bp->record = SaveData_GetRecord(fsys->savedata);
	bp->fnote_data = fsys->fnote;
...
static int GetGroundID(const FIELDSYS_WORK * fsys, int bg_id)
{
	...
	static const BasicGroundTable[] = {
		...
		GROUND_ID_CAVE,
		GROUND_ID_CAVE,
		GROUND_ID_BIG_FOUR_A,
		GROUND_ID_BIG_FOUR_B,
		GROUND_ID_BIG_FOUR_C,
		GROUND_ID_BIG_FOUR_D,
		GROUND_ID_CHAMPION,
		GROUND_ID_HAKAI,
		GROUND_ID_TOWER,
		GROUND_ID_FACTORY,
...
void	BattleParam_EnemyTrainerDataMake(BATTLE_PARAM *bp)
{
	...
	bp->trainer_data[CLIENT_NO_MINE2]=bp->trainer_data[CLIENT_NO_ENEMY];
}

...
u32 FieldOBJ_PosHitCheck( CONST_FIELD_OBJ_SYS_PTR fos, int x, int z )
{
	...
	if( FieldOBJ_SearchGridPos(fos,x,z,FALSE) != NULL ){
...
int FieldOBJ_MoveHitCheckAttr( CONST_FIELD_OBJ_PTR fldobj, int x, int z, int dir )
{
	...
	MATR next_attr = MATR_IsNotAttrGet();
	
	if( FieldOBJ_MoveBitCheck_AttrGetOFF(fldobj) == FALSE ){
		next_attr = GetAttributeLSB( fsys, x, z );
	}
...
int FieldOBJ_NowPosMapAttrSet( FIELD_OBJ_PTR fldobj )
{
	...
	FieldOBJ_OldMapAttrSet( fldobj, old_attr );
	FieldOBJ_NowMapAttrSet( fldobj, now_attr );

...
#define EOA_STA_BIT_INIT_COMP	(1<<1)						///<初期化完了
		...
		const VecFx32 *mtx, int prm, const void *ptr, int pri )
{
	...
	if( mtx != NULL ){ EOA_MatrixSet(eoa,mtx); }
	else{ VecFx32 pos={0,0,0}; EOA_MatrixSet(eoa,&pos); }
	...
	eoa_StatusBit_ON( eoa, EOA_STA_BIT_INIT_COMP );
	es->add_count++;
...
void EOA_SysDraw( EOA_SYS_PTR es )
{
	...
	do{
		ret = eoa_StatusBit_Check( eoa, EOA_STA_BIT_USE_INIT_CHECK );
		
		if( ret == EOA_STA_BIT_USE_INIT_CHECK ){
 ...
 * @param	eoa		EOA_PTR
 * @param	bit		EOA_STA_BIT_NON等
 * @retval	u32		TRUE=使用中。FALSE=未使用
 */
//--------------------------------------------------------------
static u32 eoa_StatusBit_Check( EOA_PTR eoa, u32 bit )
{
	return( eoa->status_bit&bit );
}

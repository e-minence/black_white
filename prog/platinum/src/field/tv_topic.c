 ...
 * @brief	バトルステージ状況の保持
 * @param	tvwk		テレビセーブデータへのポインタ
 * @param	monsno		挑戦したポケモン
 */
//--------------------------------------------------------------------
void TVTOPIC_StageTemp_Set(TV_WORK * tvwk, u16 monsno)
{
 ...
 */
//--------------------------------------------------------------------
void TVTOPIC_BPTemp_Set(TV_WORK * tvwk, u32 add_bp)
{
...
void TVTOPIC_Entry_Watch_OsuMesu(FIELDSYS_WORK * fsys)
{
	...
	for( i=0; i < max; i++ ){
		...
		if( PokeParaGet(pp,ID_PARA_tamago_flag,NULL) != 0 ){
			if( PokeParaGet(pp,ID_PARA_sex,NULL) == PM_MALE ){
				male = 1;
			}else{
				female = 1;
			}
 ...
 */
//--------------------------------------------------------------------
void TVTOPIC_Entry_Record_BP(FIELDSYS_WORK * fsys, u32 add_point)
{
	TOPIC_DATA TopicData;
	RECORD_BP_WORK * rbw = &TopicData.RecordBPWork;
	TOPICTEMP_BP * pbw = TVWORK_GetBPTemp(SaveData_GetTvWork(fsys->savedata));
	rbw->bp = *pbw;
	pbw->exist_flag = FALSE;
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_TVWORK)
	SVLD_SetCrc(GMDATA_ID_TVWORK);
#endif //CRC_LOADCHECK
	MakeTopic(fsys, TVTOPICTYPE_RECORD, TVTOPIC_RECORD_BP, rbw);
 ...
 */
//--------------------------------------------------------------------
void TVTOPIC_Entry_Record_GTS(FIELDSYS_WORK * fsys, u32 add_point)
{
	TOPIC_DATA TopicData;
	RECORD_GTS_WORK * rgw = &TopicData.RecordGTSWork;
	TOPICTEMP_GTS * pgw = TVWORK_GetGTSTemp(SaveData_GetTvWork(fsys->savedata));
	rgw->gts = *pgw;
	pgw->exist_flag = FALSE;
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_TVWORK)
	SVLD_SetCrc(GMDATA_ID_TVWORK);
#endif //CRC_LOADCHECK
	MakeTopic(fsys, TVTOPICTYPE_RECORD, TVTOPIC_RECORD_GTS, rgw);
}
//--------------------------------------------------------------------
///	トピック放送：GTS名人
//--------------------------------------------------------------------
...
static int TVTOPIC_Make_Event_PokemonCharactor(FIELDSYS_WORK * fsys, WORDSET * ws, TVBCD * bcd)
{
	...
	//「○に自信があるんだ」
	WORDSET_RegisterPokeStatusName( ws, 2, STATUS_001+status );
...
static int TVTOPIC_Make_Event_MovePokemon(FIELDSYS_WORK * fsys, WORDSET * ws, TVBCD * bcd)
{
	...
	//移動ポケモン名をセット
	for( i=0; i < MOVE_POKE_MAX ;i++ ){						//3匹
		if( EncDataSave_IsMovePokeValid(data,i) ){			//0-2
...
static int TVTOPIC_Make_Variety_SearchParty(FIELDSYS_WORK * fsys, WORDSET * ws, TVBCD * bcd)
{
	...
	if( SysFlag_GameClearCheck(SaveData_GetEventWork(fsys->savedata)) == TRUE ){
		rand = ( gf_rand() % 3 );
		if( rand == 0 ){
			return msg_tv_05_01_06;
		}else if( rand == 1 ){
			return msg_tv_05_01_07;
		}else{
			return msg_tv_05_01_08;
		}

	//ヨスガ到着していたら
	}else if( SysFlag_ArriveGet(SaveData_GetEventWork(fsys->savedata),FLAG_ARRIVE_C05) == 1 ){
			return msg_tv_05_01_05;

	//ハクタイ到着していたら
	}else if( SysFlag_ArriveGet(SaveData_GetEventWork(fsys->savedata),FLAG_ARRIVE_C04) == 1 ){
			return msg_tv_05_01_04;

	//ハクタイの森到着していたら(08.02.25 佐藤さんに確認済み)
	}else if( SysFlag_ArriveGet(SaveData_GetEventWork(fsys->savedata),FLAG_ARRIVE_D03) == 1 ){
			return msg_tv_05_01_03;
	};

	//ゲーム最初から
	rand = ( gf_rand() % 2 );
	if( rand == 0 ){
		return msg_tv_05_01_01;
	}else{
		return msg_tv_05_01_02;
	};
}
...
static const TOPIC_FUNC_DATA VarietyNewsFuncTable[TVTOPIC_VARIETY_TYPE_MAX - 1] = {
	...
	{	//TVTOPIC_EVENT_VARIETY_DORAMA3,		///<連続ドラマ★

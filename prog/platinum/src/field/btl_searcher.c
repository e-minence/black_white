...
enum{
	...
	BS_SEQ_OK_BATTERY_LOTTERY_SE_WAIT,
	BS_SEQ_NO_BATTERY,
	BS_SEQ_NO_TRAINER,
	BS_SEQ_EXIT,
};
...
static BOOL GMEVENT_StartBtlSearcher( GMEVENT_CONTROL* event )
{
	...
	case BS_SEQ_OK_BATTERY:
		bs_sys->player_tcb = Player_BSDrawProcSet( bs_sys->fsys );		//形態変更
	...
	case BS_SEQ_OK_BATTERY_LOTTERY_SE_WAIT:
		if( Snd_SePlayCheck(SE_BTL_SEARCHER) == FALSE ){
			Player_BSDrawProcDelete( bs_sys->player_tcb );			//形態削除

...
#include "msgdata/msg_place_name.h"		// 改訂版で追加 2006/10/24 by nakahiro
 ...
 */
//--------------------------------------------------------------------------------------------
BOOL FieldMenuCallCheck( FIELDSYS_WORK * repw )
{
	if( ZoneData_GetPlaceNameID( repw->location->zone_id ) == MAPNAME_NOTHING ){
		return FALSE;
	}
	return TRUE;
}
...
static void CellActSet( FLD_MENU * wk, u8 * list, u32 max, u8 sex )
{
	...
	FldClact_LoadResCellAnmArcH(
		&wk->fcat, p_handle, NARC_menu_gra_fmenu_cur_NANR, 0, FLDMENU_CLA_ANM_H_ID_CURSOR );

	FldClact_LoadResourceCharArcH(
		&wk->fcat, p_handle, NARC_menu_gra_fmenu_cur_NCGR,
	...
	FldClact_LoadResCellAnmArcH(
		&wk->fcat, p_handle, NARC_menu_gra_fmenu_obj_NANR, 0, FLDMENU_CLA_ANM_H_ID_ICON );

	FldClact_LoadResourceCharArcH(
		&wk->fcat, p_handle, NARC_menu_gra_fmenu_obj_NCGR,
...
BOOL FldMenu_PokeListEnd( GMEVENT_CONTROL * event )
{
	...
	case PL_RET_LVUPSHINKA:		// 進化画面へ（レベルアップ進化）
		{
			FLDMENU_SHINKA_WORK * swk = sys_AllocMemory(HEAPID_WORLD,sizeof(FLDMENU_SHINKA_WORK));
 ...
 *
 * @param	fsys		フィールドワーク
 * @param	wk			フィールドメニューワーク
 * @param	list_mode	ポケモンリストの処理モード
 *
 * @return	none
 */
//--------------------------------------------------------------------------------------------
 ...
 */
//--------------------------------------------------------------------------------------------
BOOL FldMenu_BattleRecoderEnd( GMEVENT_CONTROL * event )
{
	FIELDSYS_WORK * fsys;
	FLD_MENU * wk;

...
{ 
  ...
  pmw->biw = BTLV_INPUT_InitEx( PDC_GetGameData( psp ), BTLV_INPUT_TYPE_SINGLE, BTLV_EFFECT_GetPfd(),
                                pmw->large_font, &pmw->cursor_flag, pmw->heapID );
...
{ 
  ...
  case 0:
    ...
    BTLV_INPUT_SetFadeOut( pmw->biw, 0 );
  ...
  case 10:
    ...
    { 
      ...
      //PMSND_FadeOutBGM( 8 );  // 捕獲したときは、図鑑登録や名前入力へ遷移するので、BGMはそのままにする
      pmw->seq_no++;
    }
    break;
  case 11:

...
{ 
  ...
  RRG_RESULT finishResult;        // 画面終了結果
  BOOL finishFlag;
};
...
// シーケンス制御
static void CountUpSeqCount( RRG_WORK* work ); // シーケンスカウンタを更新する
static void SetNextSeq( RRG_WORK* work, RESEARCH_CHECK_SEQ nextSeq ); // 次のシーケンスをキューに登録する
static void FinishCurrentSeq( RRG_WORK* work ); // 現在のシーケンスを終了する
static void SwitchSeq( RRG_WORK* work ); // 処理シーケンスを変更する
static void SetSeq( RRG_WORK* work, RESEARCH_CHECK_SEQ nextSeq ); // 処理シーケンスを設定する
static void SetResult( RRG_WORK* work, RRG_RESULT result ); // 画面終了結果を設定する
static void SetWaitFrame( RRG_WORK* work, u32 frame ); // フレーム経過待ちシーケンスの待ち時間を設定する
static u32 GetWaitFrame( const RRG_WORK* work ); // フレーム経過待ちシーケンスの待ち時間を取得する
RESEARCH_CHECK_SEQ GetFirstSeq( const RRG_WORK* work ); // 最初のシーケンスを取得する    
//-----------------------------------------------------------------------------------------
// ◇LAYER 3 機能
//-----------------------------------------------------------------------------------------
// カーソル
...
// 調査データ表示タイプ
static void SetDataDisplayType( RRG_WORK* work, DATA_DISP_TYPE dispType ); // 調査データの表示タイプを設定する
//「報告を見る」ボタン
...
static void SetupMainCircleGraph( RRG_WORK* work, DATA_DISP_TYPE dispType ); // メイン円グラフを現在の調査データで構成する
static void SetupSubCircleGraph ( RRG_WORK* work, DATA_DISP_TYPE dispType ); // サブ円グラフを現在の調査データで構成する
...
// BG
static void SetupBG( RRG_WORK* work ); // BG全般にかかわるセットアップを行う
static void SetupSubBG_WINDOW( RRG_WORK* work ); // SUB-BG ( ウィンドウ面 ) をセットアップする
static void SetupSubBG_FONT( RRG_WORK* work ); // SUB-BG ( フォント面 ) をセットアップする
static void SetupMainBG_WINDOW( RRG_WORK* work ); // MAIN-BG ( ウィンドウ面 ) をセットアップする
static void SetupMainBG_FONT( RRG_WORK* work ); // MAIN-BG ( フォント面 ) をセットアップする
static void CleanUpBG( RRG_WORK* work ); // BG全般にかかわるクリーンアップを行う
static void CleanUpSubBG_WINDOW( RRG_WORK* work ); // SUB-BG ( ウィンドウ面 ) をクリーンアップする
static void CleanUpSubBG_FONT( RRG_WORK* work ); // SUB-BG ( フォント面 ) をクリーンアップする
static void CleanUpMainBG_WINDOW( RRG_WORK* work ); // MAIN-BG ( ウィンドウ面 ) をクリーンアップする
static void CleanUpMainBG_FONT( RRG_WORK* work ); // MAIN-BG ( フォント面 ) をクリーンアップする
...
// BMP-OAM
static void InitBitmapDatas( RRG_WORK* work ); // ビットマップデータを初期化する
static void CreateBitmapDatas( RRG_WORK* work ); // ビットマップデータを生成する
static void SetupBitmapData_forDefault( RRG_WORK* work ); // ビットマップデータをセットアップする ( デフォルト ) 
static void SetupBitmapData_forANALYZE_BUTTON( RRG_WORK* work ); // ビットマップデータをセットアップする (「報告を見る」ボタン ) 
static void DeleteBitmapDatas( RRG_WORK* work ); // ビットマップデータを破棄する
static void SetupBmpOamSystem( RRG_WORK* work ); // BMP-OAM システムをセットアップする
static void CleanUpBmpOamSystem( RRG_WORK* work ); // BMP-OAM システムをクリーンアップする
static void CreateBmpOamActors( RRG_WORK* work ); // BMP-OAM アクターを生成する
static void DeleteBmpOamActors( RRG_WORK* work ); // BMP-OAM アクターを破棄する
static void BmpOamSetDrawEnable( RRG_WORK* work, BMPOAM_ACTOR_INDEX BmpOamActorIdx, BOOL enable );  // 表示するかどうかを設定する
// パレットアニメーション
static void InitPaletteAnime( RRG_WORK* work ); // パレットアニメーションワークを初期化する
static void CreatePaletteAnime( RRG_WORK* work ); // パレットアニメーションワークを生成する
static void DeletePaletteAnime( RRG_WORK* work ); // パレットアニメーションワークを破棄する
static void SetupPaletteAnime( RRG_WORK* work ); // パレットアニメーションワークをセットアップする
static void CleanUpPaletteAnime( RRG_WORK* work ); // パレットアニメーションワークをクリーンアップする
...
static void StartPaletteFadeOut( RRG_WORK* work ); // パレットのフェードアウトを開始する
static void StartPaletteFadeIn ( RRG_WORK* work ); // パレットのフェードインを開始する
...
// VBlankタスク
static void RegisterVBlankTask( RRG_WORK* work ); // VBlank タスクを登録する
static void ReleaseVBlankTask( RRG_WORK* work ); // VBlank タスクの登録を解除する
...
// OBJ
static void CreateClactSystem( RRG_WORK* work ); // OBJ システムを生成する
static void DeleteClactSystem( RRG_WORK* work ); // OBJ システムを破棄する
static void RegisterSubObjResources( RRG_WORK* work ); // SUB-OBJ リソースを登録する
static void ReleaseSubObjResources( RRG_WORK* work ); // SUB-OBJ リソース 解放する
static void RegisterMainObjResources( RRG_WORK* work ); // MAIN-OBJ リソースを登録する
static void ReleaseMainObjResources( RRG_WORK* work ); // MAIN-OBJ リソースを解放する
static void InitClactUnits( RRG_WORK* work ); // セルアクターユニットを初期化する
static void CreateClactUnits( RRG_WORK* work ); // セルアクターユニットを生成する
static void DeleteClactUnits( RRG_WORK* work ); // セルアクターユニットを破棄する
static void InitClactWorks( RRG_WORK* work ); // セルアクターワークを初期化する
static void CreateClactWorks( RRG_WORK* work ); // セルアクターワークを生成する
static void DeleteClactWorks( RRG_WORK* work ); // セルアクターワークを破棄する
...
// 通信アイコン
static void SetupWirelessIcon( const RRG_WORK* work ); // 通信アイコンをセットアップする
// データの初期化/生成/破棄
static void InitSeqQueue( RRG_WORK* work ); // シーケンスキュー 初期化
static void CreateSeqQueue( RRG_WORK* work ); // シーケンスキュー 作成
static void DeleteSeqQueue( RRG_WORK* work ); // シーケンスキュー 破棄
...
// ◇LAYER 0 ユーティリティ
//-----------------------------------------------------------------------------------------
static u8 BindU8( int num ); // u8 に収まるように丸める
//-----------------------------------------------------------------------------------------
// ◇LAYER -1 デバッグ
//-----------------------------------------------------------------------------------------
static void DebugPrint_seqQueue( const RRG_WORK* work ); // シーケンスキューの中身を表示する
...
static void MainSeq_SETUP( RRG_WORK* work )
{
  ...
  SetNextSeq( work, GetFirstSeq(work) ); // 次のシーケンスをセット
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 調査項目確定の確認シーケンスへの準備シーケンス ( RESEARCH_CHECK_SEQ_FRAME_WAIT ) の処理
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief シーケンスを初期化する ( ==> RESEARCH_CHECK_SEQ_FRAME_WAIT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief シーケンス終了処理 ( ==> RESEARCH_CHECK_SEQ_FRAME_WAIT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------

...
static void SetWaitFrame( RESEARCH_CHECK_WORK* work, u32 frame ); // フレーム経過待ちシーケンスの待ち時間を設定する
static u32 GetWaitFrame( const RESEARCH_CHECK_WORK* work ); // フレーム経過待ちシーケンスの待ち時間を取得する
RESEARCH_CHECK_SEQ GetFirstSeq( const RESEARCH_CHECK_WORK* work ); // 最初のシーケンスを取得する    
//-----------------------------------------------------------------------------------------
// ◇LAYER 3 機能
//-----------------------------------------------------------------------------------------
// カーソル
...
static void MoveMenuCursorSilent( RESEARCH_CHECK_WORK* work, MENU_ITEM menuItem ); // 直接移動する ( 点滅・SEなし )
// 表示する質問
...
static void SetAnalyzeButtonCursorOn( RESEARCH_CHECK_WORK* work ); //「報告を見る」ボタンをカーソルが乗っている状態にする
static void SetAnalyzeButtonCursorOff( RESEARCH_CHECK_WORK* work ); //「報告を見る」ボタンをカーソルが乗っていない状態にする
static void SetAnalyzeButtonCursorSet( RESEARCH_CHECK_WORK* work ); //「報告を見る」ボタンをカーソルをセットした状態にする
static void SetAnalyzeButtonActive( RESEARCH_CHECK_WORK* work ); //「報告を見る」ボタンをアクティブ状態にする
static void SetAnalyzeButtonNotActive( RESEARCH_CHECK_WORK* work ); //「報告を見る」ボタンを非アクティブ状態にする
//「戻る」ボタン
...
static BOOL CheckPaletteAnime( const RESEARCH_CHECK_WORK* work, PALETTE_ANIME_INDEX index ); // パレットアニメーション中かどうかをチェックする

...
{ 
  ...
  u32                  waitFrame;     // FRAME_WAIT シーケンスでの待ちフレーム数
...
static void SetWaitFrame( RESEARCH_MENU_WORK* work, u32 frame ); // FRAME_WAIT シーケンスの待ち時間を設定する
static u32 GetWaitFrame( const RESEARCH_MENU_WORK* work ); // FRAME_WAIT シーケンスの待ち時間を取得する
static RESEARCH_MENU_SEQ GetFirstSeq( const RESEARCH_MENU_WORK* work ); // 最初のシーケンスを取得する
//------------------------------------------------------------------------------------
// ◆LAYER 2 機能
//------------------------------------------------------------------------------------
// データ更新
...
static void SetChangeButtonNotActive( RESEARCH_MENU_WORK* work ); //『調査を決める』ボタンを非アクティブ状態にする
// "new" アイコン
...
//------------------------------------------------------------------------------------
// カーソル位置
static void SetupCursorPos( RESEARCH_MENU_WORK* work ); // カーソル位置をセットアップする
...
static BOOL CheckCursorSetEnable( RESEARCH_MENU_WORK* work, MENU_ITEM menuItem ); // カーソルを指定位置にセットできるかどうかをチェックする
// 調査初期画面ワーク
...
// 調査項目
static u8 GetSelectableTopicNum( const RESEARCH_MENU_WORK* work ); // 選択可能な調査項目の数を取得する
//------------------------------------------------------------------------------------
// ◆LAYER 0 デバッグ
//------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------
/**
 * @brief フレーム経過待ちシーケンス ( RESEARCH_MENU_SEQ_FRAME_WAIT ) の初期化処理
 *
 * @param work
 */
//------------------------------------------------------------------------------------
...
static void Main_KEY_WAIT( RESEARCH_MENU_WORK* work )
{
  ...
  if( commonTouch == COMMON_TOUCH_AREA_RETURN_BUTTON ) {
    ...
    PMSND_PlaySE( SEQ_SE_CANCEL1 );                      // キャンセル音
    SetResult( work, RESEARCH_MENU_RESULT_EXIT );        // 画面終了結果を決定
    SetNextSeq( work, RESEARCH_MENU_SEQ_FRAME_WAIT );    // → 待ち
    SetNextSeq( work, RESEARCH_MENU_SEQ_FADE_OUT );      // → フェードアウト
    SetNextSeq( work, RESEARCH_MENU_SEQ_CLEAN_UP );      // → クリーンアップ
    FinishCurrentSeq( work );
    return;
  }
...
//------------------------------------------------------------------------------------
/**
 * @brief フレーム経過待ちシーケンス ( RESEARCH_MENU_SEQ_FRAME_WAIT ) の処理
 *
 * @param work
 */
//------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------
/**
 * @brief FRAME_WAIT シーケンスの待ち時間を設定する
 *
 * @param work
 * @param frame 待ちフレーム数
 */
...
//------------------------------------------------------------------------------------
/**
 * @brief FRAME_WAIT シーケンスの待ち時間を取得する
 *
 * @param work
 */
//------------------------------------------------------------------------------------

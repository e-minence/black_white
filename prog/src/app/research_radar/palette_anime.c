...
// 点滅 ( 明 )
#define BLINK_BRIGHT_FRAMES      (20) // アニメーションフレーム数
#define BLINK_BRIGHT_WAIT_FRAMES  (4) // 点滅待ちフレーム数
#define BLINK_BRIGHT_MIN_EVY      (0) // 最小フェード係数[0, 16]
#define BLINK_BRIGHT_MAX_EVY      (9) // 最大フェード係数[0, 16]
// 暗転
...
static void UpdateAnime_BLINK_BRIGHT( PALETTE_ANIME* work )
{
  ...
  GF_ASSERT( work->animeFlag ); // アニメーション停止中
  GF_ASSERT( work->setupFlag ); // セットアップされていない
  GF_ASSERT( work->frameCount <= BLINK_BRIGHT_FRAMES ); // 不正呼び出し

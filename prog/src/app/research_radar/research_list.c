...
// 状態制御
static void CountUpStateCount( RRL_WORK* work ); // 状態カウンタを更新する
...
static void RegisterFirstStateFlow( RRL_WORK* work ); // 最初の状態遷移をセットする
//-----------------------------------------------------------------------------------------
// ◆LAYER 4 機能
//-----------------------------------------------------------------------------------------
// メニュー項目カーソル
...
// 調査項目
static void SetTopicCursorPosDirect( RRL_WORK* work, int topciID ); // 調査項目カーソルの移動先を設定する ( 直値指定 ) 
...
void RRL_Main( RRL_WORK* work )
{
  ...
  CountUpStateCount( work ); // 状態カウンタをインクリメント
...
static void MainState_SETUP( RRL_WORK* work )
{
  ...
  RegisterFirstStateFlow( work ); // 次の状態遷移をセット
  FinishCurrentState( work ); // 状態終了
}
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 調査項目確定状態 ( RRL_STATE_DETERMINE ) の処理
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
static void MainState_DETERMINE( RRL_WORK* work )
{
  ...
  if( SEQ_DETERMINE_WAIT_FRAMES <= work->stateCount ) {
    FinishCurrentState( work );                         // RRL_STATE_DETERMINE 状態終了
    RegisterNextState( work, RRL_STATE_FADE_OUT );      // => RRL_STATE_FADE_OUT 
    RegisterNextState( work, RRL_STATE_PALETTE_RESET ); // ==> RRL_STATE_PALETTE_RESET 
    RegisterNextState( work, RRL_STATE_CLEAN_UP );      // ===> RRL_STATE_CLEAN_UP 
  } 
...
//-----------------------------------------------------------------------------------------
/**
 * @brief フェードイン 状態 ( RRL_STATE_FADE_IN ) の処理
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief フェードアウト 状態 ( RRL_STATE_FADE_OUT ) の処理
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief フレーム経過待ち 状態 ( RRL_STATE_FRAME_WAIT ) の処理
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief パレット復帰状態 ( RRL_STATE_PALETTE_RESET ) の処理
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief パレット復帰状態 ( RRL_STATE_SCROLL_RESET ) の処理
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_SETUP )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_STANDBY )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_KEY_WAIT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_CONFIRM_STANDBY )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_CONFIRM_KEY_WAIT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_DETERMINE )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_FADE_IN )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_FADE_OUT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_FRAME_WAIT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_PALETTE_RESET )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_SCROLL_RESET )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態を初期化する ( ==> RRL_STATE_CLEAN_UP )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_STANDBY )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_KEY_WAIT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_CONFIRM_STANDBY )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_CONFIRM_KEY_WAIT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_DETERMINE )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_FADE_IN )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_FADE_OUT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_FRAME_WAIT )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_PALETTE_RESET )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_SCROLL_RESET )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
/**
 * @brief 状態終了処理 ( ==> RRL_STATE_CLEAN_UP )
 *
 * @param work
 */
//-----------------------------------------------------------------------------------------

...
#define	BMPWIN_SEARCH_WIN_PY		( 18 )
#define	BMPWIN_SEARCH_WIN_SX		( 30 )
#define	BMPWIN_SEARCH_WIN_SY		( 2 )
#define	BMPWIN_SEARCH_WIN_PAL		( 5 )
...
static void WriteDefaultStr( ZKNSEARCHMAIN_WORK * wk )
{
	...
	if( wk->dat->sort->mode == ZKNCOMM_LIST_SORT_MODE_ZENKOKU ){
		StrPrint(
			wk, ZKNSEARCHBMP_WINIDX_LABEL1, ZKN_SEARCH_23, 0, 4, FCOL_WP15WN, PRINTTOOL_MODE_LEFT );
	}else{
		StrPrint(
			wk, ZKNSEARCHBMP_WINIDX_LABEL1, ZKN_SEARCH_22, 0, 4, FCOL_WP15WN, PRINTTOOL_MODE_LEFT );
	}
...
static void PrintSearchWin( ZKNSEARCHMAIN_WORK * wk, u32 msg )
{
	...
	StrPrint(
		...
		BMPWIN_SEARCH_WIN_SX*8/2, 0, FCOL_MP05, PRINTTOOL_MODE_CENTER );

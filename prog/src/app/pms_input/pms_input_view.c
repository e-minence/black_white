...
//	PMSIV_BUTTON*		button_wk;
  ...
  // 背景BGの転送を管理する変数
  BOOL        back_screen_req_trans;   // TRUEのときVBlankで背景BG転送
  BOOL        back_screen_is_wordwin;  // TRUEのとき単語リストウィンドウ、FALSEのとき通常の背景
  ...
  // 背景BGの転送を管理する変数
  {
    if( vwk->back_screen_req_trans )
    {
      ...
      // 背景面転送
	    ...
	    //	FRM_MAIN_BACK, 0, 0, FALSE, HEAPID_PMS_INPUT_VIEW );  // スクリーンをここで転送すると画面が一瞬崩れるので、コメントアウト。

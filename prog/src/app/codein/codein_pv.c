...
BOOL CI_pv_MainInit( CODEIN_WORK* wk )
{
	...
	CI_pv_disp_BMP_WindowAdd( &wk->sys.win, GFL_BG_FRAME0_S, 2, 21, 27, 2, wk->sys.fontHandle );
	...
	case 0:
		CI_pv_disp_CurOAM_Visible( wk, 0, FALSE );
		{
			...
			for ( i = 0; i < BAR_OAM_MAX; i++ ){
				...
				GFL_CLACT_WK_GetPos( wk->bar[ i ].clwk, &pos , CLSYS_DEFREND_MAIN );
				pos.x += wk->bar[ i ].move_wk.pos.x;
				pos.y += wk->bar[ i ].move_wk.pos.y;
				GFL_CLACT_WK_SetPos( wk->bar[ i ].clwk, &pos , CLSYS_DEFREND_MAIN );
...
void CI_pv_Input_End( CODEIN_WORK* wk )
{
	...
	STRBUF* str_dest = GFL_STR_CreateBuffer( 100, HEAPID_CODEIN );

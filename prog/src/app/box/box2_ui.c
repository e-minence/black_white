...
static const POINTER_WORK PartyOutMainKeyTbl[] =
{
...
static const POINTER_WORK PartyOutBoxSelKeyTbl[] =
{
...
static const POINTER_WORK MarkingKeyTbl[] =
{
...
static const POINTER_WORK PartyInMainKeyTbl[] =
{
...
static const POINTER_WORK BoxThemaChgKeyTbl[] =
{
...
static const POINTER_WORK WallPaperChgKeyTbl[] =
{
...
static const POINTER_WORK BoxArrangeMainKeyTbl[] =
{
	{ TRAYPOKE_PX(0)+PICUR_X,TRAYPOKE_PY(0)+PICUR_Y, 0, 0,	30,  6, 5, 1 },		// 00: ポケモン
	{ TRAYPOKE_PX(1)+PICUR_X,TRAYPOKE_PY(0)+PICUR_Y, 0, 0,	30,  7, 0, 2 },
	{ TRAYPOKE_PX(2)+PICUR_X,TRAYPOKE_PY(0)+PICUR_Y, 0, 0,	30,  8, 1, 3 },
...
static const POINTER_WORK BoxArrangePokeMoveKeyTbl[] =
{
...
static const POINTER_WORK BoxArrangePartyPokeKeyTbl[] =
{
...
static const POINTER_WORK BoxItemArrangeMainKeyTbl[] =
{
	{ TRAYPOKE_PX(0)+PICUR_X,TRAYPOKE_PY(0)+PICUR_Y, 0, 0,	30,  6, 5, 1 },		// 00: ポケモン
	{ TRAYPOKE_PX(1)+PICUR_X,TRAYPOKE_PY(0)+PICUR_Y, 0, 0,	30,  7, 0, 2 },
	{ TRAYPOKE_PX(2)+PICUR_X,TRAYPOKE_PY(0)+PICUR_Y, 0, 0,	30,  8, 1, 3 },
...
static const POINTER_WORK BoxItemArrangePartyPokeKeyTbl[] =
{
...
// Ｙステータスボタンチェック
static const GFL_UI_TP_HITTBL YStatusHitTbl = { 168, 191, 0, 135 };
//↑[GS_CONVERT_TAG]
...
static const MOVE_DATA_TBL MoveDataTable[] =
{
	...
	{	// あずける・ボックス選択
		PartyOutBoxSelHitTbl,
		PartyOutBoxSelKeyTbl,
	...
	{	// ボックス整理・メイン
		BoxArrangeMainHitTbl,
		BoxArrangeMainKeyTbl,
		&BoxArrangeMainCallBack
	},
	{	// ボックス整理・ポケモン移動
		BoxArrangePokeMoveHitTbl,
		BoxArrangePokeMoveKeyTbl,
		&BoxArrangePokeMoveCallBack
	},
	{	// ボックス整理・手持ちポケモン
		BoxArrangePartyPokeHitTbl,
		BoxArrangePartyPokeKeyTbl,
		&BoxArrangePartyPokeCallBack
	},

	{	// 持ち物整理・メイン
		BoxItemArrangeMainHitTbl,
		BoxItemArrangeMainKeyTbl,
		&BoxItemArrangeMainCallBack
	},
	{	// 持ち物整理・手持ちポケモン
		BoxItemArrangePartyPokeHitTbl,
		BoxItemArrangePartyPokeKeyTbl,
 ...
 */
//--------------------------------------------------------------------------------------------
void BOX2KEY_CursorMoveInit( BOX2_SYS_WORK * syswk )
{
	u32		id;

	switch( syswk->dat->mode ){
	case BOX_MODE_AZUKERU:
		id = BOX2KEY_INIT_ID_PTOUT_MAIN;
		break;
	case BOX_MODE_TURETEIKU:
		id = BOX2KEY_INIT_ID_PTIN_MAIN;
		break;
	case BOX_MODE_SEIRI:
	...
	syswk->app->cmwk = CURSORMOVE_Create(
							MoveDataTable[id].hit,
							MoveDataTable[id].pos,
							MoveDataTable[id].cb,
							syswk,
							TRUE,
 ...
 */
//--------------------------------------------------------------------------------------------
void BOX2KEY_CursorMoveChange( BOX2_SYS_WORK * syswk, u32 id, u32 pos )
{
	BOX2KEY_CursorMoveExit( syswk );
 ...
 */
//--------------------------------------------------------------------------------------------
void BOX2KEY_CursorMoveExit( BOX2_SYS_WORK * syswk )
{
...
static void CursorMoveCallBack_Move( void * work, int now_pos, int old_pos )
{
	...
	BOX2KEY_CursorMoveVFuncWorkSet( syswk->app, now_pos, old_pos );
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_PartyOutMain( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	u8	now;
	...
	if( ret < NELEMS(PartyOutMainKeyTbl) ){
		if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
			if( ret >= BOX2KEY_PTOUT_MAIN_AZUKERU && ret <= BOX2KEY_PTOUT_MAIN_CLOSE ){
				CURSORMOVE_PosSet( syswk->app->cmwk, now );
				CursorObjMove( syswk, now );
				return CURSORMOVE_NONE;
			}
		}else{
			if( ret == BOX2KEY_PTOUT_MAIN_RETURN ){
				return BOX2KEY_PTOUT_MAIN_CLOSE;
			}
		}
		return ret;
	}
	...
	if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == TRUE ){
		if( ret == BOX2KEY_PTOUT_MAIN_RETURN || ret == CURSORMOVE_CANCEL ){
			return BOX2KEY_PTOUT_MAIN_CLOSE;
		}
	}
	...
	if( old_pos != CURSORMOVE_ONOFF_DIRECT ){
		if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
			if( syswk->get_pos != BOX2MAIN_GETPOS_NONE &&
				...
				now_pos = BOX2KEY_PTOUT_MAIN_POKE1;
			}
		}else{
			if( now_pos != BOX2KEY_PTOUT_MAIN_STATUS &&
				now_pos != BOX2KEY_PTOUT_MAIN_MARKING &&
				now_pos != BOX2KEY_PTOUT_MAIN_FREE ){
				now_pos = BOX2KEY_PTOUT_MAIN_AZUKERU;
			}
		}
	}
...
static void PartyOutBoxSelCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( now_pos == BOX2KEY_PTOUT_BOXSEL_LEFT || now_pos == BOX2KEY_PTOUT_BOXSEL_RIGHT ){
	...
	pw = CURSORMOVE_PointerWorkGet( syswk->app->cmwk, now_pos );
...
//↑[GS_CONVERT_TAG]
	...
	if( now_pos >= BOX2KEY_PTOUT_BOXSEL_TRAY1 && now_pos <= BOX2KEY_PTOUT_BOXSEL_TRAY6 ){
...
static void PartyOutBoxSelCallBack_Move( void * work, int now_pos, int old_pos )
{
	...
	//「あずける」からトレイへの移動
	if( old_pos == BOX2KEY_PTOUT_BOXSEL_ENTER && now_pos == BOX2KEY_PTOUT_BOXSEL_TRAY1 ){
		now_pos = syswk->app->old_cur_pos;
		CURSORMOVE_PosSetEx( syswk->app->cmwk, now_pos, BOX2KEY_PTOUT_BOXSEL_ENTER, BOX2KEY_PTOUT_BOXSEL_ENTER );
	}
	...
	// トレイ左から右へのループ
	if( now_pos == BOX2KEY_PTOUT_BOXSEL_TRAY1 && old_pos == BOX2KEY_PTOUT_BOXSEL_TRAY6 ){
	...
	// トレイ右から左へのループ
	if( now_pos == BOX2KEY_PTOUT_BOXSEL_TRAY6 && old_pos == BOX2KEY_PTOUT_BOXSEL_TRAY1 ){
	...
	if( now_pos >= BOX2KEY_PTOUT_BOXSEL_TRAY1 && now_pos <= BOX2KEY_PTOUT_BOXSEL_TRAY6 ){
		if( old_pos != BOX2KEY_PTOUT_BOXSEL_ENTER ){
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_PartyInMain( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	u8	now;
	...
	if( ret < NELEMS(PartyInMainKeyTbl) ){
		if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
			if( ret >= BOX2KEY_PTIN_MAIN_TSURETEIKU && ret <= BOX2KEY_PTIN_MAIN_CLOSE ){
				CURSORMOVE_PosSet( syswk->app->cmwk, now );
				CursorObjMove( syswk, now );
				return CURSORMOVE_NONE;
			}
		}else{
			if( ret == BOX2KEY_PTIN_MAIN_RETURN ){
				return BOX2KEY_PTIN_MAIN_CLOSE;
			}
		}
		return ret;
	}
	...
	if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == TRUE ){
		if( ret == BOX2KEY_PTIN_MAIN_RETURN || ret == CURSORMOVE_CANCEL ){
			return BOX2KEY_PTIN_MAIN_CLOSE;
		}
	}
...
static void PartyInMainCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( old_pos != CURSORMOVE_ONOFF_DIRECT ){
		if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
			if( now_pos == BOX2KEY_PTIN_MAIN_LEFT || now_pos == BOX2KEY_PTIN_MAIN_RIGHT ){
				now_pos = BOX2KEY_PTIN_MAIN_NAME;
			}else if( now_pos != BOX2KEY_PTIN_MAIN_NAME && now_pos != BOX2KEY_PTIN_MAIN_RETURN ){
				if( syswk->get_pos != BOX2MAIN_GETPOS_NONE &&
					syswk->get_pos < BOX2OBJ_POKEICON_TRAY_MAX ){
					now_pos = syswk->get_pos;
				}else{
					now_pos = BOX2KEY_PTIN_MAIN_POKE1;
				}
			}
		}else{
			if( now_pos != BOX2KEY_PTIN_MAIN_STATUS &&
				now_pos != BOX2KEY_PTIN_MAIN_MARKING &&
				now_pos != BOX2KEY_PTIN_MAIN_FREE ){
				now_pos = BOX2KEY_PTIN_MAIN_TSURETEIKU;
			}
		}
	}
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_BoxArrangeMain( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	u8	now;
	...
	if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
		if( ret >= BOX2KEY_ARRANGE_MAIN_MOVE && ret <= BOX2KEY_ARRANGE_MAIN_FREE ){
			CURSORMOVE_PosSet( syswk->app->cmwk, now );
			CursorObjMove( syswk, now );
			return CURSORMOVE_NONE;
		}
	}else{
		if( ret == BOX2KEY_ARRANGE_MAIN_PARTY || ret == BOX2KEY_ARRANGE_MAIN_CHANGE ){
			CURSORMOVE_PosSet( syswk->app->cmwk, now );
			CursorObjMove( syswk, now );
			return CURSORMOVE_NONE;
		}
		if( ret == BOX2KEY_ARRANGE_MAIN_RETURN || ret == CURSORMOVE_CANCEL ){
			return BOX2KEY_ARRANGE_MAIN_CLOSE;
		}
	}
...
static void BoxArrangeMainCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( old_pos != CURSORMOVE_ONOFF_DIRECT ){
		if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
			switch( now_pos ){
			case BOX2KEY_ARRANGE_MAIN_LEFT:
			case BOX2KEY_ARRANGE_MAIN_RIGHT:
				now_pos = BOX2KEY_ARRANGE_MAIN_NAME;
				break;

			case BOX2KEY_ARRANGE_MAIN_NAME:
			case BOX2KEY_ARRANGE_MAIN_PARTY:
			case BOX2KEY_ARRANGE_MAIN_CHANGE:
			case BOX2KEY_ARRANGE_MAIN_RETURN:
				break;
			...
			default:
				if( syswk->get_pos < BOX2OBJ_POKEICON_TRAY_MAX ){
					...
					now_pos = BOX2KEY_ARRANGE_MOVE_POKE1;
				}
			}
		}else{
			if( now_pos != BOX2KEY_ARRANGE_MAIN_MOVE ||
				now_pos != BOX2KEY_ARRANGE_MAIN_STATUS ||
				now_pos != BOX2KEY_ARRANGE_MAIN_ITEM ||
				now_pos != BOX2KEY_ARRANGE_MAIN_MARKING ||
				now_pos != BOX2KEY_ARRANGE_MAIN_FREE ){

				now_pos = BOX2KEY_ARRANGE_MAIN_MOVE;
			}
		}
	}
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_BoxArrangePokeMove( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	
	if( BOX2MAIN_BoxMoveButtonCheck( syswk->app->wfrmwk ) == FALSE ){
		CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_CHANGE );
	}
...
static void BoxArrangePokeMoveCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( now_pos == BOX2KEY_ARRANGE_MOVE_LEFT || now_pos == BOX2KEY_ARRANGE_MOVE_RIGHT ){
		now_pos = BOX2KEY_ARRANGE_MOVE_TRAY1 + syswk->box_mv_pos % BOX2OBJ_TRAYICON_MAX;
		syswk->app->old_cur_pos = now_pos;
	}
...
//↑[GS_CONVERT_TAG]
	...
	if( syswk->app->poke_get_key == 0 ){
		if( now_pos >= BOX2KEY_ARRANGE_MOVE_TRAY1 && now_pos <= BOX2KEY_ARRANGE_MOVE_TRAY6 ){
			BOX2OBJ_AnmSet( syswk->app, BOX2OBJ_ID_HAND_CURSOR, BOX2OBJ_ANM_HAND_TRAY );
		}else{
			BOX2OBJ_AnmSet( syswk->app, BOX2OBJ_ID_HAND_CURSOR, BOX2OBJ_ANM_HAND_NORMAL );
		}
...
// カーソルループのための位置
#define	BAPM_TRAY_POKE_LINE1_S	( BOX2KEY_ARRANGE_MOVE_POKE1 )
#define	BAPM_TRAY_POKE_LINE1_E	( BOX2KEY_ARRANGE_MOVE_POKE1 + BOX2OBJ_POKEICON_H_MAX - 1 )
#define	BAPM_TRAY_POKE_LINE5_S	( BOX2KEY_ARRANGE_MOVE_POKE1 + BOX2OBJ_POKEICON_TRAY_MAX - BOX2OBJ_POKEICON_H_MAX )
#define	BAPM_TRAY_POKE_LINE5_E	( BOX2KEY_ARRANGE_MOVE_POKE1 + BOX2OBJ_POKEICON_TRAY_MAX - 1 )
#define	BAPM_PARTY_POKE_LINE1_S	( BOX2KEY_ARRANGE_MOVE_PARTY1 )
#define	BAPM_PARTY_POKE_LINE1_E	( BOX2KEY_ARRANGE_MOVE_PARTY1 + 1 )
...
static void BoxArrangePokeMoveCallBack_Move( void * work, int now_pos, int old_pos )
{
	...
	// ポケモンアイコンからトレイへの移動
	if( now_pos == BOX2KEY_ARRANGE_MOVE_TRAY1 &&
		( ( old_pos >= BAPM_TRAY_POKE_LINE1_S && old_pos <= BAPM_TRAY_POKE_LINE1_E ) ||
		  ...
		  old_pos == BOX2KEY_ARRANGE_MOVE_RETURN ) ){
	...
	// トレイ左から右へのループ
	if( now_pos == BOX2KEY_ARRANGE_MOVE_TRAY1 && old_pos == BOX2KEY_ARRANGE_MOVE_TRAY6 ){
		if( syswk->box_mv_pos + 1 >= BOX_MAX_TRAY ){
			syswk->box_mv_pos = 0;
		}else{
			syswk->box_mv_pos++;
	...
	// トレイ右から左へのループ
	if( now_pos == BOX2KEY_ARRANGE_MOVE_TRAY6 && old_pos == BOX2KEY_ARRANGE_MOVE_TRAY1 ){
		if( syswk->box_mv_pos - 1 < 0 ){
			syswk->box_mv_pos = BOX_MAX_TRAY - 1;
		}else{
			syswk->box_mv_pos--;
	...
	if( now_pos >= BOX2KEY_ARRANGE_MOVE_TRAY1 && now_pos <= BOX2KEY_ARRANGE_MOVE_TRAY6 ){
		if( old_pos >= BOX2KEY_ARRANGE_MOVE_TRAY1 && old_pos <= BOX2KEY_ARRANGE_MOVE_TRAY6 ){
			syswk->box_mv_pos = BOX2MAIN_MV_TRAY_DEF(syswk->box_mv_pos) + now_pos - BOX2KEY_ARRANGE_MOVE_TRAY1;
			BOX2OBJ_BoxMoveNameSet( syswk );
			BOX2BMP_BoxMoveNameNumWrite( syswk );
		}
	}
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_BoxArrangePokeMoveHand( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	...
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_LEFT );
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_RIGHT );
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_BoxArrangePartyMoveMain( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	u8	now;
	...
	if( BOX2MAIN_PokeMenuPutCheck( syswk->app->wfrmwk ) == FALSE ){
		if( ret >= BOX2KEY_ARRANGE_PARTY_MOVE && ret <= BOX2KEY_ARRANGE_PARTY_FREE ){
			CURSORMOVE_PosSet( syswk->app->cmwk, now );
			CursorObjMove( syswk, now );
			return CURSORMOVE_NONE;
		}
	}else{
		if( ret == BOX2KEY_ARRANGE_PARTY_RETURN || ret == CURSORMOVE_CANCEL ){
			return BOX2KEY_ARRANGE_PARTY_CLOSE;
		}
	}
...
static void BoxArrangePartyMoveCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( old_pos != CURSORMOVE_ONOFF_DIRECT ){
		if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
			if( now_pos >= BOX2KEY_ARRANGE_PARTY_MOVE ){
				if( syswk->get_pos >= BOX2OBJ_POKEICON_TRAY_MAX ){
					now_pos = syswk->get_pos - BOX2OBJ_POKEICON_TRAY_MAX;
				}else{
					now_pos = BOX2KEY_ARRANGE_PARTY_POKE1;
				}
			}
		}else{
			if( now_pos != BOX2KEY_ARRANGE_PARTY_MOVE ||
				now_pos != BOX2KEY_ARRANGE_PARTY_STATUS ||
				now_pos != BOX2KEY_ARRANGE_PARTY_ITEM ||
				now_pos != BOX2KEY_ARRANGE_PARTY_MARKING ||
				now_pos != BOX2KEY_ARRANGE_PARTY_FREE ){

				now_pos = BOX2KEY_ARRANGE_PARTY_MOVE;
			}
		}
	}
...
static void BoxArrangePartyMoveCallBack_Move( void * work, int now_pos, int old_pos )
{
	...
	// メニューが表示されてるとき
	if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == TRUE ){
		if( now_pos <= BOX2KEY_ARRANGE_PARTY_CHANGE ){
			if( old_pos >= BOX2KEY_ARRANGE_PARTY_RETURN ){
		...
		if( now_pos == BOX2KEY_ARRANGE_PARTY_FREE && old_pos == BOX2KEY_ARRANGE_PARTY_RETURN ){
			CURSORMOVE_PosSet( syswk->app->cmwk, BOX2KEY_ARRANGE_PARTY_RETURN );
			now_pos = BOX2KEY_ARRANGE_PARTY_RETURN;
		}
	}
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_BoxItemArrangeMain( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	u8	now;
	...
	if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
		if( ret == BOX2KEY_ITEM_A_MAIN_MENU1 ){
			CURSORMOVE_PosSet( syswk->app->cmwk, now );
			CursorObjMove( syswk, now );
			return CURSORMOVE_NONE;
		}
	}else{
		if( ret == BOX2KEY_ITEM_A_MAIN_PARTY || ret == BOX2KEY_ITEM_A_MAIN_CHANGE ){
			CURSORMOVE_PosSet( syswk->app->cmwk, now );
			CursorObjMove( syswk, now );
			return CURSORMOVE_NONE;
		}
		if( ret == BOX2KEY_ITEM_A_MAIN_RETURN || ret == CURSORMOVE_CANCEL ){
			return BOX2KEY_ITEM_A_MAIN_CLOSE;
		}
	}
...
static void BoxItemArrangeMainCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( old_pos != CURSORMOVE_ONOFF_DIRECT ){
		if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
			switch( now_pos ){
			case BOX2KEY_ITEM_A_MAIN_LEFT:
			case BOX2KEY_ITEM_A_MAIN_RIGHT:
				now_pos = BOX2KEY_ITEM_A_MAIN_NAME;
				break;

			case BOX2KEY_ITEM_A_MAIN_NAME:
			case BOX2KEY_ITEM_A_MAIN_PARTY:
			case BOX2KEY_ITEM_A_MAIN_CHANGE:
			case BOX2KEY_ITEM_A_MAIN_RETURN:
				break;
			...
			default:
				if( syswk->get_pos < BOX2OBJ_POKEICON_TRAY_MAX ){
					...
					now_pos = BOX2KEY_ITEM_A_MAIN_POKE1;
				}
			}
		}else{
			if( now_pos != BOX2KEY_ITEM_A_MAIN_MENU1 ){
				now_pos = BOX2KEY_ITEM_A_MAIN_MENU1;
			}
		}
	}
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_BoxItemArrangePartyMoveMain( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	u8	now;
	...
	if( BOX2MAIN_PokeMenuPutCheck( syswk->app->wfrmwk ) == FALSE ){
		if( ret == BOX2KEY_ITEM_A_PARTY_MENU1 ){
			CURSORMOVE_PosSet( syswk->app->cmwk, now );
			CursorObjMove( syswk, now );
			return CURSORMOVE_NONE;
		}
	}else{
		if( ret == BOX2KEY_ITEM_A_PARTY_RETURN || ret == CURSORMOVE_CANCEL ){
			return BOX2KEY_ITEM_A_PARTY_CLOSE;
		}
	}
...
static void BoxItemArrangePartyMoveCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( old_pos != CURSORMOVE_ONOFF_DIRECT ){
		if( BOX2MAIN_PokeMenuPutCheck(syswk->app->wfrmwk) == FALSE ){
			if( now_pos >= BOX2KEY_ITEM_A_PARTY_MENU1 ){
				if( syswk->get_pos >= BOX2OBJ_POKEICON_TRAY_MAX ){
					now_pos = syswk->get_pos - BOX2OBJ_POKEICON_TRAY_MAX;
				}else{
					now_pos = BOX2KEY_ITEM_A_PARTY_POKE1;
				}
			}
		}else{
			if( now_pos != BOX2KEY_ITEM_A_PARTY_MENU1 ){
				now_pos = BOX2KEY_ITEM_A_PARTY_MENU1;
			}
		}
	}
...
static void BoxThemaChgCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( now_pos == BOX2KEY_BOXTHEMA_CHG_LEFT || now_pos == BOX2KEY_BOXTHEMA_CHG_RIGHT ){
		now_pos = syswk->box_mv_pos % BOX2OBJ_TRAYICON_MAX;
		syswk->app->old_cur_pos = now_pos;
	}
...
//↑[GS_CONVERT_TAG]
	...
	if( now_pos >= BOX2KEY_BOXTHEMA_CHG_TRAY1 && now_pos <= BOX2KEY_BOXTHEMA_CHG_TRAY6 ){
		BOX2OBJ_AnmSet( syswk->app, BOX2OBJ_ID_HAND_CURSOR, BOX2OBJ_ANM_HAND_TRAY );
	}else{
		BOX2OBJ_AnmSet( syswk->app, BOX2OBJ_ID_HAND_CURSOR, BOX2OBJ_ANM_HAND_NORMAL );
	}
...
static void BoxThemaChgCallBack_Move( void * work, int now_pos, int old_pos )
{
	...
	if( old_pos == BOX2KEY_BOXTHEMA_CHG_MOVE && now_pos == BOX2KEY_BOXTHEMA_CHG_TRAY1 ){
		now_pos = syswk->app->old_cur_pos;
		CURSORMOVE_PosSetEx( syswk->app->cmwk, now_pos, BOX2KEY_BOXTHEMA_CHG_MOVE, BOX2KEY_BOXTHEMA_CHG_MOVE );
	}
	...
	// トレイ左から右へのループ
	if( now_pos == BOX2KEY_BOXTHEMA_CHG_TRAY1 && old_pos == BOX2KEY_BOXTHEMA_CHG_TRAY6 ){
		if( syswk->box_mv_pos + 1 >= BOX_MAX_TRAY ){
			syswk->box_mv_pos = 0;
		}else{
			syswk->box_mv_pos++;
	...
	// トレイ右から左へのループ
	if( now_pos == BOX2KEY_BOXTHEMA_CHG_TRAY6 && old_pos == BOX2KEY_BOXTHEMA_CHG_TRAY1 ){
		if( syswk->box_mv_pos - 1 < 0 ){
			syswk->box_mv_pos = BOX_MAX_TRAY - 1;
		}else{
			syswk->box_mv_pos--;
	...
	if( now_pos >= BOX2KEY_BOXTHEMA_CHG_TRAY1 && now_pos <= BOX2KEY_BOXTHEMA_CHG_TRAY6 ){
		if( old_pos != BOX2KEY_BOXTHEMA_CHG_MOVE ){
			syswk->box_mv_pos = BOX2MAIN_MV_TRAY_DEF(syswk->box_mv_pos) + now_pos;
			BOX2OBJ_BoxMoveNameSet( syswk );
			BOX2BMP_BoxMoveNameNumWrite( syswk );
		}
...
static void WallPaperChgCallBack_On( void * work, int now_pos, int old_pos )
{
	...
	if( now_pos == BOX2KEY_WALLPAPER_CHG_LEFT || now_pos == BOX2KEY_WALLPAPER_CHG_RIGHT ){
...
//↑[GS_CONVERT_TAG]
	...
	if( now_pos >= BOX2KEY_WALLPAPER_CHG_ICON1 && now_pos <= BOX2KEY_WALLPAPER_CHG_ICON4 ){
		BOX2OBJ_AnmSet( syswk->app, BOX2OBJ_ID_HAND_CURSOR, BOX2OBJ_ANM_HAND_TRAY );
	}else{
		BOX2OBJ_AnmSet( syswk->app, BOX2OBJ_ID_HAND_CURSOR, BOX2OBJ_ANM_HAND_NORMAL );
	}
...
static void WallPaperChgCallBack_Move( void * work, int now_pos, int old_pos )
{
	...
	//「はりかえる」からトレイへの移動
	if( old_pos == BOX2KEY_WALLPAPER_CHG_ENTER && now_pos == BOX2KEY_WALLPAPER_CHG_ICON1 ){
		now_pos = syswk->app->old_cur_pos;
		CURSORMOVE_PosSetEx( syswk->app->cmwk, now_pos, BOX2KEY_WALLPAPER_CHG_ENTER, BOX2KEY_WALLPAPER_CHG_ENTER );
	}
	...
	// トレイ左から右へのループ
	if( now_pos == BOX2KEY_WALLPAPER_CHG_ICON1 && old_pos == BOX2KEY_WALLPAPER_CHG_ICON4 ){
	...
	// トレイ右から左へのループ
	if( now_pos == BOX2KEY_WALLPAPER_CHG_ICON4 && old_pos == BOX2KEY_WALLPAPER_CHG_ICON1 ){
	...
	if( now_pos >= BOX2KEY_WALLPAPER_CHG_ICON1 && now_pos <= BOX2KEY_WALLPAPER_CHG_ICON4 ){
		if( old_pos != BOX2KEY_WALLPAPER_CHG_ENTER ){
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2KEY_BoxItemArrangeMoveHand( BOX2_SYS_WORK * syswk )
{
	u32	ret;
	...
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_RIGHT );
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_CHANGE );
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_TRAY1 );
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_TRAY2 );
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_TRAY3 );
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_TRAY4 );
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_TRAY5 );
	CURSORMOVE_MoveTableBitOff( syswk->app->cmwk, BOX2KEY_ARRANGE_MOVE_TRAY6 );
 ...
 */
//--------------------------------------------------------------------------------------------
void BOX2KEY_CursorMoveVFuncWorkSet( BOX2_APP_WORK * appwk, int now_pos, int old_pos )
{
	const POINTER_WORK * pw;
	BOX2MAIN_CURSORMOVE_WORK * cwk;
	u8	nx, ny, ox, oy;

	pw = CURSORMOVE_PointerWorkGet( appwk->cmwk, now_pos );
	PointerWkPosGet( pw, &nx, &ny );
	pw = CURSORMOVE_PointerWorkGet( appwk->cmwk, old_pos );
	PointerWkPosGet( pw, &ox, &oy );
 ...
 */
//--------------------------------------------------------------------------------------------
BOOL BOX2KEY_YStatusButtonCheck( BOX2_SYS_WORK * syswk )
{
...
//↑[GS_CONVERT_TAG]
	...
	if( GF_TP_SingleHitTrg( &YStatusHitTbl ) == TRUE ){
		return TRUE;
	}
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2TP_HitTrayPokeTrg(void)
{
	return GFL_UI_TP_HitTrg( TrayPokeHitTbl );
//↑[GS_CONVERT_TAG]
}
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2TP_HitCheckTrgPartyPokeLeft(void)
{
	return GFL_UI_TP_HitTrg( PartyPokeLeftHitTbl );
//↑[GS_CONVERT_TAG]
}
 ...
 */
//--------------------------------------------------------------------------------------------
u32 BOX2TP_HitCheckTrgPartyPokeRight(void)
{
	return GFL_UI_TP_HitTrg( PartyPokeRightHitTbl );
//↑[GS_CONVERT_TAG]
}
#endif

...
static int PartyPokeGetKeyArrange( BOX2_SYS_WORK * syswk, u32 pos, int ret );
...
//				BOX2BGWFRM_SubDispWazaFrmOutSet( syswk->app );
				...
				BOX2BGWFRM_TemochiButtonOn( syswk->app );
				return VFuncSet( syswk, BOX2MAIN_VFuncMenuCloseKeyArrange, SetCursorMoveRet(syswk,BOX2SEQ_MAINSEQ_ARRANGE_MAIN) );
			}else{
...
//		BOX2MAIN_PokeInfoOff( syswk );
		...
		return ObjButtonAnmSet( syswk, BOX2OBJ_ID_TB_CANCEL, 9, BOX2SEQ_MAINSEQ_BOXEND_DIRECT );
...
*/
	...
	case 1:
		...
		case BOX_MODE_TURETEIKU:
			BOX2BMP_PokeSelectMsgPut(
				syswk, syswk->get_pos, BOX2BMP_MSGID_PARTYIN_MENU, BOX2BMPWIN_ID_MSG3 );
			return BOX2SEQ_MAINSEQ_PARTYIN_POKE_MENU_RCV;

		case BOX_MODE_AZUKERU:
			BOX2BMP_PokeSelectMsgPut(
				syswk, syswk->get_pos, BOX2BMP_MSGID_PARTYIN_MENU, BOX2BMPWIN_ID_MSG3 );
			return BOX2SEQ_MAINSEQ_PARTYOUT_POKE_MENU_RCV;
...
static int RcvStatus_ModeTureteiku( BOX2_SYS_WORK * syswk )
{
	...
	if( syswk->y_status_flg == 0 ){
		...
		if( syswk->y_status_hand == 0 ){
			...
			BOX2BGWFRM_SubDispWazaFrmInPosSet( syswk->app );
			next = BOX2SEQ_MAINSEQ_ARRANGE_POKEGET_MAIN;
		}
...
static int RcvStatus_ModeSeiri( BOX2_SYS_WORK * syswk )
{
	...
	case SUB_PROC_MODE_TB_BOX_GET:		// ボックスのポケモンのタッチバーから（掴んでいる時）
		...
		while( BOX2MAIN_VFuncBoxMoveFrmIn(syswk) ){}
		BOX2OBJ_EndTrayCursorScroll( syswk );
		if( syswk->y_status_pos >= BOX2UI_ARRANGE_PGT_TRAY2 && syswk->y_status_pos <= BOX2UI_ARRANGE_PGT_TRAY5 ){
			BOX2OBJ_ChangeTrayName( syswk, syswk->y_status_pos-BOX2UI_ARRANGE_PGT_TRAY2, TRUE );
		}
		return BOX2SEQ_MAINSEQ_ARRANGE_POKEGET_MAIN;
	...
	case SUB_PROC_MODE_TB_PARTY_GET:	// 手持ちのポケモンのタッチバーから（掴んでいる時）
		...
		BOX2OBJ_GetPokeIcon( syswk->app, syswk->get_pos );
		BOX2MAIN_HandGetPokeSet( syswk );
		BOX2UI_CursorMoveChange( syswk, BOX2UI_INIT_ID_ARRANGE_PARTY_MOVE, syswk->y_status_pos );
		return BOX2SEQ_MAINSEQ_ARRANGE_PARTY_POKEGET_MAIN;
	}
...
static int MainSeq_StrInRcv( BOX2_SYS_WORK * syswk )
{
	...
	return FadeInSet( syswk, BOX2SEQ_MAINSEQ_ARRANGE_MAIN );
/*
...
static int RcvBag_ModeAzukeru( BOX2_SYS_WORK * syswk )
{
	...
	if( syswk->subRet == 0 ){
		...
		next = BOX2SEQ_MAINSEQ_ITEMSET_MSG;
	}
...
static int MainSeq_PokeFreeInit( BOX2_SYS_WORK * syswk )
{
	...
	case 0:
		...
		BOX2BGWFRM_PokeMenuOutSet( syswk->app->wfrm );
		syswk->app->sub_seq++;
		return VFuncSet( syswk, BOX2MAIN_VFuncPokeFreeMenuMove, BOX2SEQ_MAINSEQ_POKEFREE_INIT );
...
static int MainSeq_PokeFreeEnter( BOX2_SYS_WORK * syswk )
{
	...
	case 2:
		...
		if( syswk->get_pos < BOX2OBJ_POKEICON_TRAY_MAX ){
			...
			syswk->app->sub_seq = 0;
			return VFuncSet( syswk, BOX2MAIN_VFuncPokeFreeMenuMove, syswk->next_seq );
		}else{
...
//				BOX2BGWFRM_SubDispWazaFrmOutSet( syswk->app );
				...
				BOX2MAIN_ResetTouchBar( syswk );
				BOX2BGWFRM_BoxListButtonOn( syswk->app );
				return VFuncSet( syswk, BOX2MAIN_VFuncMenuCloseKeyArrange, SetCursorMoveRet(syswk,BOX2SEQ_MAINSEQ_ARRANGE_PARTY_MAIN) );
			}else{
...
//			Snd_SePlay( SE_BOX2_DECIDE );
			...
			return PartyPokeGetKeyArrange( syswk, ret, BOX2SEQ_MAINSEQ_ARRANGE_PARTY_MAIN );
		}
	}
...
//			Snd_SePlay( SE_BOX2_POKE_CATCH );
	...
	case BOX2UI_ARRANGE_PARTY_BOXLIST:	//「いれかえ」
//		Snd_SePlay( SE_BOX2_DECIDE );
		syswk->quick_mode = 0;
		syswk->quick_get  = 0;
...
//		return BgButtonAnmSet( syswk, BOX2MAIN_WINFRM_RET_BTN, BOX2SEQ_MAINSEQ_PARTY_END_BUTTON );
		return BOX2SEQ_MAINSEQ_PARTY_END_BUTTON;
...
//		return BgButtonAnmSet( syswk, BOX2MAIN_WINFRM_CLOSE_BTN, BOX2SEQ_MAINSEQ_ARRANGE_PARTY_CLOSE_BUTTON );
		return BOX2SEQ_MAINSEQ_ARRANGE_PARTY_CLOSE_BUTTON;
...
//	BOX2BGWFRM_SubDispWazaFrmInSet( syswk->app );
	...
	return VFuncSet( syswk, BOX2MAIN_VFuncMenuOpenKeyArrange, SetCursorMoveRet(syswk,BOX2SEQ_MAINSEQ_ARRANGE_PARTY_MAIN) );
...
//			Snd_SePlay( SE_BOX2_DECIDE );
			...
			return YStatusButtonAnmSet( syswk, BOX2MAIN_WINFRM_Y_ST_BTN, BOX2SEQ_MAINSEQ_STATUS_CALL );
		}
		return BOX2SEQ_MAINSEQ_ARRANGE_BOXPARTY_CHG_MAIN;
	}
*/
...
//					Snd_SePlay( SE_BOX2_WARNING );
					BOX2BMP_PokeSelectMsgPut( syswk, 0, BOX2BMP_MSGID_PARTYOUT_CAPSULE, BOX2BMPWIN_ID_MSG1 );
					syswk->next_seq = BOX2SEQ_MAINSEQ_ARRANGE_BOXPARTY_CHG_ERROR;
					return BOX2SEQ_MAINSEQ_TRGWAIT;
*/
...
//		return BgButtonAnmSet( syswk, BOX2MAIN_WINFRM_CLOSE_BTN, BOX2SEQ_MAINSEQ_PARTYIN_CLOSE_BUTTON );
		return BOX2SEQ_MAINSEQ_PARTYIN_CLOSE_BUTTON;
...
//		return BgButtonAnmSet( syswk, BOX2MAIN_WINFRM_CLOSE_BTN, BOX2SEQ_MAINSEQ_PARTYOUT_CLOSE_BUTTON );
		return BOX2SEQ_MAINSEQ_PARTYOUT_CLOSE_BUTTON;
...
static int MainSeq_PartyOutStartError( BOX2_SYS_WORK * syswk )
{
	return VFuncSet( syswk, BOX2MAIN_VFuncPokeFreeMenuMove, ChangeSequence(syswk,BOX2SEQ_MAINSEQ_ARRANGE_POKEMENU_RCV) );
}
...
//		return BgButtonAnmSet( syswk, BOX2MAIN_WINFRM_CLOSE_BTN, BOX2SEQ_MAINSEQ_ITEMARRANGE_CLOSE_BUTTON );
		return BOX2SEQ_MAINSEQ_ITEMARRANGE_CLOSE_BUTTON;
...
//		return BgButtonAnmSet( syswk, BOX2MAIN_WINFRM_CLOSE_BTN, BOX2SEQ_MAINSEQ_ITEMARRANGE_PARTY_CLOSE_BUTTON );
		return BOX2SEQ_MAINSEQ_ITEMARRANGE_PARTY_CLOSE_BUTTON;
...
static int TrayPokeGetKeyArrange( BOX2_SYS_WORK * syswk, u32 pos, int ret )
{
	...
	return VFuncSet( syswk, BOX2MAIN_VFuncMenuOpenKeyArrange, SetCursorMoveRet(syswk,ret) );
}
...
//		Snd_SePlay( SE_BOX2_DECIDE );
	...
	case BOX2UI_ARRANGE_PGT_STTUS:	// 40: ステータス
		if( syswk->poke_get_key == 0 ){
			u8	pos = CURSORMOVE_PosGet( syswk->app->cmwk );
			if( pos >= BOX2OBJ_POKEICON_TRAY_MAX ){
				break;
		...
		return ObjButtonAnmSet( syswk, BOX2OBJ_ID_TB_STATUS, BOX2OBJ_ANM_TB_STATUS_ON, BOX2SEQ_MAINSEQ_STATUS_CALL );
...
//		Snd_SePlay( SE_BOX2_DECIDE );
	...
	case BOX2UI_ARRANGE_PTGT_STTUS:		// 40: ステータス
		if( syswk->poke_get_key == 0 ){
			u8	pos = CURSORMOVE_PosGet( syswk->app->cmwk );
			if( pos >= BOX2OBJ_POKEICON_PUT_MAX ){
				break;
...
//				syswk->cur_rcv_pos = BOX2UI_ARRANGE_MAIN_ITEM;
			...
			if( ITEM_CheckMail( item ) == TRUE ){
				BOX2BGWFRM_PokeMenuOutSet( syswk->app->wfrm );
				syswk->app->sub_seq = 1;
				return VFuncSet( syswk, BOX2MAIN_VFuncPokeFreeMenuMove, BOX2SEQ_MAINSEQ_ITEM_MENU_CHECK );
			}
			BOX2BGWFRM_PokeMenuOff( syswk->app->wfrm );
...
//				BOX2BGWFRM_SubDispWazaFrmOutSet( syswk->app );
				...
				BOX2MAIN_ResetTouchBar( syswk );
				BOX2BGWFRM_TemochiButtonOn( syswk->app );
				return VFuncSet( syswk, BOX2MAIN_VFuncMenuCloseKeyArrange, SetCursorMoveRet(syswk,BOX2SEQ_MAINSEQ_BATTLEBOX_MAIN) );
			}else{
				CURSORMOVE_PosSet( syswk->app->cmwk, ret );
				CURSORMOVE_CursorOnOffSet( syswk->app->cmwk, TRUE );
				BOX2MAIN_PokeInfoOff( syswk );
...
//	BOX2BGWFRM_SubDispWazaFrmOutSet( syswk->app );
	...
	BOX2MAIN_ResetTouchBar( syswk );
	BOX2BGWFRM_BoxListButtonOn( syswk->app );
	return VFuncSet( syswk, BOX2MAIN_VFuncMenuCloseKeyArrange, SetCursorMoveRet(syswk,BOX2SEQ_MAINSEQ_BATTLEBOX_PARTY_MAIN) );
}
...
//			Snd_SePlay( SE_BOX2_DECIDE );
			...
			return PartyPokeGetKeyArrange( syswk, ret, BOX2SEQ_MAINSEQ_BATTLEBOX_PARTY_MAIN );
		}
	}

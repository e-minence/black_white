...
{	
  ...
  //メインシーケンス
  //p_wk->main_seq  = WifiBattleMatch_Debug_MainSeq;
...
typedef enum
{
  ...
  WIFIBATTLEMATCH_RND_SUBSEQ_RATE_MATCHING, //マッチング
  WIFIBATTLEMATCH_RND_SUBSEQ_RATE_BTL_END,  //バトル終了後
  ...
  case SEQ_WAIT_RECVRATE_SAKE:
    { 
      ...
      { 
        ...
        info_setup.btl_rule = BTL_RULE_SINGLE;
        ...
        { 
          ...
          p_player  = WIFIBATTLEMATCH_DATA_GetPlayerDataPtr( p_param->p_data );
...
{ 
  ...
  case SEQ_WAIT_MATCHING:
    if( !WIFIBATTLEMATCH_NET_CancelDisable( p_wk->p_net ) )
    { 
  ...
  case SEQ_START_SENDDATA:
    { 
      WIFIBATTLEMATCH_ENEMYDATA *p_my_data = WIFIBATTLEMATCH_DATA_GetPlayerDataPtr( p_param->p_data );
  ...
  case SEQ_WAIT_SENDDATA:
    {
      ...
      { 
        ...
        WIFIBATTLEMATCH_ENEMYDATA *p_enemy_data = WIFIBATTLEMATCH_DATA_GetEnemyDataPtr( p_param->p_data );
  ...
  case SEQ_START_SELECT_CANCEL_MSG:
    if( WIFIBATTLEMATCH_NET_SetCancelState( p_wk->p_net ) )
    { 
...
{ 
  ...
  case SEQ_START_REPORT_ATLAS:
    WIFIBATTLEMATCH_SC_Start( p_wk->p_net, BTL_RULE_SINGLE, p_param->btl_result );

...
{ 
  DEBUGWIN_REGULATION_DATA  *p_wk = userWork;
  DEBUGWIN_ITEM_SetNameV( item , "たいかいNO[%u]", p_wk->cup_no );  
}
...
static inline void DebugWin_Etc_D_ServerFlag( void* userWork , DEBUGWIN_ITEM* item )
{
  ...
  BOOL *p_is_flag  = DEBUGWIN_SERVERTIME_GetFlag();
  DEBUGWIN_ITEM_SetNameV( item , "バトルにちじけいか[%s]", sc_tbl[ (*p_is_flag) ] );
}
...
static inline void DebugWin_Etc_D_ServerFlag2( void* userWork , DEBUGWIN_ITEM* item )
{
  static const char *sc_tbl[] =
  {
    "OFF",
  ...
  BOOL *p_is_flag  = DEBUGWIN_SERVERTIME_USER_GetFlag();
  DEBUGWIN_ITEM_SetNameV( item , "にんいにちじけいか[%s]", sc_tbl[ (*p_is_flag) ] );
}

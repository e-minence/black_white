//[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[
/**
 *
 *	@file		wifibattlematch_snd.h
 *	@brief  サウンド定義ヘッダ
 *	@author	Toru=Nagihashi
 *	@date		2010.02.24
 *
 */
//]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]
#pragma once
#include "sound/wb_sound_data.sadl"

//=============================================================================
/**
 *					定数宣言
*/
//=============================================================================
#define WBM_SND_SE_MATCHING_OK  (SEQ_SE_SYS_74)
#define WBM_SND_SE_CARD_SLIDE   (SEQ_SE_SYS_75)
#define WBM_SND_SE_SAVE         (SEQ_SE_SAVE)
#define WBM_SND_SE_MATCHING     (SEQ_SE_SYS_21)
#define WBM_SND_SEQ_MAIN        (SEQ_BGM_WCS)
#define WBM_SND_SEQ_BATTLE_LIVE (SEQ_BGM_VS_WCS)
#define WBM_SND_SEQ_BATTLE_WIFI (SEQ_BGM_VS_TRAINER)
#define WBM_SND_SEQ_BATTLE_RND  (SEQ_BGM_VS_TRAINER)
#define WBM_SND_SEQ_BATTLE_PINCH (SEQ_BGM_BATTLEPINCH)

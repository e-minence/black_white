 ...
 *
 *  作った後の反省会
 *    開始画面と結果画面はGFL_PROCとして作成したほうがよかった。
...
static const GFL_CLACTPOS sc_MNGM_COUNT_MSK_MAT[ MNGM_COUNT_MSK_OAM_NUM ] = {
  {  MNGM_COUNT_MSK_X ,  MNGM_COUNT_MSK_Y },
  {  MNGM_COUNT_MSK_X ,  MNGM_COUNT_MSK_Y+MNGM_COUNT_MSK_Y_D },
};
...
static const GF_PRINTCOLOR sc_MNGM_TITLELOGO_BMP_COL[ WFLBY_GAME_P2PGAME_NUM + 1] = { //+1=padding
...
#else
	PRINT_STREAM*	print_stream[MNGM_TALKWIN_IDX_NUM];
#endif
...
#if WB_TEMP_FIX    //ラスターシステムが無いのでとりあえずfix
...
//=====================================
// バンク設定
static const GFL_DISP_VRAM sc_MNGM_ENTRY_BANK = {
...
// BGL設定
static const GFL_BG_SYS_HEADER sc_MNGM_ENTRY_BGINIT = {
...
//=====================================
// バンク設定
static const GFL_DISP_VRAM sc_MNGM_RESULT_BANK = {
  GX_VRAM_BG_256_AB,        // メイン2DエンジンのBG
  GX_VRAM_BGEXTPLTT_NONE,     // メイン2DエンジンのBG拡張パレット
  GX_VRAM_SUB_BG_32_H,      // サブ2DエンジンのBG
  GX_VRAM_SUB_BGEXTPLTT_NONE,   // サブ2DエンジンのBG拡張パレット
...
// BGL設定
static const GFL_BG_SYS_HEADER sc_MNGM_RESULT_BGINIT = {
  GX_DISPMODE_GRAPHICS,
  GX_BGMODE_0,
  GX_BGMODE_0,
  GX_BG0_AS_2D
...
static void MNGM_CLACTRES_Exit( MNGM_CLACTRES* p_wk, PLTTSLOT_SYS_PTR plttslot );
static MNGM_CLACTRESOBJ* MNGM_CLACTRES_Load( MNGM_CLACTRES* p_wk, ARCHANDLE* p_handle, u32 pal_idx, u32 palnum, u32 cg_idx, u32 cell_idx, u32 anm_idx, u32 contid, PLTTSLOT_SYS_PTR plttslot, u32 heapID );
static GFL_CLWK* MNGM_CLACTRES_Add( MNGM_CLACTRESOBJ* p_obj, GFL_CLUNIT* p_clset, s16 x, s16 y, u16 pri, u32 heapID );
static void MNGM_CLACTRES_OBJ_Init( MNGM_CLACTRES* p_wk, MNGM_CLACTRESOBJ* p_obj, ARCHANDLE* p_handle, u32 pal_idx, u32 palnum, u32 cg_idx, u32 cell_idx, u32 anm_idx, u32 contid, PLTTSLOT_SYS_PTR plttslot, u32 heapID );
static void MNGM_CLACTRES_OBJ_Exit( MNGM_CLACTRES* p_wk, MNGM_CLACTRESOBJ* p_obj, PLTTSLOT_SYS_PTR plttslot );

// BGL管理
static void MNGM_BGL_Init( MNGM_BGL* p_wk, const GFL_BG_SYS_HEADER* cp_sys, const MNGM_BGL_DATA* cp_cnt, u32 cnt_num, u32 heapID );
static void MNGM_BGL_Exit( MNGM_BGL* p_wk );
static void MNGM_BGL_VBlank( MNGM_BGL* p_wk );

// CLACT管理
static void MNGM_CLACT_Init( MNGM_CLACT* p_wk, u32 objnum, u32 charnum, u32 plttnum, const GFL_DISP_VRAM* cp_vramBank, u32 heapID );
...
static void MNGM_MSG_Print( MNGM_MSG* p_wk, u32 no, GFL_BMPWIN* p_win, u8 x, u8 y, PRINT_UTIL *printUtil );
static void MNGM_MSG_PrintRightSide( MNGM_MSG* p_wk, u32 no, GFL_BMPWIN* p_win, u8 x, u8 y, PRINT_UTIL *printUtil );
static PRINT_STREAM * MNGM_MSG_PrintScr( MNGM_MSG* p_wk, u32 no, GFL_BMPWIN* p_win, STRBUF* p_str, int wait, GFL_TCBLSYS *tcblsys, HEAPID heapID );
static void MNGM_MSG_PrintColor( MNGM_MSG* p_wk, u32 no, GFL_BMPWIN* p_win, u8 x, u8 y, GF_PRINTCOLOR col, PRINT_UTIL *printUtil );
...
static void MNGM_TALKWIN_MsgPrint( MNGM_TALKWIN* p_wk, MNGM_MSG* p_msg, u32 msgidx, u32 idx, GFL_TCBLSYS *tcblsys, HEAPID heapID );
static void MNGM_TALKWIN_MsgOff( MNGM_TALKWIN* p_wk, u32 idx );
static BOOL MNGM_TALKWIN_MsgEndCheck( MNGM_TALKWIN* p_wk, u32 idx );
...
static void MNGM_TITLELOGO_InStart( MNGM_TITLE_LOGO* p_wk, MNGM_BGL* p_bglwk, u32 heapID, PRINT_QUE *printQue, GFL_FONT *fontHandle );
...
static BOOL MNGM_RESULT_Retry_Main( MNGM_RETRY_WK* p_wk, MNGM_TALKWIN* p_talkwin, MNGM_MSG* p_msg, u32 gametype, u32 heapID, GFL_TCBLSYS *tcblsys );
...
static void MNGM_RESULT_Balloon_GraphicInit( MNGM_BALLOON_WK* p_wk, MNGM_BGL* p_bgl, MNGM_CLACT* p_clact, MNGM_CLACTRES* p_clres, ARCHANDLE* p_handle, PLTTSLOT_SYS_PTR plttslot, u32 heapID );
...
static void MNGM_COUNT_MskSetMskHBlank( GFL_TCB* tcb, void* p_work );
...
void MNGM_ENTRY_Exit( MNGM_ENTRYWK* p_wk )
{
  ...
  // グラフィック設定解除
  MNGM_CLACTRES_Exit( &p_wk->clres, p_wk->clact.plttslot );
 ...
 */
//-----------------------------------------------------------------------------
MNGM_COUNTWK* MNGM_COUNT_Init( GFL_CLUNIT* p_clset, u32 heapID, PLTTSLOT_SYS_PTR plttslot )
{
  MNGM_COUNTWK* p_wk;
 ...
 */
//-----------------------------------------------------------------------------
void MNGM_COUNT_Exit( MNGM_COUNTWK* p_wk, PLTTSLOT_SYS_PTR plttslot )
{
  // タスク破棄
  if( p_wk->p_tcb != NULL ){
    // 動作破棄
  ...
  // アクター破棄
  if( p_wk->p_anm != NULL ){
    GFL_CLACT_WK_Remove( p_wk->p_anm );
  }
  ...
  // リソース破棄
  MNGM_CLACTRES_Exit( &p_wk->resman, plttslot );
 ...
 */
//--------------------------------------------------------------
int MNGM_PalNoGet( MNGM_COUNTWK* p_wk, PLTTSLOT_SYS_PTR plttslot )
{
#if WB_FIX
...
static void MNGM_CLACTRES_Exit( MNGM_CLACTRES* p_wk, PLTTSLOT_SYS_PTR plttslot )
{
  ...
  for( i=0; i<p_wk->objnum; i++ ){
    if( p_wk->p_obj[i].data == TRUE ){
      MNGM_CLACTRES_OBJ_Exit( p_wk, &p_wk->p_obj[i], plttslot );
    }
  }
...
static MNGM_CLACTRESOBJ* MNGM_CLACTRES_Load( MNGM_CLACTRES* p_wk, ARCHANDLE* p_handle, u32 pal_idx, u32 palnum, u32 cg_idx, u32 cell_idx, u32 anm_idx, u32 contid, PLTTSLOT_SYS_PTR plttslot, u32 heapID )
{
  ...
  MNGM_CLACTRES_OBJ_Init( p_wk, p_obj, p_handle, pal_idx, palnum, cg_idx, cell_idx, anm_idx, contid, plttslot, heapID );
...
static GFL_CLWK* MNGM_CLACTRES_Add( MNGM_CLACTRESOBJ* p_obj, GFL_CLUNIT* p_clset, s16 x, s16 y, u16 pri, u32 heapID )
{
  ...
  HeadClwkData.pos_x = x;
  HeadClwkData.pos_y = y;
  HeadClwkData.anmseq = 0;
  HeadClwkData.softpri = pri;
  HeadClwkData.bgpri = MNGM_CLACTRES_DEF_BG_PRI;

  return GFL_CLACT_WK_Create(p_clset, p_obj->p_resobj_index[0], 
...
static void MNGM_BGL_Init( MNGM_BGL* p_wk, const GFL_BG_SYS_HEADER* cp_sys, const MNGM_BGL_DATA* cp_cnt, u32 cnt_num, u32 heapID )
{
  ...
  // BGコントロール設定
  {
    ...
    for( i=0; i<cnt_num; i++ ){
      ...
      // スクロール位置初期化
      GFL_BG_SetScroll( cp_cnt[i].frame, GFL_BG_SCROLL_X_SET, 0 );
      GFL_BG_SetScroll( cp_cnt[i].frame, GFL_BG_SCROLL_Y_SET, 0 );
    }
  }
}
...
static void MNGM_BGL_Exit( MNGM_BGL* p_wk )
{
  ...
  // ＢＧコントロール破棄
  {
    ...
    for( i=0; i<p_wk->tblnum; i++ ){
      GFL_BG_FreeBGControl( p_wk->cp_tbl[i].frame );
    }
  }
...
static void MNGM_CLACT_Init( MNGM_CLACT* p_wk, u32 objnum, u32 charnum, u32 plttnum, const GFL_DISP_VRAM* cp_vramBank, u32 heapID )
{
    ...
    // キャラクタマネージャー初期化
  {
...
#else
    {
      ...
      GFL_CLACT_SYS_Create( &clsys_init, cp_vramBank, heapID );
      p_wk->p_clactset = GFL_CLACT_UNIT_Create( objnum, 0, heapID );
...
static void MNGM_MSG_Print( MNGM_MSG* p_wk, u32 no, GFL_BMPWIN* p_win, u8 x, u8 y, PRINT_UTIL *printUtil )
{
  MNGM_MSG_PrintColor( p_wk, no, p_win, x, y, MNGM_MSG_COLOR, printUtil );
}
...
static void MNGM_MSG_PrintRightSide( MNGM_MSG* p_wk, u32 no, GFL_BMPWIN* p_win, u8 x, u8 y, PRINT_UTIL *printUtil )
{
  ...
  strsize = PRINTSYS_GetStrWidth( p_wk->p_str, p_wk->fontHandle_system, 0 );
  draw_x = x - strsize;
  if( draw_x < 0 ){
    draw_x = 0;
  }
  ...
  PRINT_UTIL_PrintColor(
    printUtil, p_wk->printQue, draw_x, y, p_wk->p_str, p_wk->fontHandle_system, MNGM_MSG_COLOR);
}
...
static PRINT_STREAM * _PrintStreamColor(GFL_BMPWIN* dst, u16 xpos, u16 ypos, 
  const STRBUF* str, GFL_FONT* font, int wait, GFL_TCBLSYS* tcblsys, u32 tcbpri, 
  HEAPID heapID, u16 clearColor, GF_PRINTCOLOR font_color )
{
  ...
  ret_stream = PRINTSYS_PrintStream(
    dst, xpos, ypos, str, font, wait, tcblsys, tcbpri, heapID, clearColor);
...
static PRINT_STREAM * MNGM_MSG_PrintScr( MNGM_MSG* p_wk, u32 no, GFL_BMPWIN* p_win, STRBUF* p_str, int wait, GFL_TCBLSYS *tcblsys, HEAPID heapID )
{
  ...
  return _PrintStreamColor(p_win, 0, 0, p_str, p_wk->fontHandle_system, wait, tcblsys, 10, heapID, GF_PRINTCOLOR_GET_GROUND(MNGM_MSG_TALKCOLOR), MNGM_MSG_TALKCOLOR);
}
...
static void MNGM_MSG_PrintColor( MNGM_MSG* p_wk, u32 no, GFL_BMPWIN* p_win, u8 x, u8 y, GF_PRINTCOLOR col, PRINT_UTIL *printUtil )
{
  ...
  PRINT_UTIL_PrintColor(
    printUtil, p_wk->printQue, x, y, p_wk->p_str, p_wk->fontHandle_system, col);
}
...
static void MNGM_TALKWIN_Init( MNGM_TALKWIN* p_wk, MNGM_BGL* p_bgl, SAVE_CONTROL_WORK* p_save, BOOL vip, u32 heapID )
{
  ...
  // トークウィンドウのウィンドウを転送
  TalkWinFrame_GraphicSet(GFL_BG_FRAME2_M, MNGM_TALKWIN_CGX, MNGM_TALKWIN_PAL,  type, heapID );
  TalkWinFrame_GraphicSet( GFL_BG_FRAME0_S, MNGM_TALKWIN_CGX, MNGM_TALKWIN_PAL,  type, heapID );
...
static void MNGM_TALKWIN_MsgPrint( MNGM_TALKWIN* p_wk, MNGM_MSG* p_msg, u32 msgidx, u32 idx, GFL_TCBLSYS *tcblsys, HEAPID heapID )
{
...
#else
  _PrintStreamForceStop(p_wk, idx);
#endif
  ...
  p_wk->print_stream[idx] = MNGM_MSG_PrintScr( p_msg, msgidx, p_wk->win[idx],
      p_wk->p_str[idx], 
      MSGSPEED_GetWait(),
      tcblsys, heapID);//MNGM_TALKWIN_MSG_SPEED ); ※check PLはMNGM_TALKWIN_MSG_SPEEDで固定にしていたので、MSGSPEED_GetWaitの挙動によっては固定にした方がいいかもしれない
...
static void MNGM_TALKWIN_SetPos( MNGM_TALKWIN* p_wk, u8 x, u8 y, u32 idx )
{
  GFL_BMPWIN_SetPosX(p_wk->win[idx], x);
  GFL_BMPWIN_SetPosY(p_wk->win[idx], y);
}
...
static MNGM_ENTRYWK* MNGM_ENTRY_CommonInit( const MNGM_ENRES_PARAM* cp_commparam, u32 gametype, u32 heapID )
{
  ...
  // CLACT
  MNGM_CLACT_Init( &p_wk->clact, MNGM_ENTRY_CLACT_WKNUM, MNGM_ENTRY_CLACT_CHARNUM, MNGM_ENTRY_CLACT_PLTTNUM, &sc_MNGM_ENTRY_BANK, heapID );
...
static void MNGM_ENTRY_GraphicLoad( MNGM_ENTRYWK* p_wk, u32 heapID )
{
  ...
  // 背景設定
  GFL_ARCHDL_UTIL_TransVramScreen( p_handle, NARC_wlmngm_tool_minigame_win_bg0_NSCR, 
  ...
  p_wk->p_resobj = MNGM_CLACTRES_Load( &p_wk->clres, p_handle, 
      ...
      MNGM_RESCONTID, p_wk->clact.plttslot, heapID );
...
static void MNGM_ENTRY_Tcb( GFL_TCB* tcb, void* p_work )
{
...

  {
    ...
    if(p_wk->msg.printQue != NULL){
      PRINTSYS_QUE_Main(p_wk->msg.printQue);
     	for(i = 0; i < WFLBY_MINIGAME_MAX; i++){
       	PRINT_UTIL_Trans( &p_wk->playertbl[i].printUtil, p_wk->msg.printQue );
      }
    }
  }
  ...
  case MNGM_ENTRY_SEQ_WIPEIN_INIT:
    ...
    // タイトルロゴ描画
    MNGM_TITLELOGO_InStart( &p_wk->titlelogo, &p_wk->bgl, p_wk->heapID, p_wk->msg.printQue, p_wk->msg.fontHandle_system );
    p_wk->seq ++;
    break;
  ...
  case MNGM_ENTRY_SEQ_MSGON_INIT:
    MNGM_TALKWIN_MsgPrint( &p_wk->talkwin, &p_wk->msg, msg_00, MNGM_TALKWIN_IDX_MAIN, p_wk->tcblsys, p_wk->heapID );
    p_wk->seq ++;
    break;
  ...
  // スクリーン面をスクロールさせる
  GFL_BG_SetScrollReq( GFL_BG_FRAME3_M, 
      GFL_BG_SCROLL_Y_INC, MNGM_BGSCROLL_SPEED );
  GFL_BG_SetScrollReq( GFL_BG_FRAME1_S, 
      GFL_BG_SCROLL_Y_INC, MNGM_BGSCROLL_SPEED );
}
...
//    player_idx = sc_MNGM_PLAYER_PLATE_BBIDX_DATA[ player_num-1 ][ player_idx ];
  ...
  // 表示プライオリティ設定
  GFL_BG_SetPriority( GFL_BG_FRAME1_M, 2 );
  ...
  // OAM設定する
  {
    ...
    GFL_CLACT_WK_SetAnmSeq( p_player->p_clwk, anm );
  #if WB_FIX
    CLACT_PaletteOffsetChg( p_player->p_clwk, idx );
  #else //※check　色がおかしい場合はCLWK_PLTTOFFS_MODEを変えてみる
    GFL_CLACT_WK_SetPlttOffs( p_player->p_clwk, idx, CLWK_PLTTOFFS_MODE_PLTT_TOP );
  #endif
    GFL_CLACT_WK_SetDrawEnable( p_player->p_clwk, FALSE );
    GFL_CLACT_WK_SetAutoAnmFlag( p_player->p_clwk, TRUE );
  }
  ...
  // RankOAM設定
  {
    ...
    GFL_CLACT_WK_SetAnmSeq( p_player->p_rank, 0 );
    GFL_CLACT_WK_SetDrawEnable( p_player->p_rank, FALSE );
  }
...
static void MNGM_PLATE_PLAYERTBL_DrawName( MNGM_PLATE_PLAYER* p_player, MNGM_MSG* p_msg, const MYSTATUS* cp_status, s16 x, s16 y, u32 vip )
{
  ...
  if( vip == TRUE ){
    MNGM_MSG_PrintColor( p_msg, msg_10, p_player->win, x, y, MNGM_MSG_VIP_COLOR, &p_player->printUtil );
  }else{
    MNGM_MSG_Print( p_msg, msg_10, p_player->win, x, y, &p_player->printUtil );
  }
}
...
static void MNGM_PLATE_PLAYERTBL_DrawNation( MNGM_PLATE_PLAYER* p_player, MNGM_MSG* p_msg, BOOL draw_nation, u32 nation, u32 area, s16 x )
{
  ...
  if(draw_nation){
    if( nation != country000 ){
      ...
      MNGM_MSG_Print( p_msg, msg_11, p_player->win, x,16, &p_player->printUtil );
    }
  }else{
    if( nation != country000 ){
      MNGM_MSG_ClearStrBuff( p_msg );
      MNGM_MSG_SetAreaName( p_msg, nation, area );
      MNGM_MSG_Print( p_msg, msg_12, p_player->win, x,16, &p_player->printUtil );
    }
  }
}
...
static void MNGM_PLATE_PLAYERTBL_DrawRank( MNGM_PLATE_PLAYER* p_player, u32 rank )
{
  GFL_CLACT_WK_SetDrawEnable( p_player->p_rank, TRUE );
  GFL_CLACT_WK_SetAnmSeq( p_player->p_rank, MNGM_PLAYER_OAM_ANMSEQ_TOP+rank );

  GFL_CLACT_WK_SetAutoAnmFlag( p_player->p_rank, TRUE );
}
...
static void MNGM_PLATE_PLAYERTBL_DrawScore( MNGM_PLATE_PLAYER* p_player, MNGM_MSG* p_msg, u32 score, s16 x, s16 y, u32 keta )
{
  ...
  if( keta == 5 ){
    MNGM_MSG_PrintRightSide( p_msg, msg_14, p_player->win, x, y, &p_player->printUtil );
  }else{
    MNGM_MSG_PrintRightSide( p_msg, msg_13, p_player->win, x, y, &p_player->printUtil );
  }
}
...
static void MNGM_PLATE_PLAYERTBL_DrawOamWay( MNGM_PLATE_PLAYER* p_player )
{
  GFL_CLACT_WK_SetDrawEnable( p_player->p_clwk, TRUE );
}
...
{
  if( p_player->p_clwk ){
    GFL_CLACT_WK_Remove( p_player->p_clwk );
    p_player->p_clwk = NULL;
  }
  if( p_player->p_rank ){
    GFL_CLACT_WK_Remove( p_player->p_rank );
...
//  VecFx32 mat;
  ...
  pos.x = x+draw_x;
  pos.y = y;
  GFL_CLACT_WK_SetPos( p_wk->p_clwk, &pos, CLSYS_DEFREND_MAIN);
#endif
  ...
  pos.x = x+draw_x;
  pos.y = y;
  GFL_CLACT_WK_SetPos( p_wk->p_rank, &pos, CLSYS_DEFREND_MAIN );
#endif
}
...
static void MNGM_TITLELOGO_Init( MNGM_TITLE_LOGO* p_wk, MNGM_BGL* p_bglwk, MNGM_MSG* p_msg, u32 gametype, ARCHANDLE* p_handle, u32 heapID )
{
  ...
  GFL_ARCHDL_UTIL_TransVramScreen( p_handle, NARC_wlmngm_tool_minigame_win_bg2_NSCR, 
      GFL_BG_FRAME0_M, 0, 0, FALSE, heapID );
}
...
static void MNGM_TITLELOGO_InStart( MNGM_TITLE_LOGO* p_wk, MNGM_BGL* p_bglwk, u32 heapID, PRINT_QUE *printQue, GFL_FONT *fontHandle )
{
  ...
  // 中央に表示する
  x_size  = PRINTSYS_GetStrWidth( p_wk->p_str, fontHandle, 0 );
  draw_x  = (MNGM_TITLELOGO_BMP_SIZXDOT/2) - (x_size/2);

  PRINT_UTIL_PrintColor(&p_wk->printUtil, printQue, draw_x, 0, p_wk->p_str, 
    fontHandle, sc_MNGM_TITLELOGO_BMP_COL[ p_wk->gametype ]);
...
static BOOL MNGM_TITLELOGO_OutMain( MNGM_TITLE_LOGO* p_wk, MNGM_BGL* p_bglwk )
{
  ...
  case MNGM_TITLELOGO_OUTMOVESEQ_END:
    GFL_BMPWIN_ClearTransWindow( p_wk->bmp );
...
static void MNGM_TITLELOGO_SetMoveMatrixVReq( MNGM_TITLE_LOGO* p_wk, MNGM_BGL* p_bglwk, s32 num )
{
  ...
  case WFLBY_GAME_BALLSLOW: // 玉投げ
  case WFLBY_GAME_BALANCEBALL:  // 玉乗り
    GFL_BG_SetScrollReq( GFL_BG_FRAME0_M, 
        GFL_BG_SCROLL_X_SET, num );
    GFL_BG_SetScrollReq( GFL_BG_FRAME1_M, 
        GFL_BG_SCROLL_X_SET, num );
    break;
...
static void MNGM_BACKPLTT_Trans( ARCHANDLE* p_handle, u32 gametype, u32 heapID )
{
  ...
  GFL_ARCHDL_UTIL_TransVramPaletteEx( p_handle, NARC_wlmngm_tool_minigame_win_ani_NCLR, 
...
static MNGM_RESULTWK* MNGM_RESULT_CommonInit( const MNGM_ENRES_PARAM* cp_commparam, const MNGM_RESULT_PARAM* cp_param, u32 gametype, u32 heapID )
{
  ...
  // OAM
  MNGM_CLACT_Init( &p_wk->clact, MNGM_RESULT_CLACT_WKNUM, MNGM_RESULT_CLACT_CHARNUM, MNGM_RESULT_CLACT_PLTTNUM, &sc_MNGM_RESULT_BANK, heapID );
...
static void MNGM_RESULT_GraphicLoad( MNGM_RESULTWK* p_wk, u32 heapID )
{
  ...
  MNGM_RESULT_Balloon_GraphicInit( &p_wk->balloon, &p_wk->bgl, &p_wk->clact,
      &p_wk->clres, p_handle, p_wk->clact.plttslot, heapID );
...
static void MNGM_RESULT_Tcb_BallSlowBalanceBall( GFL_TCB* tcb, void* p_work )
{
  ...
  case MNGM_RESULT_BALLSLOW_SEQ_MSGON_INIT:
    MNGM_TALKWIN_MsgPrint( &p_wk->talkwin, &p_wk->msg, msg_01, MNGM_TALKWIN_IDX_MAIN, p_wk->tcblsys, p_wk->heapID );
    p_wk->seq ++;
    break;
  ...
  case MNGM_RESULT_BALLSLOW_SEQ_GADGET_MSG00_ON:
    {
      ...
      MNGM_TALKWIN_MsgPrint( &p_wk->talkwin, &p_wk->msg, msgid, MNGM_TALKWIN_IDX_MAIN, p_wk->tcblsys, p_wk->heapID );
    }
    p_wk->seq ++;
    break;
  ...
  case MNGM_RESULT_BALLSLOW_SEQ_RETRY_MAIN:
    result = MNGM_RESULT_Retry_Main( &p_wk->retrysys, &p_wk->talkwin, &p_wk->msg, p_wk->gametype, p_wk->heapID, p_wk->tcblsys );
    if( result ){
      // リトライ設定
...
static void MNGM_RESULT_Tcb_Balloon( GFL_TCB* tcb, void* p_work )
{
  ...
  case MNGM_RESULT_BALLOON_SEQ_GADGET_MSG_00ON:
    ...
    MNGM_TALKWIN_MsgPrint( &p_wk->talkwin, &p_wk->msg, msg_07, MNGM_TALKWIN_IDX_MAIN, p_wk->tcblsys, p_wk->heapID );
    p_wk->wait = MNGM_RESULT_MSGDRAW_WAIT;
    p_wk->seq ++;
    break;
  ...
  case MNGM_RESULT_BALLOON_SEQ_GADGET_MSG_ON:
    {
      ...
      MNGM_TALKWIN_MsgPrint( &p_wk->talkwin, &p_wk->msg, msg_no, MNGM_TALKWIN_IDX_MAIN, p_wk->tcblsys, p_wk->heapID );
    }
    p_wk->seq ++;
    break;
...
{
  // パレットデータ取得
  p_wk->p_plttbuf = GFL_ARCHDL_UTIL_LoadPalette( p_handle, 
  ...
  if( (p_wk->pltt_time == 0) || 
    ...
    for( i=0; i<p_wk->comm_param.num; i++ ){
      if( p_wk->param.result[i] == 0 ){ // 1位のときだけ転送する
        ...
        // 対応したテーブルのパレットを転送
        result = NNS_GfdRegisterNewVramTransferTask( NNS_GFD_DST_2D_BG_PLTT_MAIN, tr_addr, p_rd_addr, 32);
...
static BOOL MNGM_RESULT_Retry_Main( MNGM_RETRY_WK* p_wk, MNGM_TALKWIN* p_talkwin, MNGM_MSG* p_msg, u32 gametype, u32 heapID, GFL_TCBLSYS *tcblsys )
{
  ...
  case MNGM_RESULT_RETRY_SEQ_MSGON:
    MNGM_MSG_SetGameName( p_msg, gametype );
    MNGM_TALKWIN_MsgPrint( p_talkwin, p_msg, msg_03, MNGM_TALKWIN_IDX_SUB, tcblsys, heapID );
    p_wk->seq ++;
    break;
  ...
  case MNGM_RESULT_RETRY_SEQ_RECVMSGON: // 誰かがやめるを選んだらつごうが・・・
    MNGM_TALKWIN_MsgPrint( p_talkwin, p_msg, msg_06, MNGM_TALKWIN_IDX_SUB, tcblsys, heapID );
    p_wk->seq ++;
    break;
...
//*/
  ...
  //　独自ウィンドウ
  {
    ...
    GFL_ARCHDL_UTIL_TransVramBgCharacter( p_handle, NARC_wlmngm_tool_mini_fusen_score_NCGR,
        GFL_BG_FRAME2_M, MNGM_RESULT_BALLOON_SYSWIN_CGX, 0, FALSE, heapID );
  ...
  //　OAM 
  {
    ...
    p_wk->p_resobj = MNGM_CLACTRES_Load( p_clres, p_handle, 
        ...
        MNGM_RESULT_BALLOON_CONTID, plttslot, heapID );
    ...
    for( i=0; i<MNGM_RESULT_BALLOON_Y; i++ ){
      ...
      for( j=0; j<MNGM_RESULT_BALLOON_X; j++ ){
        ...
        GFL_CLACT_WK_SetAnmSeq( p_wk->p_clwk[idx], i );
        GFL_CLACT_WK_SetBgPri( p_wk->p_clwk[idx], MNGM_RESULT_BALLOON_BG_PRI );
...
static BOOL MNGM_RESULT_Balloon_Main( MNGM_BALLOON_WK* p_wk, MNGM_BGL* p_bgl )
{
  ...
  case MNGM_RESULT_BALLOON_SEQ_ON:
    ...
    GFL_BG_ChangeScreenPalette( GFL_BG_FRAME2_M, 0, 0,
        MNGM_RESULT_BALLOON_SYSWIN_SCRNSIZX, MNGM_RESULT_BALLOON_SYSWIN_SCRNSIZY,
      ...
      if( (p_wk->idx % MNGM_RESULT_BALLOON_CLACT_NUM) == 0 ){
        ...
        if( p_wk->idx != p_wk->num ){
          ...
          for( i=0; i<MNGM_RESULT_BALLOON_CLACT_NUM; i++ ){
            ...
            // 全OAMを赤に変更
            GFL_CLACT_WK_SetAnmSeq( p_wk->p_clwk[i], MNGM_RESULT_BALLOON_ANM_RED );
          }
        }
      }
...
static void MNGM_COUNT_MskInit( MNGM_COUNT_MSKWK* p_wk, MNGM_CLACTRESOBJ* p_resobj, GFL_CLUNIT* p_clset, u32 heapID )
{
  ...
  for( i=0; i<MNGM_COUNT_MSK_OAM_NUM; i++ ){
    p_wk->p_msk[i] = MNGM_CLACTRES_Add( p_resobj, p_clset, 
        sc_MNGM_COUNT_MSK_MAT[i].x, 
        sc_MNGM_COUNT_MSK_MAT[i].y, 
        MNGM_COUNT_MSK_PRI, heapID );

    GFL_CLACT_WK_SetAnmSeq( p_wk->p_msk[i], 5 );
    GFL_CLACT_WK_SetBgPri( p_wk->p_msk[i], 0 );
  }
...
#else
  p_wk->h_tcb = GFUser_HIntr_CreateTCB(MNGM_COUNT_MskSetMskHBlank, p_wk, 1);
#endif
...
static BOOL MNGM_COUNT_MskReMain( MNGM_COUNT_MSKWK* p_wk )
{
  ...
  if( p_wk->count <= 0 ){
    ...
    GFL_CLACT_WK_SetDrawEnable( p_wk->p_msk[0], FALSE );
    GFL_CLACT_WK_SetDrawEnable( p_wk->p_msk[1], FALSE );
  }
  MNGM_COUNT_MskCommon( p_wk, -1 );
...
//  VecFx32 mat;
  ...
  for( i=0; i<MNGM_COUNT_MSK_OAM_NUM; i++ ){
    ...
    GFL_CLACT_WK_SetPos( p_wk->p_msk[i], &pos, CLSYS_DEFREND_MAIN );
  }
#endif
...
static void MNGM_COUNT_MskExit( MNGM_COUNT_MSKWK* p_wk )
{
  ...
  for( i=0; i<MNGM_COUNT_MSK_OAM_NUM; i++ ){
    if( p_wk->p_msk[i] != NULL ){
      GFL_CLACT_WK_Remove( p_wk->p_msk[i] );
      p_wk->p_msk[i] = NULL;
    }
  }
...
static void MNGM_COUNT_StartTcb( GFL_TCB* tcb, void* p_work )
{
  ...
  case MNGM_COUNT_START_SEQ_INIT:
    ...
    GFL_CLACT_WK_SetBgPri( p_wk->p_anm, 0 );
    p_wk->seq ++;
    break;
  ...
  case MNGM_COUNT_START_SEQ_ANM:
    ...
    // 変更前のフレームNO
    last_seq = GFL_CLACT_WK_GetAnmFrame( p_wk->p_anm );
    
    // アニメ
  #if WB_FIX
    CLACT_AnmFrameChg( p_wk->p_anm, FX32_CONST(2) );
  #else
    GFL_CLACT_WK_AddAnmFrame( p_wk->p_anm, FX32_CONST(2) );
  #endif
  
    // 変更後のふれーむNO
    now_seq = GFL_CLACT_WK_GetAnmFrame( p_wk->p_anm );
    ...
    #else //※check　色がおかしい場合はCLWK_PLTTOFFS_MODEを変えてみる
      if( p_wk->count == 0 ){
        GFL_CLACT_WK_SetPlttOffs( p_wk->p_anm, 0, CLWK_PLTTOFFS_MODE_OAM_COLOR );
      }else if( p_wk->count == MNGM_COUNT_STARTPALANMTIMING ){
        GFL_CLACT_WK_SetPlttOffs( p_wk->p_anm, 1, CLWK_PLTTOFFS_MODE_OAM_COLOR );
      }
    #endif
      p_wk->count ++;
    ...
    result = GFL_CLACT_WK_CheckAnmActive( p_wk->p_anm );
    if( result == FALSE ){
      p_wk->seq ++;
    #if WB_FIX
      CLACT_PaletteOffsetChgAddTransPlttNo( p_wk->p_anm, 0 );
...
static void MNGM_COUNT_TimeUpTcb( GFL_TCB* tcb, void* p_work )
{
  ...
  case MNGM_COUNT_TIMEUP_SEQ_INIT:
    ...
    GFL_CLACT_WK_SetAnmSeq( p_wk->p_anm, 1 );
    GFL_CLACT_WK_SetBgPri( p_wk->p_anm, 0 );
    p_wk->seq ++;
    break;
  ...
  case MNGM_COUNT_TIMEUP_SEQ_MSK:
    result = MNGM_COUNT_MskMain( &p_wk->msk );
    if( result == TRUE ){
      GFL_CLACT_WK_SetAutoAnmFlag( p_wk->p_anm, TRUE );
      p_wk->seq ++;
      p_wk->count = 0;
    }
    break;

...
//	OS_TPrintf( "mg start seq=%d\n", WFLBY_EVENTWK_GetSeq( p_wk ) );
	...
	case WFLBY_EV_MG_MSG_SYNCWAIT:	// メッセージ終了待ち同期待ち
		// 接続人数が減ったらエラーへ
		if( p_evwk->match_entry_num > OLDDWC_LOBBY_MG_GetEntryNum( p_param->mg_type ) ){
			OS_TPrintf( "p_evwk->match_entry_num > OLDDWC_LOBBY_MG_GetEntryNum( p_param->mg_type ) = %d > %d\n", p_evwk->match_entry_num,  OLDDWC_LOBBY_MG_GetEntryNum( p_param->mg_type ) );
	...
	case WFLBY_EV_MG_MATCHOKWAIT:	
		...
		if( OLDDWC_LOBBY_MG_MyParent() == TRUE ){
			if( OLDDWC_LOBBY_MG_GetEntryNum( p_param->mg_type ) <= 1 ){
				OS_TPrintf( "OLDDWC_LOBBY_MG_GetEntryNum( p_param->mg_type ) <= 1\n" );
				p_param->in_ok = WFLBY_EV_MG_RET_NG_DISCON;
				WFLBY_EVENTWK_SetSeq( p_wk, WFLBY_EV_MG_ERREND );
				break;
			}
	...
	case WFLBY_EV_MG_MATCHSTART:
		...
		// 募集中で
		if( OLDDWC_LOBBY_MG_CheckRecruit( p_param->mg_type ) == TRUE ){

			// もう入れないかチェック
			if( (OLDDWC_LOBBY_MG_GetRest( p_param->mg_type ) == 0) ||
				(OLDDWC_LOBBY_MG_CheckEntryOk( p_param->mg_type ) == FALSE)){
				// もうミニゲーム開催してる
			...
			// リミットがすでに0じゃないかチェック
			if( OLDDWC_LOBBY_MG_GetTimeLimit( p_param->mg_type ) == 0 ){
				p_param->in_ok = WFLBY_EV_MG_RET_NG_LOCK;
				WFLBY_EVENTWK_SetSeq( p_wk, WFLBY_EV_MG_END );
				break;
			}
		...
		// 今のリミット時間を取得
		p_evwk->timeout = OLDDWC_LOBBY_MG_GetTimeLimit( p_param->mg_type );
		...
		// P2P接続開始
		CommStateWifiP2PStart( p_param->mg_type, GFL_HEAPID_APP, 
		  WFLBY_SYSTEM_GetSaveData( WFLBY_ROOM_GetSystemData( p_rmwk ) ), 0 );
	...
	case WFLBY_EV_MG_MATCHWAIT:
		{
			...
			con_num = OLDDWC_LOBBY_MG_GetEntryNum( p_param->mg_type );
			...
			if( p_evwk->timelimit_10 == FALSE ){
				if( GFL_UI_KEY_GetTrg() & PAD_BUTTON_CANCEL ){
					...
					if( OLDDWC_LOBBY_MG_MyParent() == FALSE ){
						// 子機用処理
	...
	case WFLBY_EV_MG_INFO_INIT:	
		...
		// Info
		CommInfoInitialize( WFLBY_SYSTEM_GetSaveData( WFLBY_ROOM_GetSystemData( p_rmwk ) ) );
	...
	case WFLBY_EV_MG_STARTWAIT:
		...
		//*/
		// 人数がそろったらゲームを開始させる
		if( CommInfoGetEntryNum() >= OLDDWC_LOBBY_MG_GetEntryNum( p_param->mg_type ) ){
			if( OLDDWC_LOBBY_MG_MyParent() == TRUE ){
				if( p_evwk->start_game_set == FALSE ){
					OLDDWC_LOBBY_MG_StartGame();
					p_evwk->start_game_set = TRUE;
				}
			}

			// 親からゲーム開始命令がきたらゲームを開始する
			if( OLDDWC_LOBBY_MG_CheckStartGame() == TRUE ){
	...
	case WFLBY_EV_MG_PLIDX_CHECK:
...

		{
			...
			parent_man  = FALSE;
			parent_id	= OLDDWC_LOBBY_MG_GetParentUserID();
	...
	case WFLBY_EV_MG_COMM_START:
		...
		if( OLDDWC_LOBBY_MG_MyParent() == TRUE ){
			...
			count = OLDDWC_LOBBY_MG_GetEntryNum( p_param->mg_type );
...
static void WFLBY_EV_MG_MINIGAME_PrintMinigame_Core( WFLBY_EV_MG_KANBANWK* p_evwk, WFLBY_ROOMWK* p_rmwk, BOOL force, BOOL play_write )
{
	...
	case WFLBY_GAME_BALLSLOW:	// 玉投げ
		dwc_minigame = OLDDWC_LOBBY_MG_BALLSLOW;
		break;
	case WFLBY_GAME_BALANCEBALL:	// 玉乗り
		dwc_minigame = OLDDWC_LOBBY_MG_BALANCEBALL;
		break;
	case WFLBY_GAME_BALLOON:		// ふうせんわり
		dwc_minigame = OLDDWC_LOBBY_MG_BALLOON;
		break;
	...
	// 遊んでるかチェック
	{
		recruit = OLDDWC_LOBBY_MG_CheckRecruit( dwc_minigame );
		if( p_evwk->recruit != recruit ){
			p_evwk->write |= WFLBY_EV_MG_KANBAN_DRAW_MSK_PLAY;
		}
		p_evwk->recruit = recruit;

		play = OLDDWC_LOBBY_MG_CheckEntryOk( dwc_minigame );
		if( p_evwk->play != play ){
			p_evwk->write |= WFLBY_EV_MG_KANBAN_DRAW_MSK_PLAY;
		}
		p_evwk->play = play;
	...
	{
		if( recruit == TRUE ){
			num = OLDDWC_LOBBY_MG_GetRest( dwc_minigame );
		}else{

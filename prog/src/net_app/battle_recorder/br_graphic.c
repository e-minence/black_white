 ...
 *	{	
 *	○概要
 *	br_graphicはファイルごとコピペして使うことができる、グラフィック設定モジュールです。
 ...
 *	
 *	○中身
 *	br_graphic.cの中身は、データ部と、関数部にわかれております。
 ...
 *
 *	○使用方法
 *	１．使用したいフォルダへコピーしリネーム br, BR -> config CONFIGなど
...
{	
	...
	0,			//セルVram転送管理数
	64,32,64,64,	//ｷｬﾗ、ﾊﾟﾚｯﾄ、ｾﾙｱﾆﾒ、ﾏﾙﾁｾﾙの登録できるリソース管理最大数
 ...
 */
//-----------------------------------------------------------------------------
void BR_GRAPHIC_Exit( BR_GRAPHIC_WORK *p_wk )
{	
	//VBLANKTask消去
 ...
 */
//-----------------------------------------------------------------------------
void BR_GRAPHIC_2D_Draw( BR_GRAPHIC_WORK *p_wk )
{	
#ifdef GRAPHIC_OBJ_USE
 ...
 */
//-----------------------------------------------------------------------------
void BR_GRAPHIC_3D_StartDraw( BR_GRAPHIC_WORK *p_wk )
{	
#ifdef GRAPHIC_G3D_USE
 ...
 */
//-----------------------------------------------------------------------------
void BR_GRAPHIC_3D_EndDraw( BR_GRAPHIC_WORK *p_wk )
{	
#ifdef GRAPHIC_G3D_USE
 ...
 */
//-----------------------------------------------------------------------------
GFL_CLUNIT * BR_GRAPHIC_GetClunit( const BR_GRAPHIC_WORK *cp_wk )
{	
#ifdef GRAPHIC_OBJ_USE
 ...
 */
//-----------------------------------------------------------------------------
GFL_G3D_CAMERA * BR_GRAPHIC_GetCamera( const BR_GRAPHIC_WORK *cp_wk )
{	
#ifdef GRAPHIC_G3D_USE
 ...
 *	@brief  ミュージカル用設定
 *
 *	@param	BR_GRAPHIC_WORK *p_wk ワーク
 */
//-----------------------------------------------------------------------------
...
{ 
  ...
  //上画面のBG面だけ破棄
	{	
		...
		for( i = 0; i < NELEMS(sc_bgsetup); i++ )
		{	
      if( sc_bgsetup[i].frame < GFL_BG_FRAME0_S )
      { 
        GFL_BG_FreeBGControl( sc_bgsetup[i].frame );
      }
		}

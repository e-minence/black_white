...
struct _SND_WORK{
    ...
    SND_PMVOICE_LENGTH_WORK* pPokeWk;
//	GFL_TCB* length_tcb;								//鳴き声再生時間TCB
...
void SOUND_Main(void)
{
    ...
    Snd_PMVoiceLengthMain(wk->pPokeWk);
...
//--------------------------------------------------------------
/**
 * @brief	ポケモンボイスワーク登録=開始
 * @param	none
 * @retval	none
 */
//--------------------------------------------------------------

void Snd_SetPokemonVoiceWork(SND_PMVOICE_LENGTH_WORK* pWk)
{
   SND_WORK* wk = Snd_GetSystemAdrs();
   wk->pPokeWk = pWk;
}

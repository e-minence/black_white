...
BOOL Snd_PerapVoicePlaySub( const PERAPVOICE* perap, u32 sex, int vol, int pan )
{
	...
	//ランダムに音程を変える
	add_spd = GFUser_GetPublicRand( PERAP_WAVEOUT_SPD_RAND );
...
BOOL Snd_PerapVoicePlay( PERAPVOICE* perap, u32 sex, int vol, int pan )
{
	...
	if( ret == FALSE ){
		...
		ret = Snd_PMVoicePlayEx( PV_NORMAL, MONSNO_PERAPPU, pan, vol, GFL_HEAPID_SYSTEM, 0 );
	}
...
BOOL Snd_PerapVoiceWaitPlay( PERAPVOICE* perap, u32 sex, int vol, int pan, u8 wait )
{
	...
	if( ret == FALSE ){
		...
		Snd_PMVoiceWaitPlayEx( PV_NORMAL, MONSNO_PERAPPU, pan, vol, GFL_HEAPID_SYSTEM, wait, 0 );

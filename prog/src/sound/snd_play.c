...
#include "sound/snd_perap.h"
#include "poke_tool/monsno_def.h"

//#include "battle/battle_server.h"		//FORMNO_SHEIMI_FLOWER
#define FORMNO_SHEIMI_FLOWER (1)  ///battle_server.hがないため直  2009.01.26
...
//==============================================================================================
//ポケモン鳴き声再生時間ワーク
struct _SND_PMVOICE_LENGTH_WORK{
	int length;
//	GFL_TCB*	tcb;
} ;
...
static void Snd_PMVoiceLengthSet(int length, int heap_id );
static void Snd_PMVoiceLengthMainTCB( GFL_TCB* tcb, void* wk );
...
BOOL Snd_PMVoiceWaitPlay( u16 monsno, u8 wait, u8 form_no )
{
	Snd_PMVoiceWaitPlayEx(PV_NORMAL, monsno, WAZA_SE_PAN_N, PV_VOL_MAX, GFL_HEAPID_SYSTEM, wait,form_no);
...
//	SND_PMVOICE_LENGTH_WORK* wk = NULL;
	SND_PMVOICE_LENGTH_WORK* pPokeWk = Snd_GetParamAdrs(SND_W_ID_LENGTH_TCB);
	...
	pPokeWk = GFL_HEAP_AllocClearMemory( heap_id, sizeof(SND_PMVOICE_LENGTH_WORK) );
	if( pPokeWk == NULL ){
		GF_ASSERT( (0) && "メモリ確保失敗！" );
		return;
	}

	pPokeWk->length	= length;
    Snd_SetPokemonVoiceWork(pPokeWk);
//	*length_tcb = wk->tcb;
	return;
}
...
//--------------------------------------------------------------
//static void Snd_PMVoiceLengthMainTCB( GFL_TCB* tcb, void* wk )
void Snd_PMVoiceLengthMain(SND_PMVOICE_LENGTH_WORK* swk)
{
	u8* ch_normal_flag = Snd_GetParamAdrs(SND_W_ID_WAVEOUT_CH_NORMAL_FLAG);
	u8* ch_chorus_flag = Snd_GetParamAdrs(SND_W_ID_WAVEOUT_CH_CHORUS_FLAG);
...
void Snd_PMVoiceLengthTcbDel()
{
	...
	//登録されていたらTCB削除
	if( pPokeWk != NULL ){
		GFL_HEAP_FreeMemory( pPokeWk );
	}
    Snd_SetPokemonVoiceWork(NULL);
}

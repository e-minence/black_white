...
static void printNum(GFL_BMPWIN* bmpwin, int num, int numberSize );
//------------------------------------------------------------------
/**
 * @brief	セットアップ関数
 */
...
static void printNum(GFL_BMPWIN* bmpwin, int num, int numberSize )
{
	...
	bbbb = 1;
	for( i=0; i<(numberSize-1); i++){ bbbb *= 10;}
	f = FALSE;
	...
	if(num&0x80000000){
		GFL_TEXT_PrintSjisCode("-", &printparam);
		num = 0 - num;
	}
	...
	for( i=0; i<numberSize; i++){
		...
		if((aaaa == 0)&&(f == FALSE)&&(i!=(numberSize-1))){

...
extern void MysteryCard_DisplayCardOnly(GFL_BG_INI *bgl, void *p, int heapid);
...
//GF_PRINTCOLOR_MAKEをなかったことにして３色指定に変更
#define	MYSTERYCARD_BLACK	1,2,0
#define	MYSTERYCARD_WHITE	15,2,0
#define	MYSTERYCARD_OWHITE	14,15,0

#define MYSTERYCARD_RED		3,4,0
#define MYSTERYCARD_BLUE	5,6,0
...
typedef struct {
	...
	GFL_MSGDATA *msgman;	    		// MSGデータマネージャへのポインタ

	GFL_BMPWIN	*win[MYSTERYCARD_WIN_MAX];	// メッセージを表示するウィンドウ
	int chbase[MYSTERYCARD_WIN_MAX];

	int nowbase;
	void *tbl;

	MYSTERY_DATA *fdata;				// ふしぎデータを読んだ場所
	...
	BMPMENULIST_WORK *lw;				// メニュー
	BMP_MENULIST_DATA *bmd;
	GFL_FONT *fontHandle;

	GFL_BMPWIN	*msgwin;				// メッセージを表示するウィンドウ
	GFL_BMPWIN	*menuwin;			// メニューを表示するウィンドウ
	int menu_index;
...
//	プロトタイプ宣言
//============================================================================================
static int MysteryCardGoRemove(MYSTERYCARD_WORK *wk);
static int MysteryCardGoSendGift(MYSTERYCARD_WORK *wk);
static int MysteryCardRetMainMenu(MYSTERYCARD_WORK *wk);
static int MysteryCardRemove(MYSTERYCARD_WORK *wk);
static void MysteryCardRemoveMainMenu(MYSTERYCARD_WORK *wk);
static BOOL _nop(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardTitleWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardStatusWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardDateWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardTextWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardTransWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardMenu1Win(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardMenu2Win(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardMenu3Win(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static BOOL MysteryCardDrawEntry(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
static int MysteryCardDispConnectChildStatus(MYSTERYCARD_WORK *wk, GFL_BMPWIN *win);
static void MysteryCardDispConnectChildCount(MYSTERYCARD_WORK *wk, GFL_BMPWIN *win, int num);
static void CommMysteryCardExit(MYSTERYCARD_WORK *wk, int *seq, int next);
static int MysteryCardCreateWindow(MYSTERYCARD_WORK *wk, GFL_BMPWIN	**win, int index, int base);
...
typedef struct {
	...
	u32 msgid;
	BOOL (*func)(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol);
...
MYSTERYCARD_MSGWIN_TABLE MsgWin_Table[] = {
	/*  ------------------------------ 表面 ---------------------------- */
	/* ふしぎなカード */
	/*  0 */ { MYSTERYCARD_FRONT, 2,  2, 12,  2, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, -1,	mystery_card_001,	_nop },
	/* もらったひづけ */
	/*  1 */ { MYSTERYCARD_FRONT, 6, 17, 11,  2, 1/*FONT_TALK*/, MYSTERYCARD_WHITE, -1,	mystery_card_005,	_nop },
	/* ポケモンセンターにいる\nはいたついんから　うけとってください　など */
	/*  2 */ { MYSTERYCARD_FRONT, 2,  9, 28,  4, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, -1,	NULL,		MysteryCardStatusWin },
	/* カード名の表示 */
	/*  3 */ { MYSTERYCARD_FRONT, 2,  6, 28,  2, 1/*FONT_TALK*/, MYSTERYCARD_WHITE, -1,	NULL,		MysteryCardTitleWin },
	/* 日付 */
	/*  4 */ { MYSTERYCARD_FRONT,18, 17, 11,  2, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, -1,	mystery_card_014,		MysteryCardDateWin },

	/*  ------------------------------ 裏面 ---------------------------- */
	/* カードの説明 */
	/*  5 */ { MYSTERYCARD_BACK , 2,  3, 28, 10, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, -1,	NULL,		MysteryCardTextWin },
	/* くばれる回数とメッセージ表示 */
	/*  6 */ { MYSTERYCARD_BACK , 2, 14, 28,  4, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, -1,	mystery_card_006,	MysteryCardTransWin },

	/*  ------------------------------ メニュー ---------------------------- */
	/*  7 */ { MYSTERYCARD_MENU , 2, 19, 27,  4, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, 15,	mystery_card_001,	_nop },
	/*  8 */ { MYSTERYCARD_MENU ,18,  9, 12,  8, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, 5/*FONT_HEADER_F_COLOR*/,	NULL,		MysteryCardMenu1Win },
	/*  9 */ { MYSTERYCARD_MENU ,24, 13,  6,  4, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, 5/*FONT_HEADER_F_COLOR*/,	NULL,		MysteryCardMenu2Win },
	/* 10 */ { MYSTERYCARD_MENU ,24, 13,  6,  4, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, 5/*FONT_HEADER_F_COLOR*/,	NULL,		MysteryCardMenu3Win },

	/*  ------------------------------ おすそわけ ---------------------------- */
	/* ともだちはいしん　うけつけちゅう */
	/* 11 */ { MYSTERYCARD_SEND,  1,  1, 25,  2, 0/*FONT_SYSTEM*/, MYSTERYCARD_OWHITE, -1,	mystery_osusowake_001, _nop, 7, 0},
	/* げんざいのエントリー */
	/* 12 */ { MYSTERYCARD_SEND, 23,  7,  9,  4, 0/*FONT_SYSTEM*/, MYSTERYCARD_OWHITE, -1,	mystery_osusowake_004, _nop, 2, 0 },
	/* くばる */
	/* 13 */ { MYSTERYCARD_SEND,  6, 20,  6,  2, 0/*FONT_SYSTEM*/, MYSTERYCARD_OWHITE, -1,	mystery_osusowake_006, _nop, 8, 1},
	/* やめる */
	/* 14 */ { MYSTERYCARD_SEND, 21, 20,  6,  2, 0/*FONT_SYSTEM*/, MYSTERYCARD_OWHITE, -1,	mystery_osusowake_007, _nop, 0, 1 },
	/* げんざいのエントリー(○／４) */
	/* 15 */ { MYSTERYCARD_SEND, 25, 12,  4,  2, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, -1,	mystery_osusowake_005, MysteryCardDrawEntry },
	/* 実際にエントリしてる人を表示するウィンドウ */
	/* 16 */ { MYSTERYCARD_SEND,  2,  5, 19, 11, 0/*FONT_SYSTEM*/, MYSTERYCARD_BLACK, -1,	NULL,		     _nop },

	/* おすそわけ中＆電源をきってくださいウィンドウ */
	/* 17 */ { MYSTERYCARD_MENU , 2, 19, 27,  4, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, 14,	mystery_osusowake_008, _nop },
	/* 18 */ { MYSTERYCARD_MENU , 2, 19, 27,  4, 1/*FONT_TALK*/, MYSTERYCARD_BLACK, 14,	mystery_osusowake_009, _nop },
};
...
static BMPMENULIST_HEADER MenuListHeader = {
	...
	0/*FONT_SYSTEM*/,		/* 文字指定(本来は u8 だけど、そんなに作らないと思うので) */
 ...
 */
//------------------------------------------------------------------
static void CMC_BmpMenuWinClear(GFL_BMPWIN * win, u8 trans_sw)
{
//	if(GF_BGL_BmpWinAddCheck(win) == TRUE)
	if(win != NULL)
		GFL_BMPWIN_ClearScreen(win);
}
static void CMC_BmpTalkWinClear(GFL_BMPWIN * win, u8 trans_sw)
{
//	if(GF_BGL_BmpWinAddCheck(win) == TRUE)
	if(win != NULL)
		GFL_BMPWIN_ClearScreen(win);
 ...
 */
//--------------------------------------------------------------------------------------------
static void CreateWindowMenuData(MYSTERYCARD_WORK *wk, LISTDATA *ld, int num, GFL_BMPWIN *win, int curpos)
{
	int i;
	BOOL isLocalMsg = FALSE;
	BMPMENULIST_HEADER list_h;

	if(wk->bmd)	BmpMenuWork_ListDelete(wk->bmd);
	if(wk->lw) 	BmpMenuList_Exit(wk->lw, NULL, NULL);
	/* メニューの文字列を登録 */
	wk->bmd = BmpMenuWork_ListCreate(num,wk->heapid);
	if( wk->msgman == NULL )
	{
		wk->msgman = GFL_MSG_Create(GFL_MSG_LOAD_NORMAL, ARCID_MESSAGE, NARC_message_mystery_dat, wk->heapid);
		isLocalMsg = TRUE;
	}
	for(i = 0; i < num; i++)
		BmpMenuWork_ListAddArchiveString(wk->bmd, wk->msgman, ld[i].id, ld[i].param, wk->heapid);
	if( isLocalMsg == TRUE )
	{
	...
	list_h.count = num;
	list_h.win = win;
	wk->lw = BmpMenuList_Set( &list_h, 0, curpos, wk->heapid);
}
 ...
 */
//--------------------------------------------------------------------------------------------
static void DrawStringWindow(MYSTERYCARD_WORK *wk,GFL_BMPWIN *win, int msgid)
{
	STRBUF *msg,*baseStr;
	GFL_MSGDATA *msgman = GFL_MSG_Create(GFL_MSG_LOAD_NORMAL, ARCID_MESSAGE, NARC_message_mystery_dat, wk->heapid);
	WORDSET *word = WORDSET_Create(wk->heapid);
	...
	PRINTSYS_Print( GFL_BMPWIN_GetBmp(win) , 0, 0, msg ,  wk->fontHandle );
...
static int MysteryCardGoSendGift(MYSTERYCARD_WORK *wk)
{
	...
	DrawStringWindow(wk,wk->msgwin, mystery_card_011);
...
static int MysteryCardGoRemove(MYSTERYCARD_WORK *wk)
{
	...
	if(MYSTERYDATA_GetCardLinkDeli(wk->fdata, wk->disp_card) == TRUE)
		/* おくりもの　を　うけとってませんが\nこの　カードを　すてますか？ */
		DrawStringWindow(wk,wk->msgwin, mystery_drop_002);
	else
		/* この　カードを　すてますか？ */
		DrawStringWindow(wk,wk->msgwin, mystery_drop_001);
...
static int MysteryCardRemove(MYSTERYCARD_WORK *wk)
{
	...
	/* カードを捨ててます…の処理 */
	MysteryCardRemoveMainMenu(wk);
	DrawStringWindow(wk,wk->msgwin, mystery_drop_003);
	/* タイムアイコン表示スタート */
	//FIXME
//	wk->time_icon = TimeWaitIconAdd(&wk->msgwin, MYSTERYCARD_TFRAMECHR);
	...
	OS_TPrintf("捨てるカード %d\n", wk->disp_card);
		...
		/* カードの抹消処理 */
		if(MYSTERYDATA_GetCardLinkDeli(wk->fdata, wk->disp_card) == TRUE){
				MYSTERYDATA_RemoveCardDataPlusBit(wk->fdata, wk->disp_card);
		}
		else{
				MYSTERYDATA_RemoveCardData(wk->fdata, wk->disp_card);
		}
...
//	TimeWaitIconDel(wk->time_icon);
	...
	if(MYSTERYDATA_IsExistsCardAll(wk->fdata) == FALSE)
	...
	DrawStringWindow(wk,wk->msgwin, mystery_card_001);
	MysteryCardCreateWindow(wk, &wk->menuwin, MYSTERYCARD_INDEX_MENU+1, wk->menu_index);

	MysteryCardCreateMsgWindow(wk, 1/*FONT_TALK*/, MYSTERYCARD_FRONT);
 ...
 */
//--------------------------------------------------------------------------------------------
static BOOL _nop(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol){ return TRUE; }
...
static BOOL MysteryCardTitleWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol)
{
	...
	msg = GFL_STR_CreateBuffer( GIFT_DATA_CARD_TITLE_MAX+1, wk->heapid );
	GFL_STR_SetStringCodeOrderLength(msg, wk->card[wk->disp_card]->beacon.event_name, GIFT_DATA_CARD_TITLE_MAX);
...
static BOOL MysteryCardStatusWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol)
{
	...
	if(card->beacon.delivery_flag == FALSE){
		...
		if(wk->fdata == NULL || MYSTERYDATA_GetCardLinkDeli(wk->fdata, wk->disp_card) == TRUE)
...
static BOOL MysteryCardDateWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol)
{
	...
	WORDSET_RegisterNumber(wk->word, 0, date.year+2000, 4, STR_NUM_DISP_ZERO, STR_NUM_CODE_DEFAULT);
	WORDSET_RegisterNumber(wk->word, 1, date.month,     2, STR_NUM_DISP_ZERO, STR_NUM_CODE_DEFAULT);
	WORDSET_RegisterNumber(wk->word, 2, date.day,       2, STR_NUM_DISP_ZERO, STR_NUM_CODE_DEFAULT);
	return TRUE;
}
...
static BOOL MysteryCardTextWin(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol)
{
	...
	msg = GFL_STR_CreateBuffer( GIFT_DATA_CARD_TEXT_MAX+1, wk->heapid );
	GFL_STR_SetStringCodeOrderLength(msg, wk->card[wk->disp_card]->event_text, GIFT_DATA_CARD_TEXT_MAX);
	GFL_FONTSYS_SetColor( lcol , scol , bcol );
	PRINTSYS_Print( GFL_BMPWIN_GetBmp(*win) , 0, 0, msg ,  wk->fontHandle );
//	GF_STR_PrintColor(win, FONT_TALK, msg, 0, 0, MSG_NO_PUT, col, NULL);
	GFL_STR_DeleteBuffer(msg);
...
//	GF_BGL_BmpWinOff(win);
	...
	*win = GFL_BMPWIN_Create(	GFL_BG_FRAME2_M , tbl->sx, tbl->sy + (4 - index) * 2, 
								tbl->width, tbl->height - (4 - index) * 2,
								FONT_PALNO_NORMAL, GFL_BMP_CHRAREA_GET_F );
 ...
 */
//--------------------------------------------------------------------------------------------
static BOOL MysteryCardMenu4Win(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol)
{
	DrawStringWindow(wk,wk->msgwin, mystery_01_003);
...
static BOOL MysteryCardDrawEntry(MYSTERYCARD_WORK *wk, GFL_BMPWIN **win, int lcol, int scol, int bcol)
{
	WORDSET_RegisterNumber(wk->word, 0, 0/*wk->entry*/, 1, STR_NUM_DISP_ZERO, STR_NUM_CODE_DEFAULT);
	return TRUE;
}
...
//	wk->word = WORDSET_Create(wk->heapid);
	...
	// ウィンドウを作って…
			wk->chbase[i] = chbase;
			wk->win[i] = GFL_BMPWIN_Create( GFL_BG_FRAME0_M, tbl[i].sx, tbl[i].sy, 
						tbl[i].width, tbl[i].height, FONT_PALNO_NORMAL, GFL_BMP_CHRAREA_GET_F);
			}
			// ウィンドウをクリアして…
			...
			//col = tbl[i].color;
			// 外部ファンクションを呼び出して…
			if(tbl[i].func(wk, &wk->win[i], tbl[i].letterColor , tbl[i].shadowColor , tbl[i].backColor) == TRUE){
				// メッセージがあれば表示する
				if(tbl[i].msgid){
					baseStr = GFL_STR_CreateBuffer( 192 , wk->heapid );
					msg = GFL_STR_CreateBuffer( 192 , HEAPID_MYSTERYGIFT );
					...
					GFL_FONTSYS_SetColor( tbl[i].letterColor , tbl[i].shadowColor , tbl[i].backColor );
					PRINTSYS_Print( GFL_BMPWIN_GetBmp(wk->win[i]) , tbl[i].ox, tbl[i].oy, msg ,  wk->fontHandle );
...
#endif
	if(*win == NULL){
		*win = GFL_BMPWIN_Create( GFL_BG_FRAME2_M, tbl->sx, tbl->sy, 
					tbl->width, tbl->height, FONT_PALNO_NORMAL, GFL_BMP_CHRAREA_GET_F);
//		GF_BGL_BmpWinAdd(wk->bgl, win, GF_BGL_FRAME2_M,
...
//	GF_BGL_BmpWinDataFill(win, tbl->background);
	...
	if(win == &wk->msgwin){	// ごめんなさいコード
		GFL_BMPWIN_MakeFrameScreen( *win , MYSTERYCARD_TFRAMECHR, TWAKU_PALETTE_NUMBER);
//		BmpTalkWinWrite(win, WINDOW_TRANS_ON, MYSTERYCARD_TFRAMECHR, TWAKU_PALETTE_NUMBER);
	} else {
		GFL_BMPWIN_MakeFrameScreen( *win , MYSTERYCARD_FRAMECHR2, WAKU_PALETTE_NUMBER2);
...
static int MysteryCardNextCard(MYSTERYCARD_WORK *wk, int card, int add)
{
	...
	while(1){
		...
		if(orgcard == card)
			break;
		if(MYSTERYDATA_IsExistsCard(wk->fdata, card))
			break;
	}
#if 0//def DEBUG_ONLY_FOR_mituhara
...
{
	// メイン画面ＢＧパレット転送
	GFL_ARC_UTIL_TransVramPalette( ARCID_MYSTERY , NARC_mystery_fusigi_card_NCLR , PALTYPE_MAIN_BG ,0,16*16,wk->heapid );
//	ArcUtil_PalSet(ARC_MYSTERY_GRA, NARC_mystery_fusigi_card_nclr, PALTYPE_MAIN_BG, 0, 16*16, wk->heapid);
	// メイン画面BG1キャラ転送
	GFL_ARC_UTIL_TransVramBgCharacter( ARCID_MYSTERY , NARC_mystery_fusigi_card_NCGR , GFL_BG_FRAME1_M , 0,24*16*0x20 , 0 ,wk->heapid );
//	ArcUtil_BgCharSet(ARC_MYSTERY_GRA, NARC_mystery_fusigi_card_lz_cngr, wk->bgl, GF_BGL_FRAME1_M, 0, 24*16*0x20, 1, wk->heapid);
	...
	case MYSTERYCARD_FRONT:
		// メイン画面BG1スクリーン転送
		GFL_ARC_UTIL_TransVramScreen( ARCID_MYSTERY , NARC_mystery_fusigi_card01_NSCR , GFL_BG_FRAME1_M , 0 , 32*24*2 , 0 , wk->heapid);
//		ArcUtil_ScrnSet(ARC_MYSTERY_GRA, NARC_mystery_fusigi_card01_lz_cscr, wk->bgl, GF_BGL_FRAME1_M, 0, 32*24*2, 1, wk->heapid);
	...
	case MYSTERYCARD_BACK:
		// メイン画面BG1スクリーン転送
		GFL_ARC_UTIL_TransVramScreen( ARCID_MYSTERY , NARC_mystery_fusigi_card02_NSCR , GFL_BG_FRAME1_M , 0 , 32*24*2 , 0 , wk->heapid);
//		ArcUtil_ScrnSet(ARC_MYSTERY_GRA, NARC_mystery_fusigi_card02_lz_cscr, wk->bgl, GF_BGL_FRAME1_M, 0, 32*24*2, 1, wk->heapid);
		break;
	}

	// メイン画面ＢＧパレット転送
	GFL_ARC_UTIL_TransVramPalette( ARCID_MYSTERY , NARC_mystery_fusigi_card_NCLR , PALTYPE_MAIN_BG ,16*2*LATTICEBG_PALNO,16*2,wk->heapid );
//	ArcUtil_PalSet(ARC_MYSTERY_GRA, NARC_mystery_fusigi_bg_00_nclr, PALTYPE_MAIN_BG, 16*2*LATTICEBG_PALNO, 16*2, wk->heapid);
	...
	GFL_ARC_UTIL_TransVramBgCharacter( ARCID_MYSTERY , NARC_mystery_fusigi_bg_00_NCGR , 
				GFL_BG_FRAME3_M , LATTICEBG_CHARBASE,1*16*0x20 , 0 ,wk->heapid );
...
//				GF_BGL_FRAME3_M, LATTICEBG_CHARBASE * 1/*0x20*/, 1*16*0x20, 1, wk->heapid);
	// メイン画面スクリーン１
	//TODO 取りあえず普通にやってみる
	GFL_ARC_UTIL_TransVramScreen( ARCID_MYSTERY , NARC_mystery_fusigi_bg_00_NSCR , GFL_BG_FRAME3_M , 0 , 32*24*2 , 0 , wk->heapid);
//	CMC_ArcUtil_ScrnSet(wk, NARC_mystery_fusigi_bg_00_lz_cscr, GF_BGL_FRAME3_M, 32*24*2);
	GFL_BG_ChangeScreenPalette( GFL_BG_FRAME3_M , 0,0,32,24,LATTICEBG_PALNO );
 ...
 */
//------------------------------------------------------------------
void MysteryCard_DisplayCardOnly(GFL_BG_INI *bgl, void *p, int heapid)
{
	MYSTERYCARD_WORK *wk;
...
//	SystemFontPaletteLoad( PALTYPE_MAIN_BG, FONT_PALNO_NORMAL * 32, wk->heapid);
	GFL_ARC_UTIL_TransVramPalette( ARCID_FONT , NARC_font_default_nclr , PALTYPE_SUB_BG , FONT_PALNO_NORMAL * 32, 16*2, wk->heapid );

...
/**
 *
 *	@file		event_debug_btl_all_waza_cam_check.c
...
#include "event_debug_btl_all_waza_cam_check.h"
...
typedef struct {
  GAMESYS_WORK*   gsys;
  int count;
} DEBUG_BTL_ALL_WAZA_CAM_CHECK_WORK;
 ...
 */
//=============================================================================
static GMEVENT_RESULT BtlAllWazaCamCheck( GMEVENT* event, int* seq, void* wk_adrs );
 ...
 */
//-----------------------------------------------------------------------------
GMEVENT* EVENT_DEBUG_BtlAllWazaCamCheck( GAMESYS_WORK* gsys, void* wk_adrs )
{
  GMEVENT* event;
  DEBUG_BTL_ALL_WAZA_CAM_CHECK_WORK* wk;
  GAMEDATA* gdata;

  gdata = GAMESYSTEM_GetGameData( gsys );

  event = GMEVENT_Create( gsys, NULL, BtlAllWazaCamCheck, sizeof(DEBUG_BTL_ALL_WAZA_CAM_CHECK_WORK) );

  wk = GMEVENT_GetEventWork( event );
  GFL_STD_MemClear( wk, sizeof(DEBUG_BTL_ALL_WAZA_CAM_CHECK_WORK) );
  wk->gsys = gsys;
...
static GMEVENT_RESULT BtlAllWazaCamCheck( GMEVENT* event, int* seq, void* wk_adrs )
{
  ...
  int i;
  DEBUG_BTL_ALL_WAZA_CAM_CHECK_WORK* wk = wk_adrs;
  ...
  switch( *seq )
  {
    case SEQ_INIT:
      wk->count = 1;
      OS_FPrintf(3,"全技カメラカメラフォーカスチェック開始！\n");
      *seq = SEQ_SETUP;
      break;
...

    {
      ...
      for( i=0; i<PokeParty_GetPokeCount( mp ); i++ )
      {
        ...
        PP_SetWazaPos( pp, WAZANO_KAUNTAA, 0 );
      }
    ...
    case SEQ_RET:
      ...
      // スキップ じばく、だいばくはつ
      if( wk->count == WAZANO_ZIBAKU || wk->count == WAZANO_DAIBAKUHATU )
      {
        wk->count++;
      }
    ...
    case SEQ_EXIT:
      OS_FPrintf(3,"全技カメラカメラフォーカスチェック終了！\n");

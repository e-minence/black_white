...
static GMEVENT_RESULT FSkillSorawotobuEvent(GMEVENT * event, int * seq, void *work)
{
  ...
  case 3:
    ...
    //ブラックアウトリクエスト
    GMEVENT_CallEvent(event, EVENT_FieldFadeOut(wk->gsys, fieldWork, FIELD_FADE_BLACK));
    (*seq)++;
    break;
  case 4:
    //ノーフェードマップチェンジ
  ...
  case 7:
    ...
    if ( !FIELD_CAMERA_CheckMvFunc(camera) )
    {
      ...
      //ブラックインリクエスト
      GMEVENT_CallEvent(event, EVENT_FieldFadeIn(wk->gsys, fieldWork, FIELD_FADE_BLACK));
      (*seq)++;
    }
    break;
  case 8:

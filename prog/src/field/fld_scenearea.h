...
struct _FLD_SCENEAREA_DATA{
  ...
  // シーンパラメータ
  u8 area[FLD_SCENEAREA_SCENEPARAM_SIZE];

  // コールバック関数
  FLD_SCENEAREA_CHECK_AREA_FUNC* p_checkArea;   // 範囲内チェック
  FLD_SCENEAREA_CHECK_AREA_FUNC* p_updata;      // 情報の更新（エリア内の場合マイフレーム行う処理）
  FLD_SCENEAREA_CHECK_AREA_FUNC* p_inside;      // 範囲に入った瞬間の処理
  FLD_SCENEAREA_CHECK_AREA_FUNC* p_outside;     // 範囲を抜けた瞬間の処理
};

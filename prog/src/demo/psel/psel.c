...
#define FADE_SPEED_INSIDE_DEMO   (1)     // このデモでのフェード速度係数
...
typedef enum
{
  ...
  TARGET_POKE_DEFAULT   = TARGET_HONOO,  // キー操作のときに最初に選ばれるデフォルト
}
TARGET;
...
// 描画優先度
#define POKE_SOFTPRI_BACK   (2)  // 奥
#define POKE_SOFTPRI_MIDDLE (1)  // 中
...
typedef struct
{
  ...
  BOOL             move_req_no_change_move_step;  // move_reqを出した後にまだ一度もmove_stepやmove_step_countを変更していないときTRUE、変更したときFALSE
}
POKE_SET;
...
typedef struct
{
  ...
  // ブレンド
  u8                          ev1;  // G2_SetBlendAlphaのev1  // 0<=ev1<=16  // ev2=16-ev1
  ...
  // フェード速度係数
  int                         fade_speed_outside_demo;  // このデモに入る前のフェード速度係数を覚えておく
}
PSEL_WORK;
...
static GFL_PROC_RESULT Psel_ProcInit( GFL_PROC* proc, int* seq, void* pwk, void* mywk )
{
  ...
  // フェード速度係数
  work->fade_speed_outside_demo = GFL_FADE_GetFadeSpeed();  // このデモに入る前のフェード速度係数を覚えておく
...
//  GFL_FADE_SetMasterBrightReq( GFL_FADE_MASTER_BRIGHT_BLACKOUT, 16, 16, 0 );
  ...
  // フェード速度係数
  GFL_FADE_SetFadeSpeed( work->fade_speed_outside_demo );  // このデモに入る前のフェード速度係数に戻しておく
    ...
    case POKE_MOVE_STEP_P2_STOP:
      {
        ...
        else
        {
          if( p->move_req == POKE_MOVE_REQ_P0_TO_P2 )
          {
            ...
            if( !(p->move_req_no_change_move_step) )  // 何も変わっていないときは書く必要はないので
            {
  ...
  for( i=0; i<TARGET_POKE_MAX; i++ )
  {
    ...
    // 動き
    {
      if( i == target_poke )
      {
        ...
        //GFL_CLACT_WK_SetSoftPri( work->poke_set[i].clwk[POKE_BIG], POKE_SOFTPRI_FRONT );  // 注目していないポケモンのほうが手前のこともあるので、毎フレーム設定することにした。
      }
      else
      {
        work->poke_set[i].move_req   = POKE_MOVE_REQ_P2_TO_P0;
        work->poke_set[i].move_req_no_change_move_step = TRUE;
        //GFL_CLACT_WK_SetSoftPri( work->poke_set[i].clwk[POKE_BIG], POKE_SOFTPRI_BACK );  // 注目していないポケモンのほうが手前のこともあるので、毎フレーム設定することにした。
      }
    }
  }
}
  ...
  case S02_MAIN_SEQ_EXPLAIN_INIT:
    {
      ...
      *seq = S02_MAIN_SEQ_EXPLAIN_WAIT;
      ...
      // ボックスが開いたらポケモンのアルファアニメ開始
      Psel_PokeSetAlphaAnimeStart( work );
    }
    break;
  case S02_MAIN_SEQ_EXPLAIN_WAIT:
    {
      if(    Psel_TextExplainStreamWait( work )     // ストリームテキストが全て表示されるのを待つ
          && Psel_PokeSetIsAlphaAnimeEnd( work ) )  // ポケモンのアルファアニメが完了するのを待つ
      {
        // テキストとウィンドウは表示しっぱなし

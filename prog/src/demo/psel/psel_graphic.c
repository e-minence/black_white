...
static const GFL_DISP_VRAM sc_vramSetTable =
{
	...
	GX_VRAM_SUB_BGEXTPLTT_NONE,//GX_VRAM_SUB_BGEXTPLTT_0123_H, // サブ2DエンジンのBG拡張パレット
...
// サブ2DエンジンのBG拡張パレットを設定しない理由
// 256色1パレットのBGを使用しているので、拡張パレットを用意してしまうと、そちらを参照するようになってしまうから。
// TWL-SDK
// 関数リファレンスマニュアル
// G2S_SetBG0Control
// (注意) GX_SetBankForSubBGExtPltt() でVRAMバンクをBG拡張パレットに割り当てた場合、256色モードを使用するとBG拡張パレットを参照するようになります。それ以外の場合は標準パレットを参照します。
...
{	
	...
	{	
		GFL_BG_FRAME0_S,	//設定するフレーム
		{
			...
			GX_BG_SCRBASE_0x0000, GX_BG_CHARBASE_0x04000, GFL_BG_CHRSIZ256_256x128,//ｽｸﾘｰﾝﾍﾞｰｽ、ｷｬﾗﾍﾞｰｽ、ｷｬﾗｻｲｽﾞ
			GX_BG_EXTPLTT_01, 0, 0, 0, FALSE//拡張ﾊﾟﾚｯﾄｽﾛｯﾄ、表示優先度、ｴﾘｱｵｰﾊﾞｰ、ﾀﾞﾐｰ、ﾓｻﾞｲｸﾌﾗｸﾞ
		},
		GFL_BG_MODE_TEXT,//BGの種類
		TRUE,	//初期表示
	...
	{	
		GFL_BG_FRAME1_S,	//設定するフレーム
		{
			0, 0, 0x800, 0,	//X,Y,ｽｸﾘｰﾝﾊﾞｯﾌｧ、ｽｸﾘｰﾝｵﾌｾｯﾄ
			GFL_BG_SCRSIZ_256x256, GX_BG_COLORMODE_256,	//ｽｸﾘｰﾝｻｲｽﾞ、ｶﾗｰﾓｰﾄﾞ
			GX_BG_SCRBASE_0x0800, GX_BG_CHARBASE_0x04000, GFL_BG_CHRSIZ256_256x128,//ｽｸﾘｰﾝﾍﾞｰｽ、ｷｬﾗﾍﾞｰｽ、ｷｬﾗｻｲｽﾞ
			GX_BG_EXTPLTT_01, 1, 0, 0, FALSE//拡張ﾊﾟﾚｯﾄｽﾛｯﾄ、表示優先度、ｴﾘｱｵｰﾊﾞｰ、ﾀﾞﾐｰ、ﾓｻﾞｲｸﾌﾗｸﾞ
		},
		GFL_BG_MODE_TEXT,//BGの種類
		TRUE,	//初期表示
	...
	{	
		GFL_BG_FRAME2_S,	//設定するフレーム
		{
			0, 0, 0x800, 0,	//X,Y,ｽｸﾘｰﾝﾊﾞｯﾌｧ、ｽｸﾘｰﾝｵﾌｾｯﾄ
			GFL_BG_SCRSIZ_256x256, GX_BG_COLORMODE_16,	//ｽｸﾘｰﾝｻｲｽﾞ、ｶﾗｰﾓｰﾄﾞ
			GX_BG_SCRBASE_0x1000, GX_BG_CHARBASE_0x0c000, GFL_BG_CHRSIZ_256x128,
			GX_BG_EXTPLTT_01, 2, 0, 0, FALSE//拡張ﾊﾟﾚｯﾄｽﾛｯﾄ、表示優先度、ｴﾘｱｵｰﾊﾞｰ、ﾀﾞﾐｰ、ﾓｻﾞｲｸﾌﾗｸﾞ
		},
		GFL_BG_MODE_TEXT,//BGの種類
		TRUE,	//初期表示
	},
#if 1	//使わない場合は、このテーブルから消してください。(#if#endifじゃなくても大丈夫です)
	{	
		GFL_BG_FRAME3_S,	//設定するフレーム
		{
			0, 0, 0x800, 0,	//X,Y,ｽｸﾘｰﾝﾊﾞｯﾌｧ、ｽｸﾘｰﾝｵﾌｾｯﾄ
			GFL_BG_SCRSIZ_256x256, GX_BG_COLORMODE_16,	//ｽｸﾘｰﾝｻｲｽﾞ、ｶﾗｰﾓｰﾄﾞ
			GX_BG_SCRBASE_0x1800, GX_BG_CHARBASE_0x10000, GFL_BG_CHRSIZ_256x128,
			GX_BG_EXTPLTT_01, 3, 0, 0, FALSE//拡張ﾊﾟﾚｯﾄｽﾛｯﾄ、表示優先度、ｴﾘｱｵｰﾊﾞｰ、ﾀﾞﾐｰ、ﾓｻﾞｲｸﾌﾗｸﾞ
		},
		GFL_BG_MODE_TEXT,//BGの種類
		TRUE,	//初期表示
 ...
 */
//-----------------------------------------------------------------------------
PSEL_GRAPHIC_WORK * PSEL_GRAPHIC_Init( int display_select, HEAPID heapID )
{	
	PSEL_GRAPHIC_WORK * p_wk;
	p_wk	= GFL_HEAP_AllocMemory(heapID, sizeof(PSEL_GRAPHIC_WORK) );
	GFL_STD_MemClear( p_wk, sizeof(PSEL_GRAPHIC_WORK) );
 ...
 */
//-----------------------------------------------------------------------------
void PSEL_GRAPHIC_Exit( PSEL_GRAPHIC_WORK *p_wk )
{	
	//VBLANKTask消去
 ...
 */
//-----------------------------------------------------------------------------
void PSEL_GRAPHIC_2D_Draw( PSEL_GRAPHIC_WORK *p_wk )
{	
#ifdef GRAPHIC_OBJ_USE
 ...
 */
//-----------------------------------------------------------------------------
void PSEL_GRAPHIC_3D_StartDraw( PSEL_GRAPHIC_WORK *p_wk )
{	
#ifdef GRAPHIC_G3D_USE
 ...
 */
//-----------------------------------------------------------------------------
void PSEL_GRAPHIC_3D_EndDraw( PSEL_GRAPHIC_WORK *p_wk )
{	
#ifdef GRAPHIC_G3D_USE
 ...
 */
//-----------------------------------------------------------------------------
GFL_CLUNIT * PSEL_GRAPHIC_GetClunit( const PSEL_GRAPHIC_WORK *cp_wk )
{	
#ifdef GRAPHIC_OBJ_USE
 ...
 */
//-----------------------------------------------------------------------------
GFL_G3D_CAMERA * PSEL_GRAPHIC_GetCamera( const PSEL_GRAPHIC_WORK *cp_wk )
{	
#ifdef GRAPHIC_G3D_USE
...
static void GRAPHIC_G3D_Init( GRAPHIC_G3D_WORK *p_wk, HEAPID heapID )
{
	GFL_G3D_Init( GFL_G3D_VMANLNK, GRAPHIC_G3D_TEXSIZE,
			GFL_G3D_VMANLNK, GRAPHIC_G3D_PLTSIZE, 0x1000, heapID, Graphic_3d_SetUp );

...
static BtlAction ActOrder_Proc( BTL_SVFLOW_WORK* wk, ACTION_ORDER_WORK* actOrder );
...
static BOOL scproc_checkNoEffect_sub( BTL_SVFLOW_WORK* wk, const SVFL_WAZAPARAM* wazaParam,
  const BTL_POKEPARAM* attacker, const BTL_POKEPARAM* target, const BTL_DMGAFF_REC* affRec, BtlEventType eventID );
...
static u16 scproc_Ichigeki_Migawari( BTL_SVFLOW_WORK* wk, BTL_POKEPARAM* target, const SVFL_WAZAPARAM* wazaParam, BtlTypeAffAbout affAbout );
...
static u8 scproc_HandEx_vanishMsgWin( BTL_SVFLOW_WORK* wk, const BTL_HANDEX_PARAM_HEADER* param_header );
...
static u32 ActOrderProc_Main( BTL_SVFLOW_WORK* wk, u32 startOrderIdx )
{
  ...
  {
    currentAction = BTL_ACTION_GetAction( &wk->actOrder[i].action );

    // ローテーション完了後、プライオリティ再計算
...
/**
 *  両陣営ローテーション完了後、先頭に出たポケモン同士の素早さ再計算
 */
static void ActOrderProcSub_RecalcAgility( BTL_SVFLOW_WORK* wk, ACTION_ORDER_WORK* actOrder, u32 numActOrder )
{
  u32 i;
...
//              i, BPP_GetID(actOrder[i].bpp), agility, actOrder[i].priority, actOrder[i].priority);
  ...
  sortActionSub( actOrder, numActOrder );

  /*
  for(i=0; i<numActOrder; ++i)
  {
    TAYA_Printf("Order[%d]'s  pokeID=%d, aglity=%d, priority=%08x(%d)\n",
              i, BPP_GetID(actOrder->bpp), agility, actOrder->priority, actOrder->priority);
    ++actOrder;
  }
  */
}
...
//        scproc_TrainerItem_Root( wk, bpp, action.item.number, action.item.param, action.item.targetIdx );
      ...
      {
        u32 hem_state = BTL_Hem_PushState( &wk->HEManager );
        scEvent_ActProcEnd( wk, bpp, action.gen.cmd );
        BTL_Hem_PopState( &wk->HEManager, hem_state );
      }
...
static void scproc_Fight( BTL_SVFLOW_WORK* wk, BTL_POKEPARAM* attacker, BTL_ACTION_PARAM* action, u32 handlerSubPri )
{
  ...
  FLAG_SET  wazaFlags;
  u8 fWazaEnable, fWazaLock, fTameLock, fReqWaza, fPPDecrement, orgWazaIdx;
  ...
  do {
    ...
    if( scproc_Fight_CheckWazaExecuteFail_1st(wk, attacker, orgWaza, fWazaLock||fTameLock ) ){ break; }
    ...
    // ワザ出し失敗判定２（ＰＰ減少後）
    if( scproc_Fight_CheckWazaExecuteFail_2nd(wk, attacker, actWaza )){
       break;
    }
  ...
  // PP消費したらワザ使用記録を更新
  if( fPPDecrement || fWazaLock || (actWaza == WAZANO_WARUAGAKI) )
  {
...
static BOOL scproc_Fight_WazaExe( BTL_SVFLOW_WORK* wk, BTL_POKEPARAM* attacker, WazaID waza, BTL_POKESET* targetRec )
{
  ...
  if( fEnable )
  {
    ...
    // まもる・とくせいなど無効チェック
    flowsub_checkNotEffect( wk, wk->wazaParam, attacker, targetRec, &wk->dmgAffRec );
  ...
  BTL_POKESET* targets, BTL_DMGAFF_REC* affRec )
{
  ...
  while( (bpp = BTL_POKESET_SeekNext(targets)) != NULL )
  {
    if( scproc_checkNoEffect_sub(wk, wazaParam, attacker, bpp, affRec, BTL_EVENT_NOEFFECT_CHECK_L2) ){
      BTL_POKESET_Remove( targets, bpp );
    }
  }
  ...
  while( (bpp = BTL_POKESET_SeekNext(targets)) != NULL )
  {
    if( scproc_checkNoEffect_sub(wk, wazaParam, attacker, bpp, affRec, BTL_EVENT_NOEFFECT_CHECK_L3) ){
      BTL_POKESET_Remove( targets, bpp );
    }
  }
}
  ...
  const BTL_POKEPARAM* attacker, const BTL_POKEPARAM* target, const BTL_DMGAFF_REC* affRec, BtlEventType eventID )
{
  ...
  fNoEffect = scEvent_CheckNotEffect( wk, wazaParam, eventID, attacker, target, affRec, &wk->strParam, &fNoReaction );
  if( fNoEffect )
  {
    // 文字を出すだけのリアクション
    ...
    const BTL_POKEPARAM* attacker, const BTL_POKEPARAM* defender, const BTL_DMGAFF_REC* affRec, BTL_HANDEX_STR_PARAMS* customMsg, BOOL* fNoReaction )
{
  ...
  BTL_EVENTVAR_Push();
    ...
    BTL_EVENTVAR_SetConstValue( BTL_EVAR_TYPEAFF, dmgAff );
  ...
  while( 1 )
  {
...

    {
      ...
      if( scEvent_IchigekiCheck(wk, attacker, target, wazaParam->wazaID) )
      {
        ...
        if( BPP_MIGAWARI_IsExist(target) )
        {
          damage = scproc_Ichigeki_Migawari( wk, target, wazaParam, affAbout );
          BTL_POKESET_AddWithDamage( targets, target, damage, TRUE );
          fMigawari = TRUE;
        }
        else
        {
          ...
          if( korae_cause == BPP_KORAE_NONE ){
            ...
            BTL_POKESET_AddWithDamage( targets, target, damage, FALSE );
            fKorae = TRUE;
          }
        }
...
{
  u8 pokeID = BPP_GetID( target );
  u16 damage = BPP_MIGAWARI_GetHP( target );
...
static u8 scEvent_CheckSpecialActPriority( BTL_SVFLOW_WORK* wk, const BTL_POKEPARAM* attacker )
{
  ...
  BTL_EVENTVAR_Push();
    BTL_EVENTVAR_SetConstValue( BTL_EVAR_POKEID, BPP_GetID(attacker) );
    BTL_EVENTVAR_SetRewriteOnceValue( BTL_EVAR_SP_PRIORITY, BTL_SPPRI_DEFAULT );
...
static void HandEx_Exe( BTL_SVFLOW_WORK* wk, BTL_HANDEX_PARAM_HEADER* handEx_header )
{
  ...
  case BTL_HANDEX_VANISH_MSGWIN:      fPrevSucceed = scproc_HandEx_vanishMsgWin( wk, handEx_header ); break;
...
static u8 scproc_HandEx_delayWazaDamage( BTL_SVFLOW_WORK* wk, const BTL_HANDEX_PARAM_HEADER* param_header )
{
  ...
  flowsub_checkNotEffect( wk, &wazaParam, attacker, wk->psetTmp, &wk->dmgAffRec );

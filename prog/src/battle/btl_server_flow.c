...
// 各ポケモンの行動プライオリティ値を生成
  for(i=0, p=0; i<wk->numClient; i++)
  {
    ...
    for(j=0; j<numPoke; j++)
    {
      ...
      BTL_POKEPARAM_SetActionAgility( (BTL_POKEPARAM*)bpp, agility );
...
static void scproc_MemberOut( BTL_SVFLOW_WORK* wk, u8 clientID, u8 posIdx )
{
  ...
  BTL_POKEPARAM_OutClear( poke );
  SCQUE_PUT_OP_OutClear( wk->que, BTL_POKEPARAM_GetID(poke) );
...
static void scproc_turncheck_sick( BTL_SVFLOW_WORK* wk )
{
  ...
  while( FRONT_POKE_SEEK_GetNext( &fpsw, wk, &bpp ) )
  {
    ...
    BTL_POKEPARAM_WazaSick_TurnCheck( bpp, BTL_SICK_TurnCheckCallback, wk );
    scproc_HandEx_Root( wk, ITEM_DUMMY_DATA );
...
static SV_WazaFailCause scEvent_CheckWazaExecute( BTL_SVFLOW_WORK* wk, BTL_POKEPARAM* attacker, WazaID waza )
{
  ...
  do {
    ...
    if( BTL_FIELD_CheckEffect(BTL_FLDEFF_FUIN) )
    {
      ...
      if( BTL_POKEPARAM_GetID(attacker) != dependPokeID )
      {
        const BTL_POKEPARAM* bpp = BTL_POKECON_GetPokeParam( wk->pokeCon, dependPokeID );
        if( BTL_POKEPARAM_GetWazaIdx(bpp, waza) != PTL_WAZA_MAX ){
...
static BOOL scproc_HandEx_Root( BTL_SVFLOW_WORK* wk, u16 useItemID )
{
  ...
  while( (handEx_header = Hem_ReadWork(&wk->HEManager)) != NULL )
  {
    ...
    case BTL_HANDEX_MESSAGE_SET:    fPrevSucceed = scproc_HandEx_messageSet( wk, handEx_header ); break;
    case BTL_HANDEX_MESSAGE_STD:    fPrevSucceed = scproc_HandEx_messageStd( wk, handEx_header ); break;
...
{
  BTL_POKEPARAM* pp_user = BTL_POKECON_GetPokeParam( wk->pokeCon, param_header->userPokeID );
  if( !BTL_POKEPARAM_IsDead(pp_user) )
  {
...
static u8 scproc_HandEx_recoverHP( BTL_SVFLOW_WORK* wk, const BTL_HANDEX_PARAM_HEADER* param_header, u16 itemID )
{
  ...
  if( !BTL_POKEPARAM_IsDead(pp_target)
  &&  !BTL_POKEPARAM_IsHPFull(pp_target)
  ){
...
static u8 scproc_HandEx_decrementPP( BTL_SVFLOW_WORK* wk, const BTL_HANDEX_PARAM_HEADER* param_header, u16 itemID )
{
  ...
  if( !BTL_POKEPARAM_IsDead(targetPokemon) )
  {
...
static u8 scproc_HandEx_recoverRank( BTL_SVFLOW_WORK* wk, const BTL_HANDEX_PARAM_HEADER* param_header )
{
  ...
  if( !BTL_POKEPARAM_IsDead(pp_target) )
  {
    BTL_POKEPARAM_RankRecover( pp_target );
    return 1;
  }
  return 0;
}
...
static u8 scproc_HandEx_messageSet( BTL_SVFLOW_WORK* wk, const BTL_HANDEX_PARAM_HEADER* param_header )
{
  ...
  scPut_Message_SetExPoke( wk, bpp, param->strID, param->arg_cnt, param->args );
  return 1;
}
/**
 * STDメッセージ表示
...
{
  const BTL_HANDEX_PARAM_MESSAGE* param = (const BTL_HANDEX_PARAM_MESSAGE*)(param_header);
  scPut_Message_StdEx( wk, param->strID, param->arg_cnt, param->args );
  return 1;
}
/**
 * ポケモンターンフラグセット

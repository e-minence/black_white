...
//	BINPUT_TYPE_WALL_MISSING_PLAYER,	///<主人公キャラがいない壁
	...
	BINPUT_TYPE_PLAYBACK_STOP,	///<「さいせい　ていし」
};
...
///録画再生停止画面で表示させるシステムメッセージタイプ
enum{
	BI_SYSMSG_STOP,			///<「ちゅうだん　しています！」
	BI_SYSMSG_ERROR,		///<「ふせいな　データ」
	BI_SYSMSG_DATAFULL,		///<「しょうぶの　じかんが　ながすぎるため」
};

//COMMAND_SELECTのstatusパラメータの意味（プラチナから持ってきた）
...
//	外部関数宣言
//==============================================================================
extern void BINPUT_DefaultFrameSet( void );
extern void BINPUT_FrameExit( void );
extern void * BINPUT_SystemInit(ARCHANDLE* hdl_bg, ARCHANDLE* hdl_obj,
//		BATTLE_WORK *bw,
		GFL_MSGDATA *msg,
		GFL_FONT *font,
		GFL_TCBSYS *tcbsys,
...
extern void BINPUT_CreateBG(ARCHANDLE* hdl_bg, ARCHANDLE* hdl_obj, BI_PARAM_PTR bip, int select_bg, int force_put, void *scene_work);
...
extern void BINPUT_StockBallActorResourceLoad(ARCHANDLE* hdl, BI_PARAM_PTR bip);
...
extern void BINPUT_SystemMessagePrint(BI_PARAM_PTR bip, int sysmsg_type);
extern u8 BINPUT_CancelEscapeFlagGet(BI_PARAM_PTR bip);

extern	void Sub_TouchEndDelete(BI_PARAM_PTR bip, int obj, int bg);

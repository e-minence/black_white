...
typedef struct {
	...
	u8 out_wnd;
	u8 fade_flg;
} WIPE_TYPE_WND_ROUND_PARAM;

//-------------------------------------
//	ワーク
//=====================================
typedef struct {
	WIPE_TYPE_HB_WND hb_round;	// HblankWndデータ
	int x;			// 中心座標
	int y;
	int	r;			// 半径
	int start_angle;// 最初の角度
	int angle;		// 角度
	int add_angle;	// 角度の変化
	int a_count;	// 変化数（段階数）
	int piece_sync;	// １片のシンク数
	...
	int	wnd_no;		// WIPE_TYPE_WND_ROUND_PARAMのwndnoを保存
	int	disp;		// 引数のdispを保存

	BOOL turn_flg;	// ウィンドウ平面設定反転フラグ
...
static void scchg_v_h_WndDel( GFL_TCB * tcb, void* work );
...
static void scchg_WndSwap(GFL_TCB * tcb, void* work);
...
static void scchg_WndSwapInOutPlane(WNDP_SYS_PTR wnd_sys, int wnd_no, int disp, int fade_flg);
...
static void scchg_h_VMoveWnd_Swap( GFL_TCB * tcb, void* p_work );
...
//	ラウンドワイプ
static void scchg_WndRoundPackInit( WIPE_SYS_WIPE_WORK* wipe, const WIPE_TYPE_WND_ROUND_PARAM* param );
static BOOL scchg_WndRoundPack(WIPE_SYS_WIPE_WORK* wipe);
static void scchg_WndRoundInit(WIPE_TYPE_WND_ROUND* data, const WIPE_TYPE_WND_ROUND_PARAM* param, int division, int piece_sync, int disp, WNDP_SYS_PTR wnd_sys, WIPE_HBLANK* p_wipehb, int heap);
static BOOL scchg_WndRoundMain(WIPE_TYPE_WND_ROUND* data);
static void scchg_WndRoundSum(WIPE_TYPE_WND_ROUND* work);
 ...
 */
//-----------------------------------------------------------------------------
BOOL WipeFunc42(WIPE_SYS_WIPE_WORK* wipe)
{
	if(wipe->sequence == WIPE_INIT){
		static const WIPE_TYPE_WND_ROUND_PARAM param = {
			128,96,
			ROTA_NUM(200),
			WNDP_WND_0,
			GX_BLEND_PLANEMASK_BD, GX_BLEND_ALL,
		...
		WIPE_SetBackDropColor( wipe->color );
		scchg_WndRoundPackInit( wipe, &param );
	...
	return scchg_WndRoundPack(wipe);
}
...
static void scchg_FadeInit(WIPE_SYS_WIPE_WORK* wipe, int fade_flag)
{
	...
	wipe->wipe_work = GFL_HEAP_AllocMemory( wipe->heap, sizeof( WIPE_BRIGHTNESS_FADE ) );
	GFL_STD_MemFill( wipe->wipe_work, 0, sizeof( WIPE_BRIGHTNESS_FADE ) );
	data = wipe->wipe_work;
...
static void scchg_h_WndMake(WIPE_TYPE_HB_WND* data, int flag, int disp, int heap)
{
	...
	case WNDP_WND_1:
		data->data = GFL_HEAP_AllocMemory(heap, sizeof(WIPE_TYPE_HB_WND_ONE));
		data->wnd_data_num	= 1;
		data->disp			= disp;
	...
	case 2:
		{
			int i;
			data->data = GFL_HEAP_AllocMemory(heap, sizeof(WIPE_TYPE_HB_WND_ONE)*2);
			data->wnd_data_num	= 2;
			data->disp			= disp;
...
static void scchg_WndSwapInOutPlane(WNDP_SYS_PTR wnd_sys, int wnd_no, int disp, int fade_flg)
{
	...
	//	現在の設定取得
	in_wnd	= WNDP_GetWndInsidePlane( wnd_no, disp );
	out_wnd	= WNDP_GetWndOutsidePlane( disp );

	// ウィンドウの内外を設定
	if( fade_flg == WIPE_FADE_IN ){
		// フェードイン
		WNDP_SetWndInsidePlane(out_wnd.planeMask, FALSE, wnd_no, disp);
		WNDP_SetWndOutsidePlane(in_wnd.planeMask, FALSE, disp);

	}else{
		// フェードアウト
		WNDP_V_SetWndInsidePlane(wnd_sys, out_wnd.planeMask, FALSE, wnd_no, disp);
		WNDP_V_SetWndOutsidePlane(wnd_sys, in_wnd.planeMask, FALSE, disp);
	}
}
...
static void scchg_h_VMoveWndInit( WIPE_TYPE_WND_VMOVE* p_wnddata, int disp, int data_num, int wnd_no0, int wnd_no1 )
{
	GFL_STD_MemFill( p_wnddata, 0, sizeof(WIPE_TYPE_WND_VMOVE) );
...
static void scchg_h_VMoveWnd_Swap( GFL_TCB * tcb, void* p_work )
{
	...
	for( i=0; i<2; i++ ){
		GFL_STD_MemCopy( p_wnddata->data[i].in_out_read, p_wnddata->data[i].in_out_write, sizeof(u8)*192 );
	}
...
static void scchg_WndMovePackInit( WIPE_SYS_WIPE_WORK* wipe, const WIPE_TYPE_WND_MOVE_FP* pram )
{
	...
	// フェード開始
	wipe->wipe_work = GFL_HEAP_AllocMemory(wipe->heap, sizeof(WIPE_TYPE_WND_MOVE));
...
static void scchg_WndMoveDoublePackInit(WIPE_SYS_WIPE_WORK* wipe, const WIPE_TYPE_WND_MOVE_FP* pram1, const WIPE_TYPE_WND_MOVE_FP* pram2)
{
	...
	// フェード開始
	wipe->wipe_work = GFL_HEAP_AllocMemory(wipe->heap, sizeof(WIPE_TYPE_WND_DMOVE));
...
static void scchg_WndCirclePackInit( WIPE_SYS_WIPE_WORK* wipe, const WIPE_TYPE_WND_CIRCLE_FP* param )
{
	...
	// ワイプ開始
	wipe->wipe_work = GFL_HEAP_AllocMemory(wipe->heap, sizeof(WIPE_TYPE_WND_CIRCLE));
...
static void scchg_WndCurtainPackInit( WIPE_SYS_WIPE_WORK* wipe, const WIPE_TYPE_WND_CURTAIN_FP* param)
{
	...
	// フェード開始
	wipe->wipe_work = GFL_HEAP_AllocMemory(wipe->heap, sizeof(WIPE_TYPE_WND_CURTAIN));
...
static void scchg_WndDoorPackInit(WIPE_SYS_WIPE_WORK* wipe, const WIPE_TYPE_WND_DOOR_FP* param)
{
	...
	// フェード開始
	wipe->wipe_work = GFL_HEAP_AllocMemory(wipe->heap, sizeof(WIPE_TYPE_WND_DOOR));
...
static void scchg_WndDivisionPackInit(WIPE_SYS_WIPE_WORK* wipe, const WIPE_TYPE_WND_DIVI_FP* param)
{
	...
	// フェード開始
	wipe->wipe_work = GFL_HEAP_AllocMemory(wipe->heap, sizeof(WIPE_TYPE_WND_DIVI));
...
static void scchg_WndDivisionInit(WIPE_TYPE_WND_DIVI* data, const WIPE_TYPE_WND_DIVI_FP* param, int division, int piece_sync, int disp, WNDP_SYS_PTR wnd_sys, WIPE_HBLANK* p_wipehb, int heap )
{
	...
	// 分割データ作成
	// メモリアロック
	data->piece_tbl = GFL_HEAP_AllocMemory(heap, sizeof(WIPE_TYPE_WND_D_PIECE)*param->divi_num);
...
{
	// 確保したメモリを解放
	GFL_HEAP_FreeMemory( data->piece_tbl);
	data->piece_tbl = NULL;
}
...
static void scchg_WndDivisionSum(WIPE_TYPE_WND_DIVI* work)
{
	...
	GFL_STD_MemFill(one->wnd_tmp, 0, 768);	// クリア
...
static void scchg_WndScrewPackInit(WIPE_SYS_WIPE_WORK* wipe, WIPE_TYPE_WND_SCREW_FP* param)
{
	...
	wipe->wipe_work = GFL_HEAP_AllocMemory( wipe->heap, sizeof(WIPE_TYPE_WND_SCREW) );
	GFL_STD_MemFill( wipe->wipe_work, 0, sizeof(WIPE_TYPE_WND_SCREW) );
	data = wipe->wipe_work;
...
static void scchg_WndFlashFourPackInit(WIPE_SYS_WIPE_WORK* wipe, WIPE_TYPE_WND_FLASH_FOUR_FP* param)
{
	...
	wipe->wipe_work = GFL_HEAP_AllocMemory( wipe->heap, sizeof(WIPE_TYPE_WND_FLASH_FOUR) );
	GFL_STD_MemFill( wipe->wipe_work, 0, sizeof(WIPE_TYPE_WND_FLASH_FOUR) );
	data = wipe->wipe_work;
...
static void scchg_WndWndVMovePackInit(WIPE_SYS_WIPE_WORK* wipe, WIPE_TYPE_WND_VMOVE_PARAM* param)
{
	...
	wipe->wipe_work = GFL_HEAP_AllocMemory( wipe->heap, sizeof(WIPE_TYPE_WND_VMOVE_CONT) );
	GFL_STD_MemFill( wipe->wipe_work, 0, sizeof(WIPE_TYPE_WND_VMOVE_CONT) );
	data = wipe->wipe_work;
...
static void scchg_WndWndVMoveInit(WIPE_TYPE_WND_VMOVE_CONT* data, WIPE_TYPE_WND_VMOVE_PARAM* param, int division, int piece_sync, int disp, WNDP_SYS_PTR wnd_sys, WIPE_HBLANK* p_wipehb, int heap )
{
	...
	if( param->fade_flg == WIPE_FADE_IN ){
		GFL_STD_MemFill( data->HblankData.data[0].in_out_write, WIPE_WND_VMOVE_OUTWND, sizeof( u8 ) * 192 );
		GFL_STD_MemFill( data->HblankData.data[0].in_out_read, WIPE_WND_VMOVE_OUTWND, sizeof( u8 ) * 192 );
	}else{
		GFL_STD_MemFill( data->HblankData.data[0].in_out_write, WIPE_WND_VMOVE_INWND, sizeof( u8 ) * 192 );
		GFL_STD_MemFill( data->HblankData.data[0].in_out_read, WIPE_WND_VMOVE_INWND, sizeof( u8 ) * 192 );
	}
...
static void scchg_WndWndPesocomPackInit(WIPE_SYS_WIPE_WORK* wipe, WIPE_TYPE_WND_PESOCOM_PARAM* param)
{
	...
	wipe->wipe_work = GFL_HEAP_AllocMemory( wipe->heap, sizeof(WIPE_TYPE_WND_PESOCOM) );
	GFL_STD_MemFill( wipe->wipe_work, 0, sizeof(WIPE_TYPE_WND_PESOCOM) );
	data = wipe->wipe_work;
...
static void scchg_WndRoundPackInit( WIPE_SYS_WIPE_WORK* wipe, const WIPE_TYPE_WND_ROUND_PARAM* param )
{
	WIPE_TYPE_WND_ROUND* wwm;
	
	// ワイプ開始
	wipe->wipe_work = GFL_HEAP_AllocMemory(wipe->heap, sizeof(WIPE_TYPE_WND_ROUND));
	wwm = (WIPE_TYPE_WND_ROUND*)wipe->wipe_work;
	scchg_WndRoundInit(wwm, param, wipe->division, wipe->piece_sync, wipe->disp, wipe->wnd_sys, wipe->p_wipehb, wipe->heap);
...
static BOOL scchg_WndRoundPack(WIPE_SYS_WIPE_WORK* wipe)
{
	...
	case WIPE_END:
		scchg_h_WndDel(&wwm->hb_round);	// ウィンドウ設定データ破棄
		GFL_HEAP_FreeMemory(wipe->wipe_work);
		wipe->wipe_work = NULL;
		wipe->sequence++;
		ret_num = TRUE;
...
static void scchg_WndRoundInit(WIPE_TYPE_WND_ROUND* data, const WIPE_TYPE_WND_ROUND_PARAM* param, int division, int piece_sync, int disp, WNDP_SYS_PTR wnd_sys, WIPE_HBLANK* p_wipehb, int heap)
{
	...
	// 変化値を求める
	add_angle = ROTA_NUM(360) / division;

	// データを設定
	scchg_h_WndMake(&data->hb_round, param->wnd_no, disp, heap);
	data->r					= 0xFFFF*SUM_NUM_MINI;	//	無限円
	data->x					= param->x;
	data->y					= param->y;
	data->start_angle		= param->angle;
	data->angle				= param->angle;
	data->add_angle			= add_angle;
	data->a_count			= division;
	data->piece_sync		= piece_sync;
	data->sync_c			= 0;
	data->wnd_sys			= wnd_sys;
	...
	// ウィンドウ設定
	one = scchg_h_WndIdxDataGet(&data->hb_round, 0);
	scchg_WndSetUp(wnd_sys, param->in_wnd, param->out_wnd, param->wnd_no, disp,
			one->wnd_tmp[0][0], 0, one->wnd_tmp[1][0], 192,
			data->fade_flg);
	...
	// Hblank関数を設定
	WIPE_V_HBlankInit(data->p_wipehb, &data->hb_round, scchg_h_Wnd, disp, heap);
}
...
static BOOL scchg_WndRoundMain(WIPE_TYPE_WND_ROUND* data)
{
	...
	if(data->sync_c >= data->piece_sync){
		...
		//	if( sys.cont & PAD_KEY_RIGHT ) {
				data->angle += data->add_angle;
				data->a_count--;

/*	OS_Printf( "開始角:%d 移動角:%d\n", data->start_angle*360/0xFFFF, data->angle*360/0xFFFF );
			}
...
*/			while( data->angle >= ROTA_NUM(360) ) { data->angle -= ROTA_NUM(360); }	//	回り込み
			while( data->angle < 0 ) { data->angle += ROTA_NUM(360); }	//	回り込み
			diff_angle	= data->angle - data->start_angle;
			//	角度が180度を越えたらウィンドウマスクを反転する（凹が出来てしまうとウィンドウHブランク操作はできない）
			...
			WIPE_V_HBlankDelete(data->p_wipehb, data->hb_round.disp, data->heap);
			return TRUE;
		}
	}
 ...
 */
//-----------------------------------------------------------------------------
static void scchg_WndRoundSumSub(int c_r, int c_x, int c_y, int n_h, int s_a, int a, int* st, int* ed)
{
	int min, max;
	int r;			// 半径
	int p1_x, p1_y;
	...
	//	開始角と移動角の、円周上を動く点P1,P2を求める
	p1_x	= c_x + ((r * FX_CosIdx(s_a) ) >> FX32_SHIFT);
	p1_y	= c_y + ((r * FX_SinIdx(s_a) ) >> FX32_SHIFT);
	p2_x	= c_x + ((r * FX_CosIdx(a) ) >> FX32_SHIFT);
	p2_y	= c_y + ((r * FX_SinIdx(a) ) >> FX32_SHIFT);

	//	開始角から180度になった場合、埋めるべき領域がなくなるため例外処理
	if( a - s_a == ROTA_NUM(180) ) {
		if( 0 <= n_h && n_h < c_y ) {
			*st	= 0;
			*ed	= 255;
		}
	}
	//	移動角が水平を超えると、走査線ごとに塗りつぶすため、塗りつぶせなくなってしまう
	else if ( ( ROTA_NUM(0) <= a && a < ROTA_NUM( 180)/*s_a*/ ) ) {
		int p;
		min	= p1_y < c_y ? p1_y : c_y;
		max	= p1_y > c_y ? p1_y : c_y;

		if( min <= n_h && n_h <= max ) {
			*st	= ( p1_x - c_x ) * (n_h - p1_y ) / ( p1_y - c_y ) + p1_x;
		}
		else {
			*st	= ( p2_x - c_x ) * (n_h - p2_y ) / ( p2_y - c_y ) + p2_x;
		}
		*st	= MATH_CLAMP( *st, 0, 255 );
		*ed	= 255;
	}
	else {

		//	n_hを与えたときの中心からP1までの線分のX座標を返す
		min	= p1_y < c_y ? p1_y : c_y;
		max	= p1_y > c_y ? p1_y : c_y;
		if( min <= n_h && n_h <= max ) {
		...
		//	中心からP2までの線分のX座標を返す
		min	= p2_y < c_y ? p2_y : c_y;
		max	= p2_y > c_y ? p2_y : c_y;
		if( min <= n_h && n_h <= max ) {
			*ed	= ( p2_x - c_x ) * (n_h - p2_y ) / ( p2_y - c_y ) + p2_x;
			*ed	= MATH_CLAMP( *ed, 0, 255 );
		}
		else {
			*ed	= *st;	//	線分外なので、表示させないため、stと同じ位置
		}
...
static void scchg_WndRoundSum(WIPE_TYPE_WND_ROUND* work)
{
	WIPE_TYPE_HB_WND	* shb = &work->hb_round;
	int i;			// ループ用
	int st;			// ウィンドウ開始点
	int ed;			// ウィンドウ終了点
	int roop_end;
	...
	// 中心まで幅を求めたら下半分はコピーするようにする
	for(i=0;i < 192;i++){
		scchg_WndRoundSumSub(work->r, work->x, work->y, i,work->start_angle, work->angle, &st, &ed);
		one->wnd_tmp[0][i] = st;
		one->wnd_tmp[1][i] = ed;
	}
}

...
static void scchg_HBlankFunc(GFL_TCB * tcb, void * work);
static void scchg_v_HBlankInit(GFL_TCB * tcb, void* work);
static void scchg_v_HBlankDelete(GFL_TCB * tcb, void* work);
...
static void resetMstBrightnessVblank( GFL_TCB * tcb, void* work );
...
//----------------------------------------------------------------------------
/**
 *	@brief	両画面に一度に設定するモードです。
 *			両画面同じカラーで設定する場合は出来るだけこちらを使用してください
 */
//-----------------------------------------------------------------------------
...
void WIPE_HBlankInit(WIPE_HBLANK* p_wipehb, void* work, pHBFunc func, int disp)
{
	...
	if((p_wipehb->flg[0] == 0) &&
		(p_wipehb->flg[1] == 0)){
		WipeSysWork.HBtcb = GFUser_HIntr_CreateTCB(scchg_HBlankFunc, p_wipehb, 0);
...
{
	WIPE_V_HBLANK* data = GFL_HEAP_AllocMemoryLo(heap, sizeof(WIPE_V_HBLANK));
...
static void cleanWipeData( WIPE_SYS_DATA* p_wipe_data )
{
	GFL_STD_MemClear( &p_wipe_data->wipe_pattern_data, sizeof(WIPE_SYS_PATTERN_DATA) );
	GFL_STD_MemClear( &p_wipe_data->wipe_m, sizeof(WIPE_SYS_WIPE_WORK) );
	GFL_STD_MemClear( &p_wipe_data->wipe_s, sizeof(WIPE_SYS_WIPE_WORK) );
	GFL_STD_MemClear( &p_wipe_data->wipehb, sizeof(WIPE_HBLANK) );
	GFL_STD_MemClear( &p_wipe_data->wnd_sys, sizeof(WNDP_SYS) );
}

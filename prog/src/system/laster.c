 ...
 */
//-----------------------------------------------------------------------------
LASTER_SYS_PTR LASTER_Init( HEAPID heapId, void* p_buff1, void* p_buff2 )
{
	LASTER_SYS_PTR p_laster;

	p_laster = GFL_HEAP_AllocClearMemory( heapId, sizeof(LASTER_SYS) );
...
// ラスター用
static void LASTER_ScrollTcb( GFL_TCB *tcb, void* work );
static void LASTER_YScrollTcb( GFL_TCB *tcb, void* work );
 ...
 */
//-----------------------------------------------------------------------------
LASTER_SCROLL_PTR LASTER_ScrollInit( HEAPID heapId )
{
	LASTER_SCROLL_PTR p_lstscr;

	// オブジェクト生成
	p_lstscr = GFL_HEAP_AllocClearMemory( heapId, sizeof(LASTER_SCROLL) );
	
	// ラスターシステム生成
	p_lstscr->p_laster = LASTER_Init( heapId, p_lstscr->buff1, p_lstscr->buff2 );
...
void LASTER_ScrollStart( LASTER_SCROLL_PTR p_lstscr, u8 start, u8 end, u16 add_r, fx32 r_w, s16 scr_sp, u32 bg_no, u32 init_num, u32 tsk_pri )
{
	...
	// tcb登録
	p_lstscr->p_tcb = GFUser_VIntr_CreateTCB( LASTER_ScrollTcb, p_lstscr, tsk_pri );	
...
void LASTER_YScrollStart( LASTER_SCROLL_PTR p_lstscr, u8 start, u8 end, u16 add_r, fx32 r_w, s16 scr_sp, u32 bg_no, u32 init_num, u32 tsk_pri )
{
	GF_ASSERT( p_lstscr );
	GF_ASSERT_MSG( p_lstscr->p_tcb == NULL, "tcb登録済みです." );
	...
	// tcb登録
	p_lstscr->p_tcb = GFUser_VIntr_CreateTCB( LASTER_YScrollTcb, p_lstscr, tsk_pri );	
...
void LASTER_ScrollEnd( LASTER_SCROLL_PTR p_lstscr )
{
	...
	if( p_lstscr->p_tcb ){
		// TCBの破棄
		GFL_TCB_DeleteTask( p_lstscr->p_tcb );
		p_lstscr->p_tcb = NULL;
...
static void LASTER_YScrollTcb( GFL_TCB *tcb, void* work )
{
	...
	for( i=p_lstscr->start; i<=p_lstscr->end; i++ ){
		...
		// サインカーブ反映
//		mat_x += p_lstscr->sinTbl[ write ];
		mat_y += p_lstscr->sinTbl[ ( write + 180 ) % LASTER_SCROLL_BUFF_SIZE ];

...
static BOOL _commInit(BOOL bAlloc, int packetSizeMax)
{
    ...
    if(bAlloc){
        ...
        _commOrg = (u32)GFL_HEAP_AllocMemory(HEAPID_COMMUNICATION, sizeof(_COMM_WORK_SYSTEM)+32);
/*↑[GS_CONVERT_TAG]*/
...
/*↑[GS_CONVERT_TAG]*/
        ...
        _pComm->pServerRecvBufRing = GFL_HEAP_AllocMemory(HEAPID_COMMUNICATION, _pComm->allocSizeMax);   ///< 受け取るバッファをバックアップする
/*↑[GS_CONVERT_TAG]*/

        _pComm->pMidRecvBufRing = GFL_HEAP_AllocMemory(HEAPID_COMMUNICATION, _pComm->allocSizeMax);   ///< 受け取るバッファをバックアップする DS専用
/*↑[GS_CONVERT_TAG]*/
        // キューの初期化
...
/*↑[GS_CONVERT_TAG]*/
        ...
        GFL_HEAP_FreeMemory((void*)_commOrg);
/*↑[GS_CONVERT_TAG]*/
        _pComm = NULL;
        _commOrg = 0;
    }
...
BOOL CommUpdateData(void)
{
    ...
    if(_pComm != NULL){
        if(!_pComm->bShutDown){
            ...
            _pComm->sendCont |= (GFL_UI_KEY_GetCont() & 0x7fff);  // キーデータの取得
/*↑[GS_CONVERT_TAG]*/
            _keyRand();
            _dataMpStep();

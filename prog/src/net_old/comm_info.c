...
void CommInfoFinalize(void)
{
    ...
    if(_pCommInfo){
        ...
        if(_pCommInfo){
            GFL_HEAP_FreeMemory(_pCommInfo);
/*↑[GS_CONVERT_TAG]*/
        }
        _pCommInfo = NULL;
		TOMOYA_PRINT( "comminfo delete\n" );
    }
}
...
void CommInfoSendPokeData(void)
{
    ...
    if(_pCommInfo->pWiFiPlaceMyStatus){
        pMyStatus = _pCommInfo->pWiFiPlaceMyStatus;
    }
    else{
...
void CommInfoRecvArrayPlayerData( int netID, int size, void* pData, void* pWork)
{
	...
	// 080606	tomoya		BTS:253修正のため
	// まとめて送るモードの場合、Infoデータを削除後のデータが送られてくる可能性があるため、
	// 無効なMYSTATUSデータ（名前の中身が全部０）がやってきたら無視する
	if( MyStatus_CheckNameClear( _pCommInfo->pMyStatus[ _pCommInfo->nowNetID ] ) == TRUE ){
		return ;
	}
...
void CommInfoDeletePlayer(int netID)
{
    ...
    TOMOYA_PRINT("comminfo delete netid[%d]\n", netID);
...
//--------------------------------------------------------------
/**
 * @brief   エントリー中なのかどうか調べる
 * @param   netID           ネットワークID
 * @retval  TRUE     エントリーしている
 * @retval  FALSE    エントリーしていない
 */
//--------------------------------------------------------------

BOOL CommInfoGetEntry(int netID)
{
    return (_pCommInfo->info[netID] == _INFO_ENTRY);
}
 ...
 */
//==============================================================================
int CommInfoSearchWifiListIndex(int netID)
{
    return dwc_SearchNetID2WifiListIndex(_pCommInfo->pSaveData, netID);
}
...
//------------------------------------------------------------------
/**
 * $brief   WIFI広場用のMyStatusポインタ設定
 * @param   MyStatus マイステータスのポインタ
 * @retval  none
 */
//------------------------------------------------------------------

void CommInfoSetWiFiPlaceMyStatus(MYSTATUS* pMyStatus)
{
    _pCommInfo->pWiFiPlaceMyStatus = pMyStatus;
}

...
BOOL	AriFieldMain( void )
{
	...
	case 0:
		//基本システムセットアップ
		ariFieldWork->gs = SetupGameSystem( ariFieldWork->heapID );
	...
	case 1:
		ariFieldWork->ftbl = resistMapTbl[ariFieldWork->mapNum].dep_funcs;
		{
			...
			{	//デバッグメニュー
				ariFieldWork->d_menu = AriFldMenu_Init( ariFieldWork->heapID );
			}
	...
	case 2:
		if( GameEndCheck( GFL_UI_KEY_GetCont() ) == TRUE ){
			ariFieldWork->gamemode = GAMEMODE_FINISH;
			ariFieldWork->seq = 3;
			break;
		}
		...
		if( GFL_UI_KEY_GetTrg() == PAD_BUTTON_SELECT ){
			ariFieldWork->mapNum--;
			if( ariFieldWork->mapNum < 0 ){
				ariFieldWork->mapNum = (resistMapTblCount)-1;
			}
			ariFieldWork->seq = 3;
			break;
		}
		...
		if( !DebugMenuProc( ariFieldWork ) ) {
		
			VecFx32 pos;
			ariFieldWork->key_cont = GFL_UI_KEY_GetCont();
	...
	case 3:
		...
		ariFieldWork->ftbl->delete_func(ariFieldWork);

        ReleaseDataFieldG3Dmapper( GetFieldG3Dmapper(ariFieldWork->gs) );

		if (ariFieldWork->gamemode != GAMEMODE_FINISH) {
			ariFieldWork->seq = 1;
		} else {
			ariFieldWork->seq = 4;
		}
	...
	case 4:
		RemoveGameSystem( ariFieldWork->gs );
		return_flag = TRUE;
		break;
	}
	return return_flag;
...
#if 0
GFL_G3D_CAMERA* AriGetG3Dcamera( FIELD_SETUP* gs )
{
	return gs->g3Dcamera;
}

FLD_G3D_MAPPER* AriGetFieldG3Dmapper( FIELD_SETUP* gs )
{
	return gs->g3Dmapper;
}

GFL_BBDACT_SYS* AriGetBbdActSys( FIELD_SETUP* gs )
{
	return gs->bbdActSys;
}
...
static BOOL DebugMenuProc( FIELD_WORK *fldWork )
{
	if( fldWork->d_menu_flag == FALSE ){
		if( GFL_UI_KEY_GetTrg() == PAD_BUTTON_SELECT ){
			AriFldMenu_Create( fldWork->d_menu );

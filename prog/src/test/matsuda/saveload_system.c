...
static BOOL PMSVLD_DivSave_Main(s32 lock_id, BOOL lock_flg, BOOL * result);
...
#ifdef PM_DEBUG
int DEBUG_SaveCurrentSideGet(void);
#endif
...
SAVEDATA * SaveData_System_Init(void)
{
	...
	case LOAD_RESULT_NG:
		...
		if(sv_result == LOAD_RESULT_NG){	//OKもNGも両方ここを通るので改めてチェック
		...
		//外部セーブの存在チェック
		{
			...
			if(frontier_result == LOAD_RESULT_BREAK){
				sv->first_status |= FRONTIER_BREAK_BIT;
			}
			else if(frontier_result == LOAD_RESULT_NG){
				sv->first_status |= FRONTIER_NG_BIT;
			}
			if(video_result == LOAD_RESULT_BREAK){
				sv->first_status |= VIDEO_BREAK_BIT;
			}
			else if(video_result == LOAD_RESULT_NG){
				sv->first_status |= VIDEO_NG_BIT;
			}
		}
		break;
...
BOOL SaveData_Load(SAVEDATA * sv)
{
	...
	if (result) {
		sv->data_exists = TRUE;			//データは存在する
		sv->new_data_flag = FALSE;		//新規データではない
		{	//外部セーブのキー情報を更新する為(old一致のケア)、一度外部を読み込む
			LOAD_RESULT frontier_result, video_result;
			ExtraNewCheckLoadData(sv, &frontier_result, &video_result);
		}
		return TRUE;
 ...
 * @brief	外部セーブ初期化状態を取得
 * @param	sv			セーブデータ構造へのポインタ
 * @param	BOOL		TRUE：初期化済み。　FALSE:初期化していない
 */
//---------------------------------------------------------------------------
BOOL SaveData_GetExtraInitFlag(SAVEDATA * sv)
{
	...
	return MISC_GetExtraInitFlag(misc);
}

//---------------------------------------------------------------------------
/**
 * @brief	外部セーブ初期化状態フラグをセットする
 * @param	sv			セーブデータ構造へのポインタ
 */
//---------------------------------------------------------------------------
...
//--------------------------------------------------------------
/**
 * @brief   セーブデータが持っているCRCテーブルでハッシュ値を求める
 *
 * @param   sv			セーブデータへのポインタ
 * @param   start		データへのポインタ
 * @param   size		データサイズ
 *
 * @retval  ハッシュ値
 */
...
{
#if CRC_LOADCHECK
    return CalcTool_CrcCheck(start, size );
#else  //CRC_LOADCHECK
    return MATH_CalcCRC16CCITT(&sv->crc_table, start, size);
#endif
...
{
#if CRC_LOADCHECK
    return CalcTool_CrcCheck(start, size - sizeof(SAVE_FOOTER));
#else  //CRC_LOADCHECK
...
static void ExtraNewCheckLoadData(SAVEDATA * sv, LOAD_RESULT *frontier_result, LOAD_RESULT *video_result)
{
	...
	EX_CERTIFY_SAVE_KEY ex_key, old_key;
	u8 ex_load_flag;
	
	*frontier_result = LOAD_RESULT_OK;
	*video_result = LOAD_RESULT_OK;

	if(SaveData_GetExtraInitFlag(sv) == FALSE){
		return;	//外部セーブがまだ作られていない
	}
	...
	//バトルビデオ外部データチェック
	//※oldと一致した時、svに更新したフラグ情報を持たせる為、
	//　BREAK情報が入っても必ず全ての録画データを一度ロードする
		...
		if(ex_key != EX_CERTIFY_SAVE_KEY_NO_DATA || old_key != EX_CERTIFY_SAVE_KEY_NO_DATA){
			...
			if(result == LOAD_RESULT_NG){
				...
				*video_result = LOAD_RESULT_BREAK;
				...
				//1つ前のデータとキー一致
				if((*video_result) != LOAD_RESULT_BREAK){//破壊情報がセットされているなら上書きしない
...
static BOOL NewSVLD_Load(SAVEDATA * sv)
{
    ...
    for (i = 0; i < GMDATA_ID_MAX; i++) {
        sv->pageinfo[i].crc = CalcTool_CrcCheck(SaveData_Get(sv,i), sv->pageinfo[i].size);
        OS_TPrintf("crc create %d %x\n",i,sv->pageinfo[i].crc);
    }
#endif //CRC_LOADCHECK
...
#if CRC_LOADCHECK
		pageinfo[i].size = SVDT_GetWorkSize(i);
#else
...
//#ifdef	DEBUG_ONLY_FOR_tamada
	...
	OS_TPrintf("savedata_total_size = 0x%x\n", savedata_total_size);
...
{
	const EXSAVEDATA_TABLE * extra_table = ExtraSaveDataTable;
	int i;
	LOAD_RESULT result;
	void *ex_work;
	
	if(SaveData_GetExtraInitFlag(sv) == TRUE){
		return;
	}
	
	for (i = 0; i < ExtraSaveDataTableMax; i++) {
		if(extra_table[i].id == EXDATA_ID_DENDOU){
			continue;	//殿堂入りはDPから既に別個に処理が入っているので飛ばす
		}
		
		OS_TPrintf("%d番の外部セーブ初期化開始...\n", i);
		
		ex_work = SaveData_Extra_LoadAlloc(sv, HEAPID_SAVE_TEMP, extra_table[i].id, &result);
		GF_ASSERT(ex_work != NULL);
		MI_CpuClear8(ex_work, extra_table[i].get_size());
		extra_table[i].init_work(ex_work);
		SaveData_Extra_Save(sv, extra_table[i].id, ex_work);
		sys_FreeMemoryEz(ex_work);
		
		OS_TPrintf("%d番の外部セーブ初期化完了！！！！\n", i);
	}
...
static void MakeExtraCheckData(const SAVEDATA * sv, void * buf, EXDATA_ID id, u32 size)
{
	...
	chk->id = id;
#if CRC_LOADCHECK
    chk->crc = CalcTool_CrcCheck(buf, size + sizeof(CHECK_TAIL_DATA) - 2);
#else //CRC_LOADCHECK
...
#if CRC_LOADCHECK
	if (chk->crc != CalcTool_CrcCheck(buf, size + sizeof(CHECK_TAIL_DATA) - 2)) {
		return FALSE;
	}
#else //CRC_LOADCHECK
...
//---------------------------------------------------------------------------
/**
 * @brief	プラチナ用に追加された外部データのセーブ
 * 			内部セーブと同じように交互ミラーリング処理を行う。
 *
 * @param	sv			セーブデータ構造へのポインタ
 * @param	id			特殊セーブデータ指定ID
 * @param	data		取得したセーブデータを載せたワークへのポインタ
 ...
 * @retval	SAVE_RESULT_NG
 *
 * ここでセーブするデータは必ずSaveData_Extra_Mirror_LoadAlloc経由で取得したデータであること。
 * 本来のデータに加えて整合性チェック用のエリアが追加されているため。
 */
//---------------------------------------------------------------------------
SAVE_RESULT SaveData_Extra_Mirror_Save(SAVEDATA * sv, EXDATA_ID id, void * data)
{
	...
	do{
		new_key = gf_fix_rand(key);
	}while(new_key == EX_CERTIFY_SAVE_KEY_NO_DATA);
	Extra_SaveKeySet(sv, id, new_key, key, flag ^ 1);
	
	*((EX_CERTIFY_SAVE_KEY *)data) = new_key;
...
#ifdef DEBUG_ONLY_FOR_matsuda
	int i;
	int data_size, total = 0;
	
	for(i = 0; i < ExtraSaveDataTableMax; i++){
		data_size = ExtraSaveDataTable[i].get_size();
		total += data_size;
		OS_TPrintf("data_size %d = %d\n", i, data_size);
	}
	OS_TPrintf("total_size = %d\n", total);
...
//---------------------------------------------------------------------------
/**
 * @brief	プラチナ用に追加された外部データのロード
 * 			内部セーブと同じように交互ミラーリング処理を行う。
 * 
 * @param	sv			セーブデータ構造へのポインタ
 * @param	heap_id		ワークを取得するヒープのID
 * @param	id			特殊セーブデータ指定ID
 * @param   result		1つでも読み込めた場合:LOAD_RESULT_OK。　読めなかった場合:LOAD_RESULT_NG
 * @param   old			TRUE:old_keyとのみ一致
 * @return	void *	取得したセーブデータを載せたワークへのポインタ
 */
//---------------------------------------------------------------------------
void * SaveData_Extra_Mirror_LoadAlloc(SAVEDATA *sv, int heap_id, EXDATA_ID id, LOAD_RESULT * result, BOOL *old)
{
	...
	BOOL res1, res2;
	//u32 counter1, counter2;
	EX_CERTIFY_SAVE_KEY esk1, esk2;
	EX_CERTIFY_SAVE_KEY key, old_key;
	u8 flag;
	MISC *misc = SaveData_GetMisc(sv);
	
	GF_ASSERT(id < ExtraSaveDataTableMax);
	GF_ASSERT(id != EXDATA_ID_DENDOU);
	extbl = &ExtraSaveDataTable[id];
	GF_ASSERT(extbl->id == id);
	data_size = extbl->get_size() + sizeof(CHECK_TAIL_DATA);
	buf = sys_AllocMemory(heap_id, data_size);
	...
	PMSVLD_Load((FIRST_MIRROR_START + extbl->sector) * SECTOR_SIZE, buf, data_size);
	res1 = IsCorrectExtraCheckData(sv, buf, id, extbl->get_size());
	MI_CpuCopy8(buf, &esk1, sizeof(EX_CERTIFY_SAVE_KEY));
	//counter1 = GetCounterFromExtraData(buf, extbl->get_size());

	PMSVLD_Load((SECOND_MIRROR_START + extbl->sector) * SECTOR_SIZE, buf, data_size);
	res2 = IsCorrectExtraCheckData(sv, buf, id ,extbl->get_size());
	MI_CpuCopy8(buf, &esk2, sizeof(EX_CERTIFY_SAVE_KEY));
	//counter2 = GetCounterFromExtraData(buf, extbl->get_size());
	...
	*old = FALSE;
	...
	if (res1 == TRUE && res2 == FALSE) {
		if(key == esk1){
			if(flag == 1){
				Extra_SaveKeySet(sv, id, old_key, old_key, 0);
				*old = TRUE;
			}
			PMSVLD_Load((FIRST_MIRROR_START + extbl->sector) * SECTOR_SIZE, buf, data_size);
			return buf;
	...
	if (res1 == FALSE && res2 == TRUE) {
		if(key == esk2){
			if(flag == 0){
				Extra_SaveKeySet(sv, id, old_key, old_key, 1);
				*old = TRUE;
			}
			PMSVLD_Load((SECOND_MIRROR_START + extbl->sector) * SECTOR_SIZE, buf, data_size);
			return buf;
	...
	if (res1 == TRUE && res2 == TRUE) {
		if(flag == 0){
			if(key == esk1){
				//正常一致
				//OS_TPrintf("flag0で正常一致\n");
				PMSVLD_Load((FIRST_MIRROR_START + extbl->sector) * SECTOR_SIZE, buf, data_size);
				return buf;
			}
			else if(old_key == esk2){
				//1つ前のデータと一致
				//OS_TPrintf("flag0でerror一致\n");
			...
			if(key == esk2){
				//正常一致
				//OS_TPrintf("flag1で正常一致\n");
				PMSVLD_Load((SECOND_MIRROR_START + extbl->sector) * SECTOR_SIZE, buf, data_size);
				return buf;
			}
			else if(old_key == esk1){
				//1つ前のデータと一致
				//OS_TPrintf("flag1でerror一致\n");
				Extra_SaveKeySet(sv, id, old_key, old_key, flag ^ 1);
				*old = TRUE;
				PMSVLD_Load((FIRST_MIRROR_START + extbl->sector) * SECTOR_SIZE, buf, data_size);
				return buf;
...
static void Extra_SaveKeyGet(SAVEDATA *sv, EXDATA_ID exdata_id, EX_CERTIFY_SAVE_KEY *key, EX_CERTIFY_SAVE_KEY *old_key, u8 *flag)
{
	MISC_ExtraSaveKeyGet(SaveData_GetMisc(sv), exdata_id, key, old_key, flag);
}
...
static void Extra_SaveKeySet(SAVEDATA *sv, EXDATA_ID exdata_id, EX_CERTIFY_SAVE_KEY key, EX_CERTIFY_SAVE_KEY old_key, u8 flag)
{
	MISC_ExtraSaveKeySet(SaveData_GetMisc(sv), exdata_id, key, old_key, flag);
}
...
BOOL PMSVLD_Save(u32 src, void * dst, u32 len)
{
	...
	while (PMSVLD_DivSave_Main(lock_id, TRUE, &result) == FALSE) {
		/* Do Nothing */
	}
	return result;
#if 0
...
static BOOL PMSVLD_DivSave_Main(s32 lock_id, BOOL lock_flg, BOOL * result)
{
	if (SaveEndFlag == TRUE) {
        ...
        if(lock_flg==FALSE){
            return TRUE; // すでにキャンセル済み なのでTRUEに変更 08/06/25 k.ohno PL:0646
        }
        CARD_UnlockBackup(lock_id);
        OS_ReleaseLockID(lock_id);
...
/**
 * @brief	セーブデータのCRC検査
 * @param	SAVEDATA *sv		    セーブデータポインタ
 * @param	GMDATA_ID gmdataID		セーブのtypeID
 * @return  CRC合格時にはTRUE
 */
//---------------------------------------------------------------------------

BOOL SVLD_CheckCrc(GMDATA_ID gmdataID)
{
    SAVEDATA * pSV = SaveData_GetPointer();
    void* pData = SaveData_Get(pSV, gmdataID);
    int size = SVDT_GetWorkSize(gmdataID) - 4;
    int sizeHarf = size / 2;
    u16* pShortData = pData;
    u16 chk;
    u32 address;

    chk =CalcTool_CrcCheck(pData, size);
    if(pShortData[sizeHarf] == chk){
        return TRUE;
    }
    address  = (u32)&pShortData[sizeHarf];
    OS_TPrintf("CRCL %x %x %x %x %x\n", gmdataID, chk,pShortData[sizeHarf], address,size);
    GF_ASSERT(0);
    return FALSE;
}
...
/**
 * @brief	crcを再計算して埋め込む
 * @param	const void* pData	    セーブデータポインタ
 * @param	GMDATA_ID gmdataID		セーブのtypeID
 */
//---------------------------------------------------------------------------

void SVLD_SetCrc(GMDATA_ID gmdataID)
{
    SAVEDATA * pSV = SaveData_GetPointer();
    void* pData = SaveData_Get(pSV, gmdataID);
    u16* pShortData = pData;
    ...
    size = SVDT_GetWorkSize(gmdataID) - 4;
    sizeHarf = size / 2;
    pShortData = pData;
    crcdata = CalcTool_CrcCheck(pData, size);
    pShortData[sizeHarf] = crcdata;
    address  = (u32)&pShortData[sizeHarf];
//    OS_TPrintf("CRCS %x %x %x %x\n", gmdataID, crcdata, address,size);
}
...
int DEBUG_SaveCurrentSideGet(void)
{
	return (SaveData_GetPointer())->current_side[0];
}
#endif	//PM_DEBUG

...
#define	ID_TOOIBASYO			(3002)		//とおいばしょのIDナンバー（なぞの場所対応用）
...
typedef struct{
	SOFT_SPRITE_ANIME_DP	poke_f;
	SOFT_SPRITE_ANIME_DP	poke_b[3];
...
struct TPoke_Anime_Data {
	...
	u8					voice_wait;					///< 鳴き声開始のウェイト
	u8					prg_anm;					///< プログラムアニメ
	u8					prg_anm_wait;				///< プログラムアニメ開始ウェイト
	SOFT_SPRITE_ANIME	ssanm[ SS_ANM_SEQ_MAX ];	///< パターンアニメデータ
	
};// POKE_ANIME_DATA;

typedef struct TPoke_Anime_Data POKE_ANIME_DATA;
...
typedef struct {
	...
	POKE_ANIME_DATA		poke_anime[ 2 ];			///< ポケモンアニメデータ 0 = 正面 1 = 背面
	
	s8					height;						///< 高さ情報
	s8					shadow_ofx;					///< 影のオフセット
	u8					shadow_size;				///< 影のサイズ

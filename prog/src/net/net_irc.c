...
static void _IRC_SendRapper(u8* buf, u8 size, u8 command, u8 value)
{
  ...
  GF_ASSERT(size < IRC_TRNS_MAX-4);

  buff[0] = command;
  buff[1] = value;
  if(buf){
    GFL_STD_MemCopy(buf, &buff[4], IRC_TRNS_MAX-4);
  }
  IRC_Send(buff, size+4,  command);
...
#define _IRC_SEND_SIZE (IRC_TRNS_MAX - 2)  //４バイト減らしてある
...
typedef struct{
  ...
  u8 friendmac[6];     ///< 相手のユニークID(MACADD)
  u32 timeout;		///<再接続時のタイムアウト時間
  u32 retry_time;		///<再接続中のタイムカウンタ
...
static void _IRC_SendRapper(u8* buf, u8 size, u8 command, u8 value)
{
  ...
  IRC_Send((const u8*)&NetIrcSys.aSendBuff, size+8,  command);
}
...
static void IRC_ReceiveCallback(u8 *data, u8 size, u8 command, u8 id)
{
  ...
  send_id = IRC_TargetIDGet();
  if(NetIrcSys.recieve_func != NULL){
    if(NetIrcSys.recieve_func(send_id, (u16*)pData->buff, size-8) == FALSE){
      IRC_PRINT("こわれた\n");
      return; //データが壊れているなら受け取らない
    }
  }
...
#if 1
  if(GFL_STD_MemComp(NetIrcSys.friendmac, nullmac, 6) == 0){
    GFL_STD_MemCopy(pData->mac ,NetIrcSys.friendmac, 6); //最初ユニークIDを入れたものだけとつながれる
  }
  else if(GFL_STD_MemComp(NetIrcSys.friendmac, pData->mac, 6) != 0){
    IRC_PRINT("ユニークIDがちがう\n");

...
typedef struct
{
  ...
  Callback_DisconnectErrorEx* pDisconnectErrorCallbackEx;  //切断を待ちたい場合つかう
...
//    NHTTP_RAP_DisconnectCallbackCall(errorCode, myErrorType, ret);
  ...
  if(_dWork->bHeapError){
    //そのままreturnNoをいれてしまうと、
    //ERRORCODE_HEAPやTIMEOUT以外のモノが入ってしまうので
    //検知したらいれる

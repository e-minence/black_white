...
void  MUS_MCSS_Draw( MUS_MCSS_SYS_WORK *mcss_sys , MusicalCellCallBack musCellCb )
{
  ...
  u8              musCellArr;
  u8              flipFlg;
  VecFx32         itemOfsMc;  //アイテム差分マルチセル用
        ...
        for( musCellArr=0;musCellArr<MUS_CELL_ARR_NUM;musCellArr++ )
        {
          if( ncec->musCellIdx[musCellArr] != 0xFF )
          {
            ...
            //const fx32 itemOfsy = mcss->musInfo[clIdx].ofsX*FX_SinIdx(rotZ) + mcss->musInfo[clIdx].ofsY*FX_CosIdx(rotZ);
            {
              ...
              const fx32 scaleY = FX_Mul( anim_SRT_mc.sy , anim_SRT.sy );
              const fx32 ofsXS = FX_Mul( ofsXR , scaleX);
              const fx32 ofsYS = FX_Mul( ofsYR , scaleY);
              ofsXF = ofsXS;
              ofsYF = ofsYS;
            }
            ...
            cellData.ofs.y  = FX_Mul( ofsYF , mcss->scale.y/16 );
            //cellData.ofs.x  = FX_Mul( FX32_CONST(ofsx + anim_SRT.px ) + itemOfsx + itemOfsMc.x , mcss->scale.x/16 );
            //cellData.ofs.y  = FX_Mul( FX32_CONST(ofsy + anim_SRT.py ) + itemOfsy + itemOfsMc.y , mcss->scale.y/16 );
...
//            cellData.rotOfs.z = 0;
            ...
            cellData.itemRotZ = rotZ + MUS_POKE_GRP_TO_ROT( mcss->musInfo[clIdx].grpNo );

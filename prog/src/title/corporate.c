...
{
  CORP_WORK  *cw    = mywk;
  CORP_PARAM *param = pwk;

  enum{
    SEQ_SETUP = 0,
    SEQ_CORP1_FADEIN,
    SEQ_CORP1_FADEIN_WAIT,
    SEQ_CORP1_MAIN,
    SEQ_CORP1_FADEOUT,
    SEQ_CORP1_FADEOUT_WAIT,
    SEQ_CORP1_WAIT,
    SEQ_SETUP2,
    SEQ_FADEIN,
    SEQ_MAIN,
  ...
  case SEQ_SETUP:
    ...
    // 2回目移行のデモはNINTENDO/POKEMONCORPを出さない
    if(param->mode==ONLY_DEVELOP_VIEW){
    ...
    case SEQ_CORP1_FADEIN:
      ...
      GFL_DISP_SetDispOn();
      {
        int mode = GFL_FADE_MASTER_BRIGHT_WHITEOUT;

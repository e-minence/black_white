...
static BOOL _sendParentNegotiation(GFL_NETHANDLE* pNetHandle)
{
    ...
    for(j = 0;j < GFL_NET_MACHINE_MAX; j++){
        NEGOTIATION_RETURN_DATA* send_data = pNetHandle->negoBuff[j];
        if(send_data->bInit){
...
{
    // 親機が受け取ってフラグを立てる
    const NEGOTIATION_REQ_DATA *recv_data = pData;
    GFLNetInitializeStruct* pNetIni = _GFL_NET_GetNETInitStruct();
    NEGOTIATION_RETURN_DATA* send_data;

    
    if(GFL_NET_GetNetHandle(GFL_NET_NETID_SERVER) == pNetHandle){
        send_data=pNetHandle->negoBuff[netID];
        if(!send_data->bInit){
...
{
    const u8* pMsg = pData;
	const NEGOTIATION_RETURN_DATA *recv_data = pData;

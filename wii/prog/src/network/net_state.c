...
static void _stateChildNegotiationWait(GFL_NETSTATE* pState);
...
static void _deviceInitialize_wireless(void)
{
...
static void _deviceInitialize(GFL_NETSTATE* pState)
{
    ...
    if(pNetInit->bNetType == GFL_NET_TYPE_WIRELESS
    		|| pNetInit->bNetType == GFL_NET_TYPE_IRC_WIRELESS){
		_deviceInitialize_wireless();
    }
    if(pNetInit->bNetType == GFL_NET_TYPE_IRC || pNetInit->bNetType == GFL_NET_TYPE_IRC_WIRELESS){
		GFL_NET_IRC_InitializeFlagSet();
	}
    GFL_NET_SystemReset();         // 今までの通信バッファをクリーンにする
    if(pState->pNetStepEndCallback){
        pState->pNetStepEndCallback(pNet->pWork);
        pState->pNetStepEndCallback=NULL;
    }
    
    if(pNetInit->bNetType != GFL_NET_TYPE_IRC && pNetInit->bNetType != GFL_NET_TYPE_IRC_WIRELESS){
	    GFL_NET_WirelessIconEasyXY(pNetInit->iconX,pNetInit->iconY,pNetInit->bNetType, pNetInit->netHeapID);
	}
...
{
    if(GFI_NET_SystemParentModeInitProcess(TRUE)){
...
static void _changeoverChildRestart(GFL_NETSTATE* pState)
{
    ...
    if(pState->bAutoConnect){
        ...
        if(GFL_NET_SystemChildModeInit(TRUE, _PACKETSIZE_DEFAULT)){
            rand = GFL_STD_Rand(&pState->sRand, (_CHILD_P_SEARCH_TIME));
            NET_PRINT("子機開始 %d \n",rand);
            _CHANGE_STATE(_changeoverChildSearching, rand);
        }
...
static void _changeoverParentStart(GFL_NETSTATE* pState)
{
    if(WH_GetSystemState() == WH_SYSSTATE_MEASURECHANNEL){
        if(GFI_NET_SystemParentModeInitProcess(FALSE)){
...
static void _stateChildNegotiation(GFL_NETSTATE* pState)
{
    ...
    if(GFL_NET_HANDLE_RequestNegotiation()){
        _CHANGE_STATE(_stateChildNegotiationWait, 0);
    }
}
...
//--------------------------------------------------------------
/**
 * @brief   赤外線でネゴシエーション完了後、ワイヤレス通信に移行
 *
 * @param   pState		
 */
//--------------------------------------------------------------
static void _stateIRC_Wireless_Change(GFL_NETSTATE* pState)
{
    GFLNetInitializeStruct* pNetInit = _GFL_NET_GetNETInitStruct();
    GFL_NETSYS* pNet = _GFL_NET_GetNETSYS();
	GFL_NETHANDLE* pNetHandleParent = GFL_NET_GetNetHandle(GFL_NET_NETID_SERVER);
	GFL_NETHANDLE* pNetHandle = GFL_NET_HANDLE_GetCurrentHandle();
	int sys_id;
	
	if(GFL_NET_HANDLE_GetNumNegotiation() >= 2){	//赤外線通信なので2固定
		sys_id = GFL_NET_SystemGetCurrentID();	//赤外線を切断する前にID取得
...
//==============================================================================
/**
 * @brief   ネゴシエーション接続待ち：赤外線
 * @param   none
 * @retval  none
 */
//==============================================================================

static void _stateNegotiationIRC(GFL_NETSTATE* pState)
{
    GFLNetInitializeStruct* pNetIni = _GFL_NET_GetNETInitStruct();
    GFL_NETSYS* pNet = _GFL_NET_GetNETSYS();

    if(GFL_NET_HANDLE_RequestNegotiation()){
		//if(pNetIni->bNetType == GFL_NET_TYPE_IRC_WIRELESS){
		if(GFL_NET_IW_SystemCheck() == TRUE){
			_CHANGE_STATE(_stateIRC_Wireless_Change, 0);
		}
		else{
	        if(pState->pNetStepEndCallback){
	            pState->pNetStepEndCallback(pNet->pWork);
...
static void _changeoverConnectIRC(GFL_NETSTATE* pState)
{
    int realParent;

	if(GFL_NET_IRC_IsConnectSystem() == TRUE){
		GFL_NET_IRC_FirstConnect();
        _CHANGE_STATE(_stateNegotiationIRC, 0);
	}
	
#if 0	//赤外線にタイマーはいらない気がする
    if(pState->timer != 0){
        NET_PRINT("time %d\n",pState->timer);
        pState->timer--;
        return;
...
void GFL_NET_StateChangeover(HEAPID heapID, NetStepEndCallback callback, BOOL bAuto)
{
    GFLNetInitializeStruct* pNetInit = _GFL_NET_GetNETInitStruct();

    _pNetState->bAutoConnect = bAuto;
	if(pNetInit->bNetType == GFL_NET_TYPE_IRC || pNetInit->bNetType == GFL_NET_TYPE_IRC_WIRELESS){
	    IRC_Connect();
		GFL_NET_SystemIrcModeInit(FALSE);
	    _pNetState->pNetStepEndCallback = callback;
	    _CHANGE_STATE(_changeoverConnectIRC, 30);
	}
    else{
        if(_pNetState->bAutoConnect){
            GFL_NET_SystemChildModeInitAndConnect(TRUE, NULL,_PACKETSIZE_DEFAULT,_parentFindCallback);
        }
        else{
            GFL_NET_SystemChildModeInit(TRUE, _PACKETSIZE_DEFAULT);
        }
	    _pNetState->pNetStepEndCallback = callback;
...
static void _stateEndParentWait(GFL_NETSTATE* pState)
{
	...
	if(pNetInit->bNetType == GFL_NET_TYPE_IRC){
		if(GFL_NET_IRC_IsConnectSystem() == FALSE){
			GFL_NET_Exit(NULL);
		}
	}
	else{
...
void GFL_NET_StateRecvExit(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
    ...
    GFL_NET_SendData(pNetHandle, GFL_NET_CMD_EXIT, 0, NULL);
...
//        _stateFinalize(pState);
        ...
        if(pNetIni->bNetType == GFL_NET_TYPE_WIRELESS){
	        WH_FreeMemory();
	    }
        _stateFinalize(pState);
        _CHANGE_STATE(_stateNone, 0);
...
void GFL_NET_StateSendWifiMatchEnd(GFL_NETSTATE* pState)
{
    ...
    GFL_NET_SendData(pNetHandle, GFL_NET_CMD_WIFI_EXIT,1,&id);
}

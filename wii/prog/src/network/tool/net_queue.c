...
{
    SEND_QUEUE* pFree = (SEND_QUEUE*)pQueueMgr->heapTop;
    int i;
...
{
    // 2byte コマンド番号
    _setSendData(pSendBuff,(u8)((pQueue->command>>8) & 0xff));
    _setSendData(pSendBuff,(u8)(pQueue->command & 0xff));
    // 2byteサイズ送信
...
static BOOL _dataCopyQueue(SEND_QUEUE* pQueue, SEND_BUFF_DATA* pSendBuff,
                           RingBuffWork* pSendRing,BOOL bQueueNext)
{

    if(pQueue->bNext == FALSE){
        if(bQueueNext){
    ...
    if(pSendBuff->size < pQueue->size){  // キューのほうがでかい場合
        if(pQueue->pQData==NULL){
            GFL_NET_RingGets(pSendRing, pSendBuff->pData, pSendBuff->size, pSendBuff->size);
        }
        else{
        ...
        pSendBuff->size = _DATA_QUEUE_MAX;  // 残りを-1にする
        pQueue->bNext = TRUE;
        return TRUE;
    }
    // 同じもしくは小さい場合
    if(pQueue->pQData==NULL){
        GFL_NET_RingGets(pSendRing, pSendBuff->pData, pQueue->size,pQueue->size);
    }
    else{
...
BOOL GFL_NET_QueueGetData(SEND_QUEUE_MANAGER* pQueueMgr, SEND_BUFF_DATA *pSendBuff)
{
    ...
    while(pSendBuff->size > 0){
        ...
        // ヘッダー送信できない
        if(!_dataCopyQueue(pQueue, pSendBuff, pQueueMgr->pSendRing,bQueueNext)){
            pQueueMgr->pNow = pQueue;  // 次の最初で送信できるようにセット
            break;
        }
        bQueueNext=TRUE;
        if( _DATA_QUEUE_MAX == pSendBuff->size ){  // データが詰まっている場合
            pQueueMgr->pNow = pQueue;  // 次の最初で送信できるようにセット
            return FALSE;
        }
        else{
...
{
    int i;
    SEND_QUEUE* pNext = (SEND_QUEUE*)pQueueMgr->heapTop;

...
GFL_NETHANDLE* GFL_NET_InitHandle(int netID)
{
	...
	pHandleServer = GFL_NET_GetNetHandle(GFL_NET_NETID_SERVER);
	...
	{
		if(pHandleServer->negoBuff[netID]!=NULL)
		{
			GFL_HEAP_FreeMemory(pHandleServer->negoBuff[netID]);
		}
	}
	...
	{
		pHandleServer->negoBuff[netID] = GFL_HEAP_AllocClearMemory(pNetIni->netHeapID ,sizeof(NEGOTIATION_RETURN_DATA));
	}
...
void GFL_NET_HANDLE_RecvNegotiation(const int netID, const int size, const void* pData, void* pWork, GFL_NETHANDLE* pNetHandle)
{
	...
	if(GFL_NET_GetNetHandle(GFL_NET_NETID_SERVER) == pNetHandle){
		...
		if(!send_data->bInit){
			...
			//NETMemCpy((void*)&recv_data->MacAddress, (void*)&send_data->child_MacAddress, sizeof(recv_data->MacAddress));
			GFL_STD_MemCopy((void*)recv_data->MacAddress, (void*)send_data->child_MacAddress, sizeof(recv_data->MacAddress));

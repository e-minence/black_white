...
static void SendAddrCheckPackets(NATNegotiator neg)
{
	...
	memcpy(p->magic, NNMagicData, NATNEG_MAGIC_LEN);
	p->version = NN_PROTVER;
	p->packettype = NN_ADDR;
	p->cookie = (int)htonl((unsigned int)neg->cookie);
	p->porttype = neg->clientindex;
	p->clientindex = 0;
	p->usegameport = 0;
	p->localip = 0;
	p->localport = 0;
	packetlen = (INITPACKET_SIZE);

	gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
        "[%04x] Sending ADDRCHK (NN%d)...\n", neg->cookie, neg->clientindex);

	switch(neg->clientindex) {
		case 1:
			SendPacket(neg->negotiateSock, matchup2ip, MATCHUP_PORT, p, packetlen);
			break;
		default:
...
static void SendPingPacket(NATNegotiator neg)
{
	...
	// [nakata] cookie表示
	gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
		"[%04x] Sending PING to %s:%d (got remote data: %d)\n", neg->cookie, inet_ntoa(*(struct in_addr *)&neg->guessedIP), neg->guessedPort,   neg->gotRemoteData);
...
NegotiateError NNBeginNegotiationWithSocket(SOCKET gamesocket, int cookie, int clientindex, NegotiateProgressFunc progresscallback, NegotiateCompletedFunc completedcallback, void *userdata)
{
...
#if defined(GSI_COMMON_DEBUG)
	{
		...
		// [nakata] cookie表示
		gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
			"[%04x] Negotiate Socket: %d\n", neg->cookie, ntohs(saddr.sin_port));
	}
#endif
...
NegotiateError NNBeginAddressCheck(int cookie, int serverindex, NegotiateCompletedFunc completedcallback, void *userdata)
{
	NATNegotiator neg;
	...
	if (neg == NULL)
		return ne_allocerror;
	neg->gameSock = INVALID_SOCKET;
	neg->clientindex = serverindex; // [nakata] use as temporary value
	neg->cookie = cookie;
	neg->progressCallback = NULL;
	neg->completedCallback = completedcallback;
	neg->userdata = userdata;
	neg->negotiateSock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	neg->retryCount = 0;
...
static void NegotiateThink(NATNegotiator neg)
{
	...
	if (neg->state == ns_initsent || neg->state == ns_connectping) //see if we need to resend init packets
	{
		if (current_time() > neg->retryTime)
		{
			if (neg->retryCount > neg->maxRetryCount)
			{
				...
				neg->completedCallback(nr_inittimeout, INVALID_SOCKET, NULL, neg->userdata, neg->cookie);
	...
	if (neg->state == ns_finished && current_time() > neg->retryTime) //check if it is ready to be removed
	{
		if (neg->gameSock == INVALID_SOCKET) //we are using the negotiate socket, time to trigger the callback
		{
			...
			neg->completedCallback(nr_success, neg->negotiateSock, &saddr, neg->userdata, neg->cookie);
	...
	if (neg->state == ns_initack && current_time() > neg->retryTime) //see if the partner has timed out
	{
		...
		neg->completedCallback(nr_deadbeatpartner, INVALID_SOCKET, NULL, neg->userdata, neg->cookie);
		NNCancel(neg->cookie); //mark to-be-canceled
	}
...
static void ProcessConnectPacket(NATNegotiator neg, ConnectPacket *p, struct sockaddr_in *fromaddr)
{
	...
	if (p->finished != FINISHED_NOERROR) //call the completed callback with the error code
	{
		...
		neg->completedCallback(errcode, INVALID_SOCKET, NULL, neg->userdata, neg->cookie);
		NNCancel(neg->cookie); //mark to-be-canceled
		return;
	}
	...
	else //they have our data, and we have their data - it's a connection!
	{
		if (neg->state == ns_connectping) //advance it
		{
			...
			if (neg->gameSock != INVALID_SOCKET) //we can trigger the callback right now
				...
				neg->completedCallback(nr_success, neg->gameSock, fromaddr, neg->userdata, neg->cookie);
...
static void ProcessInitPacket(NATNegotiator neg, InitPacket *p, struct sockaddr_in *fromaddr)
{
	...
	case NN_INITACK:
		...
		// [nakata] cookie表示
		gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
			"[%04x] Got init ack for port %d\n", neg->cookie, p->porttype);
...
static void ProcessAddrPacket(NATNegotiator neg, InitPacket *p, struct sockaddr_in *fromaddr)
{
	neg->state = ns_finished;			
	neg->retryTime = current_time() + FINISHED_IDLE_TIME;

	fromaddr->sin_port = p->localport;
	fromaddr->sin_addr.s_addr = p->localip;
...
void NNProcessData(char *data, int len, struct sockaddr_in *fromaddr)
{
	...
	{ //it's a connect packet
		...
		neg = FindNegotiatorForCookie((int)ntohl((unsigned int)cp.cookie));
		if (neg)
		{
			if (ptype == NN_CONNECT)
	...
	} else if( ptype == NN_ADDRACK)
	{
		...
		if (neg)
		{
			ProcessAddrPacket(neg, &ip, fromaddr);
		}
	...
	} else //it's an init packet
	{
		...
		neg = FindNegotiatorForCookie((int)ntohl((unsigned int)ip.cookie));
		if (neg)

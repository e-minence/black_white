...
static void piListGroupRoomsCall(PEER peer, piCallbackData * data)
{
...
#else
	{
		...
		((peerListGroupRoomsCallback)data->callback)(peer, data->success, params->groupID, params->server, name_W, params->numWaiting, params->maxWaiting, params->numGames, params->numPlaying, data->callbackParam);
		gsifree(name_W);
	}
#endif
}
...
static void piListingGamesCall(PEER peer, piCallbackData * data)
{
...
#else
	{
		...
		if (params->name != NULL)
			name_W = UTF8ToUCS2StringAlloc(params->name);
		((peerListingGamesCallback)data->callback)(peer, data->success, name_W, params->server, params->staging, params->msg, params->progress, data->callbackParam);
		gsifree(name_W);
	}
#endif
}
...
static void piNickErrorCall(PEER peer, piCallbackData * data)
{
	...
	params = data->params;
#ifndef GSI_UNICODE
	((peerNickErrorCallback)data->callback)(peer, params->type, params->nick, params->numSuggestedNicks, (const char **)params->suggestedNicks, data->callbackParam);
#else
	{

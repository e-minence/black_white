...
// Encrypt some data
//    -  theEncryptedLength becomes the length of data written to theEncryptedBuffer
												 ...
												 int *        theDecryptedLength)
{
	...
	while(decryptMore != GHTTPFalse && ((inBuf.end-inBuf.start) > 0))
	{
		...
		case SSL_SEND_RESPONSE:
			...
			// The response is already encrypted, so temporarily remove the buffer encryption
			connection->sendBuffer.encrypted = GHTTPFalse;

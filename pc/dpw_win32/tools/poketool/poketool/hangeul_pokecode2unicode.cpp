...
#define HANGEUL_POKECODE_OFFSET 0x401

static const wchar_t str2uni_table[] = {
	// ハングル文字
...
// ハングルのポケコードをUnicodeに変換する。失敗したら0xffffを返す
wchar_t hangeulPokecode2Unicode(u16 pokecode)
{
    if(pokecode >= POKECODE_HANGEUL_BEGIN && pokecode <= POKECODE_HANGEUL_END){
        return str2uni_table[pokecode - HANGEUL_POKECODE_OFFSET];
    }else{
        return 0xffff;
    }
}

// ハングルのUnicodeをポケコードに変換する。失敗したら0xffffを返す。
u16 hangeulUnicode2Pokecode(wchar_t unicode)
{
    int i;
    for(i=0; i<sizeof(str2uni_table)/sizeof(str2uni_table[0]); i++){
        if(str2uni_table[i] == unicode){
            return i + HANGEUL_POKECODE_OFFSET;
        }
    }
    return 0xffff;
}
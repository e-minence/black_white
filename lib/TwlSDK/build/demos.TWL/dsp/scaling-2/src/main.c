/*---------------------------------------------------------------------------*
  Project:  TwlSDK - DSP - demos - scaling-2
  File:     main.c
...
/*---------------------------------------------------------------------------
 640x480 の画像データを、指定した領域サイズになるよう拡大（縮小）を行うデモです。
 ...
 ・その他
     - 拡大（縮小）後のサイズを変更したい場合は、OUTPUT_WIDTH, OUTPUT_HEIGHT を
       変更して再ビルドして下さい。
...
/* 設定 */
#define RUN_ASYNC     1            // 拡縮処理を非同期で実行するかどうか
#define USE_FX32      1            // DSP_Scaling の fx32 版を使用するなら 1
...
#define OUTPUT_WIDTH  HW_LCD_WIDTH  // 処理後の解像度
#define OUTPUT_HEIGHT HW_LCD_HEIGHT
...
{
    // 希望する出力サイズから、DSP_Scaling に入力する倍率を取得する
#if USE_FX32
    fx32 factx, facty;
    factx = DSP_CalcScalingFactorFx32(AREA_WIDTH, OUTPUT_WIDTH);
    facty = DSP_CalcScalingFactorFx32(AREA_HEIGHT, OUTPUT_HEIGHT);
#else
    f32  factx, facty;
    factx = DSP_CalcScalingFactorF32(AREA_WIDTH, OUTPUT_WIDTH);
    facty = DSP_CalcScalingFactorF32(AREA_HEIGHT, OUTPUT_HEIGHT);
#endif
...
#if USE_FX32
        (void)DSP_ScalingFxAsyncEx(_binary_output_dat, TmpBuf, DATA_WIDTH, DATA_HEIGHT,
                           factx, facty, ModeNames[DspMode], AreaX, AreaY, AREA_WIDTH, AREA_HEIGHT, ScalingCallbackFunc);
#else
        (void)DSP_ScalingAsyncEx(_binary_output_dat, TmpBuf, DATA_WIDTH, DATA_HEIGHT,
                           factx, facty, ModeNames[DspMode], AreaX, AreaY, AREA_WIDTH, AREA_HEIGHT, ScalingCallbackFunc);
#endif	// USE_FX32
...
#if USE_FX32
    DSP_ScalingFxEx(_binary_output_dat, TmpBuf, DATA_WIDTH, DATA_HEIGHT,
                  factx, facty, ModeNames[DspMode], AreaX, AreaY, AREA_WIDTH, AREA_HEIGHT);
#else

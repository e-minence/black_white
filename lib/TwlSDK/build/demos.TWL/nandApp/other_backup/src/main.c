...
void TwlMain(void)
{
    ...
    // NAND アプリでは他のNANDアプリのセーブデータに 
    // アクセスすることができます。
    //  NA_LoadOtherTitleArchiveを使うと、他のNANDアプリのセーブデータを
    // マウントすることができ、"otherPub:", "otherPrv:"のアーカイブ名で
    // アクセスすることができるようになります。
    // ただし、一度に1つの領域しかマウントできません。
    //
    // 他の領域をマウントする場合は、NA_UnloadOtherTitleArchiveを呼び出して、
    // 現在のセーブデータをアンマウントする必要があります。
    // 
    // 本プログラムでは、backupデモのバックアップ領域にアクセスします。 
	//
    // 1.ファイルの読み込み 
    // 2.バックアップデータの消去 
    // 3.ディレクトリツリー・ファイルの作成 
...
/*---------------------------------------------------------------------------*
  ...
  Description:  他のタイトルのセーブデータをマウントします。 
  
  Arguments:    code:   対象アプリのイニシャルコード。
                flag:   trueならPublicセーブデータを、
                        falseならPrivateセーブデータをマウント。
  Returns:      なし。
 *---------------------------------------------------------------------------*/
static void MountOtherSaveData(const char* code, BOOL flag)
{

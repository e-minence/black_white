...
static BOOL SampleMain(void)
{
    ...
    {
        OS_TPrintf("AES_Rand failed(%d)", aesResult);
        goto error_exit;
    }
    ...
    if( aesResult != AES_RESULT_SUCCESS )
    {
        OS_TPrintf("AES_SetKey failed(%d)", aesResult);
        goto error_exit;
    }
    ...
    // 暗号化
    {
        ...
        if( aesResult != AES_RESULT_SUCCESS )
        {
            OS_TPrintf("AES_CtrEncrypt failed(%d)", aesResult);
            goto error_exit;
        }
    ...
    // 復号化
    {
        ...
        aesResult = AES_CtrDecrypt( &counter,           // カウンタ初期値（暗号化時と同じでなければなりません）
                                    pEncrypted,         // 復号化するデータ
        ...
        if( aesResult != AES_RESULT_SUCCESS )
        {
            OS_TPrintf("AES_CtrDecrypt failed(%d)", aesResult);
            goto error_exit;
        }
    ...
    if( MI_CpuComp8(pDecrypted, pPlainText, srcSize) == 0 )
    {

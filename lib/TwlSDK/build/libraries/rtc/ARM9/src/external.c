...
/*---------------------------------------------------------------------------*
  ...
  Description:  RTCライブラリを初期化する。
       Notice:  コンポーネント側の初期化時に、バッテリー抜け等により RTC デバイス
                への電源供給が完全に途絶えていたことが検出された場合には、デバイス
                の内部状態はリセットされる。この時、日付は 2000/01/01 (土曜日) 、
                時刻は 00:00:00 (24時間表記) に初期化される。
                RTC アラームに関する設定値はバッテリー抜け如何に関らず本体の起動
                シーケンス内で 0 クリアされるが、ソフトウェアリセットによるアプリ
                ケーションの再起動時には設定が持ち越される。
...
/*---------------------------------------------------------------------------*
  ...
  Description:  ライブラリ内共有ワーク変数へのポインタを取得する。

  Arguments:    None.

  Returns:      RTCWork*    -   ライブラリ内共有ワーク変数へのポインタを返す。
 *---------------------------------------------------------------------------*/
RTCWork*
...
static BOOL RtcCheckDate(const RTCDate *date, RTCRawDate *raw)
{
    ...
    *(u32 *)raw = RtcHEX2BCD(date->year);
    ...
    raw->week = (u32)RTC_GetDayOfWeek((RTCDate*)date);    // 曜日については、入力データの曜日を使用せず常に再計算する
    return TRUE;
}

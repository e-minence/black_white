...
#define ERROR_TLF_FILE_SPECIFIED                13
#define ERROR_NOT_NITRO_SRL				        14
#define ERROR_ILLEGAL_NLF_FILE					15

//---- version
#define VERSION_STRING     " 1.4  Copyright 2005-2008 Nintendo. All right reserved."
...
// ver 1.4  check if rom is for NITRO
...
{
    char   *usageString[] = {
        "Usage: buryarg [OPTION]... <NITRO srl filename> [argument]...\n",
        "Bury argument strings to NITRO srl file.",
        "\n",
        "Options:\n",
...
void displayError(int errorNumber)
{
    ...
    case ERROR_TLF_FILE_SPECIFIED:
        fprintf(stderr, "cannot access to tlf file.\n");
        break;

    case ERROR_NOT_NITRO_SRL:
        fprintf(stderr, "ROM is not for NITRO.\n");
        break;
...
/*---------------------------------------------------------------------------*
  Name:         checkNitroHeader

  Description:  check if the specified ROM is for NITRO
...
void checkNitroHeader(void)
{
	...
	size_t  size = fread(romHeader, 1, readSize, gNlfFile);

	romType = (int)romHeader[ ROM_TYPE_OFS ];

	if (size != readSize || !(romType & 0x2) )
	{
        displayError(ERROR_NOT_NITRO_SRL);
	}
}
...
void parseOption(int argc, char *argv[])
{
    ...
    //---- check nlf id string
    {
		...
		{
			displayError(ERROR_TLF_FILE_SPECIFIED);
		}

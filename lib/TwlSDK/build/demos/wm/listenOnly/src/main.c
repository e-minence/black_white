...
#define USE_WM_INITIALIZE // WM_InitializeForListening 関数を利用する
...
static BOOL InitializeWMForListeningAndStartScan(void)
{
...
#ifdef USE_WM_INITIALIZE
    // WM_InitializeForListening 関数を使用する場合
...
#else
    // WM_Init, WM_EnableForListening, WM_PowerOn 関数を個別に呼び出す場合
    ...
    result = WM_EnableForListening(EnableCb, FALSE /* LED を点滅させない */);
    if (result != WM_ERRCODE_OPERATING)
    {

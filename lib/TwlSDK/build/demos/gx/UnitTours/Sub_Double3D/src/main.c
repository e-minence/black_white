...
/* ---------------------------------------------------
▼動作モードについて
    このデモでは、Yボタンを押すごとに２つのモードで動作します。
    最初のモードから順に
    ちらつき対策あり30fps -> ちらつき対策なし15fps
    となります。

★ちらつきとは
    表示キャプチャ機能を使用して3D画像をキャプチャする場合、元画像とキャプチャ画像を
    交互に表示させると、ちらつきが発生します。

    表示キャプチャ機能を使用して3D画像をキャプチャすると、元画像のRGB=(6:6:6)データの
    最下位ビットが0になり、RGB=(5:5:5)となります。
    そのため、元画像とキャプチャ画像を交互に表示させるとちらつきが発生する原因となります。

■問題点の回避方法
    3D画像を直接表示するのではなく、VRAMにキャプチャした画像を上画面、下画面の両方で表示する必要があります。
    このデモでは、キャプチャ先のVRAMをそのまま表示することで対応しています。

★ポリゴンエッジのちらつき
    上記のちらつき対策を施したとしても、設定によってはポリゴンエッジのちらつきが発生することがあります。
    その原因はアルファ値にあります。

    （ポリゴンアルファ×（アンチエイリアス係数＋補正値１））／３２＝カラーバッファアルファ
    という計算式から、有効なRGB値と共にアルファ０がカラーバッファへ格納されることがあります。
    その後の3Dキャプチャ時には、キャプチャデータのアルファが０の場合のみ、
    VRAM のアルファビットにも０が格納されます。
    （実際 VRAM をダンプしてアルファ０のポリゴンエッジを確認することができます。）
    
    VRAM表示モードではこのアルファビットが無効になるため、ポリゴンエッジの該当ピクセルが表示されます。
    BG表示（グラフィックス表示モード）ではアルファビットが有効なので抜けドット扱いになり、
    バックドロップカラーが表示されます。

    この違いがポリゴンエッジのちらつきの原因になっています。
    （クリアカラーとバックドロップカラーが同じ色で判り難くなっていますが、例えばクリアカラーの方を
      変更すると、これらが交互に背景色として表示されているのが判ります。）

■問題点の回避方法
    GX_SetCapture 関数の引数 a に指定する値を GX_CAPTURE_SRCA_2D3D にすれば、上記の場合や
    2D と 3D を合成した場合でも同じ表示になり、ちらつきを回避できます。
---------------------------------------------------- */
...
static void setupFrame2N(void)
{
    ...
    {
    case 0:
        GX_SetGraphicsMode(GX_DISPMODE_VRAM_D, GX_BGMODE_0, GX_BG0_AS_3D); // ちらつき対策している方法
        break;
    case 1:
        GX_SetGraphicsMode(GX_DISPMODE_GRAPHICS, GX_BGMODE_0, GX_BG0_AS_3D); // ちらつき対策していない方法
        break;
    }
...
static void setupFrame2N_1(void)
{
    ...
    {
    case 0:
        GX_SetGraphicsMode(GX_DISPMODE_VRAM_C, GX_BGMODE_0, GX_BG0_AS_3D); // ちらつき対策している方法
        break;
    case 1:
        GX_SetGraphicsMode(GX_DISPMODE_GRAPHICS, GX_BGMODE_0, GX_BG0_AS_3D); // ちらつき対策していない方法
        break;
...
//#define SET_FOG                       // Double3Dを使用し2画面で別々のfogを設定したい場合は
    ...
    // 2Dと3Dの合成のために前面(BG0)がブレンディングされるようにする
    ...
    while (1)
    {
        ...
        {
            operationMode = (operationMode+1)%2; // 動作モード切り替え
        }

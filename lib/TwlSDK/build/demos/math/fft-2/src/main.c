...
#define DRAW_STEP          MATH_MAX(FFT_N/2/256,1)    // 描画するステップ数
#define DRAW_MAX           MATH_MIN(FFT_N/2/DRAW_STEP, 256) // 直流成分以外の独立なパワー値と画面の幅の min
...
static void SetDrawData(void *address)
{
    ...
    for (i = 0; i <= FFT_N/2; i++)
    {
        ...
        {
            // MATH_FFTReal 関数の結果の格納方法は特殊なので、それを処理する
            if (i == 0)
            {
                real = data[0];
            }
    ...
    case DRAWMODE_REALTIME:
        ...
        for (i = 0; i < DRAW_MAX; i++)
        {
            ...
            // +1 の理由: power[0] のデータは直流成分なので表示を省いている
            index = i * DRAW_STEP + 1;
            tmpPower = 0;

...
static void OnVBlank(void)
{
    ...
    if ((p->draw_flag & DEMO_DRAW_FLIPPING) != 0)
    {
        ...
        {
            MI_CpuFillFast(tmp_surface, (u32)((col_clear << 0) | (col_clear << 16)), sizeof(tmp_surface));
        }
...
{
    DrawiCommon *const p = &drawi_common;
    MI_CpuFillFast(p->next_surface, (u32)((col << 0) | (col << 16)), GX_LCD_SIZE_X * GX_LCD_SIZE_Y * sizeof(RGB555));
}
...
void DrawTextLen(int x, int y, const char *s, int n)
{
    ...
    for (; n > 0; ++s, --n)
    {
        ...
        default:
            ...
            {
                int     tx = x + px;
                if ((tx > pc->clip.o.x - 8) && (y > pc->clip.o.y - 8) && (tx < pc->clip.t.x) && (y < pc->clip.t.y))
                {

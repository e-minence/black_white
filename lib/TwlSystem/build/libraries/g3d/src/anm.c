...
// MayaのSSC,Si3dのClassic Scale off時にスケールをブレンドする対処
                         ...
                         u32 nodeID)
{
    ...
    else
    {
                    ...
                    (p->ratio > 0))
                {
                    ...
                    if (func)
                    {
                        ...
                        if (scalingRule == NNS_G3D_SCALINGRULE_SI3D)
                        {
                            ...
                            if (nodeID == 0)
                            {
                                // nodeID=0 の時は parentID=0 なので復元する必要がある
                                NNS_G3dRS->isScaleCacheOne[0] = 1; // エンディアン依存
        ...
        {
            // pResult から SSC の親かどうか判定できないので NNS_G3dRS を見ざるをえない
            const u8* cmd = NNS_G3dRS->c;
            u8 opFlag = *(cmd + 3);
            // SSC の親ノードの場合はブレンド済みスケールの逆数をキャッシュしておく必要がある
        ...
        if (scalingRule == NNS_G3D_SCALINGRULE_SI3D)
        {
            // scaleEx1 には累積スケールをブレンドした後の逆数を格納する必要がある。
            // scaleEx0 は参照する親ノードが正しくブレンドされていれば
            // ブレンドしたものをそのまま格納して問題無い
            if (pResult->flag & NNS_G3D_JNTANM_RESULTFLAG_SCALEEX0_ONE)
            {
                // このノードは scaleEx を使用しない
                if (pResult->flag & NNS_G3D_JNTANM_RESULTFLAG_SCALE_ONE)
                {
                    // 子ノードのためにフラグを更新する
                    // このノードまでの累積スケールが1

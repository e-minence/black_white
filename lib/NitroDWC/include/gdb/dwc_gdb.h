/*---------------------------------------------------------------------------*
  Project:  NitroDWC public include file
  File:     dwc_gdb.h
...
typedef struct {
	...
	BOOL	cache_flag;		///< 検索結果をサーバでキャッシュするか否かを示します。
} DWCGdbSearchCond;
...
extern DWCGdbError DWC_GdbInitialize(int game_id, const DWCUserData* userdata);

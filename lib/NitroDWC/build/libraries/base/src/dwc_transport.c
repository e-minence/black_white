...
DWCi_IsSendableReliable( u8 aid, u16 type )
{
    ...
    // キューに一定以上たまっているかどうか確認
    if ( !DWCi_IsReliableQueueFree() )
    {
        return FALSE;
    }
...
/*	//sTransInfo->connections[aid].recvState       = 0;
    ...
    sTransInfo->connections[aid].recvBuffer = 0;
    sTransInfo->connections[aid].recvBufferSize = 0;
    sTransInfo->connections[aid].recvState = DWC_TRANSPORT_RECV_NOBUF;
    ...
    BOOL					reliable )
{
    ...
    if ( reliable )
    {
        ...
        // まずキューのたまったデータを吐き出す
        if ( DWCi_ProcessReliableQueue() )
        {
            // キューが空になったら新しいデータを送信する
            // 新しく送信する
            res = gt2Send( connection, buffer, size, reliable );

            // 成功したら終わる
            if ( res == GT2Success )
                return;
            else
                DWC_Printf( DWC_REPORTFLAG_SEND_INFO, "gt2Send() Failed code:%d\n", (int)res );
        }

        // キューが空になっていなかったり送信に失敗したらキューに入れる
        if ( !DWCi_InsertReliableQueue( connection, buffer, size ) )
        {
            // キューに入れられなかった場合はDWCエラーをセットする
            DWCi_SetError( DWC_ERROR_NETWORK, DWC_ECODE_SEQ_ETC + DWC_ECODE_GS_GT2 + DWC_ECODE_TYPE_TRANS_SEND );
        }

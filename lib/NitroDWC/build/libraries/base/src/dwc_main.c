...
void DWC_ShutdownFriendsMatch(void)
{
    ...
    // Persistentオブジェクトの開放
    DWCi_BeginCloseStatsConnection();
    CloseStatsConnection();  
    DWCi_EndCloseStatsConnection();
...
void DWC_ProcessFriendsMatch(void)
{
    ...
    case DWC_STATE_AVAILABLE_CHECK:  // ゲーム利用可能状況チェック中
        ...
        case GSIACAvailable:    // 利用可能。
            ...
            //
            gpResult = gpInitialize(&stpDwcCnt->gpObj, stpDwcCnt->logcnt.productID, 16, 11);
            ...
            gpResult = gpSetCallback(&stpDwcCnt->gpObj, GP_RECV_BUDDY_AUTH,
                                     (GPCallback)DWCi_GPRecvBuddyAuthCallback,
                                     NULL);
            if (DWCi_HandleGPError(gpResult)) return;
...
/*---------------------------------------------------------------------------*
  AIDをリセットする関数
 *---------------------------------------------------------------------------*/
void DWCi_ResetMyAID(void)
{
    stpDwcCnt->aid = 0;
    DWC_Printf( DWC_REPORTFLAG_DEBUG, "My AID has been reset as 0.\n" );
}

...
// タイムアウト計測に使う定数
#define TIMEOUT_CMD_RESET   (-1)        // タイムアウトのカウンタをリセットするマジック
#define TIMEOUT_SEC         (30*1000)   // 30秒
...
/* -------------------------------------------------------------------------
        macro
   ------------------------------------------------------------------------- */
#define DWCi_Np_TicksToMilliSeconds(a)      OS_TicksToMilliSeconds(a)
#define DWCi_Np_GetTick()                   OS_GetTick()
#define DWCTick                             u64
...
static BOOL DWCs_RnkTimedOut( int timeout_in_ms );
...
DWCRnkError	DWC_RnkPutScoreAsync(	u32 category,
    ...
    // タイムアウトカウンタのリセット 
    DWCs_RnkTimedOut( TIMEOUT_CMD_RESET );
...
DWCRnkError DWC_RnkProcess(){
    ...
    if( g_rankinginfo.state == DWC_RNK_STATE_TIMEOUT )
    {
        // タイムアウトした
        return DWC_RNK_PROCESS_TIMEOUT;
    }
	...
	case DWCi_RANKING_SESSION_STATE_SENDING_DATA:	//!< データ送信中
        ...
        // タイムアウト検出
        //
        if ( DWCs_RnkTimedOut( TIMEOUT_SEC ) )
        {
            DWC_Printf( DWC_REPORTFLAG_INFO, "Timeout!!" );

            // キャンセルする
            DWCi_RankingSessionCancel();
            
            g_rankinginfo.state = DWC_RNK_STATE_TIMEOUT;

            // DWCエラーをセットする
            DWCi_SetError( DWC_ERROR_GHTTP_ANY, DWC_ECODE_SEQ_ETC + DWC_ECODE_GS_HTTP + DWC_ECODE_TYPE_TIMEOUT );
...
DWCRnkError	DWC_RnkResGetTotal( u32* total ){
	...
	*total = res.orderheader->total;
    ...
    // 特殊対応
    //
    // 友達ランキングのときはサーバーから返ってくる母数に
    // 自分が含まれていないので、ここで加算する。
    if( res.mode == DWC_RNK_GET_MODE_FRIENDS )
        *total += 1;
...
static BOOL DWCs_RnkTimedOut( int timeout_in_ms )
{
    static DWCTick start = 0;

    // カウンタをリセット
    if ( timeout_in_ms == TIMEOUT_CMD_RESET )
    {

        start = DWCi_Np_GetTick();

        DWC_Printf( DWC_REPORTFLAG_INFO, "Timeout counter has been reset.\n" );
    ...
    // リセットされてからの経過時間を計測
    if ( DWCi_Np_TicksToMilliSeconds( DWCi_Np_GetTick() - start ) > (u32)timeout_in_ms )
    {

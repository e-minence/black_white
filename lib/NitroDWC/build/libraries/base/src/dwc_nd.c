...
#pragma unused(size)
	...
	return DWCi_NdGetFileAsync(fileinfo, buf, size, NULL);
}

/*---------------------------------------------------------------------------*
  Name:			DWC_NdGetFileExAsync

  Description:	ファイル取得処理開始関数（拡張版）

  Arguments:	filename - 取得するファイルの名前
				buf      - 取得されたファイルのデータが格納される領域へのポインタ
				size     - 取得されたファイルのデータが格納される領域のサイズ
                opt      - 取得リクエストに含める文字列(英数字かアンダーバー"_"で32文字以内のヌル終端した文字列を指定)
  Returns:		TRUE  - 通信が開始され処理完了待ち状態
				FALSE - 処理失敗
 *---------------------------------------------------------------------------*/
...
#pragma unused(size)
	...
	return DWCi_NdGetFileAsync(fileinfo, buf, size, opt);
}

...
static BOOL s_bInCloseStatsConnection = FALSE;
...
static void DWCi_GetReverseBuddiesCallback(GPConnection* connection,
...
void DWC_LogoutFromStorageServer(void)
{
    ...
    {
        if(s_bInCloseStatsConnection == FALSE)
        {
                     ...
                     int friendListLen)
{
    ...
    stpFriendCnt->reverseBuddies     = NULL;
    
    // バディリクエストを1回のログインにつき1人に1メッセージに限定するためのフラグを初期化する
    for(i=0; i<friendListLen; i++)
    {
        DWCi_SetSentBuddyReqFlag( &stpFriendCnt->friendList[i], FALSE );
    }
...
DWC_ECODE_SEQ_FRIEND+DWC_ECODE_GS_PERS+DWC_ECODE_TYPE_NETWORK);
    ...
    if (stpFriendCnt->svUpdateComplete >= 2){    
        // 友達リスト同期処理が完了したら、片思いの友達チェックの非同期処理を開始する。
        // 同期処理完了コールバックは、この非同期関数のコールバック(DWCi_GetReverseBuddiesCallback)内で呼ぶように変更した。
        if (stpFriendCnt->pGpObj && *stpFriendCnt->pGpObj){
            ...
            // ローカル友達リストでGSProfileIDを含む数をカウント
            for (i = 0; i < stpFriendCnt->friendListLen; i++)
            {
                // 友達リストのデータタイプがプロファイルIDのとき
                if ( DWC_GetFriendDataType( &stpFriendCnt->friendList[i] ) == DWC_FRIENDDATA_GS_PROFILE_ID )
                {
                    pidcnt++;
                }
            }
            ...
            if(pidcnt == 0)
            {
                // 友達リスト同期処理完了コールバックを呼ぶ。
                stpFriendCnt->svUpdateComplete = 0;
                DWCi_EndUpdateServers();
                return;
            }
            
            stpFriendCnt->reverseBuddies = DWC_Alloc(DWC_ALLOCTYPE_BASE, sizeof(GPProfile) * pidcnt);
            
            if(!stpFriendCnt->reverseBuddies){
                DWC_Printf(DWC_REPORTFLAG_DEBUG, "DWC_Alloc Error\n");
                DWCi_StopFriendProcess(DWC_ERROR_FATAL, DWC_ECODE_SEQ_FRIEND + DWC_ECODE_TYPE_ALLOC);
                return;
            }
            ...
            for (i = 0; i < stpFriendCnt->friendListLen; i++)
            {
                ...
                if ( DWC_GetFriendDataType( &stpFriendCnt->friendList[i] ) == DWC_FRIENDDATA_GS_PROFILE_ID )
                {
                    stpFriendCnt->reverseBuddies[j] = DWC_GetGsProfileId( DWCi_GetUserData(), &stpFriendCnt->friendList[i] );
                    j++;
                }
            }
            
            
            gpResult = gpGetReversBuddiesList( stpFriendCnt->pGpObj,
                                               stpFriendCnt->reverseBuddies,
                                               pidcnt,
                                               GP_NON_BLOCKING,
                                               (GPCallback)DWCi_GetReverseBuddiesCallback,
                                               NULL );
...
void DWCi_StopFriendProcess(DWCError error, int errorCode)
{
    ...
    if ((stpFriendCnt->state != DWC_FRIEND_STATE_INIT) &&
        ...
        // gpGetReverseBuddiesListの為に確保したメモリを解放
        if(stpFriendCnt->reverseBuddies != NULL)
        {
            DWC_Free(DWC_ALLOCTYPE_BASE, stpFriendCnt->reverseBuddies, 0);
            stpFriendCnt->reverseBuddies = NULL;
        }
...
/*---------------------------------------------------------------------------*
  ...
  引数　：connection GPコネクションオブジェクトへのポインタ
          arg        GPRecvBuddyAuthArg型オブジェクトへのポインタ
          param      コールバック用パラメータ
  戻り値：GPバディリクエストの返答ならTRUE
  用途　：GPバディレスポンス受信コールバック。DWCi_GPRecvBuddyMessageCallbackより
          呼び出さされる。
 *---------------------------------------------------------------------------*/
void DWCi_GPRecvBuddyAuthCallback( GPConnection* connection,
...
static void DWCi_UpdateFriendReq(DWCFriendData friendList[], int friendListLen)
{
        ...
        for (i = 0; i < numBuddy; i++){
            ...
            for (j = 0; j < friendListLen; j++){
                if (DWCi_GetProfileIDFromList(j) == status.profile){
                    ...
                    if (!DWCi_IsBuddyFriendData(&friendList[j])){
    ...
    for ( ; stpFriendCnt->buddyUpdateIdx < friendListLen; stpFriendCnt->buddyUpdateIdx++){
        // バディリクエストは1回のログインで1人に対して1回しか送らない。
        if(DWCi_IsBuddyMsgAlreadySent(&friendList[stpFriendCnt->buddyUpdateIdx]))
            continue;
        ...
        if (profileID){
            ...
            // いないだけと考えられるので何もしない
            //if (DWCi_IsBuddyFriendData(&friendList[stpFriendCnt->buddyUpdateIdx]))
            //    continue;
            ...
            if (i == -1){
                ...
                // バディ登録要求を行う
                // また、バディリクエストを1度送った事を示すフラグを立てる
                (void)DWCi_GPSendBuddyRequest(profileID);
                DWCi_SetSentBuddyReqFlag( &friendList[stpFriendCnt->buddyUpdateIdx], TRUE ); 
...
static BOOL DWCi_RefreshFriendListForth(DWCFriendData friendList[], int index, int profileID)
{
    ...
    for (i = 0; i < index; i++){
        ...
        if (pid_i && (pid_i == profileID)){
            ...
            if (DWCi_IsBuddyFriendData(&friendList[index]) &&
                !DWCi_IsBuddyFriendData(&friendList[i])){
        ...
        if (stpFriendCnt->state == DWC_FRIEND_STATE_PERS_LOGIN){
            ...
            for ( match_index = 0 ; match_index < arg->numMatches ; ++match_index )
            {
                ...
                if (idx == -1){
                    ...
                    DWCi_SetSentBuddyReqFlag( &stpFriendCnt->friendList[friendIdx], TRUE );
                }
                else {
                    ...
                    DWCi_SetReverseBuddyFlag(&stpFriendCnt->friendList[friendIdx], TRUE);
                    // バディ成立コールバックを呼び出す
    ...
    for (i = 0; i < stpFriendCnt->friendListLen; i++)
    {
        if ( DWC_GetFriendDataType( &stpFriendCnt->friendList[i] ) == DWC_FRIENDDATA_LOGIN_ID )
        {
            ...
            if (strcmp(userName, arg->lastname) == 0)
            {
                ...
                // ReverseBuddyのフラグを立てる
                DWCi_SetReverseBuddyFlag(&stpFriendCnt->friendList[i], TRUE);
...
#pragma unused(param)
    ...
    for (i = 0; i < stpFriendCnt->friendListLen; i++)
    {
                  ...
                  DWC_GetFriendDataType( &stpFriendCnt->friendList[i] ) == DWC_FRIENDDATA_FRIEND_KEY )
        {
            ...
            if ( DWC_GetGsProfileId( DWCi_GetUserData(), &stpFriendCnt->friendList[i] ) == arg->profile )
            {
                if( DWCi_IsBuddyFriendData( &stpFriendCnt->friendList[i] ) == TRUE )
                {
                    DWC_Printf(DWC_REPORTFLAG_UPDATE_SV, 
    ...
    if ( listChanged )
    {
        ...
        if ( !alreadyBuddy )
        {
            DWCi_SetReverseBuddyFlag(&stpFriendCnt->friendList[index], TRUE);
...
void DWCi_BeginCloseStatsConnection()
{
    DWC_ASSERT(s_bInCloseStatsConnection == FALSE);
    s_bInCloseStatsConnection = TRUE;
}

void DWCi_EndCloseStatsConnection()
{
    DWC_ASSERT(s_bInCloseStatsConnection == TRUE);
    s_bInCloseStatsConnection = FALSE;
}
...
/*---------------------------------------------------------------------------*
  片思いの友達リスト取得コールバック関数
  引数　：connection　使用しない
          arg   　　　(GPResult result, int numOfUniqueMatchs, GPUniqueMatch *matches)
          param       GPProfileを指定するように確保したメモリのポインタ（解放用）
  戻り値：なし
  用途　：片思いの友達リスト取得コールバック
 *---------------------------------------------------------------------------*/
static void DWCi_GetReverseBuddiesCallback(GPConnection* connection, GPGetReverseBuddiesListResponseArg* arg, void* param)
{
...
#pragma unused(param)
    ...
    if (arg->result != GP_NO_ERROR)
    {
        DWC_Printf( DWC_REPORTFLAG_DEBUG, " ERROR: GetReverseBuddies Req. why??? : %d\n", arg->result );        
        DWCi_EndUpdateServers();
        return;
    }
    
    for(i = 0; i < arg->numOfUniqueMatchs; i++)
        DWC_Printf(DWC_REPORTFLAG_DEBUG,"GetReverseBuddies   profileID = %u \n", arg->matches[i].profile);
    ...
    for (i = 0; i < stpFriendCnt->friendListLen; i++)
    {
        ...
        if ( DWC_GetFriendDataType( &stpFriendCnt->friendList[i] ) == DWC_FRIENDDATA_GS_PROFILE_ID )
        {
            //コールバックで返ってきたProfileIDの数だけ繰り返す
            for(j = 0; j < arg->numOfUniqueMatchs; j++)
            {                
                //ProfileIDが一致したら両想いの友達なので、
                if ( DWC_GetGsProfileId( DWCi_GetUserData(), &stpFriendCnt->friendList[i] ) == arg->matches[j].profile )
                {
                    // フラグを確認し、変更の必要がある場合、フラグを変更する。
                    if( DWCi_IsReverseBuddyFriendData( &stpFriendCnt->friendList[i] ) == FALSE)
                    {
            ...
            //見つからなかった時、片思いであるので、
            if(j == arg->numOfUniqueMatchs)
            {
                // フラグを確認し、変更の必要がある場合、フラグを変更する。
                if( DWCi_IsReverseBuddyFriendData( &stpFriendCnt->friendList[i] ) == TRUE)
                {
                    stpFriendCnt->friendListChanged = 1;
                    DWCi_SetReverseBuddyFlag( &stpFriendCnt->friendList[i], FALSE);
                }
            }
            
            // 一度両想い(双方向で友達関係が結べている)になったことを示すフラグを立てるための処理
            // BuddyフラグとReverseBuddyフラグの両方が立っていれば、その状態になったと判定する。
            if( DWC_IsBuddyFriendData( &stpFriendCnt->friendList[i] ) )
            {
                DWCi_SetExpIsBuddyFriendFlag( &stpFriendCnt->friendList[i], TRUE );
            }

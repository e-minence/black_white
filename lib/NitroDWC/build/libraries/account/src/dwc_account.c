...
// default MD5 friendkey
//#define DWC_CRC_FRIENDKEY
...
/*---------------------------------------------------------------------------*
  ...
  Description:  友達情報が既にサーバ上で相互に友達関係になっているかどうかを確認する。
...
{
    if( DWCi_IsBuddyFriendData( frienddata ) && DWCi_IsReverseBuddyFriendData( frienddata ) )
    {
        return TRUE;
    }
 ...
 *---------------------------------------------------------------------------*/
BOOL
DWCi_IsBuddyFriendData( const DWCAccFriendData* frienddata )
{
...
/*---------------------------------------------------------------------------*
  Name:         DWC_WasBuddyFriendData

  Description:  引数に指定した友達が、自分との友達関係を破棄したかどうかを調べます。

  Arguments:    frienddata : フレンドデータを格納するDWCAccFriendData構造体へのポインタ

  Returns:      TRUE  : 以前友達関係にあったが、友達は私との友達関係を破棄している。
                FALSE : そのようなことはない。
 *---------------------------------------------------------------------------*/
BOOL
DWC_WasBuddyFriendData( const DWCAccFriendData* frienddata )
{
    u32 type;
    
    type = DWCi_Acc_GetFlags( &frienddata->flags );
    
    if( DWCi_CHECK_FLAG( type, DWC_ACC_FRIENDDATA_EXP_ISBUDDYFRIEND ) &&
        !DWCi_CHECK_FLAG( type, DWC_ACC_FRIENDDATA_IS_REVERSEBUDDY ))
    {
        return TRUE;
    }
...
/*---------------------------------------------------------------------------*
  Name:         DWCi_IsReverseBuddyFriendData

  Description:  引数に指定する友達が自分の事を友達と思っているかを確認する。

  Arguments:    frienddata : フレンドデータを格納するDWCAccFriendData構造体へのポインタ

  Returns:      TRUE  : 友達関係にある（相手の友達リストで）。
                FALSE : 友達関係にない（相手の友達リストで）。
 *---------------------------------------------------------------------------*/
BOOL
DWCi_IsReverseBuddyFriendData( const DWCAccFriendData* frienddata )
{
    u32 type;

    type = DWCi_Acc_GetFlags( &frienddata->flags );

    return ( DWCi_CHECK_FLAG( type, DWC_ACC_FRIENDDATA_IS_REVERSEBUDDY ) ) ? TRUE : FALSE;
}

/*---------------------------------------------------------------------------*
  Name:         DWCi_IsBuddyMsgAlreadySent

  Description:  引数に指定した友達に、今回のログインにおいて既に1度バディリクエストを
                送信しているかどうかを判定する。
 ...
 *---------------------------------------------------------------------------*/
BOOL
DWCi_IsBuddyMsgAlreadySent( const DWCAccFriendData* frienddata )
{
    u32 type;

    type = DWCi_Acc_GetFlags( &frienddata->flags );

    return ( DWCi_CHECK_FLAG( type, DWC_ACC_FRIENDDATA_SENT_BUDDYREQ ) ) ? TRUE : FALSE;
}

/*---------------------------------------------------------------------------*
  Name:         DWC_GetFriendDataType
...
/*---------------------------------------------------------------------------*
  ...
  Description:  友達情報にセットされた友達成立フラグをクリアする。
...
DWC_ClearBuddyFlagFriendData( DWCAccFriendData* frienddata )
{
    ...
    flags &=  ~DWC_ACC_FRIENDDATA_IS_REVERSEBUDDY_MASK;      // 追加
    flags &=  ~DWC_ACC_FRIENDDATA_EXP_ISBUDDYFRIEND_MASK;    // 追加
    flags &=  ~DWC_ACC_FRIENDDATA_SENT_BUDDYREQ_MASK;        // 追加
    
    DWCi_Acc_SetFlags( &frienddata->flags, flags );
}

/*---------------------------------------------------------------------------*
  Name:         DWCi_SetReverseBuddyFlag

  Description:  DWCAccFriendData構造体のDWC_ACC_FRIENDDATA_IS_REVERSEBUDDYフラグをセットする。

  Arguments:    frienddata : フラグをセットするDWCAccFriendData構造体へのポインタ
                setflag    : セットするフラグ

  Returns:      なし
 *---------------------------------------------------------------------------*/
void
DWCi_SetReverseBuddyFlag( DWCAccFriendData* frienddata, BOOL setflag )
{
    u32 tempflags;

    if(setflag)
    {
        tempflags = DWCi_Acc_GetFlags( &frienddata->flags );
        tempflags = ( tempflags &  ~DWC_ACC_FRIENDDATA_IS_REVERSEBUDDY_MASK ) | DWC_ACC_FRIENDDATA_IS_REVERSEBUDDY;
    }
    else
    {
        tempflags = DWCi_Acc_GetFlags( &frienddata->flags );
        tempflags = tempflags &  ~DWC_ACC_FRIENDDATA_IS_REVERSEBUDDY_MASK;
    }
    
    DWCi_Acc_SetFlags( &frienddata->flags, tempflags );

}

/*---------------------------------------------------------------------------*
  Name:         DWCi_SetExpIsBuddyFriendFlag

  Description:　DWCAccFriendData構造体のDWC_ACC_FRIENDDATA_EXP_ISBUDDYFRIENDフラグをセットする。
 ...
 *---------------------------------------------------------------------------*/
void
DWCi_SetExpIsBuddyFriendFlag( DWCAccFriendData* frienddata, BOOL setflag )
{
    ...
    {
        tempflags = DWCi_Acc_GetFlags( &frienddata->flags );
        tempflags = ( tempflags &  ~DWC_ACC_FRIENDDATA_EXP_ISBUDDYFRIEND_MASK ) | DWC_ACC_FRIENDDATA_EXP_ISBUDDYFRIEND;
    }
    else
    {
        tempflags = DWCi_Acc_GetFlags( &frienddata->flags );
        tempflags = tempflags &  ~DWC_ACC_FRIENDDATA_EXP_ISBUDDYFRIEND_MASK;
    }
...
/*---------------------------------------------------------------------------*
  Name:         DWCi_SetSentBuddyReqFlag

  Description:　DWCAccFriendData構造体のDWC_ACC_FRIENDDATA_SENT_BUDDYREQフラグをセットする。
 ...
 *---------------------------------------------------------------------------*/
void
DWCi_SetSentBuddyReqFlag( DWCAccFriendData* frienddata, BOOL setflag )
{
    u32 tempflags;
    ...
    {
        tempflags = DWCi_Acc_GetFlags( &frienddata->flags );
        tempflags = ( tempflags &  ~DWC_ACC_FRIENDDATA_SENT_BUDDYREQ_MASK ) | DWC_ACC_FRIENDDATA_SENT_BUDDYREQ;
    }
    else
    {
        tempflags = DWCi_Acc_GetFlags( &frienddata->flags );
        tempflags = tempflags &  ~DWC_ACC_FRIENDDATA_SENT_BUDDYREQ_MASK;
    }
...
/*---------------------------------------------------------------------------*
  ...
  Description:  GSプロファイルIDから、友達登録鍵を生成します。
                NITRO-DWC 3.0 からチェック値の計算方法が変更されたため、
                本関数は過去との互換性を保つために引数を追加しています。

  Arguments:    gs_profile_id : GSプロファイルID
                gamecode      : ユーザーデータ生成時に指定したイニシャルコード
                method        : チェック値の生成方法
                         ...
                         DWCiFriendKeyGenMethod method )
{
    ...
    switch( method )
    {
    case DWCi_FRIENDKEY_GENERATOR_CRC:
        {
            MATHCRC8Table table;
            
            MATH_CRC8InitTable( &table );
            hash = MATH_CalcCRC8( &table, code, sizeof(code) ) & 0x7f; // 下位7ビット
        }
        break;

    case  DWCi_FRIENDKEY_GENERATOR_MD5:
    default:
        {
            ...
            MATH_MD5Update( &context, (void *)code, sizeof(code) );
            MATH_MD5GetHash( &context, digest );
            hash = ( digest[0] >> 1 ) & 0x7f; // 先頭7ビット
        }
        break;
    }

    return ( ((u64)hash << 32) | (u32)gs_profile_id );
}
...
{
    return DWC_Acc_CheckFriendKey( friendkey, userdata->gamecode, DWCi_FRIENDKEY_GENERATOR_MD5 );
}

BOOL
DWC_CheckFriendKeyLegacy( const DWCAccUserData* userdata, u64 friendkey )
{
    return DWC_Acc_CheckFriendKey( friendkey, userdata->gamecode, DWCi_FRIENDKEY_GENERATOR_CRC );
}
                        ...
                        DWCiFriendKeyGenMethod method )
{
    ...
    pid = (int)( friendkey & 0xffffffff );

    // 負のGSプロファイルIDは不正
    if ( pid <= 0 ) return FALSE;

    // 友達登録鍵の確認
    return (( DWC_Acc_CreateFriendKey( pid, gamecode, method ) == friendkey ) ? TRUE : FALSE );
}

/*---------------------------------------------------------------------------*
  Name:         DWC_Acc_FriendKeyToGsProfileId

  Description:  友達登録鍵からGSプロファイルIDを取り出します。
                                ...
                                u32 gamecode )
{
    ...
    for( i = 0; i < DWCi_FRIENDKEY_GENERATOR_NUM; i++ )
    {
        if ( DWC_Acc_CheckFriendKey( friendkey, gamecode, (DWCiFriendKeyGenMethod)i ) )
        {
...
DWC_CreateFriendKey( const DWCAccUserData* userdata )
{
    ...
    {
        ret = DWC_Acc_CreateFriendKey( userdata->gs_profile_id, userdata->gamecode, DWCi_FRIENDKEY_GENERATOR_MD5 );
    }
...
u64
DWC_CreateFriendKeyLegacy( const DWCAccUserData* userdata )
{
    u64 ret = 0;

    if ( userdata->gs_profile_id != 0 )
    {
        ret = DWC_Acc_CreateFriendKey( userdata->gs_profile_id, userdata->gamecode, DWCi_FRIENDKEY_GENERATOR_CRC );
    }

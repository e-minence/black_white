...
DWC_Init( void* work )
{
    ...
    if ( ret < 0 )
    {
        ...
        if ( created )
        {
            result = DWC_INIT_RESULT_DESTROY_USERID;
        }
        else
        {
            result = DWC_INIT_RESULT_DESTROY_OTHER_SETTING;
        }
    }
    else if ( created )
    {
        // NVRAMは問題ないけど、なぜか作られました。
        result = DWC_INIT_RESULT_CREATE_USERID;
    }
    else
    {
        result = DWC_INIT_RESULT_NOERROR;
    }
    
#ifndef NITRODWC_NOGS
    // Gamespy用メモリーアロケーターをセット
    //
    gsiMemoryCallbacksSet(  DWCi_GsMalloc,
                            DWCi_GsFree,
                            DWCi_GsRealloc,
                            DWCi_GsMemalign );
#endif

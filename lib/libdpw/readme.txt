...
【 更新内容 】
 ...
 beta33 (Jul. 2, 2010)
  －管理用ツール－
    ・リリース用サーバにアクセスできるツール DpwAdmin_releaseServer.exe をパッケージに追加しました。

 beta32 (Jun. 23, 2010)
  －バトルタワー－
    ・beta31 のバグ修正。DS から送信されるトークンの末尾3バイトが上書きされてしまうバグを修正しました。

 beta31 (Jun. 11, 2010)
  －共通－
    ・beta30 のバグ修正。Dpw と無関係な DWC エラーが発生した場合 FATAL ERROR 扱いにしていた点を修正し、
      無線レイヤでのエラーを検知してインターネット接続が切断されたエラーを返すようにしました。
  －世界通信交換－
    ・交換成立時のログに交換相手のpidを出力するようにスクリプトを変更しました。

 beta30 (Jun. 3, 2010)
  －共通－
    ・通信エラー取得時、DWCでエラーが発生していた場合インターネット接続が切断されたエラーを返すようにしました。

 beta29 (May. 18, 2010)
  －共通－
    ・エラー取得時に誤ったエラー番号を返すバグを修正しました。

 beta28 (May. 12, 2010)
  －共通－
    ・非同期処理の開始後にインターネット接続が切れた場合に、非同期処理をキャンセルしても正しくキャンセル処理が
      進まなかった不具合を修正しました。

 beta27 (Apr. 28, 2010)
  －バトルタワー、管理用ツール－
    ・バトルタワーにアップロードするポケモンデータ構造体を 56 バイトから 60 バイトに変更しました。
      DpwAdmin.exe をこの変更に対応するように変更しました。(version 1.07)
      
 beta26 (Apr. 22, 2010)
  －管理用ツール－
    ・DpwAdmin.exe に表示されるプレイヤーデータ、リーダーデータの名前欄を
      UTF16 エンコードされた文字列を表示するように変更しました。(version 1.06)
      旧文字コードのデータは文字化けしますので、以前のデータを扱いたい場合 version 1.05 以前のツールを使用して下さい。

...
/*! @file
	...
	@author Takeuchi Takaaki(takeuchi_takaaki@nintendo.co.jp)
...
typedef struct {
	...
	u8 langCode;			//!< 言語コード             1
	u8 countryCode;			//!< 住んでいる国コード     1
	u8 localCode;			//!< 住んでいる地方コード   1
	u8 playerId[4];			//!< プレイヤーID           4
	s8 leaderMessage[8];	//!< リーダーメッセージ     8
	union{
		struct{
			u8 ngname_f	:1;	//!< NGネームフラグ
			u8 gender	:1;	//!< プレイヤーの性別フラグ
...
extern void Dpw_Bt_GetRoomNumAsync(u8 rank);
extern void Dpw_Bt_DownloadRoomAsync(u8 rank, u8 roomNo, Dpw_Bt_Room* roomData);
extern void Dpw_Bt_UploadPlayerAsync(u8 rank, u8 roomNo, u8 win, const Dpw_Bt_Player* player, const u8 *token, u32 len);

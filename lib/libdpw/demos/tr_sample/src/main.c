...
void NitroMain()
{
	...
	while (1)
	{
		...
		case SAMPLE_TR_UPLOAD_FOR_CANCEL:
			// 「預ける」処理の開始。
			// データをアップロードする。
			Dpw_Tr_UploadAsync(&upload_data, token1, TOKEN_LEN);
		...
		case SAMPLE_TR_TRADE:
			...
			Dpw_Tr_TradeAsync(match_id, &trade_data, &download_buf, token1, TOKEN_LEN);

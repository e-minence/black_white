...
static void Print( tDbsConsole* console, int color, const char* fmt, va_list vlist )
{
    ...
    while ( str[i] )
    {
        ...
        else if ( console->nowcolumn < console->width )
        {
			if ( str[i] == 0x08 )
			{
	            linebuf->str[console->nowcolumn] = ' ';
				if (console->nowcolumn > 0) console->nowcolumn--;
			}
			else
			{
            ...
            if ( console->nowcolumn == console->width && ( str[i+1] != 0x0d && str[i+1] != 0x0a ) )
            {
                // 回り込み処理。
                console->nowcolumn = 0;
                console->currentline ++;
...
static tLine* GetLine( tDbsConsole* con, u32 line )
{
    u32 offset; // 行の先頭が何バイト目にあたるか

    offset = ( line % con->linenum ) * ( con->width + 1 );
...
#define GREATEREQ( x, y )  ( 0 <= (u32)((x) - (y)) && (u32)((x) - (y)) <= 0xFFFFFFFF/2 )
    ...
    if ( GREATER( line, bufstart ) &&
         GREATEREQ( con->currentline, line ) )
    {
        return TRUE;
    }
    else

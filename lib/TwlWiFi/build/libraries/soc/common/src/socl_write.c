                                  ...
                                  SOCLInAddr remote_ip, s32 flag_block)
{
    ...
    else
    {
        ...
        MI_CpuCopy(buffer2, command->write.buffer2, (u32) command->write.buffer2_len);
    }

    MI_CpuCopy(buffer, command->write.buffer1, (u32) command->write.buffer1_len);
...
static int SOCLi_WriteCallBack(void* arg)
{
    ...
    if (SOCL_SocketIsUDPSend(socket) && SOCL_SocketIsClosing(socket))
    {
        return SOCL_EINVAL;
    }
...
static int SOCLi_MemCpy(u8* sndbuf_ptr, int sndbuf_len, SOCLiCommandWrite* cpacket)
{
    ...
    if (buffer1_len > 0)
    {
        MI_CpuCopy(cpacket->buffer1, sndbuf_ptr, (u32) buffer1_len);
    ...
    if (buffer2_len > 0)
    {
        MI_CpuCopy(cpacket->buffer2, sndbuf_ptr + buffer1_len, (u32) buffer2_len);

...
BOOL    SOC_IsNetworkAvailable(void);
...
/*---------------------------------------------------------------------------*
  ...
  Description:  SOC_Close()が一度成功した上で、
                ソケットがCloseされたかどうか判定する。
                注意)SOC_Close()を一度も成功していないソケットは、
                     リリース済みと判断します。
  ...
  Returns:      TRUE : ソケットはリリースされている。
                FALSE: ソケットはクローズ&リリース処理中である。
                       あるいは不正なソケットである。
 *---------------------------------------------------------------------------*/
static inline BOOL SOC_IsReleased(int s)
{
    return SOCL_IsReleased( s);
}
...
{
    SDK_ASSERT ( 0 != mtu);
    (*mtu) = SOCLiConfigPtr->mtu ? (s32) SOCLiConfigPtr->mtu : SOCL_MTU_SIZE_DEFAULT;
    (void)interface;
}

...
/* file-and-directory entryr information structure.
   for FS_ReadDir() */
typedef struct
{
    union
...
/*---------------------------------------------------------------------------*
  ...
  Description:  get top positon of file image as offset from archive base
...
/*---------------------------------------------------------------------------*
  ...
  Description:  get bottom positon of file image as offset from archive base
...
/*---------------------------------------------------------------------------*
  $Log: file.h,v $
  Revision 1.38  2006/01/18 02:11:19  kitase_hirotake
  do-indent

  Revision 1.37  2005/06/28 11:30:31  yosizaki
  add FS_CreateFileFromMemory(), FS_CreateFileFromRom().

  Revision 1.36  2005/06/28 09:49:44  yada
  applied for SDK_THREAD_INFINITY

  Revision 1.35  2005/06/17 13:08:04  yosizaki
  fix FS_WaitAsync. (add FS_FILE_STATUS_OPERATING)

  Revision 1.34  2005/05/30 04:21:12  yosizaki
  add FS_End().
  add comments.

  Revision 1.33  2005/04/18 02:48:43  yosizaki
  ...
  Revision 1.29  2004/11/02 10:07:57  yosizaki
  fix typo.

  Revision 1.28  2004/09/24 13:07:14  yosizaki
  fix FS_UnloadTable.

  Revision 1.27  2004/09/24 12:08:05  yosizaki
  add FS_UnloadTable.

  Revision 1.26  2004/07/27 06:47:07  yosizaki
  small fix (warning "padding inserted")

  Revision 1.25  2004/07/19 13:11:44  yosizaki
  add FS_IsFileSyncMode.
  add FS_GetResultCode.
  delete FS_ReadServer.

  Revision 1.24  2004/07/13 02:54:44  yosizaki
  add FS_CancelFile.

  Revision 1.23  2004/07/12 13:05:10  yosizaki
  change FS_COMMAND_READFILE & FS_COMMAND_WRITEFILE (add argument)

  Revision 1.22  2004/07/08 13:46:28  yosizaki
  move some functions to card/rom.h

  Revision 1.21  2004/07/01 09:41:50  yasu
  suppoer autoload

  Revision 1.20  2004/06/30 04:30:15  yosizaki
  add new command 'FINDPATH'.

  Revision 1.19  2004/06/11 05:42:32  yosizaki
  extern FSOverlayInfo.

  Revision 1.18  2004/06/03 13:32:53  yosizaki
  fix around archive asynchronous operation.

  Revision 1.17  2004/06/01 10:17:23  yosizaki
  add FS_OpenFileDirect(). (private)

  Revision 1.16  2004/05/20 06:30:33  yosizaki
  add FS_ConvertPathToFileID.
  add some structure for archive.

  Revision 1.15  2004/05/19 02:30:34  yosizaki
  add FS_GetPathName().
  add comment.

  Revision 1.14  2004/05/17 08:31:22  yosizaki
  add comment.

  Revision 1.13  2004/05/11 09:35:55  yosizaki
  add FS_CreateReadServerThread().

  Revision 1.12  2004/05/11 04:33:17  yosizaki
  change format of FSDirPos and FSFileID.
  change archive system.

  Revision 1.11  2004/05/10 06:24:32  yosizaki
  add FS_ReadFileAsync, FS_WriteFile, FS_WriteFileAsync.

  Revision 1.10  2004/05/01 03:26:58  yosizaki
  u32 FSFile::arc_id -> void *FSFile::arc.
  change PXI protocol.

  Revision 1.9  2004/04/22 02:31:11  yosizaki
  FS_LoadTables() -> FS_TryLoadTable().
  add FS_LoadTable().
  add FS_GetTableSize().

  Revision 1.8  2004/04/15 11:39:45  yosizaki
  support CARD-on-TEG. (but CARD access routine is extern)
  add FSi_ReadServer().

  Revision 1.7  2004/04/15 00:06:27  yosizaki
  add FSi_LoadArchive().
  add FSi_LoadArchiveTables().
  add FS_LoadTables().
  add FS_GetDefaultDMA().

  Revision 1.6  2004/04/13 12:16:19  yosizaki
  fix header comment.
  add FS_IsAvailable.
  change FS_SetDefaultDMA.
  add archive selector system (internal).

  Revision 1.5  2004/04/08 10:46:48  yosizaki
  change FSDir to FSFile
  add FS_SetDefaultDMA()

  Revision 1.4  2004/04/06 11:59:18  yosizaki
  fix FS_SeekDir()

  Revision 1.3  2004/04/06 06:59:12  yosizaki
  change FS_Init()
  change around FSOverlayID

  Revision 1.2  2004/04/05 02:50:07  yosizaki
  change argtype of FSLoadOverlay()

  Revision 1.1  2004/04/01 09:30:45  yosizaki
  (none)

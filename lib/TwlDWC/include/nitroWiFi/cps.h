/*---------------------------------------------------------------------------*
  ...
  $Date: 2009-03-30 14:42:43 +0900 (Mon, 30 Mar 2009) $
  $Revision: 1155 $
...
#define USE_WCMSENDDCFDATAEX	// define this to use WCMSendDCFDataEx()

#define USE_FASTER_CHECKSUM		// define this to use faster checksum calc
#define USE_DELAYED_ACK			// define this to delay ACK-only packets
//#define USE_SOCK_OPT			// define this to support tos bit
#define USE_TCP_REORDER
#define USE_SELECTIVE_ACK       // define this to support selective ACK .

#ifdef USE_TCP_REORDER
#define CPS_RWIN_CHUNK      4
#endif
...
typedef struct {
    u32 bufp;           // index to reorder data on rcvbuf
    u32 size;        // size of reorder data
    u32 order;
} CPSSocRcvBuf;
...
typedef struct _CPSSoc {
	...
	u32 ackno_sent;				// my ACK number previously sent
	u32 tick_acked;
	u16 win_sent;				// my window size previously sent
	u8 ack_pending;
#endif
#ifdef USE_SELECTIVE_ACK
    u8 sack_enabled;            // 1:use sack.
#endif
...
#ifdef USE_SOCK_OPT
    u8 tos;						// miz : Type of service
    u8 rsv[3];					// miz : for 32 bytes aligned
#endif
#ifdef USE_TCP_REORDER
    CPSSocRcvBuf chunks[CPS_RWIN_CHUNK];
#endif
} CPSSoc;

#ifdef USE_SOCK_OPT
// miz : socket options
#define CPS_SOC_OPT_TOS	0x01    // ToS field of IP header
#endif
...
enum {
	...
	CPS_NOIP_REASON_INVALIDADDRESS
};
...
#define CPS_DONOTUSE_DHCP_FOR_DNS    0x0002
//
...
extern void CPS_SetCheckNConfigCallback(int(*f)(void));

#ifdef USE_SOCK_OPT
// miz : additional functions
extern int CPS_SocSetOpt(int optname, void* optval, int optlen); // miz
extern int CPS_SocGetOpt(int optname, void* optval, int optlen); // miz
#endif

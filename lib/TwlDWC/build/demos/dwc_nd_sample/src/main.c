...
void NitroMain( void )
{
	...
	// 実際のプログラムでは、DWC_LoginAsync()またはDWC_NASLoginAsync()で、
	// ダウンロードライブラリを使用する前に必ず認証サーバでの認証が成功していることを確認してください
...
{
	int no,entrynum;
	u32 received, contentlen, prereceived;
	...
	// ファイル取得終了待ち
	for( prereceived = 0; gdlcallback == FALSE; )
	{
	    if( DWC_UpdateConnection() )
	    {
	        // 実際のプログラムでは、エラーが発生した場合にはDWC_GetLastErrorEx関数で取得する対処法に従ってください
...
#pragma unused(servererror)
	...
	case DWC_ND_CBREASON_INITIALIZE:
		...
		break;
    case DWC_ND_CBREASON_CLEANUP:
        OS_TPrintf("DWC_ND_CBREASON_CLEANUP\n");
        NdCleanupCallback();
        break;
	}

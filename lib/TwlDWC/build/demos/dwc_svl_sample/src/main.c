...
static void GetSvlMain(void)
{
	...
	while(1) {
		...
		if(nasloginstate == DWC_NASLOGIN_STATE_SUCCESS) {
			OS_TPrintf("NAS Login Succeeded\n");
			break;
		}
	...
	// Aボタンで取得開始
	OS_TPrintf("**********************************************************\n");
	OS_TPrintf("Press A to check SVL function.\n");
	OS_TPrintf("While processing, you can abort SVL function by pressing A.\n");
	OS_TPrintf("**********************************************************\n");
	...
	if(DWC_SVLGetTokenAsync("", &result) == FALSE)
	{
	    ...
	    OS_TPanic("DWC_SVLGetTokenAsync failed.\n");
	}
	...
	while(1) {
		...
		if(state == DWC_SVL_STATE_SUCCESS) {
			OS_TPrintf("Succeeded to get SVL Status\n");
			OS_TPrintf("status = %s\n", result.status==TRUE ? "TRUE" : "FALSE");
			OS_TPrintf("svlhost = %s\n", result.svlhost);
			OS_TPrintf("svltoken = %s\n", result.svltoken);
			break;
		}
		else if(state == DWC_SVL_STATE_ERROR) {
			dwcerror = DWC_GetLastErrorEx(&errorcode, &dwcerrortype);
			OS_TPrintf("Failed to get SVL Token\n");
			OS_TPrintf("DWCError = %d, errorcode = %d, DWCErrorType = %d\n", dwcerror, errorcode, dwcerrortype);
			break;
		}
		else if(state == DWC_SVL_STATE_CANCELED) {
			OS_TPrintf("SVL canceled.\n");
			break;
		}

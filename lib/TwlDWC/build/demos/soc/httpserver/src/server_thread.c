...
#define SEND_SIZE       (CARD_ROM_PAGE_SIZE * 4)
...
static void SendResponse(int client_socket, char* filepath)
{
    ...
    char*   buffer      = OS_Alloc(SEND_SIZE + 32);
    char*   send_buffer = (char*)MATH_ROUNDUP32((u32)buffer);

    SDK_ASSERT(buffer != NULL);
...
#ifdef KEEPALIVE
        result = SendData(client_socket, "Connection: Keep-Alive\r\n");
#else
...
#pragma unused(arg)
    ...
    while (1)
    {
        ...
        OS_TPrintf("Thread %d waked up\n", OS_GetThreadId(OS_GetCurrentThread()));
            ...
            if (result != SOC_EWOULDBLOCK)
            {
                if (result <= 0)
                {
                    ...
                    do
                    {
                        ...
                        {
                            /* ソケットが正常に閉じられなかった */
                            OS_TPrintf("Socket Close Error! %d\n", result);
                            break;
                        }
                    }

...
static void SetHostSetting(SOCSockAddrIn* sa_in, const char* addr, u16 port)
{
    SOCHostEnt*  host_ent;
...
void NitroMain(void)
{
    ...
    int result;
    int i;
    static SOCSockAddrIn server_addr = { sizeof(SOCSockAddrIn) };
    static SOCSockAddrIn client_addr = { sizeof(SOCSockAddrIn) };
    ...
    OS_TPrintf("Listen....");
    result = SOC_Listen(server_socket, CONNECT_MAX);

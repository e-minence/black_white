...
void	YT_InitMultiBoot(GAME_PARAM *gp)
{
	...
	//文字表示パラメータワーク作成
	{
		...
		mbw->textParam=GFL_HEAP_AllocClearMemoryLo(gp->heapID,sizeof(GFL_TEXT_PRINTPARAM));
...
{
    GAME_PARAM *gp = pWork;
    gp->seq_no = SEQ_MB_ICHNEUMON_TRANS_WAIT;
}
...
void	YT_MainMultiBoot(GAME_PARAM *gp)
{
	...
	case SEQ_MB_ICHNEUMON_TRANS:
		GFL_NET_InitIchneumon(_initIchneumon, gp);
		gp->seq_no++;
		break;
    case SEQ_MB_ICHNEUMON_TRANS_WAIT1:
        break;
...
static	void	MultiBootInit(GAME_PARAM *gp)
{
	...
	if(!FS_OpenFile(file,mbGameList.romFilePathp)){
		...
		if(length==0){
			...
			mbw->segbuf=GFL_HEAP_AllocMemory(gp->heapID,length);
			if(mbw->segbuf==NULL){

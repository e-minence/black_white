...
/*
  ...
  */
    {_recvCLACTAnim,  NULL},
    {_recvCLACTPos,  NULL},
    {_recvCLACTPlayerAnim,  NULL},
    {_recvCLACTDelete,  NULL},
    {_recvYossyAnim,  NULL},
    {_recvCLACTAnimCreate,  NULL},
    {_recvScreenMake,  NULL},
    {_recvScreenMakeRot,  NULL},
    {_recvPlaySe,  NULL},
    {_recvPlayGameover,  NULL},
    {_recvPlayEnd,  NULL},
...
void YT_NET_SendPlayerAnmReq(int player_no,int anm_no,int pat_no,u16 line_no,int rot,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        GFL_NET_SendData(pNet->pNetHandle[1], YT_NET_COMMAND_PLAYERANIM, sizeof(COMM_PLAYER_ANIM_ST), &CommPlayerAnmSt);
    }
}
...
void YT_NetSendYossyBirthAnime(u8 posx,u8 posy,u8 count,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        GFL_NET_SendData(pNet->pNetHandle[1], YT_NET_COMMAND_YOSSYANIM, sizeof(COMM_YOSSY_ANIM_ST), &sYossy);
    }
}
...
void YT_NET_SendAnmCreate(int clactno,u16 type,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        GFL_NET_SendData(pNet->pNetHandle[1], YT_NET_COMMAND_ANIM_CREATE, sizeof(COMM_ANIM_CREATE_ST), &CommAnmCreateSt);
    }
}
...
void YT_NET_SendAnmReq(int clactno,u16 anmseq,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        GFL_NET_SendData(pNet->pNetHandle[1], YT_NET_COMMAND_ANIM, sizeof(COMM_SCREENMAKE_ST), &CommAnmSt);
    }
}
...
void YT_NET_SendPlayerScreenMake(int player_no, int old_line_no, int new_line_no,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        GFL_NET_SendData(pNet->pNetHandle[1], TY_NET_COMMAND_SCREENMAKE, sizeof(COMM_SCREENMAKE_ST), &CommScreenMakeSt);
    }
...
void YT_NET_SendPlayerRotateScreenMake(u8 player_no, u8 line_no, u8 flag,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        GFL_NET_SendData(pNet->pNetHandle[1], TY_NET_COMMAND_SCREENMAKE_ROT, sizeof(COMM_DELETE_ST), &CommScreenMakeRotSt);
    }
...
void YT_NET_DeleteReq(int clactno,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        GFL_NET_SendData(pNet->pNetHandle[1], YT_NET_COMMAND_DELETE, sizeof(COMM_DELETE_ST), &CommDelSt);
    }
}
...
void YT_NET_SendPosReq(int clactno,s16 x, s16 y,NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        CommPosSt.y = y;
        //OS_TPrintf("位置送信 %d %d\n",x,y);
        GFL_NET_SendData(pNet->pNetHandle[1], YT_NET_COMMAND_POS, sizeof(COMM_POS_ST), &CommPosSt);
    }
}
...
//            pNet->pNetHandle[1] = GFL_NET_CreateHandle();   // ハンドル作成
      ...
      case _SEARCH_CHILD:
        {
            for(i=0;i<16;i++){
                u8* pData = GFL_NET_GetBeaconMacAddress(i);//ビーコンリストの0番目を得る
                if(pData){
...
void YT_NET_Init(GAME_PARAM* gp, BOOL bParent)
{
    ...
    GFL_NET_Init(&aGFLNetInit, NULL, gp);
...
void YT_NET_PlaySE(int se_no, NET_PARAM* pNet)
{
    ...
    if(pNet){
        GFL_NET_SendData(pNet->pNetHandle[1], YT_NET_COMMAND_PLAY_SE,sizeof(int), &se_no);
    }
}
...
void YT_NET_SendGameResult(int player, int bWin, NET_PARAM* pNet)
{
    ...
    if(pNet){
        ...
        sResult.bWin = bWin;
        GFL_NET_SendData(pNet->pNetHandle[1], YT_NET_COMMAND_GAMERESULT,sizeof(COMM_RESULT_ST) , &sResult);
    }
}
